...
HF_Ready <- read.csv("HF_Ready.csv")
...
> I selected Predicted Readmission Rate because it showed the clearest relationships with hospital performance metrics and the strongest separation between hospitals with better and worse post-readmission lengths of stay. In addition, it accounts for patient mix and hospital characteristics, making it a more meaningful measure for comparing hospital performance.
...
longHCAHPS %>%  
  ggplot(aes(y = ExpectedReadmissionRate,  ## <-- CHANGE THIS!
             x = Value)) +
    geom_point(alpha = 0.25) + 
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                method = "lm", 
    ...
    labs(title = "Relationship of _ pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
        x = "Rate or Score",
        y = "Predicted Pneumonia-related Readmissions",
        color = "Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
...
> The most interesting relationships were with doctor communication, nurse communication, communication about medicines, and overall hospital rating. Hospitals with better scores in these areas generally had lower predicted readmission rates, suggesting that good communication may help patients recover and avoid returning to the hospital. One question I still have is whether these relationships are due to communication itself or if higher-performing hospitals also have other advantages that reduce readmissions.
...
> One limitation of using PredictedReadmissionRate is that it is based partly on historical data and model adjustments, so it may not fully reflect a hospital's current performance. Another limitation is that there may be factors affecting readmissions, such as patient health, economic status (how much they make, rich/poor?), or access to follow-up care, that are not completely captured in the dataset.
...
> All 31 of the features contained at least one missing value, meaning 100% of the features had some level of missingness. This suggests that handling missing data through imputation will be an important step before building predictive models.

**Which column(s) are not missing any data?**

> None of the columns are complete. Every feature contains at least one missing value.
...
```
There are 644 complete observations in the dataset. These are hospitals that have data available for every feature without any missing values.
...
> The value in cell [6,1] means that 1,788 hospitals were missing both variables at the same time. Since there are 4,816 hospitals in the dataset, that's about 37% of the observations. That seems pretty high to me and suggests the missing values in these variables may be connected instead of happening completely by chance.
