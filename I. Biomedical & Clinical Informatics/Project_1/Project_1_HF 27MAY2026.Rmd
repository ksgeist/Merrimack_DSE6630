    ...
    `Emergency department volume` =
      as.numeric(
        case_when(
          ...
          `Emergency department volume` == "very low" ~ "1",
          TRUE ~ NA_character_
        )
      ),
...
## Calculate proportion from pairwise pattern of missingness
p <- as.matrix(
  miceMatrix$mm / (miceMatrix$mr + miceMatrix$mm)
)
...
plot_df %>%
  ...
  theme_minimal() +
  labs(
    title = "Highest Correlations of Missingness \nwith Predicted Readmissions",
    x = "Variables",
...
> PredictedReadmissionRate has no missing values, so there is no meaningful co-missingness with the target. This is actually a good thing because the outcome variable is complete.

## 3.3 Drop rows that are missing from the target variable. 

We cannot use observations that are missing the target variable. In this dataset, PredictedReadmissionRate does not contain any missing values, so no rows were removed and the dataset remains at 2,305 hospitals.
...
> Not really. We still have over 2,300 hospitals and only 31 variables, so there is much more data than predictors. Because of that, I do not think we have a big-P, little-N problem. 
...
> MSE (Mean Squared Error) measures how close the imputed values are to the actual values for numeric variables. Lower MSE values indicate more accurate imputations. PCF (Proportion of Falsely Classified) measures how often categorical variables are assigned the wrong category during imputation, so lower values are also better. Based on the OOB error results, Payment had the highest error, followed by Denominator, HigherEstimate, and LowerEstimate. These variables appear to have been the most difficult for the model to impute accurately.
...
**NOTE 2**: Even though the PCF is a bit too high, I am choosing to retain `ComparedToNational_Hospital return days for heart failure patients` because it is a primary predictor. This could be a poor choice, however! Every decision we make as data scientists can _come with pitfalls_.
...
```{r}
# Numeric variable
summary(train$`Payment for heart failure patients`)
summary(imputedTrain$`Payment for heart failure patients`)
...
> The imputation did not drastically change the data. For Payment for heart failure patients, the distribution of categories remained very similar after imputation, with most hospitals still classified as “No Different Than the National Average Payment.” The imputation mainly filled in a small number of missing values without changing the overall pattern. For Emergency department volume, the category frequencies also remained fairly stable, with most hospitals classified as medium, high, or very high volume. Overall, the imputed values appear consistent with the original data and did not substantially alter the underlying distributions.
...
# Extract imputed dataframes from missForest objects if needed
if ("missForest" %in% class(imputedTrain)) {
  imputedTrain <- imputedTrain$ximp
}

if ("missForest" %in% class(imputedTest)) {
  imputedTest <- imputedTest$ximp
}

# Convert factors/characters back to numeric
...
> The Shapiro-Wilk test was significant (W = 0.994, p < 0.001), which suggests that PredictedReadmissionRate is not perfectly normally distributed. Since the p-value is much smaller than 0.05, we would reject normality. However, with a larger sample size (N = 1,893), even small differences from normality can make the test significant, so the histogram and Q-Q plot are still important to look at before deciding whether a transformation is needed.
...
We can see that the estimated λ = 0, which corresponds to a log transformation. This suggests that a logarithmic transformation may help make the distribution more normally distributed.(see lecture slides for reference). I found that $\lambda = -0.5$ in FY 2025, for those using that year's data.
...
ggplot(readyTrain, aes(x = bc_PredictedReadmissionRate)) +
  ...
  labs(
    title = paste0(
      "Box-Cox Transformed Predicted Readmission Rate, N = ",
      nrow(readyTrain),
      " hospitals"
    ),
    y = "Frequency",
...
> We must also transform the test data so it is on the same scale as the training data. We also need to use the same lambda value from the training data to avoid data leakage and keep the datasets consistent. The Box-Cox transformation improved the distribution by making it more symmetric and reducing skewness.
...
# Drop original target only if it still exists
if ("PredictedReadmissionRate" %in% names(readyTest)) {
  readyTest <- readyTest %>%
    select(-PredictedReadmissionRate)
}

# Convert factors and scale
readyTest <- readyTest %>%
  mutate_if(is.factor, as.numeric) %>%
  scale(center = TRUE, scale = TRUE) %>%
  data.frame()
...
readyTrain_corr <- readyTrain[, sapply(readyTrain, function(x) {
  is.numeric(x) &&
    !all(is.na(x)) &&
    sd(x, na.rm = TRUE) > 0
})]
...
> The highest potential for multicollinearity is among the patient satisfaction variables, such as nurse communication, doctor communication, staff responsiveness, and overall hospital rating. These variables are grouped closely together on the heatmap and appear strongly related to one another.

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> The variables that seem most correlated with bc_PredictedReadmissionRate are HigherEstimate, LowerEstimate, Score.x, Score.y, and Payment. These variables appear closest to the target on the heatmap and show stronger relationships than many of the other predictors.
...
readyTrain <- readyTrain[, colSums(is.na(readyTrain)) < nrow(readyTrain)]

# Remove rows with remaining missing values
readyTrain <- na.omit(readyTrain)
...
> Yes, multicollinearity appears to be an issue in this dataset. Variables such as Overall hospital rating, Recommend hospital, and Nurse communication have high VIF values, suggesting strong relationships among the patient satisfaction measures. HigherEstimate and LowerEstimate also have extremely large VIF values, indicating overlapping information.
...
> HigherEstimate, LowerEstimate, and Payment seem to be the most related to the target. The relationships appear to be mostly positive, meaning that higher estimated readmission measures and payments are associated with higher readmission rates. This makes sense because hospitals treating more complex patients may have both higher costs and higher readmission risk.
...
Because the goal is a deployable predictive model, we want to reduce redundant predictors and keep the more detailed variables when possible. Several patient satisfaction measures and estimate variables appear highly correlated, so removing the most collinear features should help simplify the model and reduce instability.
...
# Drop remaining ComparedToNational and PaymentCategory variables
readyTrain <- readyTrain %>%
  select(
    -contains("Compared"),
...
> The cumulative explained variance first exceeds 80% at PC8, where the cumulative proportion reaches about 80.6%. This means the first 8 principal components capture most of the variation in the dataset.
...
> PC1 explains about 21.6% of the total variance, while the final principal component (PC15) explains less than 1% of the variance. The amount explained by PC1 is moderate because the variance is spread across multiple components rather than dominated by one underlying factor.
...
> Based on the scree plot, approximately 5–6 principal components appear to be optimal. The explained variance decreases sharply from PC1 through PC5 and then begins to level off around PC6, indicating diminishing returns from retaining additional components. Retaining the first 5–6 principal components would likely capture most of the meaningful variation in the data while reducing dimensionality.
...
Additionally, we are going to color this by our grouping variable and key predictor in the dataset, Hospital.return.days.for.heart.failure.patients. This variable reflects hospital performance related to heart failure return-to-hospital days compared with the national average. Hospitals categorized as “Worse than average” had heart failure patients with longer-than-average return stays.
...
returnHospitalComparison <- factor(
  readyTrain$Hospital.return.days.for.heart.failure.patients,
...
This biplot (Figure above) shows that, on principal components 1 and 2 (the PCs that account for the most variance in the training data), there is subtle but noticeable grouping of hospitals based on heart failure return-to-hospital days. Variables such as Cleanliness, Quietness, Discharge.information, and Doctor.communication appear associated with hospitals in the “Better” category, while variables such as bc_PredictedReadmissionRate and Payment are more associated with the “Worse” category. Although the groups overlap, there appears to be some separation in the PCA space.
...
> Variables such as bc_PredictedReadmissionRate, Payment, and Healthcare.workers.given.influenza.vaccination appear to contrast with Communication.about.medicines because they point in different directions on the PCA biplot. Variables pointing in opposite directions are generally interpreted as having opposing relationships in the PCA space.
...
> The top contributing variables are mostly hospital quality, patient experience, and hospital operations measures, such as doctor communication, discharge information, influenza vaccination rates, ED wait times, and patients leaving before being seen. This suggests these factors explain much of the variation in the first two principal components. Additionally, bc_PredictedReadmissionRate falls above the significance threshold, indicating that it is also an important contributor to the explained variance, although several hospital quality and operational variables contribute more strongly.
...
> The within sum of squares (WSS) measures how close the data points are to the center of their assigned cluster. Smaller WSS values indicate tighter and more compact clusters. The elbow appears around k=4, where the decrease in WSS starts to level off, suggesting that 4 is the optimal number of clusters. This agrees with my choice from the cluster plots.
...
## Create dataset for cluster comparisons
kmeansTrain <- readyTrain %>%
...
## Important variables from PCA contribution plots
pcaImportantVars <- c(
  ...
  "bc_predicted_readmission_rate"
)

## Create boxplots comparing variables across clusters
kmeansTrain %>%
  ...
  mutate(
    Measure = case_when(
      ...
      Measure == "doctor_communication" ~
        "Doctor Communication",

      Measure == "discharge_information" ~
        "Discharge Information",
      ...
      Measure == "hospital_return_days_for_heart_failure_patients" ~
        "Hospital Return Days For Heart Failure Patients",

      Measure == "bc_predicted_readmission_rate" ~
        "Box-Cox Predicted Readmission Rate",
  ...
  scale_fill_manual(
    values = c(skittles[1], skittles[6])
  ) +
...
dict <- tribble(
  ...
  # Cluster 1
  "Predicted heart failure-related readmissions",
  "These hospitals have lower readmission rates, suggesting stronger patient outcomes and better heart failure management",

  "Return hospital days due to heart failure",
  "Readmitted patients spend below the national average number of days in the hospital, suggesting faster stabilization and treatment",

  "Doctor communication",
  "Patients report stronger communication and interactions with physicians",

  "Discharge information",
  "Patients report receiving clearer discharge instructions and follow-up information",

  # Cluster 2
  "Predicted heart failure-related readmissions",
  "These hospitals have higher readmission rates, suggesting opportunities to improve patient outcomes and continuity of care",

  "Return hospital days due to heart failure",
  "Readmitted patients tend to spend more time in the hospital compared with the national average",

  "Cleanliness and quietness",
  "Patients report lower satisfaction with the hospital environment and overall experience",

  "Doctor communication",
  "Patients report weaker communication and interactions with physicians"
)

dict %>%
  kable(
    format = "html",
    caption = "Table 9. Hospital segmentation analysis showing two broad hospital groupings."
  ) %>%
...
> Our team would test these clusters using Random Forest and logistic regression models to predict hospital performance outcomes such as heart failure readmission risk. To evaluate robustness, we would compare model performance on a separate test dataset using metrics such as accuracy, precision, and recall. We would also compare model performance with and without the cluster assignments to determine whether the clusters improve prediction accuracy.
...
> We use adjusted R-squared because it accounts for the number of predictors included in the model, while regular R-squared will almost always increase when additional variables are added, even if they contribute very little. Adjusted R-squared provides a more realistic measure of model performance and helps reduce the risk of overfitting.
...
> The RMSE and R-squared describe model performance, but they are difficult to interpret directly because the outcome variable was Box-Cox transformed and standardized. As a result, these metrics are more useful for comparing models than for interpreting actual readmission rates.
...
ols_df <- data.frame(
  testingResults = readyTest$predictions,
  actual = readyTest$bc_PredictedReadmissionRate[1:length(readyTest$predictions)],
  label = "OLS Prediction"
)

elastic_df <- data.frame(
  testingResults = elasticPredictions,
  actual = readyTest$bc_PredictedReadmissionRate[1:length(elasticPredictions)],
  label = "Elastic Net Prediction"
)

temp <- rbind(ols_df, elastic_df)

## Graph predictions relative to actual values
ggplot(temp,
       aes(x = testingResults,
           y = actual,
           fill = label,
           color = label)) +
