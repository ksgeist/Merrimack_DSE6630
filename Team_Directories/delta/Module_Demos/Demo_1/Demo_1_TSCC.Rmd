...
# hospital datasets are too large so not in GitHub
# personal path to datasets below.
# note that downloaded data from CMS,gov has _FY2025_ datasets (not 2024), file names adjusted accordingly

# read in my filepath to the variable 'filepath'
filepath <- "C:/Merrimack/DSE 6630 - Healthcare analytics/Demo1/hospital_data/"
...
# list and assign to dataframe, the names of all files in the filepath that have the pattern "Hospital.csv"
files <- list.files(path = filepath, pattern = "Hospital.csv")

# a 'for' loop, loop through all files in the 'files' list dataframe
# change all file names to upper_camel format
# in the filenames, substitute "Hospital.csv" for blank (drop from the file name).
...
# this is creating an error for me (TS) due to the inability to update htmltools to 0.5.8 for some reason.
...
>> # Yes there are multiple lines for each hospital, each row represents a question or measure, it is not Tidy.
...
> #  Many of the numeric variable also have strings mixed into the same column (some of the NAs actually a string "N/A".
...
# identify strings to replace in dataframe (just "N/A" in this case)

na_strings <- c("N/A")

FY_2025_Hospital_Readmissions_Reduction_Program <- FY_2025_Hospital_Readmissions_Reduction_Program %>% replace_with_na_all(condition = ~.x %in% na_strings)
...
> # Number of readmissions also has string "Too Few to Report" which was replaced with na in the preceeding step.
...
> # Question 7 (TS/CC)
> # You are basically indicating that this data is likely a small number (but not 0).
...
> # Question 8 (TS/CC)
> # A randomly sampled integer from 1-10 might be useful to add random variability.
...
FY_2025_Hospital_Readmissions_Reduction_Program_temp <- read_csv("C:/Merrimack/DSE 6630 - Healthcare analytics/Demo1/hospital_data/FY_2025_Hospital_Readmissions_Reduction_Program_Hospital.csv")

# Identify problem rows
problem_rows <- grepl("^[^0-9]+$", FY_2025_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions_temp)

# Replace with random values
FY_2025_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions[problem_rows] <- 
  sample(1:10, sum(problem_rows), replace = TRUE)
...
wideDF <- FY_2025_Hospital_Readmissions_Reduction_Program %>%
  pivot_wider(names_from = MeasureName, values_from = NumberOfReadmissions)

dim(wideDF)  # Confirm reshaped dimensions
```
...
merge_pneumonia_data <- function() {
  # Find all dataframes in environment matching the pneumonia_* pattern
  dfs <- mget(ls(pattern = "^pneumonia_\\d+$"), envir = .GlobalEnv)
  
  # Reduce with left_join using FacilityID as the common key
  merged_df <- Reduce(function(x, y) left_join(x, y, by = "FacilityID"), dfs)
  
  return(merged_df)
}

pneumoniaFull <- merge_pneumonia_data()
dim(pneumoniaFull)  # Confirm the size of the merged dataframe
...
> # since we can create a function that can merge all dataframes matching a certain pattern and merge them easily
...
to_drop <- c("LowerEstimate", "HigherEstimate", "Denominator", "HcahpsAnswerPercent")

# Count dropped columns
dropped_cols <- grep(paste(to_drop, collapse = "|"), names(pneumoniaFull), value = TRUE)
length(dropped_cols)  # Answer: Number of features dropped
...
pneumoniaFull <- pneumoniaFull %>%
  select(-any_of(dropped_cols), -FacilityName, -TelephoneNumber, -Address, -CityTown)
...
compare_cols <- grep("^ComparedToNational_", names(pneumoniaFull), value = TRUE)

# Create encoding function
encode_compared <- function(x) {
  case_when(
    x == "Above the national average" ~ 2,
    x == "Same as the national average" ~ 1,
    x == "Below the national average" ~ 0,
    TRUE ~ NA_real_
  )
}

# Apply encoding
pneumoniaFull[compare_cols] <- lapply(pneumoniaFull[compare_cols], encode_compared)

# Check one result
table(pneumoniaFull[[compare_cols[1]]], useNA = "ifany")
...
## specify the column names to encode and save into cols2encode
cols2encode <- c("State", 
               "CountyParish")

# save into a temp dataframe, only the columns State and parish fro pneumoniaFullEncoded
temp <- pneumoniaFullEncoded[, names(pneumoniaFullEncoded) %in% cols2encode]


# this creates a function 'add_freq' which creates a frequencey count
...
# add a frequency column for each of the columns in the temp df
for (col in names(temp)) {
  temp <- add_freq(temp, col)
}

# for the columns in the cols2encode df, join the frequency columns
...
## Expected Readmission Rate as the target variable
...
> # These would be highly correlated with the new column they created (observed_readmission_rate)
...
ggplot(pneumoniaAnalyze,
       mapping = aes(x=ExpectedReadmissionRate, y=PredictedReadmissionRate)) + geom_point()
...
> # This is a scatterplot of the Predicted readmission rate versus the Expected readmission rate.
