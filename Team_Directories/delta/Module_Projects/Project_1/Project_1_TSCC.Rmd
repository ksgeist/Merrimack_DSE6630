...
load(here("FY2024_data_files/pneumoniaAnalyzeFullyEncoded2024.Rdata"))
...
Chosing the fully encoded datset since working on # 3 (further predictive models)
...
> Your answer here.
Based on the violin plots, I would choose Excess Readmission Ratio, since there is a good separation between ordinal categories related to the national average.
...
> Your answer here.
Yes, the density plot of Excess Readmission Ratio shows separation similar to the observed readmission rate.
...
> Your answer here.
I chose ExcessReadmission Ratio, since there seemed to be good separation in the ordinal levels.
...
> Your answer here.
I'm continuing with Excess Readmission Ratio to observe its performance with this dataset.
...
> Your answer here.
Interesting that the graphs for Doctor Communication and Overall recommend hospital have opposite trends.
...
> I think it is an appropriate target that the stakeholder can relate to.  Excess readmission ratio may be difficult for a stakeholder to grasp.
...
> Your answer here.
PredictedReadmissionRate may contain some bias, since it is based on rating each hospital on its historical case and patient mix performance.  It is quite possible that current and future case and patient mix may differ.
...
dat2Analyze <- dat2Analyze %>% 
  ## create a new dat2Analyze dataframe, remove the targets we have decided not to use (these are too highly correlated with PredictedReadmissionRate).
  select(-ExpectedReadmissionRate, 
         -ExcessReadmissionRatio,
         -observed_readmission_rate, 
         -contains(c("Sample_", "NumberOfPatients")),
         -NumberSurveysCompleted) %>% 
  ## below renames some of the variables
  ...
  ## select our chosen target of PredictedReadmissionRate, and everything else (not unselected earlier)
  select(PredictedReadmissionRate, everything())

## substitute Score_|HcahpsLinearMeanValue_|_Payment
...
> Your answer here.
It is useful to remove features with low variance, since it reduces the dimensionality of the dataset, without losing much predictive power.
...
> Your answer here.
This is because unique identifiers will have a great amount of variance (due to being unique).
FacilityID was moved to rownames, so it is not seen as an additional variable to be assessed for variability (it is simply a row identifier)
...
> Your answer here.
I don't believe we have a big-p, litte-n problem, since we only have 30 variables, and we currently have 4816 observations.
...
> Your answer here.
The column of State is not missing any data
...
# using example from https://www.statology.org/nrow-function-in-r/

nrow(dat2Analyze[complete.cases(dat2Analyze), ])

# 664 rows with no missing data
...
> Your answer here.  if I am looking at the correct cell, that would be 'Compared to the National Death Rate for pneumonia patients.  It is likely a  smaller number since not all pneumonia patients will die.
...
> Your answer here.
 MRSA Bacteremia, but not sure why
...
> Your answer here.
I believe the dataset is still large enough to attempt a model. However, removing the rows with missing did lose almost half of the dataset.
...
> Your answer here.
Data is split into training test and testing sets.  This is done so that the model can learn from one set of data, and we can than test the accuracy of the learned model on a new set of (unseen data).  The new unseen data allows us to test the models prediction power on new and future data. Data leakage occurs when the training model has too much information, and the model has a performance theat is unrealistic with any new data.
...
calcSplitRatio <- function(p = NA, df) {
  ...
  ## p is equal to the number of columns in the dataframe - 1
  if(is.na(p)) {
    p <- ncol(df) -1   ## COMMENT HERE
  }
  
  ## the test number is equal to the 1/sqrt p * number of rows in our dataset (dat2Analyze)
  test_N <- (1/sqrt(p))*nrow(dat2Analyze)
  ## defining the proportion of the dataset that is the test partition
  test_prop <- round((1/sqrt(p))*nrow(dat2Analyze)/nrow(dat2Analyze), 2)
  ## defining the proportion of the dataset that is the training partition
  train_prop <- 1-test_prop
  
  ## the output that declares the ideal split ratio
...
> Your answer here.
The split ratio was 0.81:0.19 training:testing, which is very close to 80/20 split
...
# commented out since my computer cannot run
...
> Your answer here.
MSE is the mean squared error of the imputation for missing data. a higher MSE indicates more error compared to true values.
The variables with the highest OOB error were `Payment for pneumonia patients`, 	`Healthcare workers given influenza vaccination`,  `Venous Thromboembolism Prophylaxis`, `ComparedToNational_Hospital return days for pneumonia patients`
...
summary (imputedTrain$`ComparedToNational_MRSA Bacteremia`)
...
> Your answer here. 
Probably could shuffle the data and split again in an effort to have classes represented on both sides of the split.
...
> Your answer here.
According to information found on the web, a histogram can be influenced by bin size (so may or may not appear as a normal distribution, depending on bin size chosen. Q-Q plots may also have small deviations that can be difficult to interpret)
...
```
> Your answer here.
Since the p-value produced by the Shapiro-Wilk test is not greater than 0.05, the distribution of the target is NOT normally distributed.
...
> Your answer here.
We want to be sure that the transformation to the train and test data is identical.
...
> Your answer here.
The variable composite patient safety has a higher correlation

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> Your answer here.
There is some evidence of a correlation between `bc_PredictedReadmissionRate`, `State`, `Payment category for pneumonia patients`, `Medicare spending per patient`
...
> Your answer here.
VIF values greater than 5 can indicate a severe correlation between predictors.
...
> Your answer here.
Overall hospital rating and recommended hospital are the highest correlated (negative correlation).
THis would make sense, since lower rated hospitals may be associated with higher pneumonia readmission rates.
...
> Your answer here.
PC11 surpasses 80% of the explained variance
...
> Your answer here.
PC1 explains 22.4% of the variance, PC18 explains 100% of the variance.
PC1 explains a low amount of variance
...
> Your answer here.
There is a defined elbow at 6 principal components
...
> Your answer here. 
In the PCA literature, the plot is called a ‘Scree’ Plot because it often looks like a ‘scree’ slope, where rocks have fallen down and accumulated on the side of a mountain.) (medium.com)
...
> Your answer here.
The target variable `PredictedReasmissionRate` is at the dotted red line (which represents the variance of the target)
many of the top contributing variables all sound like patient complaints about hospital service.
...
> Your answer here
I would prefer K=6, beyond that there is a lot of overlap in the clusters.
...
> Your answer here
in this Scree plot it is difficult to tell. Maybe 6, but not a defined elbow (maybe at 2, which is well-defined elbow, but that seems too low).
...
> Your answer here
Surprised at the silhouette method identifying 2 clusters as optimal, but that corresponds with the WSS only visible elbow at 2 (even though 2 seemed low).
...
## run kmeans in the training data
kmeansResult <- kmeans(readyTrain, 	
                         centers = k,      ## indicate to kmeans to find the most appropriate cluster number and optimal centers
                         nstart = 50,      
                         iter.max = 1000,  
                         algorithm = "MacQueen")
## kmeansTrain is from the imputedTrain dataset
kmeansTrain <- imputedTrain %>% 
  ...
  ## Select only the columns with a VIF > 5, but drop columns that contain names of Compared, PaymentCategory, or SurveyResponseRate
  select(-any_of(vifOver5), -contains("Compared"), -contains("PaymentCategory"), -SurveyResponseRate) %>% 
  ## mutate a column called cluster that is either 0 or 1 
  ...
  ## clean_names function, to cleanup the names of the dataframe (lowercase and puts underscore in spaces)
  clean_names()
...
## convert pcaImportantVars to lowercase
...
kmeansTrain %>% 
  ...
  ## ggplot of clusters
    ...
    theme(legend.position = "bottom",
        ...
        legend.title = element_text(size = 10),
        title = element_text(size = 12, color = "maroon")) + 
    ## using facets in ggplot, 2 columns
    facet_wrap(~ Measure, 
             scales = "free_y",
             ncol = 2)
...
kmeansTrain %>% 
  ...
  ## COMMENT HERE
  select(Cluster, any_of(pcaImportantVars)) %>% 
  ## produce a cluster summary table
...
"Doctor Communication", "These hospitals had a higher rate of doctor communication verus lower-performing hospitals. ",
"Cleanliness", "Higher performing hospitals have a higher rate of overall cleanliness.",
"Discharge Information", "Lower-performing hospitals have a lower rate of discharge information, which may increase readmission risk for patients.",
"Communication about Medicines", "Lower-performing hospitals had a lower rate of communication about medicines, which may contribute to misunderstanding, and medication non-compliance.",
"Quietness", "Overall, hospital quietness was rated lower for this cluster, which may have a negative affect on patient recovery times.",
"Post-Operative Respiratory Failure Rate", "Lower-rated hospitals have a slightly higher median rate, but also a much higher range of post-operative respiratory failure"
)
...
> Your answer here.
The cluster labels themselves can be used in a supervised learning model to predict new clusters.
...
> Your answer here.
The Q-Q plot looks normal and the residals suggest homoscedasticity
...
> Your answer here.
# yes, because the shapiro-wilk test has a p value > .05
...
> Your answer here.
Adjusted R squared is more reliable to measure model accuracy when there are multiple predictors.
...
> Your answer here.
Medicare spending per patient and Death Rate for pnuemonia patients are the top 2
...
> Your answer here.
For PCA the signs do not matter since the variance would be the same, irrespective of sign
...
> Your answer here.
The elastic net and OLS regression perform similarly.
Next we will try a LASSO regression and a random forest model
...
## first try a LASSO regression on the dataset, since there is multicolinearity in the predictors.
...
# assign the training and test splits
x_train <- model.matrix(bc_PredictedReadmissionRate ~ ., readyTrain)[, -1]
y_train <- readyTrain$bc_PredictedReadmissionRate

# the lasso regression model (alpha=1)
set.seed(465)
...
plot(glmnet(x_train, y_train, family="gaussian", alpha=1),"lambd", label=T, main="")
abline(v=log(lambda.min), lty=3)

# coefficients of the model 
glm_best_model <-glmnet(x_train, y_train, alpha=1, lambda = lambda.min)
coef(glm_best_model)


# Make predictions on the training set
predictions_train <- predict(glm_best_model, x_train)

# Make predictions on the test set
#predictions_test <- predict(glm_best_model, y_train)


# prediction vs actual (Training data)
plot(predictions_train, readyTrain$bc_PredictedReadmissionRate, main="Training Data: Predicted vs Actual", xlab="Predicted", ylab="Actual", col='blue', pch=20)
abline(0, 1, col="red")

# prediction vs actual (Testing data)
#plot(predictions_test, readyTest$bc_PredictedReadmissionRate, main="Testing Data: Predicted vs Actual", xlab="Predicted", #ylab="Actual", col='blue', pch=20)
#abline(0, 1, col="red")


```

## try a random forest model to predict bc_PredictedReadmissionRate
...
ctrl <- trainControl(method='repeatedcv', number=10, repeats=10, search='random')

set.seed(1)
rf_random <- train(bc_PredictedReadmissionRate~ ., data=readyTrain, method='rf', trControl=ctrl)
...
```{r random forest on training set}

rf.train <- randomForest(bc_PredictedReadmissionRate ~ ., data=readyTrain, mtry = 6, ntree=1000, importance = TRUE )

# check the results of the random forest
rf.train
...
# or visualize variable importance
# https://hackernoon.com/

# Get variable importance from the model fit
ImpData <- as.data.frame(importance(rf.train))
...
```{r random forest predictions on training/test set}

# Make predictions on the training set
predictions_train <- predict(rf.train, readyTrain)

# Make predictions on the test set
predictions_test <- predict(rf.train, readyTest)

```

```{r random forest visualizations and accuracy}

# prediction vs actual on the training set
plot(predictions_train, readyTrain$bc_PredictedReadmissionRate, main="Training Data: Predicted vs Actual", xlab="Predicted", ylab="Actual", col='blue', pch=20)
abline(0, 1, col="red")

# prediction vs actual on the test set
plot(predictions_test, readyTest$bc_PredictedReadmissionRate, main="Test Data: Predicted vs Actual", xlab="Predicted", ylab="Actual", col='blue', pch=20)
abline(0, 1, col="red")

# Evaluate performance on the training set
rmse_train_rf <- rmse(readyTrain$bc_PredictedReadmissionRate, predictions_train)
r2_train_rf <- R2(predictions_train, readyTrain$bc_PredictedReadmissionRate)
mae_train_rf <- mae(readyTrain$bc_PredictedReadmissionRate, predictions_train)

cat("Train RMSE:", round(rmse_train_rf, 4), "\n")
cat("Train R²:", round(r2_train_rf, 4), "\n")
cat("Train MAE:", round(mae_train_rf, 4), "\n")

# Evaluate performance on the test set
rmse_test_rf <- rmse(readyTest$bc_PredictedReadmissionRate, predictions_test)
r2_test_rf <- R2(predictions_test, readyTest$bc_PredictedReadmissionRate)
mae_test_rf <- mae(readyTest$bc_PredictedReadmissionRate, predictions_test)

cat("Test RMSE:", round(rmse_test_rf, 4), "\n")
cat("Test R²:", round(r2_test_rf, 4), "\n")
cat("Test MAE:", round(mae_test_rf, 4), "\n")
