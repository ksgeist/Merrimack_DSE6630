...
# Based on the violin plots, I would choose Excess Readmission Ratio, since there is a good separation between ordinality related to the national average.
...
# I'm again sticking eith Excess REadmission Ratio to observe its performance with this dataset.
...
> Your answer here.
# Interesting that the graphs for Doctor Communication and Overall recommend hospital have oppostie trends.
...
# I think it is an appropriate target that the stakeholder can relate to.  Excess readmission ratio may be difficult for a stake holder to grasp.
...
dat2Analyze <- dat2Analyze %>% 
  ## create a new dat2Analyze dataframe, remove the targets we have decided not to use.
...
# I don't believe we have a big-p, litte-n problem, since we only have 30 variables, and we have 2726 observations.
...
# I believe the dataset is still large enough to attempt a model.
...
# THis would make sense, as a hospitals rating gets lower, it my be associated with higher pneumonia readmission rates
...
> Your answer here
# in this Scree plot it is difficult to tell. Maybe 6, but not a defined elbow (maybe at 2, but that seems too low).
...
> Your answer here.
# the Q-Q plot looks normal
# and the residals suggest homoscedasticity
...
> Your answer here.
# yes, the shapiro-wilk test has a p value > .05
