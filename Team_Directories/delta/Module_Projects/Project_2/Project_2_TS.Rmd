...
# having issues with knitting so setting wd.
setwd('C:/Users/TK/Documents/GitHub/Merrimack_DSE6630/Team_Directories/delta/Module_Projects/Project_2')
...
> The shapefiles contain the following files (with the following file extensions)
.cpg - optional file that specifies the codepage fot the character set to be uses (optional)
.dbf - dBASE file that stores the attribute information of the features (required)
.prj - Stores the coordinate information used by ArcGIS
.shp - main file that stores feature geometr (required)
.shp.xml - metadata for ArcGIS—stores information about the shapefile.
...
cali_sf <- read_sf("C:/Users/TK/Desktop/California_County_Boundaries/cnty19_1.shp")
...
> The islands cause the names of several counties (where the islands are located) to be repeated.
...
> The bounding box specifies the bounding rectangle of all shapes contained in the dataset (min_x, max_x, min_y, max_y)
source: https://en.wikipedia.org/wiki/Shapefile
If a user wanted to zoom-in or zoom-out, the coordinates of the bounding box would need to be changed.
...
> The projection specifies how a map is overlayed onto a curved surface, like the earth. All overlayed data must use the same projection.
source: https://support.esri.com/en-us/knowledge-base/how-to-define-the-projection-for-arcgis-data-1462479485-000005932
...
ggplot(data = map1) +
  ...
  labs(title = "Northernmost California Counties",
       subtitle = "(Del Norte, Siskiyou, Modoc)",
       caption = "Source: California State Geoportal")   
...
```{r}
## Create a cities data frame with coordinates, adding San Jose
cities <- data.frame(
...
## create a mortality shapefile, start with the totalDeaths dataframe
mortality_sf <- totalDeaths %>% 
  ## rename 'County' to COUNTY_NAM' for shapefile compatibility
  rename(COUNTY_NAM = County) %>%
  ## group the data by County, year, month
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## summarize total deaths by County, year, and month
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## join this data with the california shapefile, a many-to-many relationships (since for each county in the shapefile,
  ## there are many deaths by year/month, and also the islands duplicate the counties)
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## below is a function (st_as_sf) from the sf package that makes an object compatible with spatial data
  st_as_sf() %>% 
  ## but filter out the islands from the resulting shapefile
  filter(is.na(ISLAND))
```
...
> Your answer here.
We need to use full_join since it is a many-to-many join.
...
```{r}
# find the number of features in the shapefile mortality_sf
...
> we have added more dimensions to (now each county has mortality data for Year and Month) 
...
> Your answer here.
# there are 58 counties in mortality_sf$COUNTY_NAM
...
> Your answer here.
# Total deaths are always higher in counties that contain cities (Los Angeles County), probably due to high population centers, this needs to be converted to a percentage (based on population).
...
key <- "933c56c69edfba0988fb9f997ae40c74d8fe3f40"
```
...
> Your answer here.
source: https://api.census.gov/data/2020/dec/dp/variables.html
DP1_0048C is Males 65 years and over (this is a count of population)
DP1_0048P is Males 65 years and over (this is a percent population)
...
## explore the dataframe `census`

# errors when knitting so placing this here.
load("census.Rdata")
...
> Your answer here
## the census dataframe contains population census information for each of the counties in california.  
## There are 12 variables.
## Besides county name there are the following population variables:
## totalN - total population
## malesN - total N males
## femalesN - total N females
## over65 - total N over 65+ yrs
## over65perc - pct of population 65+
## males65plus - total N males 65+
## males65plusPerc - pct of males 65+
## females65plus - total N females 65+
## females65plusPerc - pct of females 65+
...
> Your answer here.
## there are several pieces to the api call:
## building the url, adding the specific details of the request, and the function call, which returns the data in json format
## other API calls could be used (in a similar way), and the steps would be similar. The target and the details of 
the request would change.  Most APIs could download data as JSON.
...
## calling the new datafile mortality_sf2a in the event of an error.

mortality_sf2a <- mortality_sf %>% 
  rename(County = COUNTY_NAM) %>%
  st_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>%
  st_as_sf()
...
load("C:/Users/TK/Desktop/mortality_sf2.Rdata")
load("census.Rdata")
```
...
> Your answer here.
# it looks like overall the death rate decreased by county during 2020.
...
> Your answer here.
# we could make a prediction using existing data to replace the last 4 months of missing data at the end of 2020.
# or replace the missing data with a median or mean of previouls data points
...
> Your answer here.
# it appears as though monthly percent mortality does increase in 2020 for several counties in southern california, but is unchanged in other couunties for 2020.
...
> Your answer here.
# the density plot of the residuals is bell shaped around 0 (not skewed), so the residuals appear normally distributed.
The Q-Q plot also shows heavy tails indicating some extreme values.
...
> Your answer here.
# Yes, I would think there would be a computational cost for calculating spatial weights on redundant data.
...
> Your answer here.
# the spatial lag regression is more useful for map data where neighbors may influence the model.
...
> Your answer here.
The R2 of both models is very similar.
olsmodel = Adjusted R-squared:  0.7165 
sarModel = Adjusted R-squared:  0.7271
The overall p-value is also similar
...
> Your answer here.
The examiniation of the sarModel is very similar to the olsModel, it is difficult to differentiate between the two models.
...
# percent of data in the training set
(nrow(mortalityTrain) / nrow(mortality_sf2)) * 100

# percent of data in the testing set
(nrow(mortalityTest) / nrow(mortality_sf2)) * 100


```

> Your answer here.
# the split is close to a desired 80/20 split - slightly more in the training set
# (there is 81.8 % training set, 18.2 % testing set).
...
> Your answer here.
# one issue is that 2020 was such a different year, that data trained on other years (excluding 2020) is very different (and therefore predictions for 2020 may not be accurate).
...
> Your answer here.
# in a random forest OOB error is used to estimate the accuracy of the model, specifically the OOB error is related to the trees not included in the training set (it is used to calculate the prediciton error on the 2020 data)
...
> Your answer here.
# the default mtry for a random forest regression is p/3, but forcing it 1-3 since there are 3 predictor variables in the model (otherwise it could default to classification and not regression).
...
> Your answer here.
According to information found on towardsdatascience: To summarize, we should expect R² to be bounded between zero and one only if a linear regression model is fit, and it is evaluated on the same data it is fitted on. Else, the definition of R² can lead to negative values.
source: https://towardsdatascience.com/explaining-negative-r-squared-17894ca26321/
...
> Your answer here.
Mtry=1 has the best results, the highest R2 (0.037) compared to mtry2,3,  Mtry 1 also has a slightly lower RMSE and MAE.  R2 is also better than the OOB fit model.
...
> Your answer here.
The train MAE = 0.7056
The test MAE = 0.7428
Seems like both test and training error are roughly equal, so it seems OK.
...
> Your answer here.
It looks like the model is over-predicting mortality, versus the actual mortality in the data
...
## using code provided by classmate June Lemieux in Canvas announcements
...
## filtering (2020 and all causes of death) was done in the above api call, however still need to filter total population only

caliDeaths_allpop <- caliDeaths %>% filter(Strata == "Total Population")
...
```{r}
## Your code here
## first need to add census data to get total population per county in 2020.
## prepare for scatterplot (keep county)

caliDeaths_2020 <- caliDeaths_allpop %>%
  full_join(census, by = "County") %>%
  group_by(Year, Month, County) %>%
  mutate(NDeaths = as.numeric(Count)) %>%
  summarize(PctMortality = sum(NDeaths, 
    na.rm = TRUE)/mean(totalN)*100) %>% 
  select(Year, Month, County, PctMortality)
...
## Make a scatterplot
  ...
  ggplot(caliDeaths_2020, aes(x = Month, y = PctMortality)) +
  geom_point(alpha = 0.7, color = skittles[4]) +
  geom_smooth(color = skittles[8], method = "lm") +
  labs(title = "All-cause Percent Mortality in California during 2020",
       subtitle = "Each point represents a county average",
       y = "% Mortality of County Population", 
       x = "Month, 2020",
       caption = "Source: California DPH Vital Records") +
  theme_minimal()
...
> Your answer here.
Accourding to source: https://www.geeksforgeeks.org/machine-learning/what-is-lag-in-time-series-forecasting/
- One important concept within time series analysis is lag, which plays a significant role in understanding and modeling the relationship between past and future values in a time series.
- Lag enables analysts to understand how current data points are related to previous ones (autocorrelation).
- Lagged variables are often used as features in models
- Seasonality can be identified, by examining values across different lags.
...
> Your answer here.
If the testing MAE with time-lagged variables = 0.129, and the training MAE = 0.134, I would think the model is doing well, and the MAE is now much lower (so performing better) now that the time-laged variables have been added.
...
> Your answer here.
# the random forest model could have used more predictors, which would have resulted in a better relative predictor importance.
...
> Your answer here.
Adding time-lagged variables reduces the error of the random forest regression model. 
...
> Your answer here.
# No, we can clearly see that Biological sex has an affect on the proportion or deaths due to injury or accident.
# This is clearly higher for Males
...
> Your answer here.
# In 2020, accidental deaths by Males and females decreased dramatically (more so for males). This is likely the result of stay-at-home orders.
...
> Your answer here.
# I would use additional faceting to make another set of maps of Year x Biosex (8 panels)
...
> Your answer here.
# It is possible that during 2020, some counties did not report their data
...
injuryDeathSummary2 <- injuryDeathSummary %>% filter(Year == "2020")

## Initialize the plot
ggplot(injuryDeathSummary2) +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +
  ...
  labs(
    ...
    subtitle = "Cumulative Monthly Deaths by Biological Sex, 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
  
  ## Use facet_wrap to make one map per Bio Sex
  facet_wrap(~ biologicalSex, ncol = 2)
...
> Your answer here.
The correlation coefficient is weakly positive, it may be interesting to produce gender-specific (faceted) maps, just to examine if those would be interpretable by stakeholders.  I would think depending on the bucketing of values on the map colors, it may not be noticeable on the maps.
...
> Your answers here.
# For spatial lags, we should use a Spatial Autoregressive model (SAR)
...
>
## HYPOTHESIS: Cause of death due to heart attack and stroke increased during 2020 in California.
## this may have been because people were afraid to go to ER with symptoms.
## and also mandatory stay-at-home order my have made people delay doctor visits.
## will focus on people over age 65

```{r}

## Step 1: join the sexdeath data with census data 
## filter for leading cause of death due for heart attack or stroke.

MICVA_Deaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ## filter for leading cause of death related to heart attack or stroke
  filter(leadingCauseDeath == "HTD" | leadingCauseDeath == "STK") %>% 
  ## Now full join with the first 4 columns of the totalDeaths table to get 
  ## total deaths also
  full_join(totalDeaths[ , 1:4], by = c("County", "Year", "Month")) %>%
  ## Rename the sexDeaths column to something more appropriate
  rename(MICVADeaths = sexDeaths) %>% 
  ## Calculate the proportion of all deaths that are from injury 
  ## RELATIVE TO ALL DEATHS (not population!!)
  mutate(MICVADeathsProportion = MICVADeaths/totalDeaths,
         ## The ratio of females to males in the population
         femaleMaleRatio = femalesN/malesN,   
         ## The ratio of 65+ females to males in the population
         femaleMaleRatio65 = females65plus/males65plus,  
         ...
         Date = as.Date(paste(Year, Month, "1", sep = "-"), 
                        format = "%Y-%m-%d")) %>%    
         ## Illegal div by 0 turned to NaN; turn back to 0
  mutate(MICVADeathsProportion = ifelse(MICVADeathsProportion == "NaN", 
                                           0, MICVADeathsProportion))  

```


```{r, echo = FALSE}
## Now plot the numbers of deaths per year due to heart attack or stroke by sex

## overall (grouping heart attack and stroke together for boxplots), collapse sex

MICVA_Deaths %>% 
ggplot(aes(y = MICVADeathsProportion, x= as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "Year", 
       y = "Proportion of All Deaths Due to Heart Attack or Stroke", 
       title = "Proportion of All Deaths Due to Heart Attack or Stroke",
       subtitle = "California Counties, Jan 2017 - Aug 2020") + 
  scale_fill_manual(values = skittles[c(5, 9:11)]) + 
  theme_classic()

## separate out heart attack and stroke, collapse sex

MICVA_Deaths %>% 
ggplot(aes(y = MICVADeathsProportion, x = leadingCauseDeath, fill = as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "Heart Attack vs. Stroke", 
       y = "Proportion of All Deaths Due to Heart Attack or Stroke", 
       fill = "Year", 
       title = "Proportion of All Deaths Due to Heart Attack or Stroke",
       subtitle = "California Counties, Jan 2017 - Aug 2020") + 
...
MICVA_Deaths %>% 
ggplot(aes(y = MICVADeathsProportion, x = biologicalSex, fill = as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "Biological Sex of Decedent", 
       y = "Proportion of All Deaths Due to Heart Attack or Stroke", 
       fill = "Year", 
...
MICVA_Deaths %>% 
  group_by(Month, biologicalSex, Year) %>% 
  summarise(MICVADeathsProportion = mean(MICVADeathsProportion, na.rm = TRUE)) %>% 
ggplot(aes(x = Month, 
           y = MICVADeathsProportion, 
           color = as.factor(Year), 
           linetype = as.factor(biologicalSex))) +
  geom_line(alpha = 0.7, size = 1.1) +
  scale_x_continuous(n.breaks = 10) +
  scale_color_manual(values = skittles[c(5, 9:11)]) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(title = "Proportion all deaths due to heart attack or stroke, by sex",
       y = "Proportion of All Deaths Due to heart attack or stroke", 
       subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
       color = "Year",
       linetype = "Sex of Decedent") +
...
ggplot(aes(x = Month, 
  ...
  scale_color_manual(values = skittles[c(5, 9:11)]) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(title = "Proportion all deaths due to heart attack or stroke",
       y = "Proportion of All Deaths Due to heart attack or stroke", 
       subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
       color = "Year",
       linetype = "Sex of Decedent") +
...
## create a mortality shapefile, for MICVADeaths

MICVA_sf <- MICVA_Deaths %>% 
  ## rename 'County' to COUNTY_NAM' for shapefile compatibility
  rename(COUNTY_NAM = County) %>%
  ## group the data by County, year, month
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## summarize total deaths by County, year, and month
  summarise(totalMICVAdeaths = sum(MICVADeaths, na.rm = TRUE)) %>%
  ## join this data with the california shapefile, a many-to-many relationships (since for each county in the shapefile,
  ## there are many deaths by year/month, and also the islands duplicate the counties)
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## below is a function (st_as_sf) from the sf package that makes an object compatible with spatial data
  ...
  ## check the counts of county names in the sf (should be 58 unique names)
  unique(MICVA_sf$COUNTY_NAM)
  n_distinct(MICVA_sf$COUNTY_NAM)
...
## Start from the MICVA spatial tibble
mortalitySummary <- MICVA_sf %>% 
  ## Group by and then summarize to total the deaths by County and Year
  group_by(COUNTY_NAM, Year) %>% 
  summarise(totalDeaths = sum(totalMICVAdeaths, na.rm = TRUE)) %>% 
  mutate(deathLabels = cut(totalDeaths, 
                          breaks = c(0, 50, 500, 5000, 50000, Inf),
                          labels = c("0-49", "50-499", "500-4,999", 
                                    "5,000-49,999", "50,000+"),
...
ggplot(mortalitySummary) +
  ...
  ## Apply manual fill scale for total deaths
  scale_fill_manual(
    name = "Deaths, Heart Attack/Stroke (per 10,000 persons)",
    values = c4a(palette="BuPu", n=6),
    drop = FALSE
  ) +
  ## Set map projection explicitly
  ...
  labs(
    title = "California Annual Deaths by County (Heart Attack & Stroke)",
    subtitle = "2017 - 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
  ## Use facet_wrap to make one map per year
...
> Summary:
It did not appear that the % mortality of heart attack and stroke increased during 2020 in California.
This was easily visible using data visualizations.
Therefore our hypothesis appears not to be supported by the data and no further investigation was necessary
