...
> (JML) The `group` variable is an identifier for the primary geographic unit (a US continguous state in our case).
...
```{r}
load("../../Module_Demos/Demo_1/pneumoniaFull.Rdata")
...
> (JML) The fully-processed dataset had been scaled, normalized, encoded, etc., according to our needs of the Project_1. Starting from a basic level will allow us to set the data according to the needs of this Demo.
...
> (JML) In the states dataframe, there are 48 continguous US states plus District of Columbia; Alaska and Hawaii are not represented. In the pneumoniaFull dataframe, all states and all territories are present. The states dataframe has full state names, and the pneumoniaFull dataframe uses the 2 character state code. Either mapping or some kind of transformation will be needed in order to be able to join or merge data.
...
# (JML) I took a bit of a different approach (I blame my SQL background). Based on the question, I interpreted the ask to be to replace the state abbreviation in `pneumoniaFull` with the full state name. To me, the full name was available in the `state.fips` dataframe, but I'd need to only get the characters to the left of the colon (:). I'd also need to reduce the resulting list to unique values. So, `pneumoniaFull_testing` is the result of 1) creating a helper table with just state names, 2) creating a 2nd table with just unique records, 3) replacing the 2-letter abbr with the full state name found in the 2nd table. Hope this approach doesn't go against the lesson's intention. I have called this df "pneumoniaFull_testing" to keep it separate from the rest of the Demo code.

library(dplyr)
state.fips_noIsland <- state.fips %>%
  mutate(polyname = trimws(sub(":.*", "", polyname))) %>%
  select(abb, polyname)

state.fips_noIsland_unique <- unique(state.fips_noIsland)

pneumoniaFull_testing <- pneumoniaFull %>%
  mutate(State = ifelse(State %in% names(state.fips_noIsland_unique),
                        state.fips_noIsland_unique[pneumonia$State],
                        state.fips_noIsland_unique$polyname))
view(pneumoniaFull_testing)
...
> (JML) In the approach I mentioned in Q2, I created a new dataframe (from state.fips) with full state names that were limited to the characters to the left of any colon that might appear in the values. Because this new dataframe now contained duplicate records, I created a 2nd dataframe that contained just 1 record of each state.
...
```{r}
 stateAggPneumonia <- pneumoniaFull %>%
  ...
  # (JML) I didn't understand this part; including it didn't work and excluding it did work?
...
> (JML) Maryland is missing Medicare spending data in the stateAggPneumonia dataset. I checked the provided .Rdata file and the same data was missing in that file too. Looking back to the pneunomiaFull dataset, I can see that Maryland is missing data for Score_Medicare spending per patient across all records. 
...
> (JML) Mean can identify a representative value when there are few/no extreme values in the dataset. It is sensitive to outliers, though. If outliers are present, showing someone a mean could be interpreted as a representative value when in actuality, it is not. In those cases, showing the median value is more represtative. (reference https://datascience.stackexchange.com/questions/46744/when-to-use-mean-vs-median)
...
> (JML) Based on the boxplots, I think that pneumonia hospital return days and pneumonia readmission rates would be better represented by the median because there are long tails on one side of the boxplots vs the other side. This to me indicates there are extreme values or outliers on one side.
...
___Interpretation___: (JML) Because $p<0.05$, we reject the hypothesis of symmetry and conclude that the death rate for pneumonia patients data are __significantly skewed__. We should use the median to aggregate this variable.
...
___Interpretation___: (JML) Because $p<0.05$, we reject the hypothesis of symmetry and conclude that Hospital return days for pneumonia patients data are __significantly skewed__. We should use the median to aggregate this variable.
...
___Interpretation___: (JML) Because $p<0.05$, we reject the hypothesis of symmetry and conclude that Medicare spending per patient data are __significantly skewed__. We should use the median to aggregate this variable.
...
> (JML) If I were to only look at the stability information in this plot, I might determine that medicare spending per patient should be analyzed using the mean. However, I feel the data in Q8A and Q8B support a decision to use median for medicare spending per patient. For the remaining measures, I believe that the stability data supports the data provided by the previous questions and that these measures should be analyzed using medians.
...
```{r}
# JML removing all mean columns, leaving all median columns
stateAggPneumonia <- stateAggPneumonia %>% select(-meanReadmissions, -meanDeathRate, -meanHospitalReturnDays, -meanMedicareSpending)
```
...
> (JML) As mentioned in class, the top 10 states don't fit an immediate pattern of one determinant (wide range of income/poverty levels, education, geography, rural vs urban). To me this suggests a set of determinants could be in play. 
...
p2 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median death rate within 30 days", 
            low = "white", high = skittles[3]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median pneumonia death rate by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
p3 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median Medicare spending per patient per hospital", 
...
p4 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median number of days readmitted pneumonia patients spend in the hospital", 
            low = "white", high = skittles[5]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median number of hospitalized days for readmitted pneumonia patients by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
# (JML) I had some trouble with this due to the location of my files. I ended up opening the .R file, modifying the paths of the source files and manually running the code. I did not push the changed file to github because I assume the path issue is isolated to my workstation setup.
```
...
## Automatically search for the Rdata files starting in your home directory
home_dir <- path.expand("~")
train_path <- list.files(home_dir, pattern = "readyTrain\\.Rdata$", recursive = TRUE, full.names = TRUE)[1]
test_path <- list.files(home_dir, pattern = "readyTest\\.Rdata$", recursive = TRUE, full.names = TRUE)[1]

## Check if the files were found
if (is.na(train_path) | is.na(test_path)) {
  stop("One or both data files were not found. Please check your folders.")
}
...
## Check for pneumoniaFull
if (!exists("pneumoniaFull")) {
  message("pneumoniaFull is missing. Please load or create it before proceeding.")
} else {
  pneumoniaFull <- pneumoniaFull %>% mutate(region = getStateName(State))

  ## Merge for TRAIN
  stateTrain <- readyTrain %>%
    left_join(pneumoniaFull %>% select(FacilityId, region, PredictedReadmissionRate,
      `Score_Hospital return days for pneumonia patients`,
      `Score_Medicare spending per patient`, `Score_Death rate for pneumonia patients`),
      by = "FacilityId") %>%
...
```{r}
# (JML)
n_distinct(stateAggTrain$region)
```

> (JML) There are 52 unique values in the stateAggTrain$region column

#### **Question 10B**: [0.25 points]

Why am I not asking you to check the states in the testing data?

> (JML) Looking at the count of observations in the Global Environment, I only see 50 observations. I believe that it is possible that the Train:Test ratio/split might not have allocated a record for each state in the test data set. 
...
> (JML) The training and testing sets had been scaled & centered and we have not done that in this exercise. Despite that, outliers/extreme values can still exist and medians will better represent the data. Median data can be easier to interpret/explain as well.
...
> (JML) The st_as_sf() function converts dataframe data into the "Simple Features" object and creates a single variable with all the data needed to draw the lines for a single record (aka state in our case). The states object, in contrast, has many many records of the coordinates, wo the states object wasn't used because it isn't in the correct format to be consumed by the map code.
...
> The rook contiguity 1) defines neighbors strictly as those spatial units with common edges (vs queen that uses either common edges or common vertex). 2) This defnition means that CO and AZ are not neighboring states. 3) Change the contiguity in poly2nb() from queen to rook by changing queen = TRUE to queen = FALSE.
...
> (JML) There are 4 numbers of 0.25 each. This feels consistent that with my thought that there are 4 neighbors. The values of 0.25 each also feels consistent with the row-normalization. This way no neighbor and no state is weighted more than another.  
...
```{r}
states_sw$weights[[19]]
```

> (JML) As discussed in class, MA is number 19. There are 5 neighbors and this matches my expectation.  
...
> (JML) For the spatial weight work, I guess that if we applied this to the full dataset, we could influence the data that would become the test data and that could impact overfitting or data leakage.  
...
> (JML) DE (low-high), WV, DC, NJ (high-high), UT (low-low). No outliers identified in (high-low)
...
> (JML) Scaling transforms data to a specific range, so using that data could impact the analysis of the Moran's I
...
```{r}
## Calculate the global Moran's I for states on training data
## temporarily resetting the target to `bc_PredictedReadmissionRate`

## Change this to change the target!
y <- states_sf_AggTrain$bc_PredictedReadmissionRate
...
> (JML) The Moran's I is 0.289 and the p-value is 0.0007. The p-value is less than 0.05 so we reject the null hypothesis that there is no spatial autocorrellation. Therefore there is a relationship between states' geographies and the predicted readmission rates.
...
> AJ-For a state to be classified as a Coldspot (Quadrant III)— it needs to have low readmission rates surrounded by other low readmission states (positive spatial autocorrelation), have a cluster of "good" performance with a local clustering that is statistically significant. Have a positive local Moran’s I (Ii > 0), a negative centered value of y , and a significant p-value Pr(z != E(Ii)) < 0.05.


Let me know if you'd like help identifying coldspots in your output using a filtering script or visual map.it must meet the opposite set of criteria from a hotspot:
...
> AJ-The spatially-lagged values are shifted because they’re calculated on the centered (mean-subtracted) readmission rates and not the original raw values. Before the spatial weights was calculated, the raw median readmission rates were mean-centered by subtracting the overall mean from each state's value.This transformation shifts the data to have a mean of zero and  makes the lagged values reflect deviations from the average, so they appear smaller and shifted.
...
```{r}
# Massachusetts is row 20 in the original dataframe
y[20]
...
> AJ-Yes, it is much closer.The centered value for Massachusetts is 0.657, which is on the same scale as the spatially-lagged value (around 0.213), unlike the original raw value (~16.7). Since the spatially-lagged value was around 0.213, it’s much closer to the centered value than the original raw rate (~16.7%).
...
```{r}
##Classify each state into LISA quadrants based on local Moran's I and spatial lag criteria

states_sf_AggTrain <- states_sf_AggTrain %>%
  mutate(
## Create a new variable 'quadrant' to classify states as Hotspot, Coldspot, Outlier, or Not Significant
    quadrant = case_when(
      ## If p-value is greater than 0.05, the local spatial autocorrelation is NOT statistically significant
      pValue > 0.05 ~ "N.S.",
## No need to specify p-value again in other cases because the first condition already filters for significance - ## so all other cases below already have p-value <= 0.05 (significant)

      ## Quadrant I: Hotspot (High-High)
      y > 0 & lag_medianReadmissions > 0 ~ "Hotspot",
      
      ## Quadrant III: Coldspot (Low-Low) 
      y < 0 & lag_medianReadmissions < 0 ~ "Coldspot",

      ## Quadrant IV: High-Low Outlier 
      y > 0 & lag_medianReadmissions < 0 ~ "High-Low Outlier",

      ## Quadrant II: Low-High Outlier 
      y < 0 & lag_medianReadmissions > 0 ~ "Low-High Outlier",

      ##For any remaining cases not covered above, assign NA
      TRUE ~ NA_character_
    )
  )
```
...
> AJ- If Indiana is a "low-high" outlier, it means that the state is doing better than expected when compared to its higher-readmission neighbors but it's noted as an exception in a region of poor performance and the spatial relationship is statistically significant (p-value < 0.05).
...
> AJ- Hot and coldspots may not perfectly match with 'individual states' high or low median readmission rates because spatial clustering (LISA) focuses on patterns among neighbors, not just isolated values. While the graph p1 shows 'individual state' rates, LISA classifies states based on both their value and the values of their neighbors. A hotspot state is not just a state with a high rate but a high-rate state surrounded by other high-rate states and a coldspot state is astate surrounded by low-rate states. Therefore a state with a high readmission rate could be simply an outlier (high surrounded by low), not a hotspot and spatial context  and LISA adds nuance beyond what p1 graph can't show  alone.
...
> AJ- Starting from raw data, we would first clean the data removing or imputing missing values, correcting errors, and standardizing formats. Then, we would engineer or select relevant features, including calculating variables like median readmission rates and  we would normalize or scale continuous predictors so they're on comparable scales, especially if using models sensitive to magnitude. We would also need to merge spatial data (like shapefiles) with our hospital readmission data to create the spatial weights matrix in order to capture neighborhood structure. Finally, we would split the data into training and testing sets so that we can evaluate model performance.
...
> AJ- (1) Fitting an OLS regression again  provides a baseline model to compare against the Spatial Autoregressive (SAR) model and help assess whether spatial relationships improve predictions.(2)  Aggregation to the state level since it's a smaller sample size, is important in order to verify that basic assumptions still hold and that predictors are still useful before adding spatial complexity.  OLS is used as a baseline for model comparison and revalidating assumptions after aggregation due to the reduced sample size.
...
> AJ-It can be concluded that there is no strong evidence of spatial autocorrelation in the residuals of the OLS regression since the p-value (0.061) is slightly above the common threshold of 0.05. This means failure to reject the null hypothesis that residuals are spatially independent, suggesting that OLS may not be severely misspecified.
However, the positive Moran's I value (≈ 0.129) hints at a possible mild spatial pattern, so using a spatial model like SAR could still be worthwhile for improving either accuracy or interpretability.
...
> AJ-Starting with residuals alone would be misleading since the p-value (0.061) is just above the typical significance threshold. This borderline result could lead us to wrongly conclude there’s no spatial autocorrelation. It’s better to assess spatial patterns both in the raw outcome and in residuals to fully understand spatial dependence.

- Why might these results disagree with what we found from the Moran's $I$ and LISA on the scaled target?

> AJ-These results might disagree because the Moran’s I and LISA on the scaled target detect spatial patterns in the raw (unexplained) data, whereas the Moran’s I on the OLS residuals detects spatial patterns left over after controlling for predictors.The raw target’s spatial patterns and the residual spatial patterns after modeling can differ, leading to differences in conclusions between the raw and modeled data.
...
> 1. Global Moran’s I: Showed significant positive spatial autocorrelation in the scaled outcome,  similar readmission rates tend to cluster geographically which suggests that spatial relationships are important and should not be ignored.
> 2. Local Moran’s I (LISA): Identified specific clusters (e.g., high-high or low-low areas) where readmission rates are unusually similar to neighbors. This gave localized insight into spatial dependence and reinforced the global Moran’s I findings.
> 3. Moran’s I on OLS Residuals: Produced a borderline p-value (~0.061), suggesting weak or no residual spatial autocorrelation. If this was the sarting point, we might have incorrectly concluded that spatial effects aren’t important. However, this only reflects patterns after controlling for predictors.
> 4. SAR Model ρ Coefficient: The spatial lag coefficient ρ was significantly greater than zero (p = 0.012), indicating that spatial dependence in the outcome remains after accounting for predictors. This justifies the inclusion of the spatial lag term in the model.

> 5. Likelihood Ratio Test (LRT): Showed that the SAR model fits significantly better than the OLS model (p = 0.0286), proving that the addition of the spatial component meaningfully improves the predictive performance.

> 6. LM Test on SAR Residuals: Had a high p-value (~0.82), indicating no remaining spatial autocorrelation in the residuals after fitting the SAR model. This confirms that the SAR successfully addressed the spatial structure in the data.


What is your **final takeaway**? Do we need to account for spatial autocorrelation in our regression model?

> AJ-Yes, we definitely need to account for spatial autocorrelation in our regression model. The evidence shows that the outcome exhibits spatial clustering, and the SAR model significantly improves fit over OLS by explicitly modeling spatial dependence. Ignoring spatial effects would risk biased or inefficient estimates therefore including spatial structure is essential for accurate modeling and inference.
...
> AJ- Perioperative complications and Medicare spending remain important predictors across all models and quietness gains significance only after accounting for spatial dependence which then highlights the added value of spatial modeling. Accounting for spatial autocorrelation sharpens the inference, separating true effects from spatially structured noise and revealing predictors that truly matter when spatial spillover is considered.
...
> AJ-To interpret RMSE, the error would need to be back-transformed (unscaled) because the model was trained on a scaled (standardized) version of the target variable. As a result, RMSE reflects error in standardized units, not in the original units of hospital readmission rates, making direct interpretation difficult.
...
> AJ-The Mean Absolute Error (MAE) represents the average magnitude of the errors between the predicted and actual values on the test set, which is  measured in the transformed (standardized) units of the target variable. It provides a straightforward interpretation of the typical size of prediction errors without regard to their direction.
The Root Mean Squared Error (RMSE) also measures prediction error on the transformed scale but places greater emphasis on larger errors because it squares the differences before averaging. This makes RMSE more sensitive to outliers. For example, an RMSE of 0.3 means that on average, predictions deviate from the true values by about 0.3 standard deviations in the transformed target scale.
When comparing the two models, the SAR model shows lower MAE and RMSE than the OLS model, indicating that it produces both smaller average errors and fewer large prediction mistakes which  suggests that accounting for spatial autocorrelation improves predictive accuracy.
In summary, the SAR model performs better than OLS when evaluated by both MAE and RMSE on the transformed target, demonstrating the value of incorporating spatial dependence in the regression. The SAR model is preferred if it shows consistently lower MAE and RMSE than OLS, demonstrating improved predictive performance by capturing spatial dependence.
...
> AJ-The null hypothesis is that lambda = 0, which means there is no spatial autocorrelation in the residuals.From the SEM output, the estimated lambda is 0.62138 with a very small p-value of 0.00000037814. Since the p-value is much less than 0.05, we reject the null hypothesis. This provides strong evidence of spatial autocorrelation in the residuals, indicating that the spatial error model is necessary to properly account for this spatial dependence.
...
> AJ- The reason would be that a significant SEM combined with a non-significant LM test for SAR residuals suggesting that the SAR model fits well by capturing spatial spillover in the outcome, but some spatial dependence remains in the residuals that the SEM detects, indicating potential room for refinement or complementary modeling.
...
> AJ- Summary for a possible workflow
1.Check spatial autocorrelation with Moran’s I or LISA.
2.If present, fit SAR to model spatial dependence in the outcome.
3.Test SAR residuals for spatial autocorrelation using LM test.
4.If residual autocorrelation remains, fit SEM to model spatial dependence in errors.
5.If needed, fit SDM to account for spatial dependence in both outcome and predictors, especially if omitted variables are suspected.
6.Use diagnostics to choose the best model and improve with additional variables if necessary.
...
> AJ-Integrating spatially lagged variables into machine learning models is a proven strategy to enhance predictive performance in spatially correlated datasets. By acknowledging and modeling spatial dependencies, data scientists can build more robust models that better reflect the underlying spatial structures of the data. A recent study published in Remote Sensing, demonstrated the effectiveness of integrating spatial features into machine learning models (https://www.researchgate.net/publication/359808809_Incorporating_Spatial_Autocorrelation_in_Machine_Learning_Models_Using_Spatial_Lag_and_Eigenvector_Spatial_Filtering_Features?utm_source=chatgpt.com) utilizing Moran’s I and Local Indicators of Spatial Association (LISA) to assess spatial autocorrelation at both global and local scales. Their findings indicate that Random Forest models incorporating spatial lag or Eigenvector Spatial Filtering (ESF) features yielded lower errors—up to 33% improvement—and significantly reduced global spatial autocorrelation of the residuals, with a 95% decrease in Moran’s I. These models outperformed benchmark Geographically Weighted Regression (GWR) models in terms of accuracy and residual spatial autocorrelation.
...
> MN: The dimensions of sarTrain are 49 rows and 17 columns, this is a very small dataset. I feel state-level is not appropriate for further machine-learning because there is not enough inforation for the model to create relationships without over-fitting. This dataset introduces the curse of dimensionality, we have too many predictors compared to observations, and not enough data points to reliably estimate relationships. This could also introduce multicollinearity, as well as if any new information is introduced, it could drastically change the results. As far as my current understanding, the SAR, SEM, and SDM can account for the small amount of data because they handle the spartial aspects.
...
> MN: I believe we could use county level data, or maybe even zip code data, as that would increase the dimensionality of our dataset, which machine learning models love and tend to perform better in those circumstances. 
...
> MN: A social determinant of health is a non-medical factor or influence of health outcomes. These could include place of birth, social norms, political structures, income level, education level, and access to healthcare, just to name a few
...
## filter out DC, ? extreme pop density alters the rest of map
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## create plot
ggplot() +
  ## creates a map using spatial geometry, using the popDensity as the fill
  geom_sf(aes(fill = popDensity), color = "grey20", alpha = 1, size = 0.2) +
  ## scale aesthetics using hundreds per square mile
  scale_fill_gradient(
    name = "Hundreds per sq. mi.",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +
  ## labels, title and captoin of source
  labs(title = "Adult population density by state",
    caption = "Source: U.S. Census Bureau, 2023") +
  ## adjusting map appearance, making elements blank or not visible
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
    plot.background = element_rect(fill = "white"),
  ...
  ## type of coordinate display, using proper lat/long
  coord_sf()
...
## remove DC from mapping plot
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## create a plot using median raw predicted readmission rate
ggplot(aes(x = popDensity, y = Median_RawPredictedReadmissionRate)) +
## create scatter plot withh trend line
...
> MN: When looking at p1 and p2 and comparing, we can see some trends overlapping. If looking at the New Jersey area on p2, we can see high population density there and in surrounding areas. Looking at p1 in the same area we can see that the median predicted readmission is also at the higher end.
If we go to the Colorado area in p2, we see that the poplation density is lower, as well as surrounding areas. In comparison to p1, we do see lower median predicted readmissions, with the exception of Nevada, which is interesting because it has a low population density. Would county level data show a different story? 
...
> MN: Looking at the trend line of p3, we can see that there is a positive correlation between population density and predicted oneumonia related readmissions. We could assume that the top 10 states from p4 are the points to the right and up on p3, maybe some point labels could confirm that. 
On p3, while there seems to be highe population density states oulling the trend line, we can also see that there is a more subtle same trend with the points closer to the left, they do trend right and up, while not as dramactically as the far right.
...
> MN: The Moran statistic is 0.1045, showing a weak positive spatial correlation. Seeing we have a p-value of over 0.05, this test is telling us that the residuals do not support spatial autocorrelation. So we could say either there isn't spatial autocorreltion, or maybe we do still have some omitted variables that were absorbed into other predictors.
...
> MN: The Rho is 0.35443, and p-value 0.016887, and this does suggest spatial dependance. I don't think I'm surprised because I beleive we were expecting dependance somewhere. 
The LM test shows a p-value of 0.68448, which would suggest that the spatial lag is capturing spatial dependence.
...
> MN: Since spatial dependence exists, the OLS model is missing key spatial influences, while SAR correctly accounts for them, leading to a better model fit. The AIC is 61.513 in the SAR, and 65.221 in the OLS. The lower AIC would indicat ethe better fitting model. Also the p-value of 0.016887 is significant and suggest that the SAR model captures the data better than OLS.
...
> MN: We do see the same predictors being significant with the addition of popDensity. Medicare Spending and Quietness actually became slightly more significant, and periop puml embolism and DVT became slightly less significant. I would interpret the beta coefficient of 0.01 as for every 1 unit increase of population density, there is an increae of 0.01 pneumonia related hospital readmissions, while holding all other variables constant.
...
> MN: Looking at thr RMSE and MAE of all 4 models, we can conclude that OLS is not the best option. SAR, SE<, and SDM all performed the same. Since SAR performed the same as the other spatial models, we could say that it is "good enoigh". Given that SDM can account to an extent for omitted variables, I feel like this model should have performed better. Since it did not, one could assume tha there may still be more omiited variables not accounted for.
...
>MN: 
**Hypothesis 2: High Elderly Populations**: States with a higher proportion of their adult population over the age of 65 have higher rates of readmission __because__ of age-related factors that increase the likelihood of severe illness, like pneumonia, and increased risk of complications that could lead to higher
hospital readmissions.

```{r echo=TRUE, message=FALSE, warning=FALSE}
## Make a map `p5` of population over 65
p5 <- states_sf_AggTrain %>% 
## filter out DC, ? extreme pop density alters the rest of map
  #filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## create plot
ggplot() +
  ## creates a map using spatial geometry, using the over65PercentPop as the fill
  geom_sf(aes(fill = over65PercentPop), color = "grey20", alpha = 1, size = 0.2) +
  ## scale aesthetics using hundreds per square mile
  scale_fill_gradient(
    name = "Percentage of population over 65",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +
  ## labels, title and caption of source
...
## create scatter plot withh trend line
  ...
  theme_minimal() +
  ## adding labels to plot
  labs(title = "Hospital readmissions by population percentage over 65",
       x = "Percentage of Population over 65",
       y = "Predicted Pneumonia-related Hospital Readmissions")

## Make a barplot `p7` that shows the highest over 65  states with their median
p7 <- states_sf_AggTrain %>% 
## filter out DC
  #filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
  ## arrange popdensity in descending order
...
ggplot(aes(x = fct_reorder(ID, over65PercentPop), y = Median_RawPredictedReadmissionRate)) +
  ...
  coord_flip() +
  ## title and labels
  labs(title = "Top 10 Over 65 States",
     y = "Median % hospital readmissions",
     x = "",
...
>MN: Before going further, and just looking at the maps created, I feel there is an interesting trend, and something to look into further.
Looking at p6 we see there is a negative correlation between population over 65 and predicted pneumonia readmissions. p7 shows the top 10 states with populations over 65. When we look at those states in p5 and compare it to p1, not all states with high over 65 population have high readmission rates. Maine for instance stands out, while in the opposite degree West Virginia stands out with a high over 65 population and high readmissin rates. My question here is, are there other SDOH at play here?


```{r}
## TRAINING
            ## Start from the training data
sarTrain_over65 <- states_sf_AggTrain %>% 
  ## Make it a dataframe/tibble
  as_data_frame() %>% 
  ## Drop the features that we dropped before the analysis in Project 1, as 
  ## well as the features we do NOT want to use as predictors!
...
## TESTING
            ## Start from the testing data
sarTest_over65 <- states_sf_AggTest %>% 
  ## Make it a dataframe/tibble
  as_data_frame() %>% 
  ## Move the ID column to the rownames
  column_to_rownames("ID") %>% 
...
```{r}
OLS_over65 <- lm(bc_PredictedReadmissionRate ~ ., data = sarTrain_over65)
## Moran's I test on residuals
moran.test(residuals(OLS), states_sw)
```

```{r}
SAR_over65 <- lagsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain_over65, 
                listw = states_sw)
## Uncomment to view results!
summary(SAR_over65)
```
...
```{r}
anova(SAR_over65, OLS_over65)
```

>MN:So far the SAR is peforming much better than OLS. Moran test showed possibly some spatial correlations ofr population over 65, but the SAR seemed to perform well, and showed a statisticly significant spatial dependance in the RHO.
Also of note the AIC for this SAR is higher than the AIC for popdensity, indicating that this may not be the better omitted variable model.

```{r}
## Print a table using gtsummary
tbl_regression(SAR_over65) %>% 
  modify_caption("Table 7. Spatial Lag (SAR_over65) Regression results.") %>% 
  modify_header(label = "**Coefficient**") %>% 
  add_significance_stars(hide_p = FALSE)
```

>MN: The coefficients are interesting, we have the same significant variables as the popdensity, but over65 percentage population does not show as significant. This could indicate it's not as strong an indicator as we previously believed.

```{r}
## Fit the SEM (spatial error model) using the training data and weights matrix
SEM_over65<- errorsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain_over65, 
                  listw = states_sw)
## Uncomment to view results!
summary(SEM_over65)

## Fit the SDM (spatial Durbin model) using the training data and weights matrix
SDM_over65 <- lagsarlm(formula = bc_PredictedReadmissionRate ~ ., data = sarTrain_over65, 
    listw = states_sw, type = "mixed") ## "mixed" is necessary to specify as SDM
## Uncomment to view results!
summary(SDM_over65)
```
...
```{r}
## Fit the predictions for the FOUR models using the testing data!
olsPredictions_over65 <- predict(OLS_over65, newdata = sarTest_over65)

sarPredictions_over65 <- predict(SAR_over65, newdata = sarTest_over65, listw = states_sw) %>% 
  as_tibble()

semPredictions_over65 <- predict(SAR_over65, newdata = sarTest_over65, listw = states_sw) %>% 
  as_tibble()

sdmPredictions_over65 <- predict(SAR_over65, newdata = sarTest_over65, listw = states_sw) %>% 
  as_tibble()
```

```{r}
## RMSE
olsRMSE_over65 <- sqrt(mean((sarTest_over65$bc_PredictedReadmissionRate - olsPredictions_over65)^2))
sarRMSE_over65 <- sqrt(mean((sarTest_over65$bc_PredictedReadmissionRate - sarPredictions_over65$fit)^2))
semRMSE_over65 <- sqrt(mean((sarTest_over65$bc_PredictedReadmissionRate - semPredictions_over65$fit)^2))
sdmRMSE_over65 <- sqrt(mean((sarTest_over65$bc_PredictedReadmissionRate - sdmPredictions_over65$fit)^2))

## MAE
olsMAE_over65 <- mean(abs(sarTest_over65$bc_PredictedReadmissionRate - olsPredictions_over65))
sarMAE_over65 <- mean(abs(sarTest_over65$bc_PredictedReadmissionRate - sarPredictions_over65$fit))
semMAE_over65 <- mean(abs(sarTest_over65$bc_PredictedReadmissionRate - semPredictions_over65$fit))
sdmMAE_over65 <- mean(abs(sarTest_over65$bc_PredictedReadmissionRate - sdmPredictions_over65$fit))

## Make a dataframe for comparison of RMSE and MAE values
data.frame(Model = c("OLS", "SAR (Spatial Lag)", "SEM (Spatial Error)", "SDM (Durbin)"),
           RMSE = round(c(olsRMSE_over65, sarRMSE_over65, semRMSE_over65, sdmRMSE_over65), 2),
           MAE = round(c(olsMAE_over65, sarMAE_over65, semMAE_over65, sdmMAE_over65),2)) %>% 
  ## Display with kable
  kable(digits = 2,
        format = "html",
        caption = "Table 13. Comparison of RMSE and MAE for all models.") %>%
...
>MN: Finally, looking at our comparison table, the same pattern as population denisty emerges. OLS performs the "worst", while all of our spatial models perform the same. We can also see that population percentage over 65 does not perform as well as population denisty did, suggesting that this variable does not capture spatial dependencies as well as population density.

>: I feel that our hypotheses may not be strongly supported. Spatial modeling does not suggest that age alone is a driving factor in pneumonia realted hospital readmissions. While it makes intuitive sense, the model does not fully and significantly enough support the hypotheses. 
