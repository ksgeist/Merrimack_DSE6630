...
> AJ-For a state to be classified as a Coldspot (Quadrant III)— it needs to have low readmission rates surrounded by other low-readmission states (positive spatial autocorrelation), have a cluster of "good" performance with a local clustering that is statistically significant. Has a positive local Moran’s I (Ii > 0), Negative centered value of y , and has a significant p-value Pr(z != E(Ii)) < 0.05.
...
states_sf_AggTrain <- states_sf_AggTrain %>%
  mutate(
    ...
    quadrant = case_when(
      ...
      pValue > 0.05 ~ "N.S.",
      ## No need to specify p-value again in other cases because the first condition already filters for significance -       
      ## so all other cases below already have p-value <= 0.05 (significant)
...
> AJ- If Indiana is a "low-high" outlier, it means that the state is  doing better than expected compared to its higher-readmission neighbors but it's noted as an exception in a region of poor performance and the spatial relationship is statistically significant (p-value < 0.05).
...
However, the positive Moran's I value (≈ 0.129) hints at a possible mild spatial pattern, so using a spatial model like SAR could still be worthwhile for improving accuracy or interpretability.
...
> 1. Global Moran’s I: Showed significant positive spatial autocorrelation in the scaled outcome, meaning similar readmission rates tend to cluster geographically. This suggests that spatial relationships are important and should not be ignored.
...
> 3. Moran’s I on OLS Residuals: Produced a borderline p-value (~0.061), suggesting weak or no residual spatial autocorrelation. If we had started here, we might have incorrectly concluded that spatial effects aren’t important. However, this only reflects patterns after controlling for predictors.
...
> 5. Likelihood Ratio Test (LRT): Showed that the SAR model fits significantly better than the OLS model (p = 0.0286), meaning the addition of the spatial component meaningfully improves predictive performance.
...
> AJ-The Mean Absolute Error (MAE) represents the average magnitude of the errors between the predicted and actual values on the test set, measured in the transformed (standardized) units of the target variable. It provides a straightforward interpretation of the typical size of prediction errors without regard to their direction.
...
> AJ-The null hypothesis is that lambda=0, which means there is no spatial autocorrelation in the residuals.From the SEM output, the estimated 
lambda is 0.62138 with a very small p-value of 0.00000037814.
Since the p-value is much less than 0.05, we reject the null hypothesis.
This provides strong evidence of spatial autocorrelation in the residuals, indicating that the spatial error model is necessary to properly account for this spatial dependence.
...
> AJ- The reason would be that a significant SEM combined with a non-significant LM test for SAR residuals suggests the SAR model fits well by capturing spatial spillover in the outcome, but some spatial dependence remains in the residuals that the SEM detects, indicating potential room for refinement or complementary modeling.
...
Recent study published in Remote Sensing demonstrates the effectiveness of integrating spatial features into machine learning models (https://www.researchgate.net/publication/359808809_Incorporating_Spatial_Autocorrelation_in_Machine_Learning_Models_Using_Spatial_Lag_and_Eigenvector_Spatial_Filtering_Features?utm_source=chatgpt.com). By utilizing Moran’s I and Local Indicators of Spatial Association (LISA) to assess spatial autocorrelation at both global and local scales. Their findings indicate that Random Forest models incorporating spatial lag or Eigenvector Spatial Filtering (ESF) features yielded lower errors—up to 33% improvement—and significantly reduced global spatial autocorrelation of the residuals, with a 95% decrease in Moran’s I. These models outperformed benchmark Geographically Weighted Regression (GWR) models in terms of accuracy and residual spatial autocorrelation.
