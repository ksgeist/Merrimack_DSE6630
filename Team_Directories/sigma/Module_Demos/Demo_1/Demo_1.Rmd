...
##### **Question 5**: [1 point] 
Look more closely at the `NumberOfReadmissions` column in the `FY_2025_Hospital_Readmissions_Reduction_Program`. What other issue do you see with this specific column that is coercing it to the character type, and why does it exist? 
...
##### **Question 6**: [1 point] 
Investigate the `gsub()` function that is part of base `R` or any other function of your choosing. Use the function you choose to **replace** the problematic text in  the `NumberOfReadmissions` column in `FY_2025_Hospital_Readmissions_Reduction_Program` with a **5**.
```{r}
?gsub
...
```{r}
# (JML) FY_2025_HAC_Reduction_Program - not sure if we'll use it but I removed footnote-related columns
# and the date columns
FY_2025_HAC_Reduction_Program <- FY_2025_HAC_Reduction_Program %>% 
...
#(JML)I added the 2nd set of column drops since that data is in hospitalInfo; also added "MeasureID" to the 1st set of column drops
...
FY_2025_Hospital_Readmissions_Reduction_Program <-  tidy_clean_2(FY_2025_Hospital_Readmissions_Reduction_Program)#(MN) even trying on one dataset is taking several minutes. I can get it to work if I only use return, but if I try to assign the changes within the function itself it just keeps processing. Assigning outside of the function itself worked much faster. But alas, is not applying to multiple sets. 
...
add_freq <- function(data, column_name) { #function creation that takes a dataset and column name
  frequency_map <- table(data[[column_name]], useNA = "always") #variable created by taking given data and column, and always have NA included. Not including NA could alter the rows and lead to error or mismatch.
...
```{r}
# JML
# start simple: scatterplot of observed_readmission_rate vs Score_Death rate for pneumonia patients
ggplot(pneumoniaAnalyze,
      aes(x=`Score_Death rate for pneumonia patients`, y=observed_readmission_rate)       ) + geom_point(size=1.0)

# scatterplot better? it's easier to see the different values
ggplot(pneumoniaAnalyze,
  aes(x=`Score_Death rate for pneumonia patients`, y=observed_readmission_rate, 
  alpha=`ComparedToNational_Death rate for pneumonia patients`)) + 
  geom_point(aes(size=`ComparedToNational_Death rate for pneumonia patients`), color="darkred") +
  ggtitle("Transparency") +
  theme_light()

# scatterplot:lattice - I like this one!
pneumoniaAnalyze_CtoNDeathRate <- pneumoniaAnalyze  %>%
  mutate(pneumoniaAnalyze_CtoNDeathRatePhrase = case_when(
    `ComparedToNational_Death rate for pneumonia patients` == -1 ~ 'Death Rate Worse than National',
    `ComparedToNational_Death rate for pneumonia patients` == 1 ~ 'Death Rate Better than National',
    `ComparedToNational_Death rate for pneumonia patients` == 0 ~ 'Death Rate Equal to National'
  ))
print(pneumoniaAnalyze_CtoNDeathRate)

xyplot(`Score_Death rate for pneumonia patients` ~ observed_readmission_rate | `pneumoniaAnalyze_CtoNDeathRatePhrase`, data = pneumoniaAnalyze_CtoNDeathRate, pch=20, cex=1, col=rgb(0.2,0.4,0.8,0.5) )


# scatterplot matrix - meh
plot(pneumoniaAnalyze[, c(12, 13, 14, 15, 50)], pch=20, cex=1, col="#F2CA19")

# scatterplot matrix - meh
plot(pneumoniaAnalyze[, c(12, 13, 14, 15, 50)], pch=20, cex=1, col="darkred")
...
# basic histogram - informative/boring
ggplot(pneumoniaAnalyze,
       aes(x=observed_readmission_rate)) + geom_histogram(binwidth=0.1, fill="#69b3a2", color="#e9ecef", alpha=0.9)

# trying a ridgeline - interesting
library(ggridges)

ggplot(pneumoniaAnalyze_CtoNDeathRate,
       aes(x=observed_readmission_rate, 
           y=pneumoniaAnalyze_CtoNDeathRatePhrase,
           fill=`pneumoniaAnalyze_CtoNDeathRatePhrase`)) +
  geom_density_ridges() +
  theme_ridges() +
  theme(legend.position="none")
  
# can this become a faceted distribution? nope/ick/not right
library(viridis)

pneumoniaAnalyze_CtoNDeathRate %>%
  ggplot( aes(x=observed_readmission_rate, 
           y=pneumoniaAnalyze_CtoNDeathRatePhrase,
           group=pneumoniaAnalyze_CtoNDeathRatePhrase,
           fill=`pneumoniaAnalyze_CtoNDeathRatePhrase`)) +
  geom_area() +
  ...
  theme_light()   +
    ...
    facet_wrap(~pneumoniaAnalyze_CtoNDeathRatePhrase, scale="free_y")
```
