...
plot(pneumoniaAnalyze[, c(12, 13, 14, 15, 50)], pch=20, cex=1.5, col="#F2CA19")

# scatterplot matrix - meh
plot(pneumoniaAnalyze[, c(12, 13, 14, 15, 50)], pch=20, cex=1.5, col="darkred")
...
pneumoniaAnalyze_CtoNDeathRate <- pneumoniaAnalyze %>% select(`ComparedToNational_Death rate for pneumonia patients`,observed_readmission_rate)

pneumoniaAnalyze_CtoNDeathRate <- pneumoniaAnalyze_CtoNDeathRate %>%
  mutate(pneumoniaAnalyze_CtoNDeathRatePhrase = case_when(
    `ComparedToNational_Death rate for pneumonia patients` == -1 ~ 'Worse',
    `ComparedToNational_Death rate for pneumonia patients` == 1 ~ 'Better',
    `ComparedToNational_Death rate for pneumonia patients` == 0 ~ 'Equal'
  ))
