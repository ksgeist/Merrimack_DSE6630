---
title: 'Project 1 (Heart Failure): Biomedical & Clinical Informatics'
...
>MN: This was not working for me, so created a new fully encoded hf file from the answers demo file, created new HF answers fully encoded

```{r}
## Make any necessary changes in the R source code
## Uncomment to run!
#source(file = "reRunDemoData_hf.R")
...
```{r}
#Cleaning up mess I made from creating hf_AnalyzeFullyEncoded
rm(hf_Full, 
   hf_FullEncoded, 
...
load("FY2024_data_files/hf_AnalyzeFullyEncoded2024.Rdata")
dat2Analyze_hf <- hf_AnalyzeFullyEncoded
```
...
#load("FY2024_data_files/HF_AnalyzeNoEncoding2024.Rdata")
#HF_dat2Analyze <- HF_AnalyzeNoEncoding
```

#### **Question 1**: [1 point]
Which dataset do you choose and why?

> MN:As noted above, made a fully encoded dataset from answers demo due to having problems with encoding the no encoding dataset when trying to update for heart failure condition. 
...
if(!exists("HF_AnalyzeFullyEncoded")) {
    ...
    HF_dat2Analyze <- doOrdinalEncoding(df = HF_dat2Analyze,
...
Now that you can compare these groups more easily, what we're looking for is a balance between **clear difference** between hospital ordinality relative to the national average (i.e., "Are better hospitals really better in heart failure readmission rates?").

Which target(s) appear the most distinct to you?

> MN: To me there are two targets that seem the most distinct in the case of heart failure, those being Excess Readmission Ratio and Predicted Readmission Rate. While all 4 targets trend in the same directions, Excess and Predicted appear to have the most seperation between each other. 
In comparison to pneumonia, Observed and Predicted were the two possible targets that stood out to me.
...
> MN: I think this denisty plot I am even more convinced of the Predicted Readmission Rate. There are distinctions among the three groups, and shapes also vary. In the Excess Readmission, there is still seperation, butthe shapes of the plots are more similar.

### 2.4 What is the relationship of heart failure related hospital readmissions to the hospital performance metrics?

Something else we could do to help us make a decision is compare across predictors to look for the most striking relationships among the proposed target (readmission rate), how well compared to national average hospitals are performing AFTER readmitting heart failure patients (how long after readmission they are in the hospital), and relationships among other hospital-performance predictors, like Medicaid spending or heart failure-related deaths. 
...
```{r, echo = FALSE}
HF_longScore <- hf_AnalyzeFullyEncoded %>%
  rename(NatlComparisonHospitalDays = `ComparedToNational_Hospital return days for heart failure patients`) %>%
  ...
  `Score_Emergency department volume` = as.numeric(case_when(
    ...
    TRUE ~ as.character(`Score_Emergency department volume`) #changed code here, having immense problems with using same code as pneumonia for some reason
  ))) %>%
...
> MN: I believe that I feel my choice here is still Predicted Readmission Rate. While this target did not show the most separation,it did have some, and it also showed some linear relationships. The Expected target did show some steeper linear relationships, but not much separation among the groups. Observed we know is missing roughly 400 points of data. And the Excess target showed no real linear relationships, but some separation. So once again, I feel Predicted Readmission Rate is a good choice here.
...
HF_longScore %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of Predicted Readmission Rate heart failure related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
        y = "Predicted heart failure related Readmissions",
...
**HINT 3**: Here's how I would interpret the __relationship between PREDICTED heart failure readmissions and heart failure death rates, looking at whether hospitals performed better, same, or worse in terms of length-of-stay after readmission__. What I notice immediately is a moderate slope for hospitals with worse readmitted lengths of stay (LOS) [maroon line] compared to same [blue line] and especially better [yellow line]. This relationship is negative, implying that as the heart failure-related death rate increases, readmissions __goes down__. Oh! So what this implies is that hospitals with higher deaths from heart failure tend to have lower readmissions - and this is especially true for the ones who are have longer (worse than average) readmitted patient stays. In other words - these hospitals are __ probably releasing their heart failure patients too soon from their first hospitalization__, leading to higher readmissions, longer readmitted stays, and even higher heart failure-related deaths! 
...
```{r, echo = FALSE}
HF_longHCAHPS <- hf_AnalyzeFullyEncoded %>%
  rename(NatlComparisonHospitalDays = `ComparedToNational_Hospital return days for heart failure patients`) %>% 
...
> MN: I still feel that Predicted Readmissions Rate seems to have the best and most distinct graphs. SEaparation is showm among the hospital ratings, and all areas have a downward sloping trend. Improving any metric seem sto have am impact on patient readmissions. 
Something I do find interesting, that in the nurse communication metric, they looks like an upward trend in the "Better" hospitals. More investigating would need to be don ein that particular metric for those hospitals.

**HINT 1**: What does higher nurse and doctor communication mean for improved patient outcomes, including readmission?

**HINT 2**: Does cleanliness have an effect here?
...
> MN: Medicare spending again stands out here as there are noticeable upwards trends in that metric. I would like to know more about the doctor and nurse communication. The "Better" hospitals have a slight upward trend to them, so I'd like to know what is causing that. 
...
* We don't lose nearly 400 data points to missingness.
...
>MN: While I am not sure of your choice for heart failure patients, I again am going with Predicted Readmission rates for the target for heart failure.  There are still 400 missing points of data in the observed target.  A close second choice was the expected readmissions target. While this target did show more significant  linear realtionships than the predicted target, there was less distinction and nearly no separation among the three hospital rating categories. In the excess readmission target, while there was separation, there weren't any predictors that jumped out. 
The predicted readmissions as a target had the best criteria of each possible target, and also did not have missingness. 
...
> MN: After reading the article on bias, and then re-reading the summary of what the predicted readmissions target encompasses, I feel there are definitely some areas that could introduce bias. One area is if a hospital has a history of treating sicker patients or dealing with ongoing care challenges, the target might make those patterns into its predictions. Instead of adjusting for these differences, it could unintentionally keep those disparities going. Predictions using historical data may reinforce healthcare inequities rather than adjust for them, making it harder for hospitals to improve their performance if the model does not properly account for change over time. 
...
```{r}
dat2Analyze_hf <- dat2Analyze_hf %>% 
  ## Here we are un-selecting the features we no longer want using sample and number of patients. Also, taking out the other targets,
  select(-ExpectedReadmissionRate, 
         -ExcessReadmissionRatio,
         -observed_readmission_rate, 
         -contains(c("Sample_", "NumberOfPatients")),
         -NumberSurveysCompleted) %>% 
  ## Here we are changing some column names for conciseness
  ...
  ## Here we are moving our target variable to the front of the dataset and keeping everything else in place
  select(PredictedReadmissionRate, everything())

## Here were are taking the column names in our dataset and subbing anythign with score, HcahpsLinear.., and payment to be space or nothing
colnames(dat2Analyze_hf) <- gsub('Score_|HcahpsLinearMeanValue_|_Payment', '', colnames(dat2Analyze_hf))
```
...
> MN: Removing zero or near-zero variance features is important because they don’t add any useful information for analysis or predictions. If a column has almost the same value for every row, it can’t help a model distinguish between different cases. Keeping these features can slow down processing, clutter your dataset, and sometimes even confuse the model. By filtering them out, you make your analysis more efficient and focused on the variables that actually matter.


```{r, echo = F}
## Identify the columns with near-zero variance
zero_var_df_hf <- dat2Analyze_hf[, nearZeroVar(dat2Analyze_hf)]

## Print the columns
data.frame(names(zero_var_df_hf)) %>% 
  rename(`Variables Dropped` = names.zero_var_df_hf.) %>% 
  kable(
    format = "html",
...
dat2Analyze_hf <- dat2Analyze_hf[, -nearZeroVar(dat2Analyze_hf)]
```
...
> MN: FacilityID does not have predictive value, it is just a label that helps distinguish individual hospitals. If left in the dataset, functions like nearZeroVar() might misinterpret them as meaningful features rather than identifiers, which could distort variance assessments.
Additionally, keeping FacilityID as a column can clutter the analysis by introducing unnecessary data that doesn’t contribute to patient outcomes or hospital performance. By moving it to rownames, you ensure that only relevant variables are considered in statistical modeling, improving efficiency and accuracy.
...
> MN: Our dataset currently has 30 predictors (p) and 4833 observations(n). We do not meet the big-P, little-n problem here with these numbers, which will help to avoid overfitting when you have many more predictors than observations. 
...
```{r}
missing_hf <- colSums(is.na(dat2Analyze_hf))#count missing values per column

missing_cols_hf <- sum(missing_hf > 0)#columns with at least one missing value

percent_missing_hf <- (missing_cols_hf / ncol(dat2Analyze_hf)) * 100
missing_hf
missing_cols_hf
percent_missing_hf
...
> MN: 29 columns have missing data, and 96.667% of the features have some sort of missingness.

**Which column(s) are not missing any data?**

> MN: State is the only one without missing data, but will be removing that, so not really helpful. Essentially all columns are missing data

**How many complete observations do we have in the dataset?** You may find `nrow()` with `complete.cases()` or `drop_na()` useful!

>MN: 596 observations have complete data

```{r}
sum(complete.cases(dat2Analyze_hf))
...
miceMatrix_hf <- dat2Analyze_hf %>% select(-State) %>% md.pairs()
```
...
> MN: To me this means that 1641 values are missing. Compared to National death rate for heart failure patients has a missing value, and predicted readmission has a missing value in 1641 instances, which is good portion of our observations. This does seem like a high degree of co-missingness, which could indicate the pattern is not just random.
...
p_hf <- as.matrix(miceMatrix_hf$mr/(miceMatrix_hf$mr+miceMatrix_hf$mm))

## Set a color palette
prettyPurples_hf <- colorRampPalette(brewer.pal(8, "PuRd"))(8)

## Make a heatmap
heatmap(p_hf, 
        col = prettyPurples_hf, 
        cexRow = 0.5, 
        cexCol = 0.5, 
        margins = c(13, 13), 
        Colv = NA)
...
## Calculate proportion from pairwise pattern of missingness
p_hf <- as.matrix(miceMatrix_hf$mm/(miceMatrix_hf$mr+miceMatrix_hf$mm))

## Set a color palette
prettyPurples_hf <- colorRampPalette(brewer.pal(10, "PuRd"))(30)

## Make a heatmap
p_hf[1, ] %>% 
  data.frame(Proportion = .) %>% 
  mutate(Variable = rownames(p_hf),
  ...
  scale_fill_manual(values = (prettyPurples_hf)) +
  coord_flip() +   
  theme_classic() +
...
> MN: The columns that seem to be most correlated with missingness are:
- MRSA Bacteremia
- ComparedToNational MRSA Bacteremia
- Postop respiratory failure rate
...
- ComparedToNational Composite patient safety
- Death rate for heart failure patients
Yes this could cause concern as this pattern of missingness suggests non-random gaps in the data, which could introduce bias. If these specific variables are frequently missing when readmission rates are observed, it might mean that hospitals with certain characteristics or reporting standards are disproportionately affected. If hospitals with high readmission rates also tend not to report certain metrics, that could skew the data, making it harder to get an accurate idea of what’s driving readmissions.
This list is also slightly different than the pneumonia correlations, so that is also an interesting find. Also of note, the columns with most correlated missingness appear to be even more significant in the heart failure group.

## 3.3 Drop rows that are missing from the target variable. 

We will not be able to use those rows at all moving forward, sadly. That is going to reduce our sample size pretty sizably, from $N = 4,833$ to only $N = 2,635$ (or from $N = 4,833$ to only $N = 2,635$ .

```{r, echo = FALSE}
dat2Analyze_hf <- dat2Analyze_hf %>% 
  filter(!is.na(PredictedReadmissionRate)) 

paste0(c("Rows: ", "Columns: "), dim(dat2Analyze_hf))
```
...
> MN: I don't feel any differently. While there was a dimension reduction, I still do not feel that we meet the Big P little N problem. Our numbers are still greater than our predictors.
...
> AJ-  1.We partition the data into testing and training set so that we can train the model on one portion known as the training set and test how well it performs on new unseen data which is the testing set. It helps to  check whether the model is learning patterns or has memorized the given  data.
        2.Data leakage is when information from the testing set heavily influences the training process. Thus the model appears to be more accurate than it actually is solely because it is relying on the previously seen answers.
...
calcSplitRatio <- function(p = NA, df) {
  ...
  # If no value is provided for p, use  the number of predictors in the dataframe  (excluding the target variable)
  if(is.na(p)) {
    p <- ncol(df) -1   ## Subtract 1 assuming the last column is the target variable
  }
  
  ## use sqrt(p) rule to calculate the ideal number of test observations 
  test_N <- (1/sqrt(p))*nrow(dat2Analyze_hf)
  ## Convert count to a proportion of the dataset
  test_prop <- round((1/sqrt(p))*nrow(dat2Analyze_hf)/nrow(dat2Analyze_hf), 2)
  ## Calculate the training proportion
  train_prop <- 1-test_prop
  
  ## print the ideal split ratio
  print(paste0("The ideal split ratio is ", train_prop, ":", test_prop, " training:testing"))
  
  ## Return the training proportion
  return(train_prop)
}

## Fill in the blanks to run:
 train_prop <- calcSplitRatio(df = dat2Analyze_hf)
...
```{r}
 ind <- createDataPartition(dat2Analyze_hf$PredictedReadmissionRate,   ## Put the target here!
                            p = train_prop,           ## Use the object you just made!
                            list = FALSE)
 
 train <- dat2Analyze_hf[ind, ]
 test <- dat2Analyze_hf[-c(ind), ]
```
...
> AJ- The  split ratio is 81% for training, and 19% for testing which is very close to the canonical 80/20 split. This is because the formula used depends on the  ~ 30 predictors (columns) within the dataset which naturally gives a number close to 80%, leaving the remainder (18%) of the data  to test how well it works.
...
```{r}
# (JML)dont' use these because they are for pneumonia, not heart failure
...
```{r}
# JML I think this is just for pneumonia, not heart failure?
...
> AJ Since missForest was used to fill in missing data, MSE and PCF will reflect how accurate /inacurate  those imputations are.  MSE is used for numerical variables. It represents how far off the imputed values are from the true values and PCF is used for categorical variables. A higher MSE means the imputation was less accurate. It shows the proportion of imputed values that were misclassified. A PCF of 0 means perfect classification; a PCF near 0.5 suggests performance close to random guessing. The closer that  PCF is to 0 the better since PCF shows how often the model got it wrong when filling in categorical values, such as labels or groupings.
...
var(train$`ComparedToNational_Hospital return days for heart failure patients`, na.rm = T)
...
cols2drop <- c("Payment for heart failure patients", "Emergency department volume")
```
...
> AJ MICE (Multivariate Imputation by Chained Equations) is an advanced method for handling missing data by iteratively modeling each incomplete variable using the other variables in the dataset. Unlike simpler imputation methods such as mean or mode imputation, MICE preserves multivariate relationships and accounts for the uncertainty in missing data by generating multiple imputed datasets. It works well with both numerical and categorical variables and is particularly useful when data are missing at random (MAR).
...
> AJ The numeric variable was well-imputed, with the mean and median values remaining nearly unchanged after imputation, suggesting minimal distortion. However, the categorical variable showed a substantial shift: the imputation process heavily favored the neutral category (0), significantly altering the original class distribution. This indicates that while the numeric imputation performed well, the categorical imputation may have introduced bias and should be interpreted with caution.
...
> AJ-The FrequencyEncoding() helper function (from the source code) builds the frequency map from the training set only and then applies that exact map to the test set. If a category appears in the test set but not in the training set, the function assigns it a default frequency of 1. This default is a safe fallback that helps prevent model errors and ensures stability when encountering unseen categories. Using the same encoding map maintains consistency and avoids data leakage between training and testing
...
> AJ-Histograms and Q-Q plots are helpful tools for data exploring, but they’re not conclusive. For a more objective assessment of normality, visualizations should be paired  with statistical tests and summary measures.These include Shapiro-Wilk test (shapiro.test),Anderson-Darling test, Kolmogorov-Smirnov test (less sensitive), Skewness and kurtosis statistics for shape and Residual analysis in model diagnostics (for regression) and to assess normality assumptions.
...
> AJ-The Shapiro-Wilk test returned W = 0.99376 with a p-value ≈ 0.00000007422, which is far below the conventional significance level of 0.05. This means we reject the null hypothesis of normality, indicating that the distribution of the target variable PredictedReadmissionRate significantly deviates from a normal distribution. 
...
> AJ We apply the Box-Cox transformation to both the training and test sets using the same lambda to ensure consistency between them. This prevents data leakage and allows the model to make valid predictions, since it was trained on data transformed with that specific lambda. Using a different lambda for the test set would apply a different transformation, leading to mismatched scales and invalid evaluation.
...
> AJ-The highest potential for multicollinearity appears among patient experience measures such as Care.transition, Nurse communication,Doctor communication,Discharge information,Communication about medicine, Staff responsiveness,and Overall hospital rating. These variables are all conceptually related to patient satisfaction and care quality, in the heatmap they show strong positive correlations with each other which suggests that these variables may be capturing overlapping aspects of hospital communication and care transitions.
...
> AJ-The variable most strongly correlated with the target 'bc_PredictedReadmissionRate' is:Hospital return days for heart failure patients (r = -0.436): This strong negative correlation suggests that hospitals with more days between discharges and returns for heart failure patients tend to have lower readmission rates.Other variables with moderate negative correlations include:
Doctor communication (r = -0.358)
Discharge information (r = -0.355)
Communication about medicines (r = -0.311)
Quietness (r = -0.224) and Cleanliness (r = -0.188).
...
**HINT 2**: Is, for example,`Payment Category for heart failure patients` created from `Payment for heart failure patients`? [These are Medicare payments]? Which one do you think we should keep amd why?

> AJ-The highest VIFs are for Overall.hospital.rating (23.35) and Recommend.hospital (15.05), well above the common threshold of 10, indicating severe multicollinearity. This suggests these variables are highly correlated with other predictors in the model, potentially inflating standard errors and making coefficient estimates unstable.

Additionally, Nurse.communication, Care.transition, and Staff.responsiveness also have moderately high VIFs (above 5), which could indicate redundancy among patient experience variables. Consider dropping or combining some of these to reduce redundancy and improve model interpretability.
...
> AJ-The variable most highly correlated with the target 'bc_PredictedReadmissionRate' is Hospital.return.days.for.heart.failure.patients, with a negative correlation of approximately –0.44 suggesting that hospitals with more return days for heart failure patients tend to have lower predicted readmission rates. The other negatively correlated variables are  Doctor.communication, Discharge.information, and Communication.about.medicines, suggesting that better communication is associated with fewer readmissions. For positive correlation 'Medicare.spending.per.patient' (0.24) is the strongest  which may reflect complex/difficult or less efficient care. Overall,the direction of these correlations aligns with expectations about hospital quality and patient outcomes.
...
Because the goal is a deployable predictive model, we likely want something more granular than most of the `ComparedToNational` features can give us, with the __exception of our key predictor__, `ComparedToNational_Hospital return days for heart failure patients`. I will remind you of what we explored in Section 2 with this variable; not only that, we can see how **how highly correlated it is with our target!**.
...
### 4.3.2 Now drop all of the `ComparedToNational` and `PaymentCategory` features **except** `ComparedToNational_Hospital return days for heart failure patients`
```{r}
readyTrain <- readyTrain %>% 
...
1. At what principal component do we surpass 80% of explained variance? (**HINT**: look at the cumulative variance row rather than the proportion of variance row).

> (JML) PC11 where the cumulative variance surpasses 80%.
...
> (JML) PC1 explains 21% of the variance. PC19 is the final PC and it explains less than 1%. I tried to find some information about high v medium v low, but didn't find anything definitive or descriptive.
...
> (JML) I think PC1 and PC2 are the optimal PCs. PC2 is where the elbow starts to bend.
...
> (JML) Yep, not what I expected at all. Scree describes loose rock fragments at the base of a cliff, which would be at a different angle than the cliff itself. 
...
Additionally, we are going to color this by our grouping variable and key predictor in the dataset, `Hospital.return.days.for.heart.failure.patients`. Recall that when we explored this earlier this variable is the performance of each hospital in terms of its heart failure-related return-to-hospital days relative to the national average. In other words, hospitals that were "Worse than average" had readmitted heart failure patients with longer-than-average stays. 
...
> JML I found that changing the ellipse.alpha values changed the transparency of the fill of the ellipse, with lower values being more transparent and higher values being less transparent.  Personally, I liked a value of 0.2 the best. Changing the contrib values higher resulted in more loading labels being present on the biplot. Keeping the repel value set to TRUE helped keep the labels from printing over one another. I didn't modify the colors because I thought the combination was different enough to well highlight the groups. I am confused about how the shapes of the "points" are being set. I see the geom="point" so the circles make sense. I also see the habillage being set to the factor variable, but the reading I did for habillage mentioned that the color would be changed by the factors and didn't mention the point styles. Is it that combination that is giving the different groups the triangles & squares for the values? I changed the col.var to hotpink and decided that was just wrong haha.  For heart failure, I noted that the "better" group was mostly on the left half of the plot/ellipses, and that ``Doctor.communication`, `Communication.about.medicines`, `Discharge.Information`, `Quietness` and `Cleanliness` were the features that contributed the most.  I found I needed to add the arrows into the biplot so I could more easily answer what contrasts with `Communication.about.medicines`. However, I think that Doctor.communication might contribute more than `Communication.about.medicines`. Regardless `bc_PredictedReadmissionRate` is the loading that contrasts with the 5 features mentioned above. To me, this signals that the 5 features contribute to lower predicted readmission rates.
...
> (JML) I limited this code to just PC1 & PC2 since PC2 was at the elbow bend in the scree plot. In this case, eight variables contributed to the overall explanation of variance, five of which came from the HCAHPS data set. Three out of the top 4 variables relate to communication. The bc_PredictedReadmissionRate is in 9th place in terms of contribution and doesn't contribute to the variance.
...
> (JML) I think that 2 are sufficient (reduced complexity)
...
> (JML) The within sum of squares (assuming this is also called "Within Cluster Sum of Squares") is the sum of the variance between observations in each cluster (1), measuring the distance between each observation and the cluster center and performs a calculation. Looking at the elbow method, I believe that 2 is the optimal number of clusters, because the "bend" starts occurring there. Yes, this matches my answer for Q23A.(1)https://365datascience.com/tutorials/python-tutorials/k-means-clustering/
...
> (JML)The silhouette method does agree with the elbow method, identifying that the best number of clusters is 2. 
...
**HINT**: If these are back to the _original_ values, this is more interpretable for our stakeholder. We will now read this just like a typical boxplot; for example, median (the middle line) `Doctor Communication` seems higher in **Cluster 2** than it does in **Cluster 1** with only some overlap in the distributions (the boxes and whiskers don't really overlap each other). The table confirms this; the median `Doctor Communication` score is 91% in Cluster 2 but only 88% in Cluster 1.   

#### **Question 25**: [2 points]
Notice, for example, that Cluster 1 hospitals seem to include the hospitals that had the __lowest__ heart failure-related readmission rates, with the average rate ~2% higher in Cluster 2 hospitals. Thus, for our stakeholder, we might decide to rename Cluster 1 as "Highest Performing Hospitals" or something along those lines. Notice also that this cluster had more __below__ national average return hospital days (a "1"). This means that they had more readmitted heart failure patients who spent less time in the hospital upon readmission than the national average. 
...
#### **Question 26**: [1 point]
Make a recommendation for our client for 1-2 machine learning analyses __your team__ would choose to use to test whether these clusters can be used for robust predictions of hospital performance for heart failure patients. How would you know whether or not your predictions were robust? What would you look for or compare?

> (JML) Similar to the pneumonia project answer, my thoughts would be 1) decision-tree modeling because it is very easy to communicate the findings with non-data scientists and 2) xgboost because it is flexible and comprehensive.
...
> (JML) I do not think the requirements of OLS are met, although they are very close. The relationship between residuals and fitted values is not linear and does have a bit of a slope (rather than being horizontal). Although there appears to be Homosecadasticity in the residuals, there is a clumping of values near the (vertical) center of the plot, and the guideline is not linear. Regarding high leverage points, there are 3 potential outliers that stand out, with another 4 points that should be evaluated as outliers. The only OLS regression criteria that appears to be met is that the residuals are normally distributed, as seen on the Q-Q plot between the values of -2 and +2.
...
> (JML) In this case, yes, I do find support for linearity. The Q-Q plot shows values between -2 and +2 as being on the line, and the Shapiro-Wilks test calculates a p of 0.6523, which is above the 0.05 value typically used to identify a linear relationship.
...
> (JML) (similar answer from the pneumonia project) The unadjusted R-squared value is the proportion of variance for a dependent variable explained by an independent variable. The adjusted R-squared adjusts for add'l independent variables that are not significant to the model.  https://corporatefinanceinstitute.com/resources/data-science/adjusted-r-squared/
...
> (JML)Yes, I believe we need to undo the Box-Cox transformation to assess the RSME and R^2. Sorry I'm not sure about the answer to the rest of this question.
...
> (JML)We are doing cross validation to test the model against validation data and flag overfitting. The repeatedcv method splits the data into 5 folds and repeats the process 5 times to generate a more accurate estimate of model performance. We are using the grid search (default) method. Lambda = 0 means no shrinkage and is equivalent to OLS regression. Alpha = 0 means the regression is equivalent to OLS regression.
...
> (JML) Again, similar to the pneumonia project, both hyperparameters indicate that that elastic net is performing like an OLS regression model.The type of tuning search is grid and this can be seen from the plot in Figure 19.
...
> (JML) Per the feature importance plot, `Hospital return days for heart failure patients` is the most important feature. This matches what the most important feature is per the OLS regression.
...
> (JML) (also from the pneumonia project) Difference between PCA and regression is that PCA is unsupervised and subjective but regression is supervised and calculable. Importance in PCA identifies how much a component explains the variance but importance in regression involves determining how a change in a variable value changes in the model's coefficient values. Regarding the signs, they are not arbitrary. A positive loading value indicates that an increase in the feature value will increase the principal component, whereas a negative loading value indicates that an increase in feature value will decrease the principal component.
...
> (JML) To me, both models performed similarly and produced similar results, so determining the "better" predictive model may come down to efficiency and interpretibility. With that in mind, I'm going to say that the OLS regression model is the better model because it has fewer steps and lower computational costs.
...
> (JML) (again similar to the pneumonia project) Since it appears that OLS regression and Elastic Net are performing similarly, I'd recommend using the OLS regression because it is simpler to compute. Although tuning isn't done on the OLS regression model, the results are very similar to Elastic Net but has the benefit of not having the additional steps. Melissa also pointed out that, if new data is introduced, Elastic Net tends to perform better. If we consider that aspect, and add in the idea that we are trying to create a deployable model for the stakeholders, maybe the Elastic Net model could also be a valid choice.
