...
A higher MSE number indicates the guesses for the numerical values were less accurate.The closer PCF is to 0 the better since PCF shows how often the model got it wrong when filling in categorical values, such as labels or groupings.
...
```{r}
# Applying the Box-Cox transformation to the test set using the lambda from training
readyTest$bc_PredictedReadmissionRate <- predict(bc_data, readyTest$PredictedReadmissionRate)
...
readyTrain_scaled <- readyTrain %>%
  ...
  scale() %>%
  as.data.frame() %>%
  mutate(bc_PredictedReadmissionRate = readyTrain$bc_PredictedReadmissionRate)
