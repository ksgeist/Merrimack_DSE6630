...
> MN:Initially chose fully encoded because I thought it would save time, but further down the RMD, ran into an issue trying to use the fully encoded dataset, so ultimately went back and went with no encoding. Section 2.4 is where I ran into the problem.
...
> MN: To me there are two targets that seem the most distinct, those being Observed Readmission Rate and Predicted Readmission Rate. While all 4 targets trend in the same directions, Observed and Predicted appear to have the most seperation between each other. Caveat to this, is that we know we are missing 600 data points in our Observed target, which could casue some bias.
...
> MN: I think this denisty plot further confirms my previous thoughts. There is a distinct seperation between hospital performance among the three categories in the Observed and Predicted targets. Though, Observed is missing 600 points of data, which leads me more to the Predicted target. In the Excess and Expected targets, the distributions appears smoother and overlapping with each other, however, there is some distinction in the Worse than Average category for Excess Readmission. 
...
> MN: After much comparison, ultimately my choice was Predicted Readmission Rates. The reason for this choice is that, when compared to the three other possible targets, Predicted Readmission Rates showed the most separation amoung performance categories, while also having clear linear relationships with some predictors, such as Death Rate for pneumonia patients and Medicare spending per patient. There were also clear separation among worse performing hospitals in those araes as compared to same or better performing hospitals. There were also distinctions in separation in DVT and venous thrombolism prophylaxis.
Higher Medicare spending could potentially indicate a few things, patients could be sicker requiring longer stays, the majority population could be older, and/or hospital located in a lower income area requiring more Medicare spending costs.
Higher scores in DVT/thrombolysis could indicate that the hospital does not recognize these illnesses until much later, potentially not until more symptoms manifest. If a hospital is increasing medical testing to find the cause of illness, this coould also increase length of stay, which can also increase Mediacre spending. More tests -> longer stays -> more money.
...
longScore %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of Predicted Readmission Rate pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
> MN: My choice here was also Predicted Readmission Rate. This target variable showed not only distinct separation among the three hospital categories, but also showed linear relationships across all categories. The expected readmissions target was a close second but there was less distinction among the three rating categories.
Nurse and doctor communication can have improved patinet outcomes because that patient can be well informed about there condition, and post care protocols, to keep them healthy and out of the hospital. Cleanliness may not have too much of an effect because this could be individual preference among patients. We all have different opionions on what clean is.
...
> MN: I think what I find most compelling at this moment is the Medicare spending per patient. It seems the most distinct and notable value of all the others. There is a clear difference in the worst performing hospitals when compared to same or better performing hospitals. I think my question, or interest to know more, are the demographics of the Medicare patients. Is there an indicator, whether socioeconomic or demographic-wise metric that can be evaulated in regards to Medicare spending. For example, is a Medicare patient elderly and they can't hear or understand the doctor and nurse instructions? Are they a low-icnome patient and may not have easy access for follow-up care? Not sure if I'm reaching here, but are there relationships with Medicare spending and patient satisfaction metrics?
...
>MN: I feel that I do agree with choice of target. As stated in some above questions, for one, in the observed data target, there are 600 missing points of data, this could be impactful, and it would be helpful to have those points represented in the data. A close second choice was the expected readmissions target. While this target did show similar linear realtionships like the predicted target, there was less distinction and separation among the three hospital rating categories. In the excess readmission target, while there was separation, there weren't any predictors that jumped out. 
...
> MN: Our dataset currently has 31 predictors (p) and 4816 observations(n). We do not meet the big-P, little-n problem here with these numbers, which will help to avoid overfitting when you have many more predictors than observations. 
...
```{r}
missing <- colSums(is.na(dat2Analyze))#count missing values per column

missing_cols <- sum(missing > 0)#columns with at least one missing value

percent_missing <- (missing_cols / ncol(dat2Analyze)) * 100
missing
missing_cols
percent_missing
...
> MN: 31 columns have missing data, and 100% of the features have some sort of missingness.

**Which column(s) are not missing any data?**

> MN: Every column is missing data

**How many complete observations do we have in the dataset?** You may find `nrow()` with `complete.cases()` or `drop_na()` useful!
>MN: 644 observations have complete data
...
> MN: To me this means that 1788 values are missing. Postop respiratory failure rate has a missing value, and predicted readmission has a missing value in 1788 instances, which is almost half of our observations. This does seem like a high degree of co-missingness, which could indicate the pattern is not just random.
...
Could consider these too as well:
...
train <- dat2Analyze[ind, ]        ## Use the index to select training rows
test <- dat2Analyze[-c(ind), ]     ## Use the complement for testing rows
...
>AJ- The  split ratio is 82% for training, and 18% for testing which is very close to the canonical 80/20 split. This is because the formula used depends on the  ~ 30 predictors (columns) within the dataset which naturally gives a number close to 80%, leaving the remainder (18%) of the data  to test how well it works.
...
> Since  missForest was used to  fill in missing data, MSE and PCF relects how accurate/innacurate  those imputations are.
A higher MSE means the imputation was less accurate and the closer PCF is to 0 the better since PCF shows how often the model got it wrong when filling in categorical values, such as labels or groupings.
...
```{r}
"Hospital return days for pneumonia patients" %in% names(train)
...
> MICE (Multivariate Imputation by Chained Equations) is a method for filling in missing data by iteratively modeling each incomplete variable using the others in the dataset. It works by preserving relationships between variables more effectively than simple methods like mean or mode imputation. MICE also accounts for uncertainty in the imputed values and performs well with datasets that contain both numerical and categorical variables.
...
> AJ-The numeric variable was well-imputed, with the mean and median values remaining nearly unchanged after imputation, suggesting minimal distortion. However, the categorical variable showed a substantial shift, the imputation process heavily overestimated the neutral category (0) which  significantly altered the balance between the  classes. This suggests that while the numeric imputation performed well, the categorical imputation may have introduced bias and should be interpreted with caution.
...
readyTest <- imputedTest %>% mutate_if(is.factor, as.character) %>%  mutate_if(is.character, as.numeric)
```
...
> AJ-FrequencyEncoding() helper function (from source code) builds the frequency map from the training set only.It then applies that map to the test set. If a category exists in the test set but not in the training set, it assigns it a frequency of 1 which is considered to be a safe default.This approach prevents model errors and ensures stability in unseen categories. 
...
> AJ- The Shapiro-Wilk test returned W = 0.985 with a p-value = 0.00000000000003085 which is far below 0.05.The null hypothesis of normality is rejected because the distribution deviated significantly from normality.
...
> AJ-We apply the Box-Cox transformation to both training and test sets using the same lambda to maintain consistency, to prevent data leakage,and to ensure valid model evaluation.




```{r}
#  Applying the same Box-Cox transformation (using lambda from training data)
...
```{r}
# Save Box-Cox transformed target
train_target <- readyTrain$bc_PredictedReadmissionRate

# Drop original and transformed target, scale predictors
readyTrain_scaled <- readyTrain %>%
  select(-PredictedReadmissionRate, -bc_PredictedReadmissionRate) %>%
  mutate_if(is.factor, as.numeric) %>%
  scale(center = TRUE, scale = TRUE) %>%
  as.data.frame()

# Reattach scaled target (which you now also scale)
scaled_target <- scale(train_target, center = TRUE, scale = TRUE)
readyTrain_scaled$bc_PredictedReadmissionRate <- as.vector(scaled_target)
...
```{r}
# Save the Box-Cox transformed targets before scaling
train_target <- readyTrain$bc_PredictedReadmissionRate
test_target  <- readyTest$bc_PredictedReadmissionRate

# Prepare training data for scaling (drop targets and convert to numeric)
train_scaled_input <- readyTrain %>%
  select(-PredictedReadmissionRate, -bc_PredictedReadmissionRate) %>%
  mutate_if(is.factor, as.numeric) %>%
  mutate_if(is.character, as.numeric)

# Get training scaling parameters
train_means <- colMeans(train_scaled_input, na.rm = TRUE)
train_sds   <- apply(train_scaled_input, 2, sd, na.rm = TRUE)

#  Scale the test data using training parameters
test_scaled_input <- readyTest %>%
  select(-PredictedReadmissionRate, -bc_PredictedReadmissionRate) %>%
  mutate_if(is.factor, as.numeric) %>%
  mutate_if(is.character, as.numeric)

readyTest <- test_scaled_input %>%
  scale(center = train_means, scale = train_sds) %>%
  as.data.frame() # ensure it stays a dataframe

# Reattach the Box-Cox transformed targets
readyTrain <- train_scaled_input %>%
  scale(center = TRUE, scale = TRUE) %>%
  as.data.frame # <-FIXED


readyTrain$bc_PredictedReadmissionRate <- scale(train_target, center = TRUE, scale = TRUE)[, 1]
readyTest$bc_PredictedReadmissionRate  <- scale(test_target, 
                                                 center = mean(train_target), 
                                                 scale = sd(train_target))[, 1]
...
=======
```{r, eval=FALSE}
# First: extract and scale using training 
readyTest_scaled <- readyTest %>%
  select(-bc_PredictedReadmissionRate) %>%
  scale(center = attr(scale(readyTrain %>% select(-bc_PredictedReadmissionRate)), "scaled:center"),
        scale  = attr(scale(readyTrain %>% select(-bc_PredictedReadmissionRate)), "scaled:scale")) %>%
  as.data.frame() %>%
  mutate(bc_PredictedReadmissionRate = readyTest$bc_PredictedReadmissionRate)

```
>>>>>>> 5f173e2b21fd16beaa84077da60305cb16355885
...
> AJ-Nurse communication,Communication about medicines,Staff responsiveness,Doctor communication,Discharge information and Overall hospital rating.



Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

AJ-  ComparedToNational_Hospital.return.days.for.pneumonia.patients(0.50) due  readmission rates and return days  likely tied to overall patient recovery and discharge effectiveness.

Staff.responsiveness (0.39),Nurse.communication (0.36), Doctor.communication (0.34) — communication and responsiveness variables — suggests better patient interactions may be reducing complications that lead to readmissions.


```{r}
correlations <- cor(readyTrain_scaled, use = "complete.obs")
target_corr <- correlations[, "bc_PredictedReadmissionRate"]
sorted_corr <- sort(abs(target_corr), decreasing = TRUE)
sorted_corr[2:6]  # Exclude the target's self-correlation (value of 1)

```

> AJ-The variable most correlated with the target bc_PredictedReadmissionRate is ComparedToNational_Hospital.return.days.for.pneumonia.patients with a correlation of approximately 0.50, indicating a moderate to strong positive relationship. This suggests that hospitals performing worse than the national average in this metric tend to have higher predicted readmission rates.Patient experience variables such as Staff.responsiveness, Nurse.communication, and Doctor.communication show moderate positive correlations (ranging from ~0.34 to 0.39). This implies that hospitals with poorer patient communication and responsiveness may also experience higher readmission rates. Lastly, PaymentCategory.for.pneumonia.patients also shows notable correlation, possibly reflecting financial or systemic differences that influence outcomes..
...
> AJ-Multicollinearity does not appear to be a major issue in this dataset, as all of the top Variance Inflation Factors (VIFs) are well below the common thresholds of 5 or 10.Overall hospital rating may have been derived from other HCAHPS items, such as Doctor communication, Staff responsiveness, and Cleanliness which could introduce artificial multicollinearity. If Payment Category for pneumonia patients is derived from 'Payment for pneumonia patients',the original continuous variable (Payment for pneumonia patients) should be kept because  continuous data retains more nuance and predictive signal, while categories can dilute variation and reduce model flexibility.
...
> AJ-The variable ComparedToNational_Hospital.return.days.for.pneumonia.patients is the most highly correlated with our target bc_PredictedReadmissionRate, showing a positive correlation of approximately 0.50. If patients are returning to the hospital sooner or more frequently, it's likely indicative of poorer outcomes and higher readmission rates and does make sense. Additionally, communication-related variables such as Staff.responsiveness, Nurse.communication, and Doctor.communication also show moderate positive correlations (ranging from ~0.34 to 0.39), which suggests that better hospital communication may be associated with fewer readmissions.
...
> AJ-The cumulative explained variance is first surpassed 80% treshold at Principal Component 10. The first 10 components are sufficient to retain most of the variability in the original dataset, and dimensionality could be reduced to these components without significant information loss.
=======
...
<<<<<<< HEAD
> PC1 captures the largest chunk of meaningful variation (22.41%) and is thus considered an informative component. However, since it does not exceed 50%, it suggests that the dataset's variance is distributed more evenly across many dimensions and  multivariable datasets for  hospital performance metrics.PC1 explains 22.41% of the variance, while the final principal component explains only 1.06%, The proportion of variance explained by PC1 is considered medium.
=======
> (JML) PC1 explains 22% of the variance. PC18 is the final PC and it explains just 1%. I tried to find some information about high v medium v low, but didn't find anything definitive or descriptive.
>>>>>>> 5f173e2b21fd16beaa84077da60305cb16355885
...
<<<<<<< HEAD
> AJ- 1st drop is at PC2, PC1-PC2 are optimal 
...
> It's a geological term, the shape resembles a rocky slope or debris pile (scree) found at the base of a mountain. The steep drop followed by a flattening mirrors how eigenvalues decrease in PCA. 
=======
...
fviz_pca_biplot(pca, 
                ...
                col.var = "black",
                habillage = returnHospitalComparison,
                select.var = list(contrib = 5),  # ← Shows top 5 contributing variables
                repel = TRUE)
...
#  Compute the squared loadings for PC1 and PC2 (contribution to variance)
squared_loadings <- loadings[, 1:2]^2

#  Sum squared loadings across PC1 and PC2
contributions <- rowSums(squared_loadings)

# Sort and get the top 5 contributing variables
top5_vars <- sort(contributions, decreasing = TRUE)[1:10]

#  Print results as a named vector
print(top5_vars)
...
> AJ- In the biplot, variables such as composite patient safety  and Postoperative.respiratory.failure.rate contrast with Communication.about.medicines. These load heavily in the opposite quadrant, suggesting that hospitals performing poorly on these metrics tend to have worse communication scores and worse pneumonia-related return outcomes. Please see top 10 charts above.
=======
...
<<<<<<< HEAD
>AJ- For hint #1- The top contributing variables are primarily drawn from patient experience and safety datasets, particularly HCAHPS and the Complications and Deaths datasets. This suggests that patient communication and safety indicators are key drivers of variation among hospitals, reflecting both how patients perceive their care and the prevalence of complications within hospitals. For hint #2- The target variable (PredictedReadmissionRate)is not a direct input into the PCA since it’s an unsupervised method. However, it is likely influenced—directly or indirectly—by communication and safety metrics, suggesting that these features are valuable predictors to include in downstream modeling.


=======
> JML Seven variables contribute to the overall explanation of variance, and five of these came from the HCAHPS data set originally. Most of the contributing variables relate to communication. Based on the definition about the dotted red-line, it looks like bc_PredictedReadmissionRate doesn't contribute to the variance explanation? Is that because it is orthogonal to communication.about.medicines? 
>>>>>>> 5f173e2b21fd16beaa84077da60305cb16355885
...
> (JML) Maybe 3 is good? With any more than that, the clusters seem to pile one on top of the other (and make interpretibility challenging)?
...
> (JML) The within sum of squares (assuming this is also called "Within Cluster Sum of Squares") is the sum of the variance between observations in each cluster (1), measuring the distance between each observation and the cluster center and performs a calculation. Looking at the elbos method, I believe that 2 is the optimal number of clusters, because the "bend" starts occurring there. No, this doesn't match my answer to Q23A, but it's not far off either.(1)https://365datascience.com/tutorials/python-tutorials/k-means-clustering/
...
kmeansResult <- kmeans(readyTrain, 	
                         centers = k,      ## k=2 and is the optimal # of clusters based on silouette method
                         nstart = 50,      
                         iter.max = 1000,  
                         algorithm = "MacQueen")
...
kmeansTrain %>% 
  ...
  tbl_summary(by = "Cluster",
              statistic = list(all_continuous() ~ c("{mean} ({median}) {var}"))) %>% 
...
"Postoperative respiratory failure rate", "These hospitals have a lower rate of postoperative respiratory failure, suggesting that these hospitals may be better prepared to handle respiratory crises",
"Serious blood clots after surgery", "These hospitals have a lower rate of post-surgical blood clot complications, suggesting these hospitals work hard to prevent these events",
...
dict %>% 
  kable(
    ...
    pack_rows("2: Hospitals with strong patient feedback", 5, 8) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> (JML) My thoughts would be 1) decision-tree modeling because it is very easy to communicate the findings with non-data scientists and 2) xgboost because it is flexible and comprehensive.
...
> (JML)My impression is that the OLS assumptions are close to being met but aren't met. The residuals are normally distributed according the the Q-Q plot. There is almost a linear relationship betwee residuals and fitted values. There isn't obvious heteroscedacity in that neither ends of that plot are flared, but there is a clumping of values in the middle. There are at least 3 observations that should be reviewed for high influence/leverage.
...
> (JML) This is so close to being linear, I'm not sure "how" close is good enough, though.
...
> (JML)Both hyperparameters indicate that that elastic net is performing like an OLS regression model.The type of tuning search is grid and this can be seen from the plot in Figure 19.
...
> (JML) Per the feature importance plot, `Hospital return days for pneumonia patients` is the most important feature. The most important feature per OLS regression was Communication About Medicines (per the contributions of variables to Dmin 1-2 plot). I think.
...
> (JML)I'm not sure but I think I'd choose the OLS regression because it is simpler?
...
> (JML)Since it appears that OLS regression and Elastic Net are performing similarly, I'd recommend using the OLS regression because it is simpler to compute. Although tuning isn't done on the OLS regression model, the results are very similar to Elastic Net but has the benefit of not having the additional steps.                                                                                                                                                                                                                                                                                                                                                                                                                                                           
