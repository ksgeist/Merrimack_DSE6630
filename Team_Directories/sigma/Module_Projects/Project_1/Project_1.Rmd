...
A higher MSE number indicates the guesses for the numerical values were less accurate and the closer PCF is to 0 the better since PCF shows how often the model got it wrong when filling in categorical values, such as labels or groupings.
...
> AJ- The Shapiro-Wilk test returned W = 0.985 with a p-value = 0.00000000000003085 which is far below 0.05.The null hypothesis of normalityis rejected because the distribution deviated significantly from normality.
...
> (JML) Per the feature importance plot, `Hospital return dayes for pneumonia patients` is the most important feature. The most important feature per OLS regression was Communication About Medicines (per the contributions of variables to Dmin 1-2 plot). I think.
