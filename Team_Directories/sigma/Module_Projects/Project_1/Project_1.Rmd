...
> AJ-1.We partition the data into testing and training set so that we can train the model on one portion known as the training set and test how well it performs on new unseen data which is the testing set. It helps to  check whether the model is learning patterns or has memorized the given  data. 2.Data leakage is when information from the testing set heavily influences the training process. Thus the model appears to be more accurate than it actually is solely because it is relying on the previously seen answers.
...
> AJ-Since  missForest was used to  fill in missing data, MSE and PCF relects how accurate/innacurate  those imputations are.
...
> AJ- MICE (Multivariate Imputation by Chained Equations) is a method for filling in missing data by iteratively modeling each incomplete variable using the others in the dataset. It works by preserving relationships between variables more effectively than simple methods like mean or mode imputation. MICE also accounts for uncertainty in the imputed values and performs well with datasets that contain both numerical and categorical variables. van Buuren, S., & Groothuis-Oudshoorn, K. (2011). mice: Multivariate imputation by chained equations in R. Journal of Statistical Software, 45(3), 1–67. https://doi.org/10.18637/jss.v045.i03.
...
> AJ- It's a geological term, the shape resembles a rocky slope or debris pile (scree) found at the base of a mountain. The steep drop followed by a flattening mirrors how eigenvalues decrease in PCA. 
