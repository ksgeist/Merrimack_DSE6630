...
filepath <- "C:/Users/mnoon/OneDrive/Desktop/Datasets/hospitals_10_2024/"
...
cleanUpReadmissions_hf <- function(df) {
...
## Run the function
readmissionsClean_hf <- cleanUpReadmissions_hf(FY_2024_Hospital_Readmissions_Reduction_Program)

############## This is where you will filter for whatever condition(s) you are choosing to focus on!
## Filter for just pneumonia:
readmissionsClean_hf <- readmissionsClean_hf %>% 
...
datList_hf <- list(Healthcare_Associated_Infections,
                paymentOnly,
...
filterList_hf <- list("MRSA Bacteremia", 
...
tidyNjoin_hf <- function(datList_hf, filterList_hf, hospitalInfo) {
  ...
  for(i in 1:length(datList_hf)) {
    ...
    dat_hf <- dat_hf[which(dat_hf$MeasureName %in% filterList_hf[[i]]), ]
    ### Pivot wider
    wide <- pivot_wider(dat_hf, names_from = "MeasureName", 
                        values_from = 2:ncol(dat_hf))
  ...
  df <- full_join(df, readmissionsClean_hf, by = "FacilityId") 
...
hf_Full <- tidyNjoin_hf(datList_hf, filterList_hf, hospitalInfo)
...
cols2encode_hf <- colnames(temp_hf)
...
for (i in names(temp_hf)) {
  print("Before encoding:")
  print(table(temp_hf[[i]], useNA = "always"))
  temp_hf[[i]] <- ifelse(grepl("^No|^Average", temp_hf[[i]]), 0, 
                      ifelse(grepl("^Better|^Fewer Days", temp_hf[[i]]), 1,
                             ifelse(grepl("^Worse|^More Days", temp_hf[[i]]), -1,
                                    NA)))
...
for (col in 1:length(cols2encode_hf)) {
  hf_Full[, cols2encode_hf[col]] <- temp_hf[, cols2encode_hf[col]]
}
...
hf_AnalyzeFullyEncoded <- hf_Full %>% 
...
################# End my answer to Question 17

################# MY ANSWER TO QUESTION 18:

## Choose the columns I want to encode
cols2encode_hf <- c("State", 
                 "CountyParish")

## Move them to a temporary data frame
temp_hf <- hf_AnalyzeFullyEncoded[, names(hf_AnalyzeFullyEncoded) %in% cols2encode_hf]

## Create a function that will do the frequency encoding
add_freq_hf <- function(data, column_name) {
  ## Creates a map using the table() function for each category
  frequency_map_hf <- table(data[[column_name]], useNA = "always")
  ## Then goes to the column and matches the category to its frequency from the map
  data[[column_name]] <- frequency_map_hf[match(data[[column_name]],
                                             names(frequency_map_hf))]
  return(data)
}

## Runs the add_freq() function on each column I have chosen
for (col in names(temp_hf)) {
  temp_hf <- add_freq_hf(temp_hf, col)
}

## For each of the columns in the dataset, replaces it in the original dataset
for (c in 1:length(cols2encode_hf)) {
  hf_AnalyzeFullyEncoded[, cols2encode_hf[c]] <- temp_hf[, cols2encode_hf[c]]
}
################# End my answer to Question 18

#########################################
## Now, let's make pneumoniaAnalyze:
hf_AnalyzeFullyEncoded <- hf_AnalyzeFullyEncoded %>% 
  select(-CountyParish)
...
if ("Score_Percentage of healthcare personnel who completed COVID-19 primary vaccination series" %in% names(hf_AnalyzeFullyEncoded)) {
  hf_AnalyzeFullyEncoded <- hf_AnalyzeFullyEncoded %>% 
    select(-`Score_Hospital return days for heart failure patients`, 
...
save(hf_AnalyzeFullyEncoded, file = "FY2024_data_files/hf_AnalyzeFullyEncoded2024.Rdata")
