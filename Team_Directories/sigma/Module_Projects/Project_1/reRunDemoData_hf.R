...
readmissionsClean_HF <- readmissionsClean %>% 
...
filterList <- list("MRSA Bacteremia",
                 ...
                 "Abdomen CT Use of Contrast Material", 
                   ...
                   c("Death rate for heart failure surgery patients", 
...
tidyNjoin <- function(datList = datList, 
  ...
  ## Prep for joining
  temp <- readmissionsClean_HF %>%   ## Can pull from global environment
...
## To run the function
HF_Full <- tidyNjoin(datList, filterList, hospitalInfo, quiet = F)

## Save file for students:
save(HF_Full, file = "FY2024_data_files/HF_Full2024.Rdata")
## Student load file:
load("FY2024_data_files/HF_Full2024.Rdata")
...
HF_AnalyzeNoEncoding <- HF_Analyze %>% 
...
HF_AnalyzeNoEncoding <- cbind(HF_AnalyzeNoEncoding, columns2encode) 
...
if ("Score_Percentage of healthcare personnel who completed COVID-19 primary vaccination series" %in% names(HF_AnalyzeNoEncoding)) {
  HF_AnalyzeNoEncoding <- HF_AnalyzeNoEncoding %>% 
...
## Save file for students:
save(HF_AnalyzeNoEncoding, file = "FY2024_data_files/HF_AnalyzeNoEncoding2024.Rdata")
## Student load file:
load(file = "FY2024_data_files/HF_AnalyzeNoEncoding2024.Rdata")
