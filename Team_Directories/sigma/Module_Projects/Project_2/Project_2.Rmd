...
> AJ- These files are a  family of files and  work together as a set of components that define both the geometry (shapes and locations) and attributes (names, codes, and metadata) of geographic features for counties in California  and  must keep them together in the same folder for your GIS or R software to read the shapefile correctly:

> 1. `cnty19_1.cpg` – Specifies the character encoding (e.g., UTF-8) used for reading the text in the .dbf file. It ensures special characters or multilingual text are interpreted correctly.

> 2. `cnty19_1.dbf` – The attribute table stored in dBase format, holding all the non-spatial data—like county names, population values, or other metadata—associated with each shape.

> 3. `cnty19_1.prj`-Stores the projection and coordinate system information used in the spatial data. This is what allows the map to be placed accurately on a globe or another reference system (e.g., WGS84, NAD83, etc.).

> 4. `cnty19_1.shp`-The main file that stores the geometry (points, lines, or polygons) of the spatial features and also the boundary outlines of California's counties.

> 5. `cnty19_1.shp.xml`-This is a metadata file in XML format. It contains information about the dataset such as who created it, when, and provides descriptions of the fields and is mainly used for documentation purposes.

> 6. `cnty19_1.shx`-This index file allows software to efficiently locate each shape in the .shp file with the records in the .dbf file and also  speeds up access to the geometry data. 
...
```{r, eval = FALSE}
read_sf("/Users/aleej/Desktop/California_County_Boundaries/cnty19_1.shp") 
...
```{r}
cali_sf <- read_sf("C:/Users/Sean/Desktop/California_County_Boundaries/cnty19_1.shp")
...
> AJ-TRUE printed 69 times because the shapefile contains 69 geographic features, each representing a polygon (or multiple polygon) that maps a spatial area. Since California has only 58 counties, it is likely that some counties are represented by multiple polygons especially those with island or non-contiguous regions, such as parts of San Francisco, Marin, or Los Angeles Counties.
...
xmax: -12704980  
ymax: 5162403
The bounding box defines the spatial extent of the California shapefile in the WGS 84 / Pseudo-Mercator projection and  adjusting it lets you zoom in or out of your map. It tells us the rectangular boundary that contains all 69 geometries which includes counties and islands and any spatial feature outside this box will not be displayed.
...
> AJ-The file uses the WGS 84 / Pseudo-Mercator projection, which is ideal for web mapping due to its simplicity and 2D structure, though it's known to  distort area and distance, especially at larger scales. Projection defines how spatial data is transformed from the Earth’s curved surface to a flat map. It determines the coordinate system whether the measurements are in degrees, meters and feet etc.and affects how shape, area, and distance are represented.  Changing the projection alters the spatial reference system and can impact how your data looks and behaves: Coordinate values will differ (e.g., from meters to degrees), and map features may shift slightly depending on the distortion characteristics of the new projection.  Therefore, it’s important to choose a projection suited to your region and analysis type more and also that  preserves area and distance relationships more accurately for regional analysis.
...
map1 <- cali_sf %>%
  ...
  ggplot() +
    ...
    labs(
      title = "Northernmost California Counties: Del Norte, Siskiyou, Modoc",
      caption = "Data source: California GIS Portal"
    )
...
```{r}
## Add San Jose to the cities data frame
cities <- data.frame(
...
cities_sf <- st_as_sf(cities, coords = c("lon", "lat"), crs = 4326) %>%
  st_transform(crs = st_crs(cali_sf))

## Update color palette to include a color for San Jose
skittles <- c("#E74C3C", "#F1C40F", "#2ECC71", "#3498DB", "#9B59B6")

## Plot the map with updated cities
ggplot(data = cali_sf) +
  geom_sf(fill = skittles[5], color = "gray20", linewidth = 0.25, alpha = 0.65) +
  coord_sf(expand = FALSE) +
  geom_sf(data = cities_sf,
          aes(color = city, shape = cityType, size = cityType)) +
  scale_shape_manual(values = c(capital = 18, major = 16), guide = "none") +
  scale_size_manual(values = c(capital = 6, major = 3), guide = "none") +
  scale_color_manual(values = skittles, name = "City") +
  labs(title = "California Counties with Major Cities Marked (Now Including San Jose)",
       caption = "Source: California State Geoportal")
...
## Total monthly mortality data by county and year
mortality_sf <- totalDeaths %>% 
  ## Rename County column  to match shapefile’s COUNTY_NAM field
  rename(COUNTY_NAM = County) %>%
  ## Group by county and time to  get total deaths per county per month/year
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## Summarize total deaths counts within each group, removing any NA values
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## Perform a full join with the shapefile using COUNTY_NAM as the key
  ## relationship = "many-to-many"' ensures that multiple geometries per county are preserved in the merge
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Convert the joined table back into an sf object so it retains spatial features
  st_as_sf() %>% 
  ## Filter out separate island geometries to reduce clutter in maps,keep only rows where ISLAND is NA
  filter(is.na(ISLAND))
```
...
> AJ-`st_join()` performs a spatial join by  merging  attributes from one sf object to another based on a spatial predicate (e.g., intersection, contains). It is not designed for simple attribute-based joins. In this case, since we are matching on the county name column rather than on spatial relationships, `full_join()` or another dplyr join is appropriate. Since your mortality data is a non-spatial tibble and being matched based on a shared attribute (COUNTY_NAM), a dplyr join is what is  needed. Spatial joins like `st_join()` only come into play when both data layers have geometries and  their physical relationships (e.g., containment, distance, intersection) can be rely on.
...
```{r}
## Count the number of features in the spatial object
nrow(mortality_sf)
```


2. Why is it so many more than in `cali_sf`?
> AJ-`Mortality_sf` has many repeated records for each county across time since it’s a spatiotemporal table: it includes one row per county for each month across each year in the  mortality data (e.g., 58 counties × 12 months × 4 years = up to 2,784 records). In contrast, `cali_sf` has just one row per spatial feature (county polygon or island part). This time dimension multiplies the number of rows.`cali_sf` contains 69 geometry features, reflecting California’s 58 counties plus extra polygons for islands or non-contiguous areas.The time dimension in mortality_sf significantly multiplies the number of rows.
...
> AJ-In Question 2, 69 features were identified because those were geometry parts (counties plus separate island polygons), whereas here the count is  unique to county names.
...
> AJ-The pattern of highest total deaths in Los Angeles County and similarly high totals in other populous urban counties like San Diego, San Francisco, and Sacramento reflects population size rather than necessarily higher mortality risk. The raw totals are driven by how many people live in a large county metro areas and will always show darker colors. There’s a potentially misleading pattern when interpreting total deaths without adjusting for population size.Total deaths is not meant as indicating “worse” outcomes without adjusting for population or age structure. To make the maps more informative, compute per-capita (or age-standardized) mortality rates so to compare risk rather than sheer counts (e.g., deaths per 10,000 people) to show true spatial disparities and not just population effects.
...
key <- "aabaee4339a0e6eaa48e4435f58e46f49687ca2a"
```
...
> AJ-`DP1_0048C` gives you the raw number, while `DP1_0048P` gives you the proportion relative to the total population but both refer to the same demographic group (male population aged 65 years and over). This distinction is important depending on whether  analyzing absolute population size or demographic composition.
...
> AJ-The census dataframe contains county level demographic data for all California counties which has been sourced from the 2020 Decennial Census, specifically the Demographic Profile (DP) tables. It includes nine numerical variables that describe population counts and percentages. This dataset provides a valuable demographic snapshot that is ideal for normalizing mortality data and exploring age-specific public health trends..
...
> AJ- Overall, the structure of the function from building the URL and sending the request, to parsing the JSON response and cleaning the results remains consistent across APIs, regardless of the domain (e.g., Census, weather, finance). However, the specific details such as the base URL, query parameters, and response format—must be adapted to align with the design of the new API.
...
```{r}
## Standardize the join column
census <- census %>% mutate(COUNTY_NAM = as.character(COUNTY))
mortality_sf <- mortality_sf %>% mutate(COUNTY_NAM = as.character(COUNTY_NAM))

## Join the datasets by FIPS code
mortality_sf2 <- mortality_sf %>%
  left_join(census, by = "COUNTY_NAM")
...
load("C:/Users/Sean/Desktop/mortality_sf2.Rdata")
#load("C:/Users/mnoon/OneDrive/Desktop/mortality_sf2.Rdata")
load("census.Rdata")
```
...
> MN: One pattern I see from the map is that as we get to 2020, the previous years maps have darker coloring overall, indicating to me that 2017-2019 have higher all-cause Annual Percent Mortality. One would want to believe thought that 2020 should be higher overall with the increased deaths due to COVID.
...
> MN: I think that if we had a full 12 months of data, then that would increase accuracy because you are looking at the full comparison of other years plus a full year of 2020. Perhaps comparing monthly rates would provide a more complete picture.
...
> MN: I found these plots interesting. 2017-2019 do not have too much change, aside from that one dark purple county in 2018. Looking at 2020 though, there does seem to be some dareking of the map, specifically the Southern California area, while mid to Northern California tends to remina unchanged or just a few counties that have darkened in color.
...
> MN: Looking at my model plots, there are a lot of things that wouldlead me to belive this is not a robust model.The Residuals vs Fitted plot has a few high leverage points or outliers that may be effecting the results. Although it looks like the plots are fairly evenly scattered otherwise.Looking at the Q-Q plot, a majority of our points follow our dashed line, but the tails on both ends are quite far off, with the high leverage points causing deviation.Scale-Location plot is some ot ideal patterns. The red line is curved, and there appears to be some deviation from homoskedascity. The final plot, Residulas vs Leverage, we see those same outliers again. I think we could make this more robust by using some spatial regressions.
...
> MN: The dimensions of mortality_sf2 is 2552 rows by 5 columns. 2552/58 = 44. We want to calculate just based on distinct neighbors, 2552 rows means that each county appears 44 times. We would create redundancy if we calculated that many times for the counties. Also, if calculating on that much redundant info, it would take longer and use much more space than needed.
...
> According to this comparison, it would seem that the OLS model is the better fit. I wanted it to be the sarModel, so I feel either I did not calculate correctly, or the OLS model really is better. OLS included county, so that could be the driving factor, while the sarModel used averages.
...
> MN: Using the olsModel, the R-squared is 0.723 which means this models explains 72.3% of variance, suggesting a strong fit. Alipne and Mono counyies had the least mortality rates, while Shasta and Tuolumne had the highest. We can also see that Year and Month had significance. All other variables equal, Year has a 0.72 increae in mortality.
...
> MN: I feel that the olsModel is a good baseline model based on the higher R-squared result, and statistically significant coefficient results, but there is room for improvement. There is no spatial autocorrelation showing how counties influence each other, and there could be omitted variables underlying county name. Therefore, I don't feel we can entirely trust this model.
...
```{r}
round(100 * c(
  train <- nrow(mortalityTrain) / (nrow(mortalityTrain) + nrow(mortalityTest)),
  test <- nrow(mortalityTest) / (nrow(mortalityTrain) + nrow(mortalityTest))
), 2)
```

> MN: With a train/test spli of 81.82 and 18.18, this is very close to 80/20, and I feel this is still a safe split ratio to continue.

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> MN; While 2020 was a stand-out year and far from typical, leaving it out could provide some limitations. Reduced training set size which would mean weaker estimations of seasonal trends, and if the model only learns trends prior to 2020, it could under predict mortality in the pandemic era data.
...
> MN: Out-of-bag error is a technique to check how good model is without having a separate test set. It can give a quick reference for model performance, and helps to avoid overfitting.
...
> MN: The mtry is number of variables selected at each split to find the best one. The default mtry for an RF regression is the number of predictors divinded by 3. https://parsnip.tidymodels.org/reference/details_rand_forest_randomForest.html
We are trying to overide it because we want to find the best spot between bias and variance. If the default mtry was left alone, it would only use 1.
...
> MN: Mtry of 1 provided the best results. R-squared values should not be negative. In question 16 my residual standard error was 0.01449, which lower than oob RF of 0.98538, however, the olsModel was not centered and scaled, so I don't believe we can make a direct comparison.
...
> MN: In this CV model, we no longer see negative R-squared. The best mtry again looks to be 1. The RMSE and R-squared are SLIGHTLY lower than the OOB, which suggests small improvements. If both models showed mtry of 1 as the bestoption, then I think that is a good indicator it's the best choice.
...
> MN: There does not seem to be a big gap between training and test errors, so I don't think overfitting is a concern here. There may be some slight underfitting due to RMSE and MAE being very similar. 
...
> MN: I think what this plot is showing is that actual mortality values look more evenly spread across, while the predicted mortality values are more centered around the 0 to -0.25 area, when comparing the histograms. I would say this model under predicting high mortality and over predicting low mortality. This could still be valuable to stakeholders because we can go back and with limitations of current data and ask for better or newer data and predictors. There is more to the story.
...
```{r}
#wanted to not mess up my good census dataframe
census_clean <- census %>%
  distinct(County, .keep_all = TRUE)

caliDeaths <- caliDeaths %>%
  mutate(Count = as.numeric(Count))  #converts character to numeric, turning "NA" into NA
##### Calculate the monthly percent mortality
mortalitySummary_cali_deaths <- caliDeaths %>% 
  left_join(census_clean, by = "County") %>%
  ## Group by County as well as Year
  group_by(County, Month) %>%   
  ## Summarize by calculating a monthly percent mortality
  ## Note that I have to divide by the mean of the N from the Census  
  ## or I'd get an error
  summarize(monthlyPercMortality_new = sum(Count, 
                                       na.rm = TRUE)/mean(totalN)*100) %>% 
  ## Ungroup - necessary to remove the grouping for future use
  ungroup() #%>% 
  ## NOW, you can group again by county and year and calculate the true annual
  ## average that has been adjusted for differing number of months!
  #group_by(County, Year) %>%   ## <- no Month now!
  #summarize(annualPercMortality_new = mean(monthlyPercMortality_new, na.rm = TRUE)*12)

#### Find the five number summary
summ_cali_deaths <- fivenum(mortalitySummary_cali_deaths$monthlyPercMortality_new)
paste0("The five-number summary of monthly % total mortality is: ")
paste(c("Min", "Lower Quartile", "Median", "Upper Quartile", "Max"), 
      round(summ_cali_deaths, 3))

##### Now add on the cuts and labels for mapping:
mortalitySummary_cali_deaths <- mortalitySummary_cali_deaths %>% 
  mutate(deathLabels = cut(monthlyPercMortality_new, 
                          breaks = c(summ_cali_deaths, Inf),
                          labels = c("0-0.571", "0.572-0.689", "0.690-0.861", 
                                    "0.862-1.391", "1.392+"),
                          ordered_result = TRUE, right = F)) 
```
...
```{r}
# Filter to Sept–Dec 2020
plot_data <- mortalitySummary_cali_deaths %>%
  filter(Month %in% c("09", "10", "11", "12"))

#wanted to plot month as continuous so my trend line would follow the data
mortalitySummary_cali_deaths$Month <- as.numeric(mortalitySummary_cali_deaths$Month)

#updated plot
plot2 <- ggplot(mortalitySummary_cali_deaths, aes(x = Month, y = monthlyPercMortality_new)) +
  geom_point(alpha = 0.5, color = skittles[4]) +
  geom_smooth(method = "lm", color = skittles[3], linetype = "dashed", se = TRUE, size = 1.5) + 
  theme_minimal() +
  labs(
    title = "2020 Monthly Mortality Trend",
    x = "Month",
    y = "% Mortality"
  )
...
corr_data <- plot_data %>% #to perform the correlation test I need the same length of values
  group_by(Month) %>%
  summarize(actual = mean(monthlyPercMortality_new, na.rm = TRUE)) %>%
  mutate(Month = as.numeric(Month))

corr_joined <- inner_join(corr_data, temp, by = "Month") #join dataframes so that i can perform correlation test

#filter to Sept–Dec 2020 before correlation test
corr_filtered <- corr_joined %>%
  filter(Year == 2020, Month %in% c(9, 10, 11, 12))

cor.test(corr_filtered$actual, corr_filtered$percMortality)
...
>MN: According to the scatterplot, the trend line has a slight upward path from roughly month 3-12. While I was expeting a more dramatic slope, there is obviously an upward trend.
Looking at the correlation test, the correlaton coefficient is -0.88, so as predicted values go up, actual values tend to go down. A high p-value would suggest thatthis is not significant. There are also only 4 values, so not a lot of data to go on. 
...
> >MN: Using a predictor such as Month alone, may not fully capture the full picture because it is just telling you when something occured as opposed to the history behind it, or causes. The same for County, it can tell you where something happened but not history of county insights. 
A time-series lag can capture past trends leading up to current data, and how current data is correlated to past data. 
...
>MN: I would say with the large drop in MAE is a good sign that time-lagged is helping the model. With the training and testing MAE nearly identical, the model is generalizing well.
...
> MN: We could use some interactions between predictors instead of just treating them as independant factors. Perhaps treating the months as quarters(seasons, pre/post-holiday) and treating the counties as rural or urban. I think creativity is the limit with feature engineering. 
...
> MN: The model's initial predictions didn’t align closely with the actual data. The estimates were off by a noticeable margin. After adding new features that help the model learn from past months, accuracy improved dramatically. The predictions are now much closer after adjusting for time. 
That said, when we looked specifically at September through December 2020, the relationship between predicted and actual values wasn’t as strong as expected. With only four data points, it’s hard to draw firm conclusions.
Next steps: we recommend exploring region-specific seasonal patterns and layering in additional data sources like population changes, or social determinants of health.  This would help the model become more resilient with public health planning.
...
> MN: I think that you could not bring sex into the proportion if you only were interested in overall numbers. By seperating the genders, you can see a pattern of death between the two, with females clearly being lower across all years. I think there is value in that information and could be helpful in further analysis. If we lumped both genders together, with males being so much higher than females, this could cause some disproportionate results.
...
> MN: Overall, males are still much higher than females. Aside from 2020, both groups follow similar trends year to year. The possible cause of the May 2020 drop could be due in fact to the stay at home orders. Though that is 2 mpnths after the issue of that order, so could be another cause?
...
> MN: Based on these maps, I would say there is limited evidence that there is a change in trend due to stay ay home order. While on a county level there are some variations, statewide there is limited change. To address shuutdown, we could focus on monthly or quarterly data, and filter the maps by sex, and even possibly age, and type of death.
...
> MN: Some counties are filled in gray because there were too few deaths in that category for the data to be reported. This isnt always missing data, it can be an intentional suppression to protect individual privacy and avoid misleading statistics based on very small numbers. In small counties, only a few deaths could make someone identifiable, and going from a death rate of 1 to 3 in a small county can look like a VERY large increase.
...
ggplot(injuryDeathSummary) +
  ...
  labs(
    title = "California Annual Percent Deaths due to Accident or Injury, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020 \nby Sex and Stay-At-Home Order Status",
    caption = "Source: California DPH Vital Records"
  ) + 
  
  ## Use facet_wrap to make one map per year
  facet_wrap(~ biologicalSex + stayAtHome, ncol = 4)
```
...
```{r}
#JML
test <- cor.test(injDeaths$femaleMaleRatio, injDeaths$deathsInjuriesProportion)
test
...
> (JML) Based on the very small p-value, we'd reject the null hypothesis and conclude that there is a relationship between the two variables. However, because the correlation coefficient is approx 28%, we'd need to think about whether corrections should be made to the maps vs the models. The coefficient is less than 30%, which would be generally considered as small, but is getting close to being considered moderate. We have a large dataset of > 1800 records so there is a good amount of support for the idea that the evalutaion metrics are robust. I think that if we had unlimited time, we could modify the maps to account for this relationship, but in this case I'd probably leave the maps as they are as they wouldn't affect the visualizations tremendously.
...
injDeaths %>% 
  ...
  geom_smooth(color = skittles[2], method = "lm") +
...
> (JML) Yes, the scatterplot's gradual slope & direction from lower left to upper right is consistent with the correlation coefficient of .28. 
...
> (JML) My data science question is "Did the California stay-at-home orders in spring 2020 have an effect on the rate of deaths at the county level due to accidents & injuries, and if so, what were the observed patterns of influence?" My hypothesis would be that the stay-at-home orders lowered the rate of deaths at the county level. I would use a random forest model, and I would include both spatial and temporal lags.
...
```{r}
## (JML)
## Join the sex deaths table with the census data; then filter for STK
stkDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ## Here is where you filter for a specific cause of death relative to sex
  filter(leadingCauseDeath == "STK") %>% 
  ## Now full join with the first 4 columns of the totalDeaths table to get 
  ## total deaths also
  full_join(totalDeaths[ , 1:4], by = c("County", "Year", "Month")) %>%
  ## Rename the sexDeaths column to something more appropriate
  rename(strokeDeaths = sexDeaths) %>% 
  ## Calculate the proportion of all deaths that are from injury 
  ## RELATIVE TO ALL DEATHS (not population!!)
  mutate(deathsStkProportion = strokeDeaths/totalDeaths,
         ## The ratio of females to males in the population
         femaleMaleRatio = femalesN/malesN,   
         ## The ratio of 65+ females to males in the population
         femaleMaleRatio65 = females65plus/males65plus,  
         ...
         Date = as.Date(paste(Year, Month, "1", sep = "-"), 
                        format = "%Y-%m-%d")) %>%    
         ## Illegal div by 0 turned to NaN; turn back to 0
  mutate(deathsStkProportion = ifelse(deathsStkProportion == "NaN", 
                                           0, deathsStkProportion))  
...
stkDeathSummary <- stkDeaths %>% 
  ## Group by stay-at-home order, year, county, AND sex
  group_by(stayAtHome, Year, County, biologicalSex) %>% 
  ## Change the name to make the join easier
  rename(COUNTY_NAM = County) %>% 
  ## Mean proportion of deaths for each month-year-county-sex combo
  summarise(deathsStkPercent = mean(deathsStkProportion, na.rm = TRUE)*100) %>% 
  ## Join to get the geometries for mapping
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Coerce to make sure it is seen as a shapefile
  st_as_sf() %>% 
...
#### Find the five number summary
summ <- fivenum(stkDeathSummary$deathsStkPercent)
paste0("The five-number summary of % of deaths that are stroke-related is: ") 

paste(c("Min", "Lower Quartile", "Median", "Upper Quartile", "Max"), 
      round(summ, 3))

##### Now add on the cuts stkDeathSummary labels for mapping:
stkDeathSummary <- stkDeathSummary %>% 
  mutate(stkDeathLabels = cut(deathsStkPercent, 
                          breaks = c(0, 3.302, Inf),
                          labels = c("0-3.301", "3.302+"),
                          ordered_result = TRUE, right = F)) 	  

## Initialize the Chloropeth plot
ggplot(stkDeathSummary) +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = stkDeathLabels), color = "gray20", size = 0.2) +

  ## Apply manual fill scale for total deaths
  scale_fill_manual(
    name = "% of Deaths Due to Stroke",
    values = c4a(palette="brewer.dark2", n=3),
    na.value = "gray70",
    drop = FALSE
  ) +
  ## Set map projection explicitly
  ...
  # Add title and caption
  labs(
    title = "California Annual Percent Deaths due to Stroke, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
  ## Use facet_wrap to make one map per year
...
## Proportion all deaths due to stroke, by sex
stkDeaths %>% 
  group_by(Month, biologicalSex, Year) %>% 
  summarise(deathsStkProportion = mean(deathsStkProportion, na.rm = TRUE)) %>% 
ggplot(aes(x = Month, 
           y = deathsStkProportion, 
           color = as.factor(Year), 
           linetype = as.factor(biologicalSex))) +
  geom_line(alpha = 0.7, size = 1.1) +
  scale_x_continuous(n.breaks = 10) +
  scale_color_manual(values = skittles[c(5, 2:4)]) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(title = "Proportion all deaths due to stroke, by sex",
       y = "Proportion of All Deaths Due to Stroke", 
       subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
       color = "Year",
       linetype = "Sex of Decedent") +
  theme_minimal()  
...
stkDeaths %>% 
  group_by(County) %>%
  summarise(femaleMaleRatio = mean(femaleMaleRatio),
            deathsStkPercent = mean(deathsStkProportion, 
                                         na.rm = TRUE)*100) %>% 

  ggplot(aes(x = femaleMaleRatio, y = deathsStkPercent)) +
  geom_point(alpha = 0.7, color = skittles[5]) +
  geom_smooth(color = skittles[2], method = "lm") +
  labs(title = "Stroke-related Deaths vs. Female:Male Adults in California counties",
       subtitle = "Each point represents a county average",
       y = "Average % of Total Deaths Due to Stroke", 
       x = "Adult Female:Male (California Decennial Census, 2020)",
       caption = "Source: California DPH Vital Records") +
  theme_minimal()

## correlation test
strkeTest <- cor.test(stkDeaths$femaleMaleRatio, stkDeaths$deathsStkProportion)
strkeTest
```  

##### Interpretations of visualizations

Interpretation of chloropeth map

> (JML) Looking at the chloropeth maps for each of the 4 years, there doesn't appear to be much change in the rates of death due to stroke. However, having only two rate classifications does make it challenging to find meaningful differences.

Interpretation of proportion of all deaths due to stroke by sex

> (JML) According to this plot, females represent a higher proportion of deaths due to stroke than males. That trend occurs in each of the four years being presented. None of the years appears to have much higher or lower rates of stroke deaths than other years.

Interpretation of scatterplot

> (JML) The scatterplot's gradual slope and direction from lower left to upper right is consistent with the correlation coefficient of .43. 

Interpretation of correlation test

> (JML) Based on the very small p-value, we'd reject the null hypothesis and conclude that there is a relationship between the two variables. Additionally, because the correlation coefficient is approx 43%, we should need to strongly consider making corrections to the maps. We have a large dataset of > 2300 records so there is a good amount of support for the idea that the evalutaion metrics are meaningful. 

Data science question to be tested

> (JML) Did the California stay-at-home orders in spring 2020 have an effect on the rate of deaths at the county level due to stroke, and if so, what were the observed patterns of influence?

Data science hypothesis

> (JML) The hypothesis would be that the stay-at-home orders had no effect on the rate of deaths due to stroke at the county level. I believe that this hypothesis is supported by the visualizations developed.