...
> AJ- These files are a family of files and work together as a set of components that define both the geometry (shapes and locations) and attributes (names, codes, and metadata) of geographic features for counties in California  and  must keep them together in the same folder in order for the GIS or R software to read the shapefile correctly:
...
> AJ-`DP1_0048C` gives the raw number, while `DP1_0048P` gives  the proportion relative to the total population but both refer to the same demographic group (male population aged 65 years and over). This distinction is important depending on whether analyzing absolute population size or calculating demographic composition.
