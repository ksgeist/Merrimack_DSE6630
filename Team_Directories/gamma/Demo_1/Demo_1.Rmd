...
> The data are stored in a long format rather than a wide format. Each hospital appears multiple times because different measures are recorded in separate rows. For example, Facility ID 010001 appears repeatedly in both the HCAHPS and Hospital Readmissions datasets, while the measure being reported changes from row to row. This creates a dimensionality issue because a single hospital is represented by many observations instead of one row containing all relevant information. To analyze hospitals at the facility level, the data will likely need to be reshaped (pivoted wider) so that each hospital is represented by a single row and each measure becomes its own column.
...
> The NumberOfReadmissions variable has data quality issues that prevent it from being treated as a numeric variable. Although most values are numeric counts, the column is stored as a character variable because it contains the text value "Too Few to Report" (4,187 occurrences). This non-numeric value forces R to classify the entire column as character rather than numeric. In addition, the dataset contains a substantial number of missing values (6,697 NAs). Before analysis can be performed, the text values and missing data will need to be addressed so the variable can be converted to a numeric format.
...
> The purpose of this function was to practice cleaning, filtering, reshaping, and merging multiple datasets into a single analysis-ready dataset. These are important skills for Project 1, where I will need to combine and prepare healthcare datasets to answer my own research question.
...
> Using the contains() function, I removed all columns containing LowerEstimate, HigherEstimate, Denominator, and HcahpsAnswerPercent. These variables were not needed because the analysis focuses on the primary point estimates rather than confidence interval bounds, denominators, or response percentages. In total, 25 features were dropped from the dataset.

```{r}
# Count columns before dropping
old_cols <- ncol(pneumoniaFull)
...
> The ComparedToNational_* variables should be encoded using ordinal encoding because the categories have a meaningful ranking. For example, values such as "Better than National Benchmark," "No Different than National Benchmark," and "Worse than National Benchmark" represent ordered performance levels rather than unrelated categories. Ordinal encoding preserves this relationship by assigning numerical values to the categories (e.g., Better = 2, No Different = 1, Worse = 0), allowing machine learning models to incorporate the relative ordering of hospital performance.


##### **Question 16**: [1 point] 
What other column do we also need to perform this kind of encoding on?

> We should also perform ordinal encoding on the PaymentCategory column because its categories have a natural order (e.g., Above National Average, No Different Than National Average, Below National Average). Ordinal encoding preserves this ranking for analysis.
...
tempDF <- pneumoniaFull

# Encode all ComparedToNational columns
tempDF <- tempDF %>%
  mutate(across(
    starts_with("ComparedToNational"),
    ~ case_when(
      grepl("Better", .) ~ 2,
      grepl("No Different", .) ~ 1,
      grepl("Worse", .) ~ 0,
      TRUE ~ NA_real_
    )
  ))

# Encode PaymentCategory column
tempDF$`PaymentCategory_Payment for pneumonia patients` <- case_when(
  grepl("Above", tempDF$`PaymentCategory_Payment for pneumonia patients`) ~ 2,
  grepl("No Different", tempDF$`PaymentCategory_Payment for pneumonia patients`) ~ 1,
  grepl("Below", tempDF$`PaymentCategory_Payment for pneumonia patients`) ~ 0,
  TRUE ~ NA_real_
)

# Replace original dataframe
pneumoniaFull <- tempDF

# Verify results
table(
  pneumoniaFull$`PaymentCategory_Payment for pneumonia patients`,
  useNA = "ifany"
)

table(
  pneumoniaFull$`ComparedToNational_Death rate for pneumonia patients`,
  useNA = "ifany"
)
...
# **Create a vector containing the names of the categorical columns that will be frequency encoded.**
cols2encode <- c("State", 
               "CountyParish")

# **Create a temporary dataframe containing only the columns selected for encoding. This prevents accidental changes to the full dataset while the encoding is performed.**
temp <- pneumoniaFullEncoded[, names(pneumoniaFullEncoded) %in% cols2encode]

# **Define a function that performs frequency encoding on a selected column.**
add_freq <- function(data, column_name) {

# **Count how often each category appears in the column, including missing values.**
frequency_map <- table(data[[column_name]], useNA = "always")

# **Replace each category with its frequency count. This preserves geographic information while reducing the risk of overfitting that could occur if every state or county were treated as a separate category.**
  data[[column_name]] <- frequency_map[match(data[[column_name]],
                                             names(frequency_map))]

# **Return the modified dataframe after encoding.**
  return(data)
}

# **Loop through each column in the temporary dataframe and apply the frequency encoding function.**
for (col in names(temp)) {

# **Frequency encode the current column and save the result back into the temporary dataframe.**
  temp <- add_freq(temp, col)
}

# **Loop through each encoded column.**
for (c in 1:length(cols2encode)) {

# **Replace the original categorical columns in pneumoniaFullEncoded with their frequency-encoded versions from the temporary dataframe.**
...
# Compare unique values before and after frequency encoding
...
# Examine duplication of encoded values

table(pneumoniaFullEncoded$State)

table(pneumoniaFullEncoded$CountyParish)

# Remove CountyParish because frequency encoding creates excessive duplication
...
> State can be retained because it has relatively few categories and the encoded values still provide meaningful information. CountyParish should be removed because many counties share the same frequency value, resulting in excessive duplication and loss of information after frequency encoding.
...
> observed_readmission_rate is based on actual readmissions and discharges, while PredictedReadmissionRate and ExpectedReadmissionRate are risk-adjusted estimates used to calculate the ExcessReadmissionRatio. I would choose observed_readmission_rate as the target because it represents the actual outcome at each hospital. Using the predicted rate would mean predicting another model’s estimate rather than actual hospital performance.

##### **Question 21**: [1 points] 

Why did I have you drop the two columns used to make `observed_readmission_rate`?

> The two columns were dropped because they are used to calculate observed_readmission_rate. Keeping them would provide the model with direct information about the target variable, making the predictions easy.
...
ggplot(
  pneumoniaAnalyze,
  aes(
    x = factor(`ComparedToNational_Hospital return days for pneumonia patients`),
    y = PredictedReadmissionRate
  )
) +
  geom_boxplot() +
  labs(
    title = "Predicted Readmission Rate by Hospital Return-Day Performance",
    x = "Compared to National Benchmark",
...
> The plot suggests that hospitals performing worse than the national benchmark tend to have higher observed readmission rates. This indicates that hospital return-day performance may be a useful predictor of readmissions.
