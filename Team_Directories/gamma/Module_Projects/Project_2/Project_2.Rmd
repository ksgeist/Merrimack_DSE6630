...
* `cnty19_1.cpg`:This file contains the code page information for the shapefile. It specifies the character encoding used in shapefile. In short it tells the computer how to read the text in the other files.
* `cnty19_1.dbf`:This file contains the attribute data for the shapefile. It is a dBASE file that stores the tabular data associated with the spatial features in the shapefile.
* `cnty19_1.prj`:This File contains the projection information for the shapefile. It defines the coordinate reference system (CRS) used by the shapefile.
* `cnty19_1.shp`:This file contains the geometric data for the shapefile. It stores the spatial features, such as points, lines, and points that make up the shapefile.
* `cnty19_1.shp.xml`:This is an XML file that contain metadata about the shapefile.
* `cnty19_1.shx`:This file contains the Index data for the shapefile. It provides a quick way to access the geometric data in the shapefile.
...
```{r}
cali_sf <- read_sf("C:/california_county_boundaries/cnty19_1.shp")          
#("~/Desktop/California_County_Boundaries/cnty19_1.shp") 
...
> Your answer here. by collins
calif_sf object contains 69 observations (features) and 8 variables (columns). of the 69 features we have 58 counties. 3 counties that is santa barbara, Ventura and Los Angeles are islands. Santa Barbara and Ventura appears five times with a unique GlobalID and Geometry. this is the same for Ventura, appearing five times and Los Angeles three times.
...
Bounding box explains the spatial extent of the shapefile, that is, it provides information about the Westernmost, Easternmost, Southernmost, and Northernmost points of thr shapefile geometry.
To zoom out or in on a geographical feature, let say for example Los Angeles. We would need to adjust the boundary box coordinates to focus on a smaller area in this regard Los Angeles.
...
> Your answer here. By Collins
Projection is a variant of the Mercator projection, which is a cylindrical map projection that presents the earth as a flat, two-dimensional surface. its used for mapping application due to its simplicity. 

>Projection matters because it affects how geographic data is presented and analyzed, For example the Mercator projection preserves angles and shapes that suitable for navigation. However it distorts areas and distances (for example: Russia appears to be bigger than Africa in world map, even though Africa " Aprox. 30m km^2" is almost twice the size of Russia "aprox. 17m km^2" in terms of land area).

> If projection is changed, then features such as scale, cordinate values, spatial relationships, and visualizations will be changed too, impacting representation and analysis of geographical data.
...
## YOUR CODE HERE. By Collins
   ggplot() +
  ## Give it specific fill and border colors
  geom_sf(data = map1, fill = skittles[6], 
          color = "gray20", 
          linewidth = 0.25, 
          alpha = 0.65) +
  ...
  ## Give it a title and footnote/caption 
  labs(title = "California Northernmost Counties Bordering Oregon",
       caption = "Source: California State Geoportal") 
...
mortality_sf <- totalDeaths %>% # creating an object mortality_sf
  ## YOUR COMMENT HERE using rename function to rename county in totalDeaths into COUNTY_NAM so as to match COUNTT-NAM column in calif_sf
  rename(COUNTY_NAM = County) %>%
  ## YOUR COMMENT HERE use group_by function to aggregate data by COUNTY_NAM, Year, and Month
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## YOUR COMMENT HERE summarise here is being used to calculate totaDeaths using sum () and na.rm = True to ignore any missing value
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## YOUR COMMENT HERE - make sure to comment on the relationship parameter
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% # joins cali_sf with mortality_sf using COUNTY_NAM
  ## YOUR COMMENT HERE relationship = "many-to-many" is set to allow for multiple matches between the two datasets
  st_as_sf() %>%  #converts the joined tibble back to shapefile
  ## YOUR COMMENT HERE filters out ISLAND
  filter(is.na(ISLAND))
```
...
> Your answer here. By Collins
`st_join()` function is used to perform spatial join between two datasets. It allows to combine data from two sources based on their spatial relationships. In our case, while cali_sf has spatial information, totalDeath dataset which initiates mortality_sf has none.
>full_join function is preferred for non-spatial or spatial + non-spatial like in our case. Where we are using COUNTY_NAM to join.
...
> Your answer here. By Collins
By performing a full_join, it meant that the resulting dataset (motality_sf) is supposed to contain all features (rows) from both datasets and since mortality_sf has more rows than cali_sf then the total rows of mortality_sf is observed on the joined dataset
...
```{r}
# Your code here.
unique(mortality_sf$COUNTY_NAM)
```

> Your answer here. By Collins
we have 58 counties. 11 counties less than they were in cali_sf. This is because we dropped all the Islands when creating mortality_sf
...
> Your answer here. By Collins
for year 2017, 2018, and 2019 the total deaths pattern seems to be pretty consistent. This pattern changes for the year 2020 with a significant drop in total deaths. But after a little digging I realised that year 2020 has only 8 months worth of data. Therefore, its obvious total deaths for this year will be less compared to other years.

>Los Angeles is the most populated county in California with close to 10 million people. A similar trend in big cities. High populated areas are expected to have high total deaths in relation to their population size. This explains why there is more total deaths in the cities as compared to outside cities.
...
key <- "4551b0aef112cfb37dee5c5fcafe14b1858b8325"
```
...
> Your answer here. By Collins
`DP1_0048C` Annotation of Number!!SEX AND AGE!!Male population!!Selected Age Categories!!65 years and over

>`DP1_0048P` Annotation of Percent!!SEX AND AGE!!Male population!!Selected Age Categories!!65 years and over

> variable `DP1_0048P` is a proportion (Percentage) representation of variable `DP1_0048C`. in brief suffix C in `DP1_0048C`represents count while suffix P in `DP1_0048P` represents percentage. This offers an alternative to analyze the data in different perspectives and gain more comprehensive understanding of the demographic characteristics of the population.
...
> Your answer here. By Collins
1. What key pieces of the function would you need to change?
The URL: since the URL is case specific, it will need to be tweeked to suit the emerging needs
API KEY: if a new API (Application Programming Interface) requires a new API Key then it will have to change but i want to believe API KEY is more constant in nature
Parameters: parameters passed in the request changes with the needs/(data type).

> 2. What general steps stay the same regardless of the API?
sending a request to an API endpoint remains the same
regardless of the API we need to handle the response that is checking for errors and parsing the data
data cleaning is usually important it cuts across all APIs
...
census <- census%>%
  rename(COUNTY_FIP = COUNTY)
  
mortality_sf2 <- mortality_sf %>%
  left_join(census, by = "COUNTY_FIP", relationship = "many-to-many") %>%
  st_as_sf()
...
> Your answer here. By Collins
The maps of 2017, 2018, & 2019 show a very consistent death rates. With some subtle increases and decreases in different states. For example a close comparison of year 2017, 2018, and 2019, we deduce the following:
>Mendocino county death rate was high in 2017 decreased in 2018 and increased again in 2019
>shasta county death rate was higher in 2018 compared to 2017 and remained same level in 2019
>Glenn County had lower deaths in both 2017 and 2019 compared to 2018
>Calavelas county death rates was low in 2017, increased in 2018 and decreased again in 2019
>Mariposa county death rates were high in 2017, reduced in both 2018 and 2019.
2020 shows a decline pattern in deathrates but we do know the decrese is caused largely by the absence of 4 months of data.
...
> Your answer here. Collins
While a Map in section 2.7 shows some informative patterns for year 2017, 2018, and 2019. It fails to incorporate the absence of data in 2020. 
>Our formula: summarize(annualPercMortality = 100*(sum(totalDeaths, na.rm = TRUE)/mean(totalN))). TotalN fails to consider the fact that in 2020, we accounted for only 8 months, therefore, our map was bound to show a decrease in death rates for this year even a complete would have changed the trend of the death rates. 
>Adjusting the data to analyse per month has helped significantly to understand death rates in year 2020 as compared to similar months of years 2017, 2018, and 2019.
...
> Your answer here. By Collins
Yes there seems to be an increase in death rates in 2020 as compared to previous years. For example the forllowing counties have showed an upward trajectory in relation to death rate increase: Los Angeles, Orange, Imperial, Ventura, Kern, San Bernardino, Tulare, Kings, Madera, Mendocino, Plumas, and Sonoma. Riverside and  San Diego death rates remained relatively the same.
>Mariposa county shows a decline in number of Death rates in 2020
...
>1. Checking Linearity using Residuals Vs Fitted Plot. This plot does not show a clear linear relationship, which could indicate a non-linear relationship between the dependent and independent variables. Based on linearity assumption, our OLS model is not rubust.

>2. Checking for Normal distribution assumption using Q-Q plot: The heavy_tailed nature of residuals indicate that there are outliers/ Extreme values in the data that are not accounted for in the model.Therefore, residuals from our OLS model are not normally distributed a further indication that our model is not robust enough.

>3. Homoscedasticity: the non-horizontal red line in the scale location plot, suggest some level of heteroscedasticity. That is a varying spread of of residuals across different fitted values, negating the assumption of constant variance.
>Based on Residuals VS Leverage plot. points 1084, 1091, and 1097 seems to be influential observations in our linear regression model. These points have high leverage and large residuals which could impact the robustness of our model.
...
> Your answer here. By Collins
Calculating neighbors on just the distinct counties is preferred because it avoids redundant computations.And since mortality_sf2 has dimensions of 2552, 5 which is a multiple of 58, then it suggests there are approximately 44X duplicates entries. Computing spatial weights or neighbors on such redundant data would be computationally wasteful and could lead to inefficiencies in the model. Therefore by isolating unique counties using (unique()), we ensure that each county is only considered once.
...
> Your answer here. By Collins
Pr(>F) in Anova indicates that our P-value = 2.2e-16 *** which is a very small number, less than 0.05. Therefore, we reject the Null hypothesis and conclude there is a significant difference between sarModel and OLS. This tells us that sarModel is a better fit to our data because it accounts for spatial autocorrelation in the data, which is not captured by the OLS
...
> Your answer here. By Collins
The coefficient for the "year" is 0.0008920, which means that for every one-unit increase in year, monthlyPercMortality increases by 0.0008920 units cetriparibus. However, the p-value for the year is 0.080223 which is greater than 0.05, indicating that the year and monthlyPercMortality is not statistically significant.

>The coefficient for Month is -0.0006300, which means for every one unit increase in month, monthlyPercMortality decreases by -0.0006300 cetriparibus. The P-value for the month 0.000118 *** is less than 0.05, therefore we conclude that the relationship between month and monthlyPercMortality are statistically significant.

>The coefficient for lagged_monthlyPercMortality is -0.2190860, which means for every one-unit increase in lagged_monthlyPercMortality, monthlyPercMortality decreases by -0.2190860 units, ceteriparibus. The P-value for lagged_monthlyPercMortality is 4.95e-06 *** which is less than 0.05 indicating that the relationship between lagged_monthlyPercMortality and monthlyPercMortality is statistically significant.

>Multiple R-squared:  0.01361 indicates that the sarModel explains only a small proportion (1.36%) of the variance in monthlyPercMortality. This suggests that the model may not be a good fit to the data. and that other factors may be influencing monthlyPercMortality that are not captured by the current model.
...
```{r}
## Your code here.
plot(sarModel) # am not sure if this was necessarily or the right way
```

> Your answer here. By Collins
based on R^2 of just 0.01361 (1.36) i would not trust the results of the model. It explains very little relationship between independent and dependent variables.

>Residual standard error: 0.02705. In comparison to the range of our target monthlyPercMortality, the RSE is approximately 16.68%. This explains that for every predicted value of monthlyPercMortality we are making approximately 16.68% error. based on our stakeholders and how accurate they want the prediction, I would not trust the results of this model

```{r}
range(mortality_sf2$monthlyPercMortality)
```
...
```{r}
## Your code here.
train_split <- 100*(nrow(mortalityTrain)/ nrow(mortality_sf2))
print(train_split)
```

> Your answer here. By Collins
yes the split ratio is 81.81-18.19 which is very close to canonical 80-20 split

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> Your answer here.By Collins
1. Lack of Representation: By excluding 2020 data from the training set, the model may not be able to capture patterns or trends that are specific to that year.
2. Inability to Account for changes.
3.limited generalizability: The model may not generalize well to new, unseen data, especially if the underlying patterns or relationships change over time.
...
> Your answer here. By Collins
We use out-of-bag (OOB) error in random forest to evaluate the performance of the model without the need for a separate validation set. Each decision tree is trained on a bootstrap sample of training data, This means that some samples are left out of the training process for each tree. The OOB error is calculated by predicting the response variable for these left-out samples using the tree that did not see them during training.

Why do we do it?
>
1. no need for a separate validation set
2. Hyperparameter tuning. by trying a range of number of trees
3. Ubiased estimate of the model performance: OOB error is calculated on samples that were not used during training
 
 What can it tell you here? 
 > Model performance
 >comparison with cross-validation: By comparing OOB error and 10 fold cross-validation, we can determine if the model is overfitting or underfitting.
 >Variable Importance. the OOB error can be used to calculate variable importance
...
> Your answer here. By Collins
mtry is a hyperparameter that determines the number of features to consider at each split when building trees in a random forest
>The default mtry is set to a square root of the number of features in the dataset
>We are overi=riding the default mtry value to be able to experiment with differnt values and find the best for our small non-complex dataset. This will improve computational efficiency and interpretability.
...
Which value of  `mtry` gave the best results? (**Hint**: Can $R^2$ values be negative?!?). Are you able to compare the $RMSE$ of this model to the one you fitted in Question 17C? Why or why not? (**Hint**: Does your pre-processing step apply to the target too? You may have to do a little research!)

> Your answer here. By Collins
mtry of 1 shows the best Rsquared of 0.0322177 (3.22%) and low RMSE of 0.9835237. No the R62 cannot be negative, because its used to explain the variance in the model. No we can't compare the results of random Forest R^2 to sarModel R^2, this is beacuse in random forest we center and scaled unlike in sarModel. The pre-processing steps include the target variable
...
> Your answer here. By Collins
the Cross-validation 10 fold, transforms R^2 from negative to positive as witnessed in mtry 2 and 3. 
mtry 1 still post the best results with an Rsquared of 0.0452929 (4.53%) variance explained and a low RMSE of 0.9846155.
mtry 1 of cross-validation (10-fold) is 0.9846155 which is slightly higher compared to RMSE of OOB model 0.9835237. though the difference is very minute!
...
> Your answer here. By Collins
Given that the data was centered and scaled, the RMSE and MAE values (1.0076 and 0.7424 respectively) are in transformed units. Therefore to interpret these results meaningfully, we will ideally need to know the original scale of the data before scalling. However, we can still make a general observation based on the relative performance. That is if the RMSE and MAE values are close, it would suggest the model's errors are relatively consistent across the dataset. A large difference indicates presense of outliers. Given our difference between RMSE and MAE 1.0076 - 0.7424 = 0.2652. In my own opinion i can conclude that this difference is not large enough and therefore the model's performance is somewhat acceptable, But further analysis is necessary to draw concrete conclusions.

>RMSE Comparison: The RMSE values from the training set range from approximately 0.98 to 1.06 across mtry 1,2 & 3. The test set RMSE is 1.0076, which falls within the range of training set RMSE values but is more close to high end of the range (high RMSE/ less robust model)

>MAE Comparison: The MAE values from the training set range from approximately 0.71 to 0.79. the test set Mae is 0.7424, which also falls within the range of the training set MAE just around the middle point.

>Given that the test RMSE and MAE values are within the ranges observed in the training set and are not significantly higher than the training set metrics, it suggest that the model is not severly overfitting. That being said, since the test metrics are comparable to training set metrics, then we can conclude that the performance of the model is more balanced. However, the overall performance as indicated by both RMSE and MAE  suggest that there might be room for improvement in terms of model accuracy.
...
> Your answer here. Collins
The Histogram on the right side of the plot indicates that the predicted values are concentrated between -0.25 and 0.25, while the actual values range from -2 to 4.

>we can make the following observations
1. the predicted values are not accurately capturing the full range of actual values
2. the model tends to underpredict mortality rates, as most predicted values are lower than the actual values

>Therefore, the scatter plot and the Histogram suggest that the training model is leading to an underprediction of mortality rates. That is the model is consistently predicting lower mortality rates that the actual values.

>Despite the model underperformance, the scatterplot and Histogram provide valuable insights for stakeholders:
1.Model Limitations. The underprediction of mortality rates suggests that the model may not be suitable for making accurate predictions, especially in situations where high mortality rates are expected
2. Mortality Rates trends. The actual values show a range of mortality rates, indicating that there may be underlying trends or patterns that the model is not capturing.
...
```{r}
## Your code here
caliDeaths_filtered <- caliDeaths%>%
  filter(Year == "2020", Month %in% c("09", "10", "11", "12"), Cause_Desc == "All causes (total)")
```
...
```{r}
## Your code here
#Group by month (september to dec 2020) and calculate monthly death 
monthly_deaths_postshutdown <- caliDeaths_filtered %>%
  group_by(Month) %>%
  summarise(Total_deaths = sum(as.numeric(Count), na.rm = TRUE))

# add the monthly deaths to form total death for the four months
total_deaths_postshutdown <- sum(monthly_deaths_postshutdown$Total_deaths, na.rm = TRUE)

#calculate percentage monthly deaths

monthly_deaths_postshutdown$percentage_mortality_post <- 100*(monthly_deaths_postshutdown$Total_deaths / total_deaths_postshutdown)

monthly_deaths_postshutdown$Month <- as.numeric(monthly_deaths_postshutdown$Month)
```
...
```{r}
## Your code here
ggplot(monthly_deaths_postshutdown, aes(x = as.numeric(Month), y = percentage_mortality_post)) +
  geom_point() +
  geom_smooth(method = "lm" , se = FALSE) +
  labs(title = "Monthly Percent Mortality Trend",
       subtitle = "September to December 2020",
       x = "Month", 
       y = "Percent Mortality post shutdown") +
  theme_classic()
```
...
```{r}
correlation_test <- cor.test(monthly_deaths_postshutdown$Month, monthly_deaths_postshutdown$percentage_mortality_post)
...
> I got a little bit confused with this specific question, am not sure where am supposed to get the predictions. 
> I Think this was very straight forward that the correlation will be very high since monthly percent mortality was created from monthly mortality. The value 0.833812 confirms the trend that monthly percent mortality increased with increade in months from september to December. The slope is positive slanting upwards which resonates with our strong positive correlation.
...
> Your answer here.
Time series lags can help capture the monthly seasonality in deaths that is not fully explained by the month variable alone. By including lagged values of the death rate we can better model the temporal autocorrelation and seasonal momentum in the data. This will allow us to model our past values influence the current values and better capture periodic patterns in the data
...
> Your answer here. By Collins
Adding time-series lagged variables reduced the testing MAE from 0.745 to 0.129. This indicates an improvement in the model's performance on the test data.
> The training MAE is 0.134, this is slightly higher than the testing MAE with the lagged variables (0.129). In an ideal situation, we would expect the training MAE to be smaller than the test MAE due to overfitting. However, since the difference is small, then it suggest that the model is not severly overfitting but doing pretty well. This conclusion would need further annlysis since the target and predictors are scalled.
...
> Your answer here. By Collins
I think adding the number of features can introduce more complexity to the Random Forest model thus improving the model.
...
> Your answer here. By Collins
Our analysis of Non-Covid mortality trends from 2017 to 2020 using random forest model has provided valuable insights. The model has shown promising performance in predicting mortality rates, although it tends to slightly underestimate them. Notably, incorporating time-series lagged variables has enhanced the model's accuracy, highlighting the significance of temporal patterns in mortality trends. This suggest that mortality rates are influenced by factors that exhibit patterns over time, and capturing these patterns can improve our understanding and prediction of mortality trends.
To build upon this work, we recommend continuing to refine the model by exploring additional features and or alternative machine learning  approaches. Regular updates and  monitoring of the model will also be crucial to ensure continued relevance and accuracy in tracking evolving mortality trends.
...
> Your answer here. By Collins
based on the box plot, it wouldn't be fair to ignore the effects of sex. The plot suggests there is a significant difference in the proportion of deaths due to accident and injury between males and females, with males having a higher median proportion and greater variability. Ignoring sex could lead to biased or inaccurate models, as it would fail to capture this important factor.
...
> Your answer here. By Collins
it shows there was higher proportion of males death compared to females. The trend for females was more subtle as compared to males. For males it show huge highs and lows.
I think the dip in the trend of death starting the 5th month of 2020 was hugely contributed by stay at home orders
...
> Your answer here. By Collins
Based on the maps above there is no clear evidence that the percent of deaths due to accident and injury changed as a result of the stay-at-home orders. However, to provide stronger evidence, we would need to filter the data to only include deaths from April to August 2020 and facet the data by bilogical sex to check for ineraction
...
> Your answer here. By Collins
one reason could be the data was not submitted by respective counties or another reason which i think is the main reason. Is that the data was intentionally ommitted to for data privacy purposes to protect Personal Identifying Information. The data was suppressed, maybe because the number of death was too small that it could lead to imdividual identification.
...
before_shutdown <- injuryDeathSummary %>% filter(Year < 2020) #filtering data for year before the shutdown

After_shutdown <- injuryDeathSummary %>% filter(Year >= 2020) #filtering data for year after the shutdown


## creating map for years before shutdown
ggplot(before_shutdown) +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +
...
correlation_inj <- cor(injDeaths$femaleMaleRatio, injDeaths$deathsInjuriesProportion, use = "pairwise.complete.obs")

print(correlation_inj)
...
> Your answer here.
0.279791 is a weak positive correlation and therefore I don't have to make correction in my maps, correction in my model will be enough.
...
> Your answer here. By Collins
My scatter plot shows a weak positive correlation between the average percentage of total deaths due to Injury or Accident and the adult female-male ratio in california counties. The best-fit line has a slight upward slope, indicating that as the female-male ratio increases, the average percentage of total deaths due to Injury or Accident also tends to increase. This is consistent with our correlation coefficient value 0.279791
...
> Your answers here. By Collins

>Data Science Question: Does age influence the leading cause of death?

>Research Question: Is there a significant relationship between age and the likelihood of death by injury in California from 2017 to 2020 

>Null Hypothesis: There is no significant relationship between age and the likelihood of death by injury in California from 2017 to 2020.

>Prediction: The mechanism of injury varies by age group. Younger individuals are likely to die from motor vehicle accidents and other traumatic injuries, while older individuals (over65) are more likely to die from falls or age-related injuries.

>Model selection
1. Random Forest: I would deploy random Forest to handle the relationship between Age and death by Injury, while accounting for potential confounding variables.
2.Spatial Lag: now that i have 58 counties of california, I would want to understand the distribution of deaths among counties. The pattern of deaths in relation to age in different counties.
3.Temporal Lag: It would be interesting to understand the injury death by age patterns witnessed before covid shutdown were consistent with those after shutdown. It would also be interesting to understand injury death by age patterns during shutdown.

>Are there any variables you pulled from the Census that would be worth considering here as well?
over65 variable
household income (not in this census data)
amenities (hospitals). Am not sure about this one but i think it might be a factor in deciding deaths





                           UNCHARTERED WATERS SECTION
...
```{r}
sexDeaths%>%
  count(leadingCauseDeath)
```


## 5.1 Death from Suicide

Because of the mandatory COVID-19 shutdown in California, which has led to generally reduced movement throughout California, your stakeholder would like to know if the number of suicide deaths increase enough **during COVID-19 shutdown** to generate a signal.

**HYPOTHESIS**: You hypothesize that because of intense mandatory stay-at-home measures, increased family disputes, and reduced personal space, the number of deaths attributed to suicide(`SUI`) increased during the COVID-19 shutdown during the months of 2020 in California.

You will start, as with all good analysis plans, with some more __Exploratory Data Analysis__.

### 5.1.1 Sex-specific Effects on `SUI` Mortality
...
* Proportion of deaths from Suicide out of all deaths (no longer adjusting for population size)
...
```{r}
## Join the sex deaths table with the census data; then filter for INJ
suicideDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ## Here is where you filter for a specific cause of death relative to sex
  filter(leadingCauseDeath == "SUI") %>% 
  ## Now full join with the first 4 columns of the totalDeaths table to get 
  ## total deaths also
  full_join(totalDeaths[ , 1:4], by = c("County", "Year", "Month")) %>%
  ## Rename the sexDeaths column to something more appropriate
  rename(suicideDeaths = sexDeaths) %>% 
  ## Calculate the proportion of all deaths that are from suicide 
  ## RELATIVE TO ALL DEATHS (not population!!)
  mutate(deathssuicideProportion = suicideDeaths/totalDeaths,
         ## The ratio of females to males in the population
         femaleMaleRatio = femalesN/malesN,   
         ## The ratio of 65+ females to males in the population
         femaleMaleRatio65 = females65plus/males65plus,  
         ...
         Date = as.Date(paste(Year, Month, "1", sep = "-"), 
                        format = "%Y-%m-%d")) %>%    
         ## Illegal div by 0 turned to NaN; turn back to 0
  mutate(deathssuicideProportion = ifelse(deathssuicideProportion == "NaN", 
                                           0, deathssuicideProportion))  
```


```{r, echo = FALSE}
## Now plot the numbers of deaths per year due to suicide by sex
suicideDeaths %>% 
ggplot(aes(y = deathssuicideProportion, x = biologicalSex, fill = as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "Biological Sex of Decedent", 
       y = "Proportion of All Deaths Due to Suicide", 
       fill = "Year", 
       title = "Proportion of All Deaths Due to Suicide",
       subtitle = "California Counties, Jan 2017 - Aug 2020") + 
  scale_fill_manual(values = skittles[c(5, 9:11)]) + 
  theme_classic()
```
...
Would it be fair to model the proportion of deaths due to Suicide without accounting for an effect of sex? Why or why not?

> Your answer here. By Collins
>Analysing the Boxplot

Females
>Median is at 0.00 indicating that atleast half of the counties have a proportion of deaths by suicide close to zero
>Box: the interquartile range (IQR) is narrow, suggesting that the middle 50% of the data is concentrated near the median
>Whiskers: The whiskers are relatively short, indicating that most data points are close to the median
Outliers: There are several outliers above the upper whiskers, suggesting that some counties have a higher proportion of deaths due to suicide among females.

Males
>Median is at 0.00 similar to females indicating that atleast half of the counties have a proportion of deaths by suicide close to zero.
>Box: the interquartile range (IQR) is slightly wider than the females, suggesting a slightly greater spread in the middle 50% of the data.
>Whiskers: The whiskers are longer than females, indicating a greater range of values.
Outliers: There are fewe outliers compared to females, suggesting that some counties have a higher proportion of deaths due to suicide among males.

Would it be fair to model the proportion of deaths due to Suicide without accounting for an effect of sex? Why or why not?
>Based on the analysis, its not fair to model the proportion of deaths due to suicide without accounting for an effect of sex. Although, the median for both sex is at 0.00, the distribution and variability between males and females are significant enough to warrant consideration of sex as a factor in the analysis.
...
suicideDeaths %>% 
  group_by(Month, biologicalSex, Year) %>% 
  summarise(deathssuicideProportion = mean(deathssuicideProportion, na.rm = TRUE)) %>% 
ggplot(aes(x = Month, 
           y = deathssuicideProportion, 
           color = as.factor(Year), 
           linetype = as.factor(biologicalSex))) +
  geom_line(alpha = 0.7, size = 1.1) +
  scale_x_continuous(n.breaks = 10) +
  scale_color_manual(values = skittles[c(5, 9:11)]) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(title = "Proportion all deaths due to Suicide, by sex",
       y = "Proportion of All Deaths Due to Suicide", 
       subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
       color = "Year",
       linetype = "Sex of Decedent") +
...
> Your answer here. By Collins
it shows there was higher proportion of males death compared to females. The trend for females was more subtle as compared to males. For males it show huge highs and lows.
Year 2020 shows a decline trend in suicide deaths among males upto the 5th month of the year. The trend changes trajectory and it plateaus between the 6th month and 7th month. Then begins to drop again.


### 5.1.2 Mapping injury-related deaths by stay-at-home orders & sex {#chloropethInjSex}

So, hopefully you've noticed what I've noticed about the relationship between the stay-at-home orders and the proportion of overall deaths due to suicide, as well as the effect of the shutdown in terms of both decedent biological sexes. You know from your earlier graph that females have a lower proportion of overall death due to suicide; next, it would be nice to see what the relationship is spatiotemporally (before and after shutdown) on average suicide mortality overall, as well as for each of the sexes separately. This will allow you to see if there is a geographic pattern, as one might expect in denser areas with more population pressure, congested housing, social disputes with the effect or not.
...
Next, let's calculate a summarized dataframe that groups by `stayAtHome`, `Year`, and `County`, then summarizes an average `deathssuicideProportion` for use with the maps. Notice that rather than storing the summarized dataframe, I simply piped it through to join it with the geometry from the shapefile for map-making this time. I was feeling a bit lazy. 
```{r}
##### Calculate the proportion of all deaths due to suicide
suicideDeathSummary <- suicideDeaths %>% 
  ## Group by stay-at-home order, year, county, AND sex
  group_by(stayAtHome, Year, County, biologicalSex) %>% 
  ## Change the name to make the join easier
  rename(COUNTY_NAM = County) %>% 
  ## Mean proportion of deaths for each month-year-county-sex combo
  summarise(deathssuicidePercent = mean(deathssuicideProportion, na.rm = TRUE)*100) %>% 
  ## Join to get the geometries for mapping
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Coerce to make sure it is seen as a shapefile
  st_as_sf() %>% 
...
#### Find the five number summary
summ <- fivenum(suicideDeathSummary$deathssuicidePercent)
paste0("The five-number summary of % of deaths that are suicide-related is: ")
paste(c("Min", "Lower Quartile", "Median", "Upper Quartile", "Max"), 
      round(summ, 3))
## Notice that, with the minimum, 1st quartile, AND median all equal to 0, we
## really only have three groups

##### Now add on the cuts suicideDeathSummary labels for mapping:
suicideDeathSummary <- suicideDeathSummary %>% 
  mutate(deathLabels = cut(deathssuicidePercent, 
                          breaks = c(0, 3.343, 18.489, Inf),
                          labels = c("0-3.342", "3.343-18.489", "18.489+"),
                          ordered_result = TRUE, right = F)) 
```
...
```{r, echo=FALSE}
## Initialize the plot
ggplot(suicideDeathSummary) +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +

  ## Apply manual fill scale for total deaths
  scale_fill_manual(
    name = "% of Deaths Due to Suicide",
    values = c4a(palette="brewer.or_rd", n=3),
    na.value = "gray70",
    drop = FALSE
  ) +
  ...
  # Add title and caption
  labs(
    title = "California Annual Percent Deaths due to Suicide, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
...
Do you think there is any evidence that the percent of deaths due to suicide changed as a result of the stay-at-home orders? What would provide stronger evidence of a change in percent of deaths due to suicide as a result of the shutdown? Do these maps include any evidence of interaction with the sex of the decedent?

**Hint**: Make sure to explain what you would want to do to these maps, either in terms of filtering and/or additional faceting, to better address whether (1) the shutdown has had a likely effect and (2) whether there is an interaction with biological sex on the proportion of deaths due to accident or injury.

> Your answer here. By Collins
Based on the maps above there is no clear evidence that the percent of deaths due to suicide changed as a result of the stay-at-home orders. However, to provide stronger evidence, we would need to filter the data to only include deaths from April to August 2020 and facet the data by bilogical sex to check for interaction
...
> Your answer here. By Collins
one reason could be the data was not submitted by respective counties or another reason which i think is the main reason. Is that the data was intentionally ommitted for data privacy purposes. That is to protect Personal Identifying Information. The data was suppressed because the number of death was too small that it could lead to imdividual identification.
...
before_shutdown <- suicideDeathSummary %>% filter(Year < 2020) #filtering data for year before the shutdown

After_shutdown <- suicideDeathSummary %>% filter(Year >= 2020) #filtering data for year after the shutdown
...
ggplot(After_shutdown) +
  ...
  labs(
    title = "California Annual Percent Deaths due to Suicide, by County",
    subtitle = "Cumulative Monthly Deaths, March 2020 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
...
Even if you didn't explore biological sex on the map, it seems that there are likely some sex-specific effects in percent of deaths due to suicide. But what if some counties have a **skewed sex ratio**? You might need to adjust for that! Although you might expect, _a priori_, for all counties to have a female to male ratio of 1:1, in reality that isn't always the case. 
...
femaleMaleRatio_vector <- suicideDeaths$femaleMaleRatio
mean_femaleMaleRatio <- mean(femaleMaleRatio_vector)
...
ggplot(aes(x = femaleMaleRatio)) +
  ...
  geom_histogram(alpha = 0.7, fill = skittles[5], color = "white") +
  ## Add the mean as a solid, dark blue line
  geom_vline(aes(xintercept = mean_femaleMaleRatio), 
             color = skittles[9], 
             linetype = "solid", 
             size = 1.25)+
  ## Add the reference 1:1 line as a black, dashed line
...
You can see that you have a very strongly __left-skewed__ distribution, with the mean of the female:male ratio dragged to the left. What does this mean, exactly? **You have more counties with a higher proportion of men than women than otherwise!** And since men seem to have a higher proportion of deaths by suicide  as opposed to females, you need to decide if you think you need to account for this in your maps. Accounting for it in the model will be easy - you just include the female:male ratio as a predictor variable. However, in the maps it is actually a little trickier and would require a mathematical correction that could be difficult for your stakeholders to understand. Is it even necessary then?

#### Question 22 [1 point]:

Calculate the correlation of `femaleMaleRatio` and the `deathssuicideProportion`, making sure to be careful to drop or exclude any NAs before making your calculation. 

```{r}
## Your code here.

correlation_sui <- cor(suicideDeaths$femaleMaleRatio, suicideDeaths$deathssuicideProportion, use = "pairwise.complete.obs") #use = "pairwise.complete.obs" removes the N/A

print(correlation_sui)
```

Do you think that this correlation coefficient indicates that you need to make a correction on your maps, or is it likely sufficient just to make the correction in your models? In other words, large correlation coefficients (in the absolute value sense) would indicate a need to correct the maps; smaller coefficients would indicate that it's probably not a big deal for your visualizations provided to stakeholders.

> Your answer here.
0.2126415 is a positive weak correlation, therefore I do not need to make correction on my maps and a correction in my model will be sufficient
...
suicideDeaths %>% 
  group_by(County) %>%
  summarise(femaleMaleRatio = mean(femaleMaleRatio),
            deathssuicidePercent = mean(deathssuicideProportion, 
                                         na.rm = TRUE)*100) %>% 

  ggplot(aes(x = femaleMaleRatio, y = deathssuicidePercent)) +
  geom_point(alpha = 0.7, color = skittles[5]) +
  geom_smooth(color = skittles[8], method = "lm") +
  labs(title = "Suicide-related Deaths vs. Female:Male Adults in California counties",
       subtitle = "Each point represents a county average",
       y = "Average % of Total Deaths Due to Suicide", 
       x = "Adult Female:Male (California Decennial Census, 2020)",
       caption = "Source: California DPH Vital Records") +
  theme_minimal()
```
>My scatter plot shows a weak positive correlation between the average percentage of total deaths due to suicide and the adult female to male ratio in california counties. The best-fit line has a slight upward slope, indicating that as the female-male ratio increases, the average percentage of total deaths due to suicide also tends to increase. This is consistent with our correlation coefficient value 0.2126415 
