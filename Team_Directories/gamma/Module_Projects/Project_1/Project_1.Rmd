...
In my opinion, I think excess readmission rate would be the best feature to set as target variable. Its violin plot is very distinctive show "worse' being skilled towards the higher values than national average, "same" is distributed towards the centre as expected since its represent equal value to national average. lastly "better" shows some level of a left skeweness towards lower values an indication of excess readmission being less than national average. The unknown which are the missing values seems to centered around national average as represented by "same"
...
> Your answer here. By Collins. Yes this density plot shows clearly three distinctive peaks with distinctive skewness. the same density plot is centrally place as expected while better skews to the left and worse skews towards the right
...
#by Collins, Higher medicare spending per patient could indicate that the hospital is dealing with age advanced population (old folks) who probably have chronic donditions requiring Intensive care
...
#by Collins I think higher scores indicates a very stron performance of the hospital in terms of thromolism prevention, early identification and interventions and high quality of hospital resources either personel, equipments and methofology and procedures
...
longScore %>%  
  ggplot(aes(y = ExcessReadmissionRatio,  ## <-- CHANGE THIS!
             x = Value)) +
    ...
    labs(title = "Relationship of Excess readmission ratio-pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
>cleanliness is a critical asspect of hospital care, particularly for communicable diseases like pneumonia. However, our plot suggest that cleanliness may not have a significant impact on pneumonia-related readmissions, this might be because:
1. Readmissions are more often related to underlying health conditions such as chronic obstruction pulmonary disease (COPD) or heart failure than hospital cleanliness.
2. Cleanliness in a hospital is more of a norm than exception, that is hospitals are expected to maintain high standards of cleanliness, and deviations from these standards are very rare. As a results cleanliness may not be a significant differentiator in terms of readmission rates.
...
longHCAHPS %>%  
  ggplot(aes(y = ExcessReadmissionRatio,  ## <-- CHANGE THIS!
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                ...
                show.legend = T) + 
    ## CHANGE THE TITLE FOR FULL CREDIT!
    labs(title = "Relationship of Excess Readmission Ratio and pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
        x = "Rate or Score",
        y = "Predicted Pneumonia-related Readmissions",
        color = "Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
...
> Your answer here.BY Collins
in my own opinion, I find communication about medicines and discharge information to be the most compelling. they both display moderate negative correlations indicating that effective communications about medicines and better discharge of information are associated with lower readmission rates. These findings may suggest that hospitals can focus on improving these aspects of patients care to reduce pneumonia-related hospital readmissions.
...
#Collins. All along I was of the opinion that Excess readmission rate was the best target because it took into account both the observed and expected readmission rates, somehow providing a more comprehensive picture of hospital performance. after ploting denssty plot my opinion was conflicted between Excess readmission rate and predicted readmission rates. The two plots are closely similar. Howerver, after reading your supporting narrative for predicted readmission and reviewing the stakeholders question, I do agree with your opinion that predicted readmission is a better target
...
dat2Analyze <- dat2Analyze %>% 
  ## COMMENT HERE # selected to remove other potentail target variables, columns containing sample, number of patients and, NumberSurveyscompleted
...
> Your answer here. By Collins. Moving `FacilityID' to rownames transforms it into an index thus representing each row as a unique observation this also prevents it to be used as a feature in the model which could lead to errors or incorrect outcomes. 
