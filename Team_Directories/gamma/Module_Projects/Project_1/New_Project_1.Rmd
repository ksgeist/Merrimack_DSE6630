...
> Your answer here. 644 observations
...
> Your answer here.BY Collins. cell $[6,1]$ shows 1788 observations where both Postoperative respiratory failure rate and PredictedReadmissionRate were missing. In my opinion I think this level of missingness is significant enough to be regarded as high degree of co-missingness
...
> Your answer here. By Collins. 
MRSA Bacteremia is the most correlated in terms of missingness to our target variable PredictedReadmissionRate.
yes this gives a reason to be concerned:
1.  MRSA Bacteremia is a serious disease, therefore this huge correlation may indicate high cases of readmission for this specific disease. This conclusion is backed by the fact that missing data doesn't necessarily mean absense of data. That is, the disease was not diagnosed.
maybe the patients were readmitted for MRSA Bacteremia but the data was omitted.
...
> Your answer here.By Collins
We partition the data into training and testing sets to train the model and evaluate it performance separately. This separation helps prevent overfitting, where the model becomes to specialized to the training data and unable to generalize and fit new unseen data. By testing on unseen data, we get an unbiased estimate of the models performance and can make necessary adjustments to enable it to generalize well.
...
> Your answer here.By Collins
I got split ratio of 82% for train and 18% for test, which technically very close to the canomical 80-20
...
> Your answer here.By Collins.
MSE (Mean Squared Error) measures the average squared dufference between predicted and actual values as observed in the dataset. In this case MSE is used as a metric to assess theOOB error rates for imputed variables in training dataset. In my own opinion, i think low MSE indicates a closedness between the predicted and observed value therefore, suggesting the missforest is making accurate predictions for the out-of-bag samples.
...
Since p-value = 1.058e-13 is less than 0.05 significance level we reject the null hypothesis that our target variable PredictedReadmissionRate is normally distributed.
...
> Your answer here. By Collins. 
**Why did we choose to use "imputedTest" dataset rather than "ReadyTest"? Am of the opinion that it should be readyTest dataset
...
> Your answer here.
Top left corner shows a darker blue shade indicating a potential positive correlation. In my opinion, this section is made up of patient experience variables (nurse, doctor communication, information discharge, quiteness etc.,) Vs Hospital performance (predicted readmission rate) and resource variable (medicare spending per patient).. This might be explained by the fact that hospitals that perform well on patient experience may aslo have better perfomance in terms lower readmission rate. Hospitals with high medicare spending per patient may want to invest more in there staff, for example in better communication.

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> Your answer here. By Collins
ComparedToNational_Hospital Return days for Pneumonia patients, Nurse communication and, staff responsiveness are the variables showing light blue color an indication that they are relatively correlated with our target variable
...
> Your answer here. By Collins
Yes, I think multicollinearity will be an issue. We have two variables with a vif(variance inflation factors) of around "4" suggesting moderate corelation. However, Overall hospital rating, Recommend Hospital, Nurse communication, care transition and staff responsiveness show high correlation with a vif of more than 5, suggesting they are highly correlated to each other. these vatriables are most likely to cause multicollinearity problem.

>Am speculating that, since Overall hospital rating is a score, its possible it could have been created from weighted average or a combination of various patient experience metrics such as nurse communication, care transition, staff responsivenes, communication about medicines etc., and that is why it has the largest vif and highly correlated with these variables.
...
> Your answer here. By Collins 
principal component (PC11) = 0.83983 (83%)
...
> Your answer here. By Collins
principal component (PC1) =  0.2241 cumulative proportion (22.41%)
principal component (PC18) =  1.00000 cumulative proportion (100%)
22.41 % attributable portion of the variance in PC1 is on the lower end of the spectrum and therefore I would classify it as low. Being the first Preincepal component, I would expect it to ex[plain atleast 50% of the variance.
...
> Your answer here. By Collins
Am going with five principal components, this cumulatevely explains around 55% of the variance which is well above the 50%
...
> Your answer here. By Collins
composite.patients safety, postoperative respiratory failure rate, bc-predicted readmission rate
...
> Your answer here.By Collins
The top contributing variables: Communication about medicines
...
cleaniness. 
All these variables are mostly likely from the same source. They represent patient experience
PredictedReadmissionRate variable is the last in line of of the  variables that are significantly exlaining variance. We can say it sit on the borderline
...
> Your answer here. By Collins
Am going with K4. it shows clearly four cluster which account for close to 8-% of the data. K5 feels like its starting to get a little clouded. The cluster represented by green in K5 feels like a little bit of everything. K2 and K3 on the other hand feels like they are too shallow. clusters represents a very huge area explaining alot of generalizations rather than capture unique patterns or clusters
...
> Your answer here By Collins
Within-sum-of-squares (WSS) measures the quality of clustering, it calculates the sum of squared distances between each data point and itd assigned cluster centroids. A lower wss value suggest better clusting in that the data points are closer to their repsective centroids.
according to the plot, the optimal number of clusters seems to be leaning towards 5 rather than 4 but i would still feel comfortable settling with 4.
...
> Your answer here By Collins
Honestly am very surprised, because all along i have downplayed the significance of cluster two even though there was compelling evidence. My choice of cluster four is with no doubt not the best, not even the second best but a distance third. Going by the Silhouette scores (average silhouette width) cluster two has the highest score at 0.15. At this point I do agree that this score confirms the consistency shown by WSS, Kmeans and PCA.
...
#I think you meant higher in clusetr 2 than cluster 1 
...
"Communication About Medicines", "These hospitals excel inclearly explaining medications to patients, ensuring what they are taking and why ",
"Composite Patients Safety", "Hospitals in this cluster have a strong track record of patients safety, with lower rates of complications and adverse events  ",
...
4. No high leverage/ influence points. This assumption is measured using cooks distance value. A high value of more than 1 indicates that such points are highly influential. Going by our cooks distance plot, we see some peaks but those peaks are well below one at 0.03, 0.02, and 0.01. We can conclude, therefore, that even though there are some peaks indicating some level of influence the threshold is not signicance enough and thus our no high leverage assumption was met.
...
> Your answer here. By Collins
 Linearity assumption of Ols. Based on residuals Vs Fitted plot, it appears that the linearity assumption of OLS regression is met. The residuals are randomly distributed around the horizontal axis without any obvious pattern, suggesting that the relationship between the variables can be adequately modeled with a linear equation
...
> Your answer here.By Collins
In our case above we are using 5-fold repeated cross-validation, which means that the data is splitted into five folds, and the model is trained in four folds while being evaluated on the remaining 5th fold. We repeat the process multiple times to obtain a more roburst estimates of the model performance.
...
1. Hospital return days for pneumonia patients
2. medicare spending per patients
3. Death rate for pneumonia patients
4. state
5. Venous thromboembolism prophylaxis
In comparison to important features from OLS regression, these same features ranked the most important in absolute terms
...
> Your answer here.By Collins
Going by RMSE, the training data achieved 0.78 which increased in testing data to 0.81. This slight increase in RMSE may indicate the model is experiencing a slight overfitting, however, since the increase is not drastic its an indication that the model is still generalizing reasonably well to new data
...
> the graph above showing Elastc net vspredictive power shows a somewhat linear relationship between the predicted and the actual; readmissions rates, but there is a considerable scatter around the regression lines.This suggests that while there might be some degree of linearity, other factors could be contributing to the low R-squared values.
