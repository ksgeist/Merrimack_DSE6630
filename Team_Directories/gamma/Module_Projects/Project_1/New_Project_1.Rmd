---
title: 'New_Project 1: Biomedical & Clinical Informatics'
...
## 1.2 Objective Adventure 1.

To use Heart Failure as our condition of choice and Predicted Readmission rate as our target variable to run through the analyses. focus is on understanding cleaning, pre-processing, and the ML analyses. 
...
# load("FY2024_data_files/dataAnalyzeFullyEncoded2024.Rdata")
# dat2Analyze <- dataAnalyzeFullyEncoded
```
...
load("FY2024_data_files/dataAnalyzeNoEncoding2024.Rdata") #dataAnalyzeNoEncoding2024.Rdata
dat2Analyze <- dataAnalyzeNoEncoding
```
...
Am going with option two. As a matter of best practise, data encoding should come after data partitioning. The essense is to avoid data leakage which will lead to overfitting. Data encoding before partition may expose the model to information in the test set. That is the model learns from the same data it's supposed to be tested on. In our case here, ordinal encoding is exceptional and doesn't lead to data leakage, but for the purpose of best practice I choice option two
...
if(!exists("dataAnalyzeFullyEncoded")) {
...
longTargets %>% 
  ggplot(aes(x = Value, y = NatlComparisonHospitalDays, fill = NatlComparisonHospitalDays)) +
  geom_violin(alpha = 0.7) +
   geom_boxplot(width = 0.1) + #my first thought was to add it immediately before but then the boxplot was somehow obscured and not visible, add it after like it is makes its more clear and visible by Collins
  labs(title = "Which target to use?",
       subtitle = "By readmitted LOS in hospital compared to national average",
        x = "",
        y = "Rate or Score",
...
In my opinion, I think Predicted readmission rate would be the best feature to set as target variable. Its violin plot is very distinctive with different median values for each sub-category: "better", "Same", "Worse", "Uknown" . Hospitals categorized as better tend to have lower predicted readmission rates as shown by a slightly lower median, indicating better quality of care.
The distribution of predicted readmission rates is relatively symmetric across hospital vcategories, suggesting that the prediction model is fairly consistent in its estimates.
...
> Your answer here. By Collins. 
Yes the predicted Readmission density plot shows clearly three distinctive peaks with distinctive skewness. the same density plot is centrally place as expected while better skews to the left indicating a lower predicted Readmission rate and worse skews towards the right indicating a higher readmission rate compared to the "same" which represents the national average
...
#by Collins, Higher medicare spending per patient could indicate that the hospital is dealing with age advanced population (old folks) who probably have chronic donditions requiring Intensive care. The medicare spending seems to be increasing with number of readimission, but at a higher rate in "worse hositals" compared to "Better hospitals"
...
#by Collins I think higher scores indicates a very strong performance of the hospital in terms of thromolism prevention, early identification and interventions and high quality of hospital resources either personel, equipments and methodology and procedures
...
longScore %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of Predicted readmission rates-pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
> Your answer here.By Collins
The plots of both Doctor and Nurse communication show a negative relationship between readmission rate and effective communication. Thats is, as doctors' and nurses' communication effectiveness increases the rate of predicted readmission rate decreases significantly for all categories (Better, same, and Worse)
Higher nurse and doctor communication is associated with improved patients outcomes, including reduced readmissions. Effective communication may lead to:
better understanding of treatment plans and medication regimens
increased oatient engagement and adherence  to care plans
enhanced patient satisfaction and trust in healthcare providers
more accurate diagnoses and timely interventions

>cleanliness is a critical aspect of hospital care. However, cleanliness plot shows a very mild negative relationship (that is as the level of cleanliness increases predicted readmission rate decreases mildly). In "Better" hospitals the predicted readmission rate is almost unchanged as cleanliness increases. Our plot, threfore, suggest that cleanliness may not have a significant impact on pneumonia-related readmissions, this might be because:
1. Cleanliness in a hospital is more of a norm than exception, that is hospitals are expected to maintain high standards of cleanliness, and deviations from these standards are very rare. As a results cleanliness may not be a significant differentiator in terms of readmission rates.
2. Readmissions are more often related to underlying health conditions such as chronic obstruction pulmonary disease (COPD) or heart failure than hospital cleanliness.
...
longHCAHPS %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS!
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                ...
                show.legend = T) + 
    ## CHANGE THE TITLE FOR FULL CREDIT!
    labs(title = "Relationship of Predicted Readmission Rates and pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
        x = "Rate or Score",
        y = "Predicted Pneumonia-related Readmissions",
        color = "Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
...
> Your answer here.BY Collins
in my own opinion, I find Hospital rating and Recommend Hospital to be the most compelling. Hospital rating and Recommend Hospital plots shows a strong negative correlation with readmission rate.This could be because these two features are probably ratings made up of features such (quality care, patient outcome, communication etc.). I think these two features are more compelling and better in describing a hospital in general.
>communication about medicines and discharge information, both display moderate negative correlations indicating that effective communications about medicines and better discharge of information are associated with lower readmission rates. These findings may suggest that hospitals can focus on improving these aspects of patients care to reduce heartfailure-related hospital readmissions.
...
#Collins. Yes I do agree with your choice. Both Violin and Density plots shows that predicted readmission rate is the most distinctive target in terms of median and symmetry in classifying hospitals in different sub-categories that is "Better", "Same", "Worse" and "Unkown". 
...
> Your answer here.
One potential source of bias in our data collection process is the reliance on patient surveys completed after receiving medical attention. The health status of patients at the time of responding may influence their perceptions of the service quality, leading to biased responses. Additionaly there could be systematic errors introduced during data collection such as incomplete or inaccurate records as evidenced by the huge missingness (close to 330) witnessed in the target feature observed readmission rate.
...
> Your answer in the comments. By Collins

```{r}
dat2Analyze <- dat2Analyze %>% 
  ## COMMENT HERE # selected to remove other potential target variables, columns containing sample, number of patients and, NumberSurveyscompleted
  select(-ExpectedReadmissionRate, 
         -ExcessReadmissionRatio,
         -observed_readmission_rate, 
         -contains(c("Sample_", "NumberOfPatients")),
         -NumberSurveysCompleted) %>% 
  ## COMMENT HERE renaming the following columns to shorten their names for clarity and easy to read
  ...
  ## COMMENT HERE selects only the relevant columns (predictedReadmissioRate and everything else) for further analysis
  select(PredictedReadmissionRate, everything())

## COMMENT HERE uses gsub to replace the string 'Score_|HcahpsLinearMeanValue_|_Payment' with an empty string "" removing it from the column name
...
> Your answer here.By Collins. Removing features with little variance is important because they don't provide much information for modeling purposes. Features with Near-zero variance are essentially constant and do not contribute to the variability in the data. By reducing them, we can reduce dimensionality of the data and improve performance as well as prevent overfitting.
...
> Your answer here. By Collins. Moving `FacilityID' to rownames transforms it into an index thus representing each row as a unique observation. This also prevents it from being used as a feature in the model which could lead to errors or incorrect outcomes. 
...
> Your answer here. By Collins. Typically a big-P, Little-N problem occurs when there are more features (P) than observations (n) sometimes leading to overfitting and difficult in interpretation. However in our context we are not experiencing this problem, Furthermore actionable steps such as removing near-zero variance features, help in reducing our dimensions thus mitigating against big-p, little-n problem. For example this problem is experienced in gene expression analysis that is (P[number of genes] is large e.g., tens of thousands of genes Vs N[number of samples] is relatively small, maybe a couple hundreds samples)
...
#100% of the features has some level of data missingness
...
#from my code, it shows that all the 31 columns have missing data, so technically no column has a complete data
...
> Your answer here. 643 observations
...
library(dplyr)
library(mice) #  Ihad to load mice here because without it i constantly got md.mce error even after updating it
miceMatrix <- dat2Analyze %>% select(-State) %>% md.pairs()

sum(is.na(dat2Analyze$State) > 0) # Collins. my first thought was that maybe state has no missingness even though my earlier confirmation revealed that all 31 variables had missingness. my 2nd reasoning was that maybe because state is a character variable but then its not the only character variable. My last opinion is maybe because missingness in state is not important in this specific analysis. 
#To add on this, i think we are dropping state because it has high number of unique values (that is all the state are unique) this will lead to issues in analysis that rely on pairwise comparisons or correlation. This situation is known as High cardinality
```
...
> Your answer here.BY Collins. cell $[6,1]$ shows 1865 observations where both Postoperative respiratory failure rate and PredictedReadmissionRate were missing. In my opinion I think this level of missingness is significant enough to be regarded as high degree of co-missingness
...
> Your answer here. By Collins. 
MRSA Bacteremia and ComparedToNational_MRSA Bacteremia are the most correlated in terms of missingness to our target variable PredictedReadmissionRate.
yes this gives a reason to be concerned:
1.  MRSA Bacteremia is a serious disease, therefore, huge correlation may indicate high cases of readmission for this specific disease. This conclusion is backed by the fact that missing data doesn't necessarily mean absense of data. That is, the disease was not diagnosed.
maybe the patients were readmitted for MRSA Bacteremia but the data was omitted and this gives us another reason to probably understand the equal missingness in ComparedToNational_MRSA Bacteremia.

## 3.3 Drop rows that are missing from the target variable. 

We will not be able to use those rows at all moving forward, sadly. That is going to reduce our sample size pretty sizably, from $N = 4,816$ to only $N = 2,635$ (or from $N = 4,775$ to only $N = 2,731$ .
...
> Your answer here. By Collins
Yes, considering the fact that we are losing close to half the rows, I think this significant reduction in sample size may affect the representation of the data and the power of any model.
...
> Your answer here.By Collins
We partition the data into training and testing sets to train the model and evaluate it performance separately. This separation helps prevent overfitting due to data leakage, where the model becomes to specialized to the training data and test data unable to generalize and fit new unseen data. By testing on unseen data (test set), we get an unbiased estimate of the models performance and can make necessary adjustments to enable it to generalize well.

> Data leakage happens when the model gets information it shouldn't have, making it perform well on test data but poorly on new data. This can lead to overestimating the model's performance.
...
calcSplitRatio <- function(p = NA, df) {
 ...
 #By Collins ## COMMENT HERE sets a condition if the number of (p) is not given
  if(is.na(p)) {
    p <- ncol(df) -1   ## COMMENT HERE then let (p) be the number of columns (features) minus 1
  }
  
  ## COMMENT HERE calculating the optimal number of the testing set
  test_N <- (1/sqrt(p))*nrow(dat2Analyze)
  ## COMMENT HERE turning test optimal number into proportion
  test_prop <- round((1/sqrt(p))*nrow(dat2Analyze)/nrow(dat2Analyze), 2)
  ## COMMENT HERE calculating training proportion. We do know test proportion plus training proportion equals to 1
  train_prop <- 1-test_prop
  
  ## COMMENT HERE printing out the results of each set proportion
  print(paste0("The ideal split ratio is ", train_prop, ":", test_prop, " training:testing"))
  
  ## COMMENT HERE returning the size of training proportion
  return(train_prop)
}

## Fill in the blanks to run: Here we use the function that we have built and pass it to our df to give us train_prop
...
> Your answer here.By Collins
I got split ratio of 82% for train and 18% for test, which technically is very close to the canomical 80-20. Apparently this is the exact same split ratio I observed in pneumonia data set
...
data_sans_encoded = train %>% 
  select(-PredictedReadmissionRate,
         -State,
         -`PaymentCategory for pneumonia patients`, # this one was missing, its categorical
         -`Emergency department volume`, 
         -contains("ComparedToNational"))

data_cat = train %>% 
  select(`Emergency department volume`, `PaymentCategory for pneumonia patients`, 
         contains("ComparedToNational")) %>% 
...
> Your answer here.By Collins.
MSE (Mean Squared Error) measures the average squared dufference between predicted and actual values as observed in the dataset. In this case MSE is used as a metric to assess the OOB error rates for imputed variables in training dataset. low MSE indicates a closedness between the predicted and observed value, suggesting that missforest is making accurate predictions for the out-of-bag samples.
PCF(Proportion of Falsely Classified) values indicates the proportion of falsely classified observations for each variable in random forest model. In short it measures the percentage of times a variable is incorrectely classified as important when actually it is not.
Variables with the highest OOB error for MSE include: Payment for pneumonia patients, Median time (minutes) patients spent in ED, Healthcare workers given influenza vaccination, Venous Thromboembolism Prophylaxis. For PCF, include: Emergency department volume and ComparedToNational_Hospital return days for pneumonia patients
...
var(train$`ComparedToNational_Hospital return days for pneumonia patients`, na.rm = T) 
#var(train$`Median time (minutes) patients spent in ED`, na.rm = T) #ByCollins my train doesn't seem to have this feature Median time (minutes) patients spent in ED
...
# Van Buuren, S. (2018). Boca Raton, FL.: Chapman & Hall/CRC Press.
```
...
#BY Collins
#dim(imputedTrain$State)
#dim(imputedTrain$ximp) # this is returning null
#dim(imputed_data$ximp) #object not found
#dim(train$PredictedReadmissionRate)
```
...
data_sans_encoded = test %>% 
  select(-PredictedReadmissionRate,
         -State, -`PaymentCategory for pneumonia patients`, - `Emergency department volume`,
         -contains("ComparedToNational"))

data_cat = test %>% 
  select(contains("ComparedToNational"), `PaymentCategory for pneumonia patients`, `Emergency department volume`) %>% 
  mutate_all(as.factor)
...
summary(data.frame(Original = test$`MRSA Bacteremia`, Imputed = imputedTest$`MRSA Bacteremia` ))

table(Original = test$`PaymentCategory for pneumonia patients`) 
table (imputed = imputedTest$`PaymentCategory for pneumonia patients`)
```

> Your answer here. By Colllins
Summary statistics of MRSA Bacteremia shows that the imputed values have a slightly higher 1st quartile, median, and 3rd quartile compared to original values, however, the overall distribution hasn't changed drastically. Similarly, the table comparing original and imputed values of categorical variable PaymentCategory for pneumonia patients shows a very similar distribution for both values with only a slight increase in the number of patients with "No Different Than the National Average Payment". Based on this analysis, it appears that the imputation did not drastically change the distributions or frequences of these variables and that the imputation process was reasonably successful.
...
> Your answer here. By Collins
In a case where we have a class or N/As in Train which happens not to be in test, then we have have to replace such a class or N/A with a zero.
...
ggplot(readyTrain, aes(x = PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 ...
                 color = "orchid") +
  theme_minimal() +
  labs(title = "Predicted Readmission Ratio, N = 2,635 hospitals",
       y = "Frequency",
...
> Your answer here. By Collins
Looking at the histogram above, one could be easily mistaken and conclude it somewhat normal. Which I think it is, but from a close look, we can see the distribution is skewed to the right a little bit. Histogram and Q-Q plots are insufficient to assess normality because they majorly depend on visual intrepretation which more often than not is influenced by personal bias. In addition, these plots fails to capture subtle deviations from normality. 
...
> Your answer here. By Collins
The null hypothesis of Shapiro-Wilk test is that the data is normally distributed and the alternative hypothesis is that the data is not normally distributed all tested at 0.05 significance level.
Since p-value = 0.00000003308 is less than 0.05 significance level we reject the null hypothesis that our target variable PredictedReadmissionRate is normally distributed.
...
We can see that the estimated $\lambda = -0.0999999999999999$, so __approximately a square-root transform__ (see lecture slides for reference). I found that $\lambda = -0.5$ in FY 2025, for those using that year's data.
...
ggplot(readyTrain, aes(x = bc_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 ...
                 color = "hotpink") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Predicted Readmission Ratio, N = 2,635 hospitals",
       y = "Frequency",
       x = "Predicted Readmission Ratio")
...
> Your answer here. By Collins. 
When performing a Box-Cox transformation, its crucial to use the same lambda value for both the training and testing datasets to ensure consistency and prevent overfitting. This consistency allows for fair comparison between the training and testing data, enabling accurate model evaluation. Transforming the test dataset is also necessary to simulate real-world scenarios and assess the model's ability to generalize to new data. By applying the same transformation to both datasets, you can evaluate the model's performance without introducing data leakage

```{r}
# Your code here. By Collins
#bc_datatest <- BoxCoxTrans(imputedTest$PredictedReadmissionRate) This plan backfired when it turned out the lambda was -0.9999... compared to -3 found in train
#print(paste0("Box-cox lambda = ", bc_datatest$lambda))

imputedTest$bc_test_PredictedReadmissionRate <- predict(bc_data, imputedTest$PredictedReadmissionRate)

# Make sure to check that your transform looks successful!
ggplot(imputedTest, aes(x = bc_test_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6,
                 fill = "brown",
                 color = "brown") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed imputedTest$Predicted Readmission Ratio, N = 488 hospitals",
       y = "Frequency",
       x = "Testset Predicted Readmission Ratio")

qqnorm(imputedTest$bc_test_PredictedReadmissionRate)
qqline(imputedTest$bc_test_PredictedReadmissionRate,
       col = "brown", lty = 2, lwd = 2)
```

repeating all of the steps to perform a Box-Cox transformation on ReadyTest dataset by Collins

```{r}
readyTest$bc_test_PredictedReadmissionRate <- predict(bc_data, readyTest$PredictedReadmissionRate)

# Make sure to check that your transform looks successful!
ggplot(readyTest, aes(x = bc_test_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6,
                 fill = "blue",
                 color = "blue") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Readytest$Predicted Readmission Ratio, N = 488 hospitals",
       y = "Frequency",
       x = "Testset Predicted Readmission Ratio")

qqnorm(readyTest$bc_test_PredictedReadmissionRate)
qqline(readyTest$bc_test_PredictedReadmissionRate,
...
> Your answer here.
Top left corner shows a darker blue shade indicating a potential positive correlation. In my opinion, this section is made up of patient experience variables (nurse, doctor communication, information discharge, quiteness etc.,) Vs Hospital performance (predicted readmission rate) and resource variable (medicare spending per patient, payment category for pneumonia patients).. This might be explained by the fact that hospitals that perform well on patient experience may aslo have better perfomance in terms lower readmission rate. Hospitals with high medicare spending per patient may want to invest more in there staff, for example in better communication.

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> Your answer here. By Collins
Nurse & doctor communication, Discharge Information, and staff responsiveness are the variables showing light blue color an indication that they are relatively correlated with our target variable
...
> Your answer here. By Collins
Yes, I think multicollinearity will be an issue. We have three variables with a vif(variance inflation factors) of around "4" suggesting moderate corelation. However, Overall hospital rating, Recommend Hospital, care transition, and Nurse communication show high correlation with a vif of more than 5, suggesting they are highly correlated to each other. these vatriables are most likely to cause multicollinearity problem.

>Am speculating that, since Overall hospital rating is a score, its possible it could have been created from weighted average or a combination of various patient experience metrics such as nurse communication, care transition, and communication about medicines etc., and that is why it has the largest vif and highly correlated with these variables.

> most likely payment category for pneumonia patients was created from payment for pneumonia patients and is likely part of medicare payments. Since our goal is to predict readmission rate, I think retaining payment category for pneumonia patients would be more relevant especially consideraing results interpretation. Personally I would be interested to know what category of pneumonia patients based on their medicare payment suffered the most readmissions.
...
> Your answer here. Overall hospital rating and Recommend.Hospital both have a vif of more than 10 that is 25.19 and 16.33 concecutively. These variables are in the top left corner area shaded dark blue. This explaines that they are positively correlated with our target variable. I also think the fact that their vif is positive explains their relationship with target variable
...
#JUST WONDERING WHY IS IT THAT WE DID NOT DROP "PAYMENT CATEGORY AND SURVEY RESPONSE RATE" IN READYTEST? DOES IT MATTER IF THE NUMBER OF FEATURES IN READYTRAIN DIFFER WITH READYTEST?
...
> Your answer here. By Collins 
principal component (PC11) = 0.8129 (81.29%)
...
> Your answer here. By Collins
principal component (PC1) =  0.2412 cumulative proportion (24.12%)
principal component (PC20) =  1.00000 cumulative proportion (100%)
24.12 % attributable portion of the variance in PC1 is on the lower end of the spectrum and therefore I would classify it as low. Being the first Preincepal component, I would expect it to ex[plain atleast 50% of the variance.
...
> Your answer here. By Collins
In my own opinion the "elbow" point appears to be around 3 or 4. There is a noticeable drop from PC1 to PC2 but the amount of variance explained is very minimal, for this reason am settling with four principal components, this cumulatevely explains around 48.5% of the variance which is almost half.
...
> Your answer here. By Collins
After a little digging I found that its refereed as scree plot due to it resemblance to the loose rock debris (scree) at the base of mountain. The plot's shape, with a steep drop-off in eigenvalues, is likened to the accumulation of rocks and debris at the base of a slope.
...
> Your answer here. By Collins
mediam time patients spent in (ED) and Composit.patients safety
...
> Your answer here.By Collins
Yes, all these variables are mostly likely from the same source. They represent patient experience.The top contributing variables include: 
Communication about medicines
staff responsiveness
Doctors communication
discarge information
quiteness
Composite patients safety
cleaniness. 
...
for(k in 2:9) {
  ...
  kmeansResult <- kmeans(readyTrain, 	
                     ...
                     nstart = 25,   ## number of times to repeat the process 
                                    ## with random initialization; increase 
                                    ## if you're failing to converge
                     iter.max = 1000,    ## number of iterations to run k-means
...
> Your answer here. By Collins
Am going with K3. it shows three cluster which account for close to 80% of the data.The remaining 20% or less of the total data seems to be scattered (I oresume the are outliers) K4 feels like its starting to get a little clouded where clusters are sharing common data points.
...
> Your answer here By Collins
Within-sum-of-squares (WSS) measures the quality of clustering, it calculates the sum of squared distances between each data point and cluster centroids. A lower wss value suggest better clusting in that the data points are closer to their repsective centroids.
according to the plot, the optimal number of clusters seems to be leaning towards 2 where we witness the first "elbow". 
...
> Your answer here By Collins
Going by the Silhouette scores (average silhouette width) cluster two has the highest score at 0.15. At this point I do agree that this score, it confirms the consistency shown by WSS, Kmeans and PCA.
...
new_imputedTrain <- imputedTrain[rownames(readyTrain),] # I have added this line of code because i was running into errors due to difference in the number of observations between readyTrain and imputedTrain. By Collins


## COMMENT HERE calculating k means for each k-classes in readytrain
kmeansResult <- kmeans(readyTrain, 	
                         centers = k, # number of clusters (k)     ## COMMENT HERE
                         nstart = 50, # number of times to repeat runnining the process    
                         iter.max = 1000,  # number of iterations to run k-means
                         algorithm = "MacQueen") # the algorithm we will be using
## COMMENT HERE create kmeanstrain dataframe with renamed and droped off unwanted variables
kmeansTrain <- new_imputedTrain %>%  #b4 had imputedTrain updated to new_imputedTrain just to match then number of observations between readyTrain and imputedTrain datasets
  ## Rename the column we want to keep
  rename(Pneumonia.Hospital.Return.Days = `ComparedToNational_Hospital return days for pneumonia patients`) %>% 
  ## COMMENT HERE drop the selected variables
  select(-any_of(vifOver5), -contains("Compared"), -contains("PaymentCategory"), -SurveyResponseRate) %>% 
  ## COMMENT HERE this code seeks to encode categorical pneumonia hospital return days into 1, 0 and -1
  ...
  ## COMMENT HERE this cleans and standardizes column names
  clean_names()
...
## COMMENT HERE converts pcaImportantVars into lower case
pcaImportantVars <- tolower(pcaImportantVars)
## COMMENT HERE replaces periods (.) with (_)
pcaImportantVars <- gsub("\\.", "_", pcaImportantVars) %>% 
## COMMENT HERE converts variable names to title case
  to_any_case("title")

## COMMENT HERE performs k-means clustering
kmeansTrain %>% 
  ## COMMENT HERE renames the following variables (columns)
  ...
  ## COMMENT HERE clean column names by converting them into case title format
  clean_names(case = "title") %>% 
  ## COMMENT HERE selects columns in pcaImportantVars
  select(Cluster, any_of(pcaImportantVars)) %>% 
  ## COMMENT HERE converts the data from wide to long and exclude (-1) the first column (cluster column), assigns names_to  measure and values_to value
  pivot_longer(-1, names_to = "Measure", values_to = "Value") %>% 
  ## COMMENT HERE filters the data to only include the rows with levels where the measure column us present in pcaImportantVars
  ...
  ## COMMENT HERE creates a boxplot to visualize the distribution of values across different clusters
    ...
    theme(legend.position = "bottom",
        ...
        legend.title = element_text(size = 10),
        title = element_text(size = 12, color = "maroon")) + 
    ## COMMENT HERE facet the plot by measure
    facet_wrap(~ Measure, 
             scales = "free_y",
             ncol = 2)

## COMMENT HERE renames columns in the kmeansTrain dataframe
kmeansTrain %>% 
  ## COMMENT HERE renames the following two columns
  rename(Predicted.Readmission.Rate = predicted_readmission_rate,
         Periop.Rate.Of.Embolism.Or.Thrombosis = perioperative_pulmonary_embolism_or_deep_vein_thrombosis_rate) %>% 
  ## COMMENT HERE clean column names by converting them into case title format
  clean_names(case = "title") %>% 
  ## COMMENT HERE selects columns that match the pattern
  select(Cluster, any_of(pcaImportantVars)) %>% 
  ## COMMENT HERE creates a summary table grouped by the clusters
...
"Communication About Medicines", "These hospitals exceled in explaining medications to patients, ensuring they knew what they were taking and why ",
"Quiteness", "These hospitals provided a very quit and peaceful environment, enabling the patients to heal quickly",
"Discharge Information", "Patients receive comprehensive discarge instructions, including follow-up appointments and medication management plans ",
"Cleanliness", "Hospitals in this cluster maintain high standards of cleanliness, reducing the risk of hopsital-acquired infections "
...
dict %>% 
  kable(
    ...
    pack_rows("2: Other Notable Features", 5, 6) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> Your answer here.By collins
Based on analysis I would recommend Random Forest classifier to test whether these clusters can be used for robust prediction of hospital performance. This is because Random Forest classifiers are known to perform well on complex datasets and can handle high-dimensional data. To evaluate the robustness of the predictions I would look for the following:
High accuracy (> 0.8)
High precision (>0.8)
High recall (>0.8)
High AUC-ROC (>0.8)
If the model shows a good performance on these metrics, then it indicates clusters can be used for robust prediction of hospital performance for pneumonia patients.
...
> Your answer here.By Collins
1. Linearity assumption of Ols. Based on residuals Vs Fitted plot, it appears that the linearity assumption of OLS regression is met. The residuals are randomly distributed around the horizontal axis without any obvious pattern, suggesting that the relationship between the variables can be adequately modeled with a linear equation
2.Normally distributed residuals. Our Q-Q plot shows residuals that are largely normally distributed with minor deviations at the extremes. This therefore, suggest that the normality assumption is largely met.
3.Homoscedasticity. Given our scale-location plot shows residuals that are randomly scattered without a clear pattern of changing variance across different levels of fitted values, it suggests that the our assumption of homoscedasticity was met
4. No high leverage/ influence points. This assumption is measured using cooks distance value. A high value of more than 1 indicates that such points are highly influential. Going by our cooks distance plot, we see some peaks but those peaks are well below one at 0.015, 0.013, and 0.0057. We can conclude, therefore, that even though there are some peaks indicating some level of influence the threshold is not signicance enough and thus our no high leverage assumption was met.
...
> Your answer here. By Collins
The shapiro-wilk normality test above shows a width (W) of 0.9996 which is very close to 1 indicating that the data is most likely normally distributed. Our p-value = 0.9551 is greater that 0.05 confidence level, therefore, we fail to reject Null hypothesis that the data is normally distributed. 
Shapiro-Wilk normality test confirms our residuals Vs Fitted plot that the linearity assumption of OLS regression is met. The residuals are randomly distributed around the horizontal axis without any obvious pattern, suggesting that the relationship between the variables can be adequately modeled with a linear equation
...
> Your answer here.By Collins
Adjusted R-squared is a more accurate measure of model's goodness fit. Unlike unadjusted R-squared, adjusted R-squared only increases if the new predictor improves the model's fit more than would be by chance. On the other hand unadjusted R-squared increases with the increase in the number of predictors even if they are not significant, therefore, can be misleading when comparing models' fit with different number of predictors.
...
> Your answer here.B y Collins
While, RMSE and R^2 are useful metrics for evaluating model performance, they may not be directly interpretable in this case due to the Box-Cox transformation and centering and scaling of the data. Undoing the transformations would be necessary to interpret the results in terms of the originl units, but this may not be practical. Therefore at this point, our ability to communicate results to our client is limited.
...
ggplot(readyTest, 
       aes(x = predictions, y = bc_test_PredictedReadmissionRate)) +
...
> Your answer here.By Collins
In our case above we are using 5-fold repeated cross-validation, which means that the data is splitted into five folds, and the model is trained in four folds while being evaluated on the remaining fold of the 5 folds. We repeat the process multiple times to obtain a more roburst estimates of the model performance.
we have employed expand.grid that allow alpha to range from 0 to 1 and lambda from 0 to 5 thus enabling us to explore a range of regularizations strength. When alpha = 0, the model becomes ridge regression (L2 regularization only)
When lambda = 0 there is no regularization, and the model becomes ordinary least square (OLS regression that is L1 only). Our intentions for using expand.grind is to tune lambda and alpha so that we can find the optimal combination that gives us the best performance on the training data.
...
> Your answer here. By Collins
Both best lambda and alpha values are zero. This suggest that the optimal regularization being fit is an ordinary least squares (OLS) regression.
...
> Your answer here.By Collins. The most important features are
1. Doctor communication
2. Hospital return days for pneumonia patients
3. state
4. Discharge Information
5. medicare spending per patients
6. Death rate for pneumonia patients
In comparison to important features from OLS regression, the order of importance is the same.
...
> Your answer here. By Collins
Feature importance: In PCA, feature importance is determined by the magnitude of the coefficients (loadings) of the original variables in the principal components.Variables with higher absolute loadings are considered more important for explaining the variance in the data. In short PCA calculates the contribution of each variable to the principal components and then ranks them in the order of their contribution. On the other hand in Regression, feature importance is typically evaluated through coefficients that represents the change in an independent variables. The magnitude and significance (often deternibed by p-values) of these coefficients indicate the importance of each feature in predicting the outcome variable. In summary, Regression focus on the prediction of the outcome variable. Features with larger absolute coefficients and low p-values are considered more important

>In PCA, importance refers to how much a variable contributes to the principal components, explaining the variance in the data. While in Regression, importance refers to how much a variable affects the prediction of the outcome/target variable, often measured by coefficients and their significance.

>The sign of loading in PCA indicates the direction of the relationship between the variable and the principle component. However, the sign is arbitrary because PCA is sensitive to the direction of the data. Multiplying a variable by say (-1) would flip the sign of its loading but not the change in its importance.

#### 6.2.3.2 Check model performance

Assessing __model performance__ means assessing how well our training model does when applied to "unseen" data, here our `readyTest` dataset. Because we know the right answer - the true value of `PredictedReadmissionRate`, we can assess the error that the training model produces when fitted against the testing data. We measure that error as $RMSE$ and $MAE$. We briefly reviewed what the $RMSE$ measures back in [Question 26]{#rmse}, so I will only review what the $MAE$ is here.
...
prOLS <- postResample(pred = predict(mod, newdata = readyTest), 
                      obs = readyTest$bc_test_PredictedReadmissionRate)
...
temp$actual <- rep(readyTest$bc_test_PredictedReadmissionRate, 2)
...
> Your answer here.By Collins
Both OLS and Elastic net have similar performance on both training and test sets.low Rsquared values indicate that the performance was not exceptionally well and only explain a small portion of the variance in the actual readmission rates. This could be potentially because of fcators such as non-linearity in the data, omitted variables bias, or high variability in the data the model cannot capture.
> the graph above showing Elastc net vs predictive power, a somewhat linear relationship between the predicted and the actual; readmissions rates, but there is a considerable scatter around the regression lines.This suggests that while there might be some degree of linearity, other factors could be contributing to the low R-squared values.

>RECOMMENDATIONS
1.Explore non-linear models. Given the scatter in the data, exploring non-linear models such as decision trees or support vector machines might better capture the underlying relationships and improve predictive power.
2.Feature engineering: creating new feature from the existing ones (polynomial transformations) could help in capturing non-linear relationships and improving model performance
