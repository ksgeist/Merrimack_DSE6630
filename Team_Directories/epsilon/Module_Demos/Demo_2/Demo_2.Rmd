...
>This is to make sure where ever the point on the grid/matrix (map) it can line up and connect the lines to form the polygon in the given area it needs to represent a specific area.
...
#for MH: load("C:/Users/User/Documents/GitHub/Merrimack_DSE6630/Merrimack_DSE6630/Team_Directories/epsilon/I. Biomedical & Clinical Informatics/Demo_1/pneumoniaFull.Rdata")
...
> From the last set of data we did need to delete the information we would need for our maps, specifically data like 'Address', 'CityTown', 'Zipcode'; since this type of data we really didn't need for our previous analysis so we didn't keep our bloated with info unneeded for our previous models.  
...
> The states data set has the states by lowercase fully spelt out. The pneumoniaFull data set have states in their abbreviated form. We have to convert the abbreviations to full state name to have an easier time merging the state.fips information into the 'pneumoniaFull' df.
...
```{r}
state.name[match(pneumoniaFull$State, state.abb)]
...
> We would need to see where these particular islands would line up with the counties which would be tedious work. We'd then need to seperate the colons, delete them, then match up the islands to the particular counties; which can potentially break the table into more sections than we'd really need.
...
> Each state is represented as expected. Maryland is missing medicare spending, otherwise there isn't any missing data since the data from every state was split into seperate rows, and will find the mean and median to their state respectively. It'll leave behind all of the other info we don't need to do our specific region / state analysis.
...
> It can give a bias account for what is really going on in the data from the mean can be manipulated by really high outliers on either side of the specific skew. It's kind of like if you get a population of folks income and majority is in range of 40 - 90k but a few folks have a couple million dollars rolling in; it'll put a skewed idea people are living very comfortly in actuality that is far from the truth from those few outliers, the millionairs for distorting the whole data set.
...
Day 5, 5000
-----------------
Mean Sales: 1134 (skewed by the extremely high sales on Day 5)
Median Sales: 150 (a better representation of the typical daily sales)
Using the mean sales might lead to overestimating typical sales, resulting in incorrect decisions. In contrast, the median sales provide a more accurate representation of typical sales, enabling better business decisions."
Resource: <https://www.linkedin.com/pulse/understanding-mean-vs-median-key-accurate-business-decisions-thakur-fo7ff>
...
> For myself I would likely say from the results shown down below, 'Pneumonia Hospital Return Days', 'Pneumonia Readmission Rate', and 'Pneumonia Death Rate'.
Since for the majority of the ones said have large amount of outliers on either side with the mean not really being in the middle the 'y' value on the side. Also all of the these aren't usually a normal occurance enough to use an average of them from the means but more so using the median instead since areas within a feature (state) can effect the major outcome for the state as a whole. There be a potential effect for more spending per subsection inside the polygon (state) but usually states are founded as a state and not per city area. So with that thinking I feel the three I said should be used for their median instead of the mean.
...
#### **Question 8B**: [1 point]
Your task is to repeat this test, with interpretations, for `Score_Death rate for pneumonia patients`, `Score_Hospital return days for pneumonia patients`, and          :
'Score_Medicare spending per patient'
```{r}
agostino.test(pneumoniaFull$`Score_Hospital return days for pneumonia patients`)
```

__`Score_Hospital return days for pneumonia patients`__: Because $p<0.05$, we can reject the hypothesis of symmetry and conclude that Hospital Return Days are __significantly skewed__. We should use the median to aggregate this variable.
...
__'Score_Medicare spending per patient'__: Because $p<0.05$, we can reject the hypothesis of symmetry and conclude that Medicare spending per patients are __significantly skewed__. We should use the median to aggregate this variable.

```{r}
agostino.test(pneumoniaFull$`Score_Hospital return days for pneumonia patients`)
```

__'Score_Death rate for pneumonia patients'___: Because $p<0.05$, we can reject the hypothesis of symmetry and conclude that Hospital return days for pneumonia patients are __significantly skewed__. We should use the median to aggregate this variable.
...
> The means still show some outliers skewing the data, therefore we are comfortable moving forward with median.
...
stateAggPneumonia <- stateAggPneumonia %>%
  select(-meanReadmissions, -meanDeathRate, -meanHospitalReturnDays, -meanMedicareSpending)
```

```{r, label = "Testing for StateAggPneumonia"}
names(stateAggPneumonia)
...
p2 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median % of death rate within 30 days", 
            low = "white", high = skittles[3]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median pneumonia-related death rate by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
## Store the plot into a variable called p3
p3 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
                               ## !!Update the fill to the variable you want
                               fill = medianHospitalReturnDays)) +  
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median of hospital return days within 30 days", 
            low = "white", high = skittles[5]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median pneumonia-related hospital return days by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
## Store the plot into a variable called p4
p4 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
                               ## !!Update the fill to the variable you want
                               fill = medianMedicareSpending)) +  
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median of Medicare spending days within 30 days", 
            low = "white", high = skittles[7]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median pneumonia-related spending from Medicare per patient by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
#Median Prediction Readmissions by State
stateAggPneumonia %>% 
...
#Median Death Rates by State
stateAggPneumonia %>% 
  arrange(desc(medianDeathRate)) %>%
  select(region, medianDeathRate) %>% 
  rename(Region = region, 
         `Median Death Rates by Pneumonia` = medianDeathRate) %>% 
  top_n(10) %>% 
   kable(digits = 2,
    format = "html",
    caption = "Table 4. Top 10 states for pneumonia-related readmissions.") %>%
...
#Median Medicare spent per Patient by State
stateAggPneumonia %>% 
  arrange(desc(medianMedicareSpending)) %>%
  select(region, medianMedicareSpending) %>% 
  rename(Region = region, 
         `Median Medicare spending per Patient infected w/ Pneumonia by State` = medianMedicareSpending) %>% 
  top_n(10) %>% 
   kable(digits = 2,
    format = "html",
    caption = "Table 4. Top 10 states for pneumonia-related infections expenses by Medicare.") %>%
    kable_styling(bootstrap_options = c("hover", full_width = F)
  )

#Median Medicare spent per Patient by State
stateAggPneumonia %>% 
  arrange(desc(medianHospitalReturnDays)) %>%
  select(region, medianHospitalReturnDays) %>% 
  rename(Region = region, 
         `Median Hospital Return Days by State` = medianHospitalReturnDays) %>% 
  top_n(10) %>% 
   kable(digits = 2,
    format = "html",
    caption = "Table 4. Top 10 states for pneumonia-related hospital return days.") %>%
    kable_styling(bootstrap_options = c("hover", full_width = F)
  )
```
...
> At the moment I find a hypthesis from these to visuals.
...
```{r}
unique(stateTrain$region)
```

> Looks like there is 52 including NA, or not including NA 51.

#### **Question 10B**: [0.25 points]

Why am I not asking you to check the states in the testing data?

> Since the Testing data could have any amount of new data that may not contain all of the regions in the Training DF.
...
> So we could specifically see which variables we should look at and what the outcomes could look like if either test or training dataframes are some how off.
...
>The st_as_sf() initailizes the process to convert already made data like our data frame we have to shapefiles w/ geometry columns. Other functions mainly try to import or create geometry objects. This uses what is already been made data. It can also build into the columns like wkt, CRS if provided, and returns a sf data frame after it.
...
By default, the `sf` package now uses Google's S2 geometry engine (a library for geometric calculations on the sphere developed by Google for their maps), which is strict and throws errors when geometries are even slightly invalid (like if edges should cross each other even slightly). We will choose to turn off the S2 engine just for this step because it will fail otherwise. This is often going to be the case __when you are not importing shapefiles directly or if your shapefiles are older and predate the S2 engine__. But, we will turn it back on when we're done. 
...
> What "Rook" contiguity means is when two polygons are neighbors only if they share a common border. It works with "Queen" contiguity since at least the point at the famous "Four Corners" touch. To change it you'll need to have the queen = FALSE for the contiguity to be changed to Rook. It can matter since "Rook" can get rid of numerical amount of neighbors. This can help with the weight of spatial matrix to be more sparse, so it can't influence some potential neighbor data to skew the data.
...
> For queen contiguity it'll be 4.
Based on queen contiguity, you would expect Alabama to have 4 neighbors. 
...
> We only fit the preprocessing towards the training data to help control the amount of possible data leakage that the test data can gain from the fulldata set.
...
# EG: commented out for 14B
# EG: I was getting a case_when error in 15D, so I made this numeric, which fixed the error and did not appear to change any previous answers
y <- as.numeric(scale(states_sf_AggTrain$Median_RawPredictedReadmissionRate,
           center = TRUE, scale = FALSE))

# EG: uncommented this for 14B
...
moran.plot(as.vector(y), 
           ...
           col = skittles[2],
           xlim = c(-3,3), # EG commented out for 14B
...
> Based off of the information in Section 2.2, the quadrants are as follows: top left is low-high (Quadrant II), the top right is high-high (Quadrant I), the bottom left is low-low (Quadrant III), and the bottom right is high-low (Quadrant IV).

> Delaware is a low-high outlier and is located in Quadrant II. This indicates that Delaware has a low readmission rate but is generally surrounded by other states that have high readmission rates.

> West Virginia, District of Columbia, and New Jersey are high-high outliers and are located in Quadrant I. This indicates that these states have high readmission rates and are generally surrounded by other states that have high readmission rates.

> Utah is a low-low outlier and is located in Quadrant III. This indicates that Utah has a low readmission rate and is generally surrounded by other states that also have low readmission rates.

> There are no high-low outliers in Quadrant IV.
...
> If we used the transformed and scaled version, bc_PredictedReadmissionRate, the results would be more difficult to interpret. As it currently is, the high-high, low-low, low-high, and high-low outliers are all explainable and interpretable. Additionally, these values makes sense in terms of lower or higher readmissions. Since centering makes the mean zero, if we had transformed and scaled the values, the analysis would have become more complicated and less interpretable. If y and lagged y are not going in the same general direction, it will be difficult to identify which quadrant they fall into.
...
> The results of the plot change slightly. Delaware is still a low-high outlier located in Quadrant II. Utah (mostly cut off in the plot), is still a low-low outlier located in Quadrant III. There are still no high-low outliers in Quadrant IV. Quadrant I still has New Jersey and District of Columbia as high-high outliers. However, West Virgina is no longer listed here as an outlier. 

> When the target variable was Median_RawPredictedReadmissionRate, the standard deviate was 2.9895 and the p-value was 0.001397. Additionally, the Moran I statistic was 0.268531084, the Expectation was -0.020833333, and the Variance was 0.009368833. When the target variable was changed to bc_PredictedReadmissionRate, the standard deviate increased to 3.1939. The p-value decreased to 0.0007019. The remaining statistics were similar, with a Moran I statistic of 0.288790751, an Expectation of -0.020833333, and a Variance of 0.009397939.

> However, with both target variables, the p-values are < 0.05. Therefore, the null hypothesis of spatial randomness is rejected. Although Moran I statistic does increase slightly from 0.268 to 0.288, the overall results do not change significantly. We are still seeing similar trends for the outliers and the overall plot. The value we choose for the analysis does not seem to have a signficant impact on the results. The overall results do not change significantly. 
...
> According to Table 6, coldspot (Quadrant III) states have low readmissions and also have neighbors with low readmissions. These states would also have a significant p-value (p < 0.05). The centered median readmission rate and the spatially-lagged median admission rate will both be negative. Overall, these coldspots are regions of low pneumonia-related readmissions that co-occur spatially (page 23 of module 3 ppt).
...
>  The spatially-lagged variables seem so shifted compared to the original value. As mentioned earlier in the demo, spatial lags are the weighted average value of neighboring states. The raw readmission values are the raw percentages. Therefore, the values are very shifted since they are not on the same scale when comparing values. This ties back into why we chose the target varaible that was centered but not scaled. It makes it much easier to compare values when they are on the same scale.
...
# Rename code from line 873, which defines y, which is the centered target variable
massachusetts_readmissions <-scale(states_sf_AggTrain$Median_RawPredictedReadmissionRate,
           center = TRUE, scale = FALSE)

# Find Massachusetts data (row 20)
massachusetts_readmissions[20]
...
> As shown in section 2.3.2.1, the spatially-lagged value for Massachusetts is 0.2126469. The centered value for Massachusetts is 0.6571469. Both of these values are positive, indicating that they would be located in Quadrant I, which would be considered high-high and a hot spot.
...
> NOTE: I had to go back to the beginning where we defined y and make it numeric, because I was getting a case_when error. Once I did that, it fixed the error in the below code.

```{r}
# Assign each state to a LISA quadrant
states_sf_AggTrain <- states_sf_AggTrain %>%
  mutate(
 # Create a new variable quadrant
    quadrant = case_when(
  # If P > = 0.05, it is not statistically significant and would therefore be considered not significant (N.S).
      pValue >= 0.05 ~ "N.S.",
      ## COMMENT HERE - Why do I not have to specify p-value again?
  # You don't have to specify the p-value again because we have labeled any non-significant values as N.S.
      
      ## COMMENT HERE ABOUT LINE BELOW - INCLUDE QUADRANT!
      # A hotspot is represented by high-high, which is located in Quadrant I
      y > 0 & lag_medianReadmissions > 0 ~ "Hotspot",
      
      ## COMMENT HERE ABOUT LINE BELOW - INCLUDE QUADRANT!
      # A coldspot is represented by low-low, which is located in Quadrant III
      y < 0 & lag_medianReadmissions < 0 ~ "Coldspot",

      ## COMMENT HERE ABOUT LINE BELOW - INCLUDE QUADRANT!
      # A high-low outlier is located in Quadrant IV, representing states with high readmission rates that are surrounded by states with low readmission rates
      y > 0 & lag_medianReadmissions < 0 ~ "High-Low Outlier",

      ## COMMENT HERE ABOUT LINE BELOW - INCLUDE QUADRANT!
      # A low-high outlier is located in Quadrant II, representing states with low readmission rates that are surrounded by states with high readmission rates
      y < 0 & lag_medianReadmissions > 0 ~ "Low-High Outlier",

      ## COMMENT HERE
      # If none of the instances are true, label the record as NA
      TRUE ~ NA
    )
  )
```
...
> If Indiana is a low-high outlier, it means that it has a low readmission rate, but that it is surrounded by states with high readmission rates. Looking at the graph, this makes sense, as nearby and neighboring states, such as Ohio, Pennsylvania, and Virginia are hotspots (high-high).
...
> The hotspots and coldspots in the graph make sense. In the LISA plot, the following states are in a coldspot cluster: Idaho, Wyoming, Utah, Colorado, and Nebraska. All of the neighboring states surrounding them are gray, indicating that they are not significant. There is a cluster of hotspot states that include Ohio, Pennsylvania, Virginia, and Maryland. Most of their neighboring states are also not signficant. However, Indiana is a low-high outlier.

> Looking at the p1 graph that shows the median pneumonia-related hospital readmissions by state, the midwest states are all relatively lightly shaded, indicating that they have low median predicted readmissions within 30 days. The states in the hotspot cluser are much darker, indicating a higher median predicted readmissions within 30 days. Indiana has a color somewhere in between these two extremes, representing its low-high outlier status. This makes sense when comparing p1 to the results of our LISA plot.

> Hotspots and coldspots will not necessarily correspond with individual states high and low median readmission rates because they are are based on the relationships between clusters, not necessarily the individual states.
...
> If we did not already have the data fully pre-processed, we would have to first import it. Next, we would have to perform any necessary cleaning. Exploratory Data Analysis (EDA) would then be performed, with some basic visuals to better understand the data. After EDA, the test/train split would be created. Next, pre-processing would be performed, which includes centering, scaling, and imputation. Lastly, the spatial weights matrix, W, would be calculated.
...
> We are fitting an OLS regression again because the sample size has decreased due to aggregation. Refitting the model is crucial to ensure the baseline model can compare fairly to other models we run, such as SAR.
...
> The Moran's I test on the residuals shows a p-value of 0.06133, which is greater than 0.05. Since p > 0.05, we fail to reject the null hypothesis of no spatial autocorrelation.
...
> Yes, we could have been potentially mistaken about spatial relationships if we had started here. Without determining the clusters and if there is spatial autocorrelation prior to this step, incorrect assumptions about the data and clusters may have been made. By calculating the Moran's I and LISA, we are able to calculate statistical values that we can use to strengthen our understanding of the data.

- Why might these results disagree with what we found from the Moran's $I$ and LISA on the scaled target?

> These results may disagree with what we found from the Moran's I and LISA on the scaled target because Moran's I and LISA are looking at spatial autocorrelation. Since the scaled target is on a different scale, the various statistical values that are run will be differernt, therefore, changing how the results are interpreted.
...
> 1. Global Moran's I tests for spatial autocorrelation and uses spatial weights. It also measures clustering and allows us to determine whether there is clustering or not. When running this, we were able to determine clusters and states that were outliers in each of those clusters.
Source: https://r-spatial.github.io/spdep/reference/moran.test.html
> 2. LISA looks for states that contribute to the pattern, identifying clusters that have similar values. This ties into our earlier work in the Demo where we were looking at high-high, low-low, low-high, and high-low clusters. High-high clusters are called hotspots, while low-low clusters are called coldspots.
> 3. Moran's I on the residuals of the OLS model tests for spatial autocorrelation. This allows us to see if we have left out spatially correlated variables.
> 4. When testing for spatial autoregression using the coefficient rho, the outcome is dependent on neighboring outcomes. This also ties back into the clustering we saw earlier, and having these hotspots and coldspots defined helps the analysis.
> 5. The Likelihood Ratio Test (LRT) compare how well a nested regression models fits compared to an unnested model. We can use this in model comparison to see which model is performing better.
> 6. The lagrange multiplier (LM) test for residual spatial autocorrelation after fitting the SAR model is based on spatially lagged residuals.

What is your **final takeaway**? Do we need to account for spatial autocorrelation in our regression model?

> We do need to account for spatial autocorrelation in our regression model. If we do not account for it, our regression model will not have strong results.
...
#MH Adding for comparison in question 27
Compare_SAROLD<- tbl_regression(SAR) %>% 
  modify_caption("Table 7. Spatial Lag (SAR) Regression results.") %>% 
  modify_header(label = "**Coefficient**") %>% 
  add_significance_stars(hide_p = FALSE) %>% 
  as_tibble()
...
> In Table 7, the Spatial Lag (SAR) Regression Results, the following coefficeints are significant:

> Perioperative.pulmonary.embolism.or.deep.vein.thrombosis.rate, with a p-value of <0.001

> Medicare.spending.per.patient, with a p-value of <0.001

> Quietness, with a p-value of 0.019

> Table 10 from Project 1 looks at the Parsimonious OLS Regression Results. The results of this table show that all predictors aside from Constant were statistically signficiant, with p-values < 0.05. When compared to Table 7 in Demo 2, the following variables were in both: Medicare.spending.per.patient adn Quietness. Therefore, these two varaibles are the only ones that remain statistically signficiant after accounting for spatial autocorrelation.

> Lastly, Figure 20 shows the Feature Importance from Elastic Net. The top 6 variables with the most importance in this model are Hospital return days for pneumonia,  Medicare spending per patient, Death rate for pneumonia pateints, State, Venous Thromboembolism Prophylaxis, and Quietness.

> Medicare.spending.per.patient and Quietness are in both Table 7 and Feature Importance from Elastic Net.

> As noted earlier, Perioperative.pulmonary.embolism.or.deep.vein.thrombosis.rate is signficiant in the SAR model, but is not in the Parsimonious OLS Regression Results or in teh Feature Importance of the Elastic Net model.

> Therefore, from these three tables, Medicare.spending.per.patient and Quietness show up in both as being statistically signficiant.
...
> It is challenging to interprete this RMSE in terms of the original/raw median pneumonia-related hospital readmissions because it was calculated on the transformed and scaled dataset. Therefore, it does not match up well with the raw percentages that we were seeing earlier.
...
Source: https://www.sciencedirect.com/topics/engineering/root-mean-square-error
Source: https://statisticsbyjim.com/glossary/mean-absolute-error/

> The OLS model has a root mean square error (RMSE) of 1.33 and a mean absolute error (MAE) of 0.84. The SAR (Spatial Lag) has a RMSE of 1.15 and a MAE of 0.79. RMSE measures the difference between predicted and actual values, while MAE evaluates the accuracy of the model. The SAR model has a slightly smaller RMSE, representing a smaller difference between predicted and actual values. It also has a slightly smaller MAE, indicating that has better accuracy than the OLS model. It is important to note that these values are measured in the same units as the transformed target. Since the SAR has smaller values for both of these errors, it is the better model.
...
> The p-value for lambda = 0 is is 0.014877. This is less than 0.05, so we reject the null hypothesis. This shows that there is a statistically signficiant spatial autocorrrelation in the residuals.
...
> A significant SEM but the LM test from the SAR is not signficant means that the SAR model does not have the spillover effect, which means that the neighbor's outcomes do not directly affect them. However, the SEM has unobserved factors that are impacting the region. This suggests that the SAR model is not the correct model to use.
...
> A possible workflow has the following steps:
1. Use Moran's I and/or LISA to measure clustering. These methods will tell us if there is clustering in the data and we can look at it on a global or local level.
2. SAR is a model used when an outcome in one location is influenced by neighboring outcomes. It has a spatial dependence in the target variable.
3. SEM is a model that is used when there are unmeasured spatial effects. it can also be used when there is spatial error correlation. It has spatial dependence in the residuals.
4. SDM has spatial dependence in both the target variable and predictors. It finds spatial spillover of both outcome variables and the predictor(s). Similarly, SDM can be used when the SAR model did not do a sufficient enough job at handling the spatial autocorrelation.

> As mentioned earlier in the Demo, SEM and SDM are appropriate next models to run if there is still spatial autocorrelation in the residuals after we run the SAR model.
...
EG
Source found through Google Scholar: https://www.mdpi.com/2220-9964/11/4/242

> Yes, data scientists do extract the spatially lagged targets/predictors and put them into machine learning models. An example of this is in the article Incorporating Spatial Autocorrelation in Machine Learning Models Using Spatial Lag Eigenvector Spatial Filtering Features by Liu et al. This article discusses how spatial lag can be used within Random Forest models. Meuse and California housing datasets were used in this article. By using spatial lag, the errors were smaller (up to 33% lower) and the global spatial autocorrelation of the residuals was reduced.
...
```{r}
# Check the dimensions of sarTrain
dim(sarTrain)
```


> The dimensions of sarTrain are 49 rows and 17 columns. Therefore, the state-level spatial dataset is inaappropriate for any further machine learning. Since the dataset is so small and only contains 49 observations, there is a large risk of overfitting. Additionally, a dataset this small will have trouble learning predictive patterns. We are restricted to just using SAR, SEM, or SDM models with the data as-is because they work well with smaller sets of spatial data. Additional machine learning models would require a much larger dataset to provide accurate results.
...
> Yes, an additional spatial structure to this dataset that could be explored is the county-level. This would increase the dimensions of the dataset as well, as it would be much more granular. It could also provide much more information, as some states have highly populated areas (ex. NYC) and very rural areas (ex. upstate NY), that could have much different readmissions data.
...
> Social Determinants of Health (SDOH) are used often in public health research, and are defined by the WHO as "the conditions in which people are born, grow, live, work and age, and people’s access to power, money and resources." These can vary from socio-economic information, to neighborhood walkability information to healthcare access.
SOURCE: https://www.who.int/health-topics/social-determinants-of-health#tab=tab_1 
SOURCE: https://pmc.ncbi.nlm.nih.gov/articles/PMC3863696/ 
...
## Remove district of columbia due to the size of the district of columbia, `popDensity` is difficult to visualize across the US
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why! 
## initiate plot
ggplot() +
  ## draw state polygons and set fill color to grey based on population density 
  geom_sf(aes(fill = popDensity), color = "grey20", alpha = 1, size = 0.2) +
  ## Display color scale gradient titled "Hundreds per sq. mi." for density colored Low = white, high = skittles 3
  scale_fill_gradient(
    name = "Hundreds per sq. mi.",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +
  ## Create title and reference/data source
  labs(title = "Adult population density by state",
    caption = "Source: U.S. Census Bureau, 2023") +
  ## set backgrounds to white, remove axes, ticks, gridlines, auto map titles, place gradient key at bottom
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
    plot.background = element_rect(fill = "white"),
...
## Remove district of columbia due to the size of the district of columbia, `popDensity` is difficult to visualize across the US
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## Create scatter plot where x axis is population density and y axis is median raw predicted readmission rate
ggplot(aes(x = popDensity, y = Median_RawPredictedReadmissionRate)) +
## set the size of the point, and add linear regression line and set color
...
## Remove district of columbia due to the size of the district of columbia, `popDensity` is difficult to visualize across the US
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
  ## set order data from highest to lowest on population density
  arrange(desc(popDensity)) %>% 
  ## Only show top 10 most densely populated
  head(10) %>% 
  ## create bar plot where x axis is the state name ordered by population density, and y axis is median raw predicted readmission rate
ggplot(aes(x = fct_reorder(ID, popDensity), y = Median_RawPredictedReadmissionRate)) +
  ## establish bar code colors
  geom_col(fill = skittles[3]) +
  theme_minimal() + 
  ## swap the x and y axes
  coord_flip() +
  ## Establish axis titles and data source. Note that x axis will be set to blank since each tick is a state name
...
> With the exception of a couple of states, like Nevada and North Dakota there appear to be a correlation between higher readmissions and higher population density.
...
> There is a positive correlation between population density and predicted hospital readmissions. The most dense states all have 15% or greater median hospital readmissions, but there are still a handful of states with some of the highest readmission rates, but smaller densities (e.g. West Virginia, Nevada and North Dakota).

```{r}
# #MH
# #Show pop densities of top 'Median_RawPredictedReadmissionRate`
# states_sf_AggTrain %>% 
#   filter(ID != "district of columbia") %>% 
#   arrange(desc(Median_RawPredictedReadmissionRate)) %>% 
#   slice(1:10) %>% 
#   select(
#     ID,
#     popDensity)
...
> The p-value of the Moran's I is greater than 0.05, therefore we would conclude there is no spatial autocorrelation.
...
> The SAR model results with a Rho is 0.354 with a p-value of 0.017. Since the p value is less than 0.05 there is sufficient spatial dependency in the target variable to justify including the lag term in this model. These are very similar results as we saw earlier in the demo, prior to the inclusion of population density.
...
> The p-value < 0.05 suggests the SAR is better performing model than the OLS.
...
#MH Adding New and then joining to compare the 
Compare_SARNEW<- tbl_regression(SAR) %>% 
  modify_caption("Table 7. Spatial Lag (SAR) Regression results.") %>% 
  modify_header(label = "**Coefficient**") %>% 
  add_significance_stars(hide_p = FALSE) %>% 
  as_tibble()

Compare_SARNEW  <- Compare_SARNEW %>% 
  rename(pvalue_new = `**p-value**`)

Compare_SAROLD  <- Compare_SAROLD %>% 
  rename(pvalue_old = `**p-value**`)
...
> With the addition of `popDensity`, one feature lost it's signficiance: Doctor communication's p-value increased (signficiance dropped) from 0.052 to 0.2. Several predictors became more signficiant, but did not change whether they were considered significant. These include Quietness (p-value changed form 0.019 to 0.009), Cleanliness (p-value changed from 0.2 to 0.07), Postoperative.respiratory.failure.rate (p-value changed from 0.11 to 0.068). `popDensity` is considered signficiant with a p-value of 0.042 (< 0.05).
> Higher population density is associated with higher predicted pneumonia readmission rates. For each additional 100 people per square mile increase, the median predicted readmission rate increases by about 1 percentage point.
...
```{r} 
#MH UPDATED Prediction so that it accurately reflected the model to make predictions on
...
#   as_tibble()
semPredictions <- predict(SEM, newdata = sarTest, listw = states_sw) %>% 
  as_tibble()

# sdmPredictions <- predict(SAR, newdata = sarTest, listw = states_sw) %>% 
#   as_tibble()

##MH NOTE: when updating this code to predict SDM instead of SAR an error occurred:
#Error in .subscript.2ary(x, i, j, drop = TRUE) : subscript out of bounds
#trying to investigate using LLM suggested this is an expected error, but was unable to truly find fix
#therefore, RMSE and MAE of SDM are omitted from below, and instead SDM results are used to identify if there is still spatial autocorrelation occurring
# sdmPredictions <- predict(SDM, newdata = sarTest, listw = states_sw) %>% 
#   as_tibble()
```
...
#   )
#MH Removed SDM
data.frame(Model = c("OLS", "SAR (Spatial Lag)", "SEM (Spatial Error)"),
           RMSE = round(c(olsRMSE, sarRMSE, semRMSE), 2),
           MAE = round(c(olsMAE, sarMAE, semMAE),2)) %>% 
  ## Display with kable
  kable(digits = 2,
        format = "html",
        caption = "Table 13. Comparison of RMSE and MAE for all models.") %>%
...
**QUESTION**: What does the comparison of the four models' $RMSE$ and $MAE$ imply? Which model is the superior model? Is SAR still "good enough"? Do you think that, despite adding population density, we're likely STILL omitting spatial variables? Explain. ***NOTE MH: Only compared 3 models since could not get SDM to predict()***

> With `popDensity`, the OLS model improves marginally from the previosu model where RMSE was 1.33 and MAE 0.84. 

> The sptial models models both outperform OLS with a smaller RMSE And MAE. SEM performs the best with the lost RMSE and MAE. Notably, as we saw before adding in `popDensity`, we are again seeing the hypothesis test in the Durbin model with a non signficant p-value: 0.32053, but LMtest is still signficant p-value: 0.041149. This suggests that we still have spatial structure in our data. SAR is still good enough for interpreting, but we need to test incorporating additional spatial data to find the source of our spatial structure.
...
> For this question we will be investigating `High Elderly Populations`. Questions and code blocks from section 3.2 are pasted and altered below as needed to fit the new predictor. Before we begin however, we need to set up the new test/train data for this. `states_sf_AggTrain` and `states_sf_AggTest` include `over65PercentPop` as the percent of the population that is over the age of 65 relative to the total adult population over the age of 65.


## 3.3.2 Testing Hypotheses: Readmissions by Elderly Population Perentage


**Hypothesis 2**: States with a higher proportion of their adult population over the age of 65 have higher rates of readmission __because__ of age-related factors that increase the likelihood of severe illness, like pneumonia, and increased risk of complications that could lead to higher hospital readmissions.

We have a hypothesis now, but how can we test it? (Hypotheses will always be a "because" statement!)

### 3.3.2.1 Heuristic (Visual) Tests
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
#MH updating for `over65PercentPop`
## Make a map `p2` of population density:
p2 <- states_sf_AggTrain %>% 
## keeping district of columbia out for continuity
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why! 
## initiate plot
ggplot() +
  ## draw state polygons and set fill color to grey based on percent elderly population 
  geom_sf(aes(fill = over65PercentPop), color = "grey20", alpha = 1, size = 0.2) +
  ## Display color scale gradient for colored Low % elderly pop = white, higher % elderly pop = skittles 3
  scale_fill_gradient(
    name = "Percent of Population 65+",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +
  ## Create title and reference/data source
  labs(title = "Elderly Population (65+) Percentage by State",
    caption = "Source: U.S. Census Bureau, 2023") +
  ## set backgrounds to white, remove axes, ticks, gridlines, auto map titles, place gradient key at bottom
  theme(
    panel.grid = element_blank(),
    ...
    axis.title = element_blank(),
    legend.position = "bottom",
    legend.key.width = unit(2.5, "cm"),   # widen legend
    legend.key.height = unit(0.5, "cm")   # shrink height
  ) +
  ## default coordinates for sf
  coord_sf()
...
## keeping district of columbia out for continuity
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## Create scatter plot where x axis is over65PercentPop and y axis is median raw predicted readmission rate
...
## set the size of the point, and add linear regression line and set color
  ...
  theme_minimal() +
  ## Add title and axis labels
  labs(title = "Hospital Readmissions by \n Elderly Population Percentage",
       x = "Percent of Population Age 65+",
       y = "Predicted Pneumonia-related Hospital Readmissions")
...
## keeping district of columbia out for continuity
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
  ## set order data from highest to lowest % of elderly population
  arrange(desc(over65PercentPop)) %>% 
  ## Only show top 10 highest elderly populations
  head(10) %>% 
  ## create bar plot where x axis is the state name ordered by elderly population percent density, and y axis is median raw predicted readmission rate
ggplot(aes(x = fct_reorder(ID, over65PercentPop), y = Median_RawPredictedReadmissionRate)) +
  ...
  coord_flip() +
  ## Establish axis titles and data source. Note that x axis will be set to blank since each tick is a state name
  labs(title = "Top 10 States with Highest \nElderly Population (Age 65+)",
     y = "Median % hospital readmissions",
     x = "",
     caption = "Source: CMS, 2024 & U.S. Census, 2023; n.b. Wash D.C. excluded")
```
...
#### **Question 29.2.1**: [0.5 points]
Compare the new map, `p2` of population density to the original map, `p1`. It may be hard to tell at first, but do you see a correlation between population density and median pneumonia-related admissions?

> Comparing `p2` showing elderly population percentage by state with the original `p1` map, it looks like maybe West Virigina and Florida have both higher percentage of elderly population along with higher pneumonia readmissions rates.

#### **Question 29.2.2**: [0.5 points]
Now look at the scatterplot, `p3`, and the column graph, `p4`. What conclusions can you make, if any, from these two graphs and why?

> There is a slight positive correlation between eldery population percentage and predicted pneumonia related hospital readmissions. It is not as strong of a relationship as was seen with the population density maps.


```{r}
# #MH
# #Show elderly population percentages percentages of top 'Median_RawPredictedReadmissionRate`
# states_sf_AggTrain %>%
#   filter(ID != "district of columbia") %>%
#   arrange(desc(Median_RawPredictedReadmissionRate)) %>%
#   slice(1:10) %>%
...
### 3.3.2.2 Adding Population Density to the SAR

Now, we can also calculate the spatial statistics using the updated dataset now that we've add population density. Let's make a new `sarTrain` and `sarTest` so we can repeat the spatial regressions we did in Part 2.

#### 3.3.2.2.1 Make the updated `sarTrain` and `sarTest`
```{r}
#MH Updating to keep `over65PercentPop` this time!
## TRAINING
            ## Start from the training data
sarTrain <- states_sf_AggTrain %>% 
  ## Make it a dataframe/tibble
  ...
  ## test from the Census data!!
         ...
         -over65Pop, 
         -percBelowPoverty,
         -landSqMi) #note that we are keeping `over65PercentPop` this time!
...
#### 3.3.2.2.2 Fit the OLS regression and test Moran's I on the residuals
```{r}
OLS2 <- lm(bc_PredictedReadmissionRate ~ ., data = sarTrain)
## Moran's I test on residuals
moran.test(residuals(OLS2), states_sw)
```
...
#### 3.3.2.2.3 Fit the SAR and look at $\rho$ and the LM test
```{r}
## SAR (spatial lag model)
SAR2 <- lagsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain, 
                listw = states_sw)
## Uncomment to view results!
summary(SAR2)
```
...
> The SAR model results with a Rho is 0.3599 with a p-value of 0.019. Since the p value is less than 0.05 there is sufficient spatial dependency in the target variable to justify including the lag term in this model. These are very similar results as we saw earlier in the demo, prior to the inclusion of population density.

#### 3.3.2.2.4 Compare the SAR to the OLS model with a Likelihood Ratio Test (LRT)
```{r}
## Give it the larger model first, then the null model.
## SAR is our larger model; the null OLS is nested within the SAR because
## the SAR = OLS when rho = 0!
...
#### 3.3.2.2.5 Inferential statistics
```{r}
## Print a table using gtsummary
tbl_regression(SAR2) %>% 
  modify_caption("Table 7. Spatial Lag (SAR) Regression results.") %>% 
...
#MH Adding New and then joining to compare the 
Compare_SARNEW2<- tbl_regression(SAR2) %>% 
  modify_caption("Table 7. Spatial Lag (SAR) Regression results.") %>% 
  modify_header(label = "**Coefficient**") %>% 
  add_significance_stars(hide_p = FALSE) %>% 
  as_tibble()

Compare_SARNEW2  <- Compare_SARNEW2 %>% 
  rename(pvalue_new = `**p-value**`)
...
**QUESTION**: Did any of the predictors change significance from the **second** SAR we ran? What about `popDensity`? **what about `over65PercentPop`?** Is it significant, and how do you interpret its beta coefficient (the slope) AFTER adjusting for spatial dependence in readmission rate?

> The addition of `over65PercentPop` leads to `popDensity` no longer being signficiant (0.042 -> 0.061). `over65PercentPop` is not signficiant (p-value 0.9).  This suggests that `over65PercentPop` adds noise to the model and does not help predict pneumonia readmissions.

```{r}
#checking relationship between population density and %elderly by state
# states_sf_AggTrain %>% 
#   filter(ID != "district of columbia") %>%
#   ggplot(aes(x = popDensity, y = over65PercentPop)) +
#   geom_point(alpha = 0.5) + 
#   geom_smooth(method = "lm", se = T, color = skittles[3]) +
#   theme_minimal() +
#   labs(
#     title = "Correlation Between Population Density and % Elderly by State",
#     x = "Population Density (per sq mile)",
#     y = "Percent of Population Age 65+"
#   ) +
#   theme_minimal()
```
...
#### 3.3.2.2.6 Fit SEM and SDM
```{r}
## Fit the SEM (spatial error model) using the training data and weights matrix
SEM2 <- errorsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain, 
                  listw = states_sw)
## Uncomment to view results!
## summary(SEM)

## Fit the SDM (spatial Durbin model) using the training data and weights matrix
SDM2 <- lagsarlm(formula = bc_PredictedReadmissionRate ~ ., data = sarTrain, 
    listw = states_sw, type = "mixed") ## "mixed" is necessary to specify as SDM
## Uncomment to view results!
 summary(SDM2)
```

#### 3.3.2.2.6 Predictions using the testing data on SAR, SEM, and SDM

```{r} 
## Fit the predictions for the FOUR models using the testing data!
olsPredictions <- predict(OLS2, newdata = sarTest)

sarPredictions <- predict(SAR2, newdata = sarTest, listw = states_sw) %>% 
  as_tibble()

semPredictions <- predict(SEM2, newdata = sarTest, listw = states_sw) %>% 
  as_tibble()

# sdmPredictions <- predict(SDM2, newdata = sarTest, listw = states_sw) %>% 
#   as_tibble()
```

#### 3.3.2.2.7 Calculate the $RMSE$ and $MAE$ and compare

```{r}
## RMSE
ols2RMSE <- sqrt(mean((sarTest$bc_PredictedReadmissionRate - olsPredictions)^2))
sar2RMSE <- sqrt(mean((sarTest$bc_PredictedReadmissionRate - sarPredictions$fit)^2))
sem2RMSE <- sqrt(mean((sarTest$bc_PredictedReadmissionRate - semPredictions$fit)^2))
# sdmRMSE <- sqrt(mean((sarTest$bc_PredictedReadmissionRate - sdmPredictions$fit)^2))

## MAE
ols2MAE <- mean(abs(sarTest$bc_PredictedReadmissionRate - olsPredictions))
sar2MAE <- mean(abs(sarTest$bc_PredictedReadmissionRate - sarPredictions$fit))
sem2MAE <- mean(abs(sarTest$bc_PredictedReadmissionRate - semPredictions$fit))
# sdmMAE <- mean(abs(sarTest$bc_PredictedReadmissionRate - sdmPredictions$fit))
...
data.frame(
  Model = c("OLS",
            ...
            "SEM (Spatial Error)",
            "OLS2",
            "SAR2 (Spatial Lag + Elderly %)",
            "SEM2 (Spatial Error + Elderly %)"),
  
  RMSE = round(c(olsRMSE,
                 sarRMSE,
                 semRMSE,
                 ols2RMSE,
                 sar2RMSE,
  ...
  kable(
    digits = 2,
    format = "html",
    caption = "Table 14. Comparison of RMSE and MAE for all models, including models with elderly population percentage."
  ) %>%
...
**QUESTION**: What does the comparison of the OLS, and two SAR and SEM models' $RMSE$ and $MAE$ imply? Which model is the superior model? Is SAR still "good enough"? Do you think that, despite adding population density, we're likely STILL omitting spatial variables? Explain.

> Adding `over65PercentPop` did not seem to improve the OLS or the SAR results, but did result in some slight imrpovement to SEM. The best model of this is the SEM model that includes both `popDensity` and `over65PercentPop` (SEM2). We are still seeing the hypothesis test in the Durbin model with a non signficant p-value: 0.206, but LMtest is still signficant p-value: 0.00157. This suggests that we still have spatial structure in our data, but there is improvement with the inclusion of Elderly Population Percentage; however, given the lack of significance to the feature and the only slight improvement to the model, it's not clear that the `over65PercentPop` feature provides much information the model except for noise.
