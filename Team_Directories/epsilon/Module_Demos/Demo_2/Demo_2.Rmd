...
```{r}
## We need a grouping variable since its possible that two long/lat pairs could be identical (like on a state border)
```
...
```{r}
load("../Demo_1/pneumoniaFull.Rdata")
...
```{r}
## We specifically encoded state data into frequency values, which would make this spatial analysis impossible (nothing to join to)
```
...
```{r}
## The states data set lists the full name of the state, while the pneumonia data set only lists abbreviations
```
...
## commented out as we perform a different operation below

# pneumoniaFull <- pneumoniaFull %>%
#   mutate(State = str_to_upper(State)) %>%
#   inner_join(state.fips, by = join_by(State == abb)) %>%
#   mutate(State = str_to_title(polyname)) %>%
#   select(names(pneumoniaFull))
...
```{r}
## If that data (island information) is not necessary for our analysis, can't we just split on the first ':' character and take the state only?
```
...
```{r}
## It looks like Maryland doesn't have data available for `Score_Medicare spending per patient`

pneumoniaFull %>%
  filter(State == "MD") %>%
  select(`Score_Medicare spending per patient`) %>%
  unique()
...
```{r}
## The main issue is that if aggregating (mean/median), it can be assumed that is the "central tendency" of the distribution. If a data set is highly skewed or impacted by significant outliers, a mean could differ substantially from the median, and depending on the analysis that could lead to different conclusions.
```
...
```{r}
## Each measure skews right with some significant outliers - I'd recommend using the median over the mean so the outliers do not impact the aggregated value.
...
## The results show that due to the p-value being < 0.05, the null hypothesis (that the data is symmetrical) is rejected. The median should be used
```
...
# skew = 0.687, z = 15.149, p-value < 0.00000000000000022
# alternative hypothesis: data have a skewness
...
# skew = 0.54487, z = 11.43008, p-value < 0.00000000000000022
# alternative hypothesis: data have a skewness
...
```{r}
## I feel comfortable with the median selection. Some of the plots show that the medians exist at the edges of the interquartile, but from what I know, that just shows that the data is skewed and thus we're choosing the right approach.
...
```{r}
## I don't see any commonalities yet!
```
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median death rate (# deaths / # cases) within 30 days", 
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name="Median Medicare spend (per patient)", 
            low = "white", high = skittles[7]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median Medicare spend by state",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
p1 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  ## Hospital return days are the number of days readmitted pneumonia patients spend in the hospital. Readmissions are only measured within 30 days of the original admission to the hospital for pneumonia.
  scale_fill_continuous(name="Median Pneumonia-related Hospital Readmission days (vs previous admission)", 
            low = "white", high = skittles[5]) +       ## !!Update color here   
            ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median Hospital Readmission Days per State",
   ## Leave the caption as-is
   caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
```{r}
source(file = "C:\\Users\\mike_\\source\\repos\\healthcare-analytics\\Team_Directories\\epsilon\\Module_Demos\\Demo_2\\loadTrainTest.R")
```
...
nrow(cbind(unique(stateTrain$region)))
## 52
...
```{r}
## Its entirely possible that the test data will not have all 52 regions as it was cut at random
```
...
```{r}
## The data we ended up with in project 1 was centered and scaled, essentially making the median == the mean.
```
...
```{r}
## st_as_sf() converts coordinate data into proper spatial objects that R can interpret. The map('state') call returns general state data minus the island information that was in the `states` data set
...
```{r}
## Again, using the chess analogy, rook continuity means that only neighbors that share a common edge are actually neighbors. Using this contiguity would mean that Arizona & Colorado are no longer neighbors. Based on the documentation, `queen = FALSE` should require rook continuity in the poly2nb() function.
...
```{r}
## Yes this is consistent - 4 neighbors divide the weight equally
```
...
```{r}
## No - I would expect it to have six neighbors
...
```{r}
## If we attempt to do pre-processing with test data, we can cause leakage, affecting the integrity of our model
```
...
```{r}
## Utah: low-low outlier
## Delaware: Low-High outlier
## West Virginia, New Jersey, DC: High-high outliers
```
...
```{r}
## Isn't the box-cox transform already centering values?
```
...
# weights: states_sw    
# 
# Moran I statistic standard deviate = 3.1939, p-value = 0.0007019
# alternative hypothesis: greater
# sample estimates:
# Moran I statistic       Expectation          Variance 
#       0.288790751      -0.020833333       0.009397939 


## All the outliers land in the same quadrants.
...
```{r}
## - Coldspots will be low-low equivalents
...
```{r}
## Just prior to calculating spatial weights, we modified the spatial engine
```
...
```{r}
states_sf_AggTrain[20,]$Median_RawPredictedReadmissionRate
```
...
states_sf_AggTrain <- states_sf_AggTrain %>%
  mutate(
    ...
    quadrant = case_when(
      ...
      ## The case of `pValue > 0.05` has been satisfied, no other cases need to be evaluated
...
```{r}
## It means that Indiana has a low rate of readmissions but it is surrounded by states with high readmissions
```
...
```{r}
## Yes, it correlates with what we saw in p1, although much more pronounced. The hot/coldspots may not correspond with individual rates since hot/coldspots are calculated as relative to neighbors.
```
...
```{r}
## The steps we would need to follow to process the data up to this point:
## - Align our pneumonia data to the states data set so we can extract spatial information for states
## - Choose the median values as shown above (determined by analysis)
## - Create spatial objects using the poly2nb() method
## - Scale our data 
## - Perform the LISA to create clusters & neighbor analysis
...
```{r}
## Maybe to evaluate how much the box-cox scaling/transforming compares to the raw median aggregation?
```
...
```{r}
## Per above:
## Just as with a canonical correlation, $p$-values < 0.05 suggest significant spatial autocorrelation (assuming we are using the 5% significance threshold). It's important to note that an $I$ = 0 would indicate randomness; thus, the null hypothesis ($H_0$) is that median pneumonia readmission rates are randomly distributed across the states.

## Based on this, the p-value is more than the 5% significance threshold, suggesting there is not correlation.
```
...
```{r}
## Yes, we could have been mistaken - the OLS regression indicates there isn't much to look at!
...
```{r}
## Maybe because the we used box-cox for the OLS and the raw median for the Moran's / LISA?
```
...
> 1. Global Moran's results: the I statistic indicates some level of spatial correlation (.26)
> 2. Local Moran's results: we encounter Pr(z != E(Ii)) values that indicate hot/cold spots
> 3. The OLS model indicated that there were not significant spatial impacts
> 4. The RHO test suggests there is a spatial component to the target variable value
> 5. The LRT test suggests that the models (SAR / OLS) are significantly different
> 6. The Language Multiplier test suggests that the SAR model (spatially-aware) explains the spatial correaltions in the target variable.

What is your **final takeaway**? Do we need to account for spatial autocorrelation in our regression model?

> Yes - although not all the tests indicate we need to, the majority do.
...
```{r}
## It appears that `Perioperative.pulmonary.embolism.or.deep.vein.thrombosis.rate` & quietness have become much more significant. 
```
...
```{r}
## Its difficult to interpret this data as it has been scaled & centered
```
...
```{r}
## The MAE indicates the average difference between predicted & actual values, while the RMSE does the same thing, but is scaled. Lower values for both indicate a better model. Both of these indicators show that the spatial lag model performs better.
```
...
```{r}
## The p-value of 0.014877 indicates that we can reject the null hypothesis, meaning that there isn't any autocorrelation in the residuals.
```
...
```{r}
## In the case of "Incorporating Spatial Autocorrelation in Machine Learning Models Using Spatial Lag and Eigenvector Spatial Filtering Features"[1], the team was able to combine spatial lag measurements with random forest models to gain significant prediction capabilities.

## [1] Liu, X.; Kounadi, O.; Zurita-Milla, R. Incorporating Spatial Autocorrelation in Machine Learning Models Using Spatial Lag and Eigenvector Spatial Filtering Features. ISPRS Int. J. Geo-Inf. 2022, 11, 242. https://doi.org/10.3390/ijgi11040242
```
...
## We have created a dataset that describes relative values compared to states' neighbors. Feeding this data into another model would not make sense as these values are only relevant to this usage - that is, analyzing values as they compare to other nearby states.
...
```{r}
## Maybe something like population density or proximity to population centers?
```
...
```{r}
## SDOH => non-medical factors that influence health outcomes
## The phrase originated during studies of public health around the time of the industrial revolution.
```
...
## get all regions that are not the district of columbia
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## create a plot
ggplot() +
  ## generating spatial data, with color intensity based on population density
  geom_sf(aes(fill = popDensity), color = "grey20", alpha = 1, size = 0.2) +
  ## calculate a gradient based on the density value
  scale_fill_gradient(
  ...
  ## keep the map scaled (preventing stretching)
  coord_sf()
...
## get all regions that are not the district of columbia
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## plot a graph of population density vs readmission rate
...
## get all regions that are not the district of columbia
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
  ## order the results by population density (descending)
  arrange(desc(popDensity)) %>% 
  ## get only the top 10 records
  head(10) %>% 
  ## create a plot, reordering factor levels by populationDensity and compare it to readmission rates
ggplot(aes(x = fct_reorder(ID, popDensity), y = Median_RawPredictedReadmissionRate)) +
  ## generate bars using this color
...
```{r}
## Yes, it seems that higher densities have higher readmission rates
```
...
```{r}
## Honestly, not much. There doesn't appear to be strong correlations between these data points. The scatterplot has residuals all over the place.
```
...
```{r}
## The I test (0.10453179) indicates there is some positive correlation, but not much (relatively close to 0)
```
...
```{r}
## The rho & p-value are 0.35443 and 0.016887, respectively
## Because the p-value is less than 0.05, there is sufficient spatial dependency in the target variable to justify including the lag term in this model. 
```
...
```{r}
## With a p-value LESS than 0.05, there is sufficient evidence that a spatial model improves our predictive ability over what an OLS alone would.
```
...
```{r}
## Based on the calculations, the SAR, SEM and SDM models have the same RMSE & MAE values. Compared to the previous calculations (without pop density), the model has improved slightly. It does seem like we're still missing some spatial variable as these calculations are still relatively high.
```
