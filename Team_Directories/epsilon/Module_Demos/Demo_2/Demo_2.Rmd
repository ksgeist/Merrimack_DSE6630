...
```{r}
load("/Users/elizabethgreenan/Documents/DSE6330/Merrimack_DSE6630/Team_Directories/epsilon/I. Biomedical & Clinical Informatics/Demo_1/pneumoniaFull.Rdata")
...
> There isn't any missing data since the data from every state was split into seperate rows, and will find the mean and median to their state respectively. It'll leave behind all of the other info we don't need to do our specific region / state analysis.
...
__`Score_Hospital return days for pneumonia patients`__: Because $p<0.05$, we can't reject the hypothesis of symmetry and conclude that Hospital Return Days are __significantly skewed__. We should use the median to aggregate this variable.

```{r}
agostino.test(pneumoniaFull$`Score_Medicare spending per patient`)
```

__'Score_Medicare spending per patient'__: Because $p<0.05$, we can't reject the hypothesis of symmetry and conclude that Medicare spending per patients are __significantly skewed__. We should use the median to aggregate this variable.

```{r}
agostino.test(pneumoniaFull$`Score_Hospital return days for pneumonia patients`)
```

__'Score_Death rate for pneumonia patients'___: Because $p<0.05$, we can't reject the hypothesis of symmetry and conclude that Hospital return days for pneumonia patients are __significantly skewed__. We should use the median to aggregate this variable.
