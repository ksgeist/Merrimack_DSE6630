...
```{r} 
#DS, EG, MH
files <- list.files(path = filepath, pattern = "Hospital.csv")  # adds list of files from 'filepath' to object 'files' if file name contains "Hospital.csv"

for(f in 1:length(files)) { # loop for each file index do the following
    dat <- read_csv(paste0(filepath, files[f]), # create dataframe called 'dat' of the csv file from 'files'
                    show_col_types = FALSE, # suppress column type message 
                    ## HOT FIX of the "N/A" characters that were problematic
                    na = c("", "NA", "N/A", "Not Applicable", "Not Available","NULL", ".") #Note: added not applicable and not available
                    ) %>%
    ...
    filename <- gsub(".Hospital\\.csv", "", files[f]) # creates 'filename' object removing "Hospital.csv" and one prior character (_,-.) from respective indexed 'files'
    assign(filename, dat) # assign `filename` to `dat` dataframe
}

files <- gsub(".Hospital\\.csv", "", files) %>% data.frame() # create dataframe "files" from list "files". Remove "Hospital.csv" and one prior character (_,-.) from respective 'files'
names(files) <- "File Name" # rename column "File Name"
```
...
sum(duplicated(HCAHPS))
sum(duplicated(FY_2024_Hospital_Readmissions_Reduction_Program))
```
```{r}
#DS, EG, MH
#how many measures per hospital facilityids
HCAHPS %>%
    count(FacilityId)
...
```{r}
#DS, EG, MH
#How many Hospitals (FacilityID)?
cat("Hospital Count by FacilityID (HCAHPS):",
    n_distinct(HCAHPS$FacilityId), "\n")

cat("Hospital Count by FacilityID (Readmissions Program):",
    n_distinct(FY_2024_Hospital_Readmissions_Reduction_Program$FacilityId), "\n")

#How many Hospitals (FacilityName)?
cat("Hospital Count by FacilityName (HCAHPS):",
    n_distinct(HCAHPS$FacilityName), "\n")

cat("Hospital Count by FacilityName (Readmissions Program):",
    n_distinct(FY_2024_Hospital_Readmissions_Reduction_Program$FacilityName), "\n")
```

```{r}
#DS, EG, MH
#How many FacilityIDs per Hospital Name?
FY_2024_Hospital_Readmissions_Reduction_Program %>%
    group_by(FacilityName) %>%
    summarise(
        n_facility_ids = n_distinct(FacilityId)
    ) %>%
    filter(n_facility_ids > 1) %>%
    arrange(desc(n_facility_ids))
```



> Looking at the structure of the two data frames `HCAHPS` and `FY_2024_Hospital_Readmissions_Reduction_Program`, we can see that a few columns that we may excpect to have been converted to numeric are still in character format as a result of character feature "Too Few to Report" or "Not Applicable". At 447,702 x 22 and 18,774 x 12 these datasets are long, and notably each row appears to represent one specific measure per hospital resulting in many lines representing one hospital's survey results. In `HCAHPS` there are 93 measures (rows in this case) per `FacilityID`. In `FY_2024_Hospital_Readmissions_Reduction_Program` there are 6 measures (rows) per `FacilityID`.

> While it does not appear that there are duplicate rows of data, When checking for the number of hospitals represneted in these two datasets it is important to note that there are less facility names than there are IDs, and there are less hospitals represented in the Readmission Reduction Program than in the HCAHPS survey results.

> 4814 Hospitals are represented in `HCAHPS`, 3129 are represented in `FY_2024_Hospital_Readmissions_Reduction_Program` when checking for `FacilityID`, but 4689 and 3058, respectively, are represented when looking into unique `FacilityName`. This could be because one hospital has multiple locations but share a name, or because it is a popular hospital name. For example, within the `FY_2024_Hospital_Readmissions_Reduction_Program` dataset, based on `FacilityId`, there are 6 different Memorial Hospitals represented in the dataset.
...
```{r}
#DS, EG, MH
#Count how many missing data points for Number of Readmissions
sum(is.na(FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions))

#How many are 'Too Few to Report`?
cat("The number of rows which `NumberofReadmissions` is labeled `Too Few to Report` is:",
    length(which(FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions == "Too Few to Report")), "\n")
```

```{r}
#DS, EG, MH
#When NumberofReadmission is NA, what is NumberofDischarges?
FY_2024_Hospital_Readmissions_Reduction_Program %>%
  filter(is.na(NumberOfReadmissions)) %>%
  distinct(NumberOfDischarges)
```


> The `NumberOfReadmissions` column specifically remained a character column despite the work done to correct the data type when the .csv files were being pulled into the environment. There are 6697 missing datapoints, but additionally there are 4187 rows where the `NumberofReadmissions` are "Too Few to Report". This suggests that there is a reporting threshold, and the number reported is below that threshold for these rows.

> Those where `NumberofReadmissions` are NA, the `NumberofDischarges` is either 0 or NA which seems logical. 
...
#DS, EG, MH
FY_2024_Hospital_Readmissions_Reduction_Program %>%
  filter(NumberOfReadmissions == "Too Few to Report") %>%
  distinct(Footnote)
```
...
FY_2024_Hospital_Readmissions_Reduction_Program %>%
  filter(NumberOfReadmissions != "Too Few to Report") %>%
  summarise(min_readmissions = min(as.numeric(NumberOfReadmissions), na.rm = TRUE))
```


> The Data Dictionary lists the Number of Readmission column as a character feature that can support up to 17 characters. As mentioned in question 3, the coercion to character type is due to the "Too Few to Report" values. There are not direct footnotes associated when Number of Readmissions are Too Few to Report. The minimum number in the `NumberofReadmissions` column is 11 which could be an indicator that the minimum reporting threshold was >10; however, I was unable to fully coroborate this in the data dictionary. 
...
#Commenting this out for Question 9
#FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions <- as.numeric(gsub("Too Few to Report", 5, FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions))
...
> This is likely due to the "Too Few to Report" readmission minimum cut off being 10 and 5 captures the midpoint of 1 to 10. NA and 0 would both be incorrect or misleading as these suggest no readmissions at all. Given that there are a large number (4187) instances where the number of readmissions were too few to report it's possible that selecting 5 would average out across all the true values; however, midpoint imputation does remove variance, and it does risk underestimating if the trend of these 4000 readmission datapoints is closer to 10.
...
> Randomly assigning an integer from 1 to 10 would reduce potential bias by adding variance to the imputation. gsub() is used for text replacement rather than random number assignment, so would not be appropriate for this execution, whereas lappply() and for() would allow us to loop through and apply a gsub() throughout all of the defined "Too Few to Report".
...
#Source: https://www.rdocumentation.org/packages/micropan/versions/1.0/topics/gregexpr
#Source: https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/regmatches
#Source: https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/lapply

#find which rows are "Too Few to Report"
too_few <- gregexpr("Too Few to Report", FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions)

#Identify which rows contain at least one match
too_few_rows <- which(lengths(regmatches(FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions, too_few)) > 0)

#Replace each matched row with a random integer 1–10
FY_2024_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions[too_few_rows] <- unlist(
  lapply(too_few_rows, function(x) sample(1:10, 1))
)

#Additional Method: DS
#FY_2024_Hospital_Readmissions_Reduction_Program_edited$NumberOfReadmissions = str_replace_all(FY_2024_Hospital_Readmissions_Reduction_Program_edited$NumberOfReadmissions, "5", ~sample(1:10, 1))

#convert to numeric - originally had this here, but realized the next question asks us to to do this, and this is the only aberrant character column remaining
...
#NOTE the only column that needs to be converted to numeric is NumberOfReadmissions; however, including additional features that were noted prior to the hotfix
...
#Test if dataframes are identical
#note that readmissionsClean dataset has NumberofDischarges as character feature, and Start and End date are character as well.
#differences with imputation for number of readmission due to randomness

identical(FY_2024_Hospital_Readmissions_Reduction_Program, readmissionsClean)
all.equal(FY_2024_Hospital_Readmissions_Reduction_Program, readmissionsClean)
```
...
# Your code here.
#DS, EG, MH
wideDF <- readmissionsClean %>% 
 select(-c(Footnote)) %>%  #This might be a mistake, but stops duplicate rows forming because of some instances of footnotes and others without
  pivot_wider(
...
#Checking for duplicate FacilityIDs
wideDF %>%
    count(FacilityId) %>%
    filter(n > 1)
...
#Note: This info (Except FacilityId) is dropped in the tidyNjoin function below
```
...
#Added "Not Applicable" to HOTFIX from Question1

#Not Available occurs in HCAHPS, but also occurs in valueOnly and paymentOnly. In character columns such as PaymentCategory and ValueofCareCategory, maybe makes sense to keep?? Or. Remove altogether. Currently removed altogether

#Make payment numeric
paymentOnly$Payment <- as.numeric(paymentOnly$Payment)
```
...
```{r}
#DS, EG, MH
#Remove `startDate` `endDate` columns, as well as the `footnote` and `MeasureId` from readmissions clean
...
#Coding assisted by ChatGPT through pseudocode and troubleshooting as suggested in hint 6

tidyNjoin <- function(datList, filterList, hospitalInfo) {

  clean_list <- map2(datList, filterList, function(df, filt) {

    df %>%
      
      # Filter rows
      filter(MeasureName %in% filt) %>%
  ...
  # Join all datasets
  final_df <- reduce(clean_list, full_join, by = "FacilityId")

# #---------------------------------------
# # Clean hospitalInfo due to multiple duplicate
# #---------------------------------------
# 
hospitalInfo_clean <- hospitalInfo %>%
...
# #---------------------------------------
# 
final_df <- hospitalInfo_clean %>%
  left_join(final_df, by = "FacilityId"
  )

# 
# #---------------------------------------
# # Join readmissionsClean
# #---------------------------------------
# 
final_df <- final_df %>%
  left_join(readmissionsClean, by = c("FacilityId", "State","FacilityName")
  )
  
  return(final_df)
}

func_pneumoniaFull <- tidyNjoin(
  datList,
  filterList,
  hospitalInfo)

dim(func_pneumoniaFull)
...
> This function can be used again for another condition or intervention to merge multiple dataframes.
...
```{r}
#DS, EG, MH
#Check Differences between pneumoniaFull and func_pneumoniaFull
check1<-sort(colnames(pneumoniaFull)) 
check2<-sort(colnames(func_pneumoniaFull)) 
identical(check1, check2)

list(
  only_in_pneumoniaFull = setdiff(check1, check2),
  only_in_func = setdiff(check2, check1)
)
rm(check1,check2)
```
...
> `pneumoniaFull` contained 100 features before dropping columns containing `LowerEstimate`, `HigherEstimate`, `Denominator` or `HcahpsAnswerPercent`. A total of 25 features were dropped through this process. Further dropping `FacilityName`, `TelephoneNumber`, `Address`, & `CityTown`, leaves our dataset with 71 variables.
...
> Columns that are comparing to something, like those listed above, have a specific order. Therefore, the type of encoding that should be performed on these columns is ordinal. This encoding method will ensure that the order of these variables will be preserved so they don't lose meaning.


##### **Question 16**: [1 point] 
What other column do we also need to perform this kind of encoding on?

> Another column that might need ordinal encoding is `Score_Emergency department volume` which has values beginning from low to medium, high and very high.

```{r}
#DS, EG, MH
unique(pneumoniaFull$`Score_Emergency department volume`)
```
...
# Your code here.
#DS, EG, MH
#For features that begin with ``ComparedToNational_`
TEST_pneumoniaFull <- pneumoniaFull %>%
  mutate(across(starts_with("ComparedToNational_"), ~ case_when(
    ...
    . == "No Different Than the National Value" ~ 0,
    . == "Worse Than the National Value" ~ -1,
    . == "Better Than the National Value" ~ 1,
    . == "More Days Than Average per 100 Discharges" ~ -1,    
    . == "Average Days per 100 Discharges" ~ 0,        
    . == "Fewer Days Than Average per 100 Discharges" ~ 1,      
    TRUE ~ NA_real_
  ))) 


#For EmergenctDepartmentVolume
TEST_pneumoniaFull <- TEST_pneumoniaFull %>%
  mutate(
    `Score_Emergency department volume` = case_when(
      `Score_Emergency department volume` == "low" ~ 1,
      `Score_Emergency department volume` == "medium" ~ 2,
      `Score_Emergency department volume` == "high" ~ 3,
      `Score_Emergency department volume` == "very high" ~ 4,
      TRUE ~ NA_real_))

#unique(TEST_pneumoniaFull$`Score_Emergency department volume`)
...
#unique(pneumoniaFull$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`)
...
#DS, EG, MH
table(TEST_pneumoniaFull$`Score_Emergency department volume`)
table(pneumoniaFull$`Score_Emergency department volume`)

cols <- pneumoniaFull %>%
  select(starts_with("ComparedToNational_")) %>%
  names()

lapply(cols, function(col) {
  table(pneumoniaFull[[col]], useNA = "ifany")
})

cols2 <- TEST_pneumoniaFull %>%
  select(starts_with("ComparedToNational_")) %>%
  names()

lapply(cols, function(col) {
  table(TEST_pneumoniaFull[[col]], useNA = "ifany")
})
...
#DS, EG, MH
cols2encode <- c("State", 
               "CountyParish") #Create list containing State and Countyparish

temp <- pneumoniaFullEncoded[, names(pneumoniaFullEncoded) %in% cols2encode] #Create dataframe of state and countyparish

add_freq <- function(data, column_name) {
  frequency_map <- table(data[[column_name]], useNA = "always") #count frequency of each feature in column
  data[[column_name]] <- frequency_map[match(data[[column_name]], 
                                             names(frequency_map))] #replace original values with frequency value
  return(data)
}

for (col in names(temp)) {
  temp <- add_freq(temp, col) #Run add_freq function in temp dataframe//replace with encoded values
}

for (c in 1:length(cols2encode)) {
  pneumoniaFullEncoded[, cols2encode[c]] <- temp[, cols2encode[c]] #Replace State and CountyParish with encoded versions from temp
}
```             
...
# Your code here.
#DS, EG, MH
#Counts of frequency of each State 
pneumoniaFull %>%
    count(State) %>%
    filter(n > 1)


# Check the counts for State field in the pneumoniaFull table
state_freq <- table(pneumoniaFull$State)
state_freq

# Check the counts for State field in the pneumoniaFullEncoded table
state_freq_encoded <- table(pneumoniaFullEncoded$State)
state_freq_encoded

# Check the counts for CountyParish field in the pneumoniaFull table
countyparish_freq <- table(pneumoniaFull$CountyParish)
#countyparish_freq

# Check the counts for CountyParish field in the pneumoniaFullEncoded table
countyparish_freq_encoded <- table(pneumoniaFullEncoded$CountyParish)
countyparish_freq_encoded

# Look at the length of State prior to encoding
length(unique(pneumoniaFull$State))

# Look at the length of State after encoding
length(unique(pneumoniaFullEncoded$State))

# Look at the length of CountyParish prior to encoding
length(unique(pneumoniaFull$CountyParish))

# Look at the length of CountyParish after encoding
...
> Prior to encoding, the State field had 57 categories, while the CountyParish field had 1,549. After encoding, the State field has 50 categories, while the CountyParish field has 36. This shows that encoding only reduced the number of categories in the State field by 7, but the CountyParish field was reduced by 1,513. Therefore, the CountyParish field will now be heavily duplicated and it would not be advised to keep it in the dataset.

```{r}
#DS, EG, MH
# Remove CountyParish from the dataset

pneumoniaFullEncoded <- pneumoniaFullEncoded %>% select(-CountyParish)
...
```{r, echo=FALSE}
#DS, EG, MH
## Also remove those created by DS, EG, MH that we're longer using at this step:
rm(datList, filterList, func_pneumoniaFull, pneumoniaFINAL, TEST_pneumoniaFull,too_few,wideDF)
```
...
# Drop NumberOfReadmissions and NumberOfDischarges
pneumoniaAnalyze <- pneumoniaAnalyze %>% select(-NumberOfReadmissions, -NumberOfDischarges)

head(pneumoniaAnalyze$observed_readmission_rate)
```
```{r}
#DS, EG, MH
#Compare observed_readmission_rate to `PredictedReadmissionRate`, `ExpectedReadmissionRate`, and the `ExcessReadmissionRatio`
summary(pneumoniaAnalyze[, c(
  "observed_readmission_rate",
...
```{r}
#DS, EG, MH
comparison_vars <- pneumoniaAnalyze[, c(
  "observed_readmission_rate",
  "PredictedReadmissionRate",
  "ExpectedReadmissionRate",
  "ExcessReadmissionRatio"
...
> Based on the data dictionary `ExcessReadmissionRatio` is calculated by dividing a hospital's `PredictedReadmissionRate` by the `ExpectedReadmissionRate`. `ExpectedReadmissionRate` appears to be based on the nations performance with that hospital's case load, and `PredictedReadmissionRate` appears to be a within-hospital, 30day readmission rate based on the hospitals specific population. `PredictedReadmissionRate` and `ExpectedReadmissionRate` both appear to be highly correlated with `observed_readmission_rate`.     

> One advantage to predciting the `PredictedReadmissionRate` would be that it is adjusted for a hospital's case load and complexity of patients seen. While `observed_readmission_rate` is the actual for the 2024 year, it has roughly 500 less observations and does not take hospital characteristics or populations into consideration.

> Source: https://www.cms.gov/medicare/quality/value-based-programs/hospital-readmissions 
> Source: https://www.qualityreportingcenter.com/globalassets/iqr-2023-events/vbp081723/hvbp_qa-summary-document_hrrp081723_vfinal508.pdf
> Source: "2024 Hospital-Wide Readmission Measure Updates and Specifications Report — Version 13.0" Prepared for Centers for Medicare & Medicaid Services (CMS)
...
> Since `observed_readmission_rate` was created from two variables already in the dataset, inclulding them would cause collinearity. This would skew the results and could lead to incorrectly high accuracy metrics.
...
# Create a histogram of the PredictedReadmissionRate variable
hist(pneumoniaAnalyze$PredictedReadmissionRate,
     main = "Distribution of Predicted \nReadmission Rate",
     xlab = "Predicted Readmission Ratio")


#Medicare Spending and Predicted Readmission Rate
plot(
  pneumoniaAnalyze$'Score_Medicare spending per patient',
  pneumoniaAnalyze$PredictedReadmissionRate,
  main = "Predicted Readmission Rate \nvs Medicare Spending",
  xlab = "Medicare Spending Score",
  ylab = "Predicted Readmission Rate (%)",
  pch = 19,
  col = rgb(0, 0, 1, 0.4)
)

abline(
  lm(PredictedReadmissionRate ~ `Score_Medicare spending per patient`,
...
> The histogram for `PredictedReadmissionRate` shows a mostly normal distribution with a slight right skew, with the average predicated readmission falling slightly below the median.

> The scatterplot shows relationship between `PredictedReadmissionRate` and `Score_Medicare spending per patient`. There appears to be a postive relationship between these features, which is counterintuitive as we expect Readmissions to be smaller with hiugher Medicare spending. This is further investigated below. 
...
pneumoniaAnalyze <- pneumoniaAnalyze %>%
  mutate(
      ReturnForPneumonia = case_when(
      `ComparedToNational_Hospital return days for pneumonia patients` == 0 ~ "Average",
      `ComparedToNational_Hospital return days for pneumonia patients` == 1 ~ "Lower",
      `ComparedToNational_Hospital return days for pneumonia patients` == -1 ~ "Higher",
      TRUE ~ "Missing"),
    medicare_spend_group = case_when(
      `Score_Medicare spending per patient` == 1 ~ "Avgerage",
      `Score_Medicare spending per patient` < 1 ~ "Lower",
      `Score_Medicare spending per patient` > 1 ~ "Higher",
      TRUE ~ "Missing"),
    medicare_spend_group = factor(
      medicare_spend_group,
      levels = c("Lower", "Avgerage", "Higher", "Missing")))


#Violin plot showing Predicted Readmission Rate and grouped medicare spending
ggplot(pneumoniaAnalyze,
       aes(x = medicare_spend_group,
           y = PredictedReadmissionRate)) +
  geom_violin(fill = "lightblue") +
  geom_boxplot(width = 0.1) +
  labs(title = "Readmission Rate by Medicare Spending Score") + 
  coord_flip()
```

> The violin plots further anaylses the relationship between medicare spending and predicted readmission. `Score_Medicare spending per patient` was broken down in to 3 categories Average if Score was 1, Lower if Score was below 1 and Higher if Score was above 1. Here we can see the Lower spending group having a slightly lower predicted readmission rate than average and higher spend groups. 

```{r}
#DS, EG, MH
ggplot(pneumoniaAnalyze,
       aes(
         x = PredictedReadmissionRate,
         fill = ReturnForPneumonia
       )) +
  geom_density(alpha = 0.4) +
  labs(title = "Predicated Readmission Rate by Compared to National Number \n of Days for Hospital Return")
```

> This density plot shows hospitals with a lower `PredictedReadmissionRate` also have a lower `ComparedToNational_Hospital return days for pneumonia patients`. Average and Higher `ComparedToNational_Hospital return days for pneumonia patients` have higher predicte readmission rates, respectively. 

```{r}
#DS, EG, MH
#faceted density plots showing Predicted Readmission Rate based on National Comparison for Days for Hospital Return broken down by Medicare Spending Scores
ggplot(pneumoniaAnalyze,
       aes(
         x = PredictedReadmissionRate,
         fill = ReturnForPneumonia
  ...
  geom_density(alpha = 0.4) +
  facet_wrap(~ medicare_spend_group) +
  labs(
    title = "Predicted Readmission Rate by National Comparison for Number\nof Days for Hospital Return broken down by Medicare Spending Scores",
    x = "Predicted Readmission Rate",
    fill = "Return for Pneumonia"
  )
```

> This faceted display breaks down the relationship between `PredictedReadmissionRate` and `ComparedToNational_Hospital return days for pneumonia patients` by Medicare spending score. There does not appear to be any missing medicare spending score date for hospitals that have a lower compared-to-national-return-days. Notably, there are also no missing observations for `ComparedToNational_Hospital return days for pneumonia patients` in any of the remaining plots.
