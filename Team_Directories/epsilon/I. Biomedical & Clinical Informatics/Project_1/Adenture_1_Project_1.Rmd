...
# EG: Don't need to update the line below because the reRunDemoData.R file sets the condition to AMI 
...
#MH Alternative because html error keeps occuring...
dict %>% 
  kable(
    format = "simple",
    caption = "Table 1. List of possible target variables from the AMI-related hospital readmission data. Which one to choose?"
...
```{r, include = TRUE}
# EG: I had an error because facilityId was still in the data, so I removed it
...
data_sans_encoded = train %>% 
  select(-PredictedReadmissionRate,
         ...
         -facilityId, # Added this to the list of fields to remove
...
> The variable with the highest OOB was Payment for heart attack patients, which had a MSE of 1,978,951.75. Similarly to pneumonia, the payment field had the highest OOB, but the value for heart attack patients was much higher. The pneumonia dataset had a MSE value of approximately 900,000, while the heart attack dataset had a MSE of approximately 1.9 million. Once again, this was by far the largest MSE in this dataset.

> The second highest was Median time (minutes) patients spent in ED, which had a MSE value of 1,621.80. This was not only a different variable than the second highest in the pneumonia dataset, but it also was a much higher value. In the pneumonia dataset, the second highest being the Healthcare workers given influenza vaccination variable, which had a MSE of 291.21. For the heart attack dataset, this was now the third highest, with a MSE value of 273.26, showing that the numbers were quite similar between datasets. Once again, Medicare spending per patient had a MSE of 0, indicating that there were no imputation errors for this variable.
...
```{r}
# EG: remove facilityId as it was giving an error

# Remove encoded categorical variables for imputation, plus the target and state (which have no missing)
data_sans_encoded = test %>% 
  select(-PredictedReadmissionRate,
         -State,
         -facilityId, # EG removed this
         -contains("ComparedToNational"))
...
ggplot(readyTrain, aes(x = PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 ...
                 color = "orchid") +
  theme_minimal() +
  labs(title = "Predicted Readmission Ratio, N = 1,404 hospitals",
       y = "Frequency",
       x = "Predicted Readmission Ratio")
# Changed N from 2,726 to 1,404
```
...
> The p-value is small (1.009-11). However, it is larger than the p-value in the pneumonia dataset, which was 3.085e-14. In both datasets, the null hypothesis (H0) is rejected. Therefore, the distribution is very likely not normally distributed. W is close to 1 (0.98318) and it was 0.98 in the pneumonia dataset.
...
# EG: Updated the text below, as our new lambda is -0.2.

We can see that the estimated $\lambda = -0.2$, so __approximately a square-root transform__ (see lecture slides for reference). I found that $\lambda = -0.5$ in FY 2025, for those using that year's data.
...
ggplot(readyTrain, aes(x = bc_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 ...
                 color = "hotpink") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Predicted Readmission Ratio, N = 1,404 hospitals",
       y = "Frequency",
       x = "Predicted Readmission Ratio")
...
> We surpass 80% of explained variance at PC11, where the cumulative variance equals 0.81107.
...
> 22.24% of variance is explained by just PC1.
> 0.833% of variance is explained by the final PC (PC20).
> The attribution portion of the variance in PC1 is moderate. Exact thresholds that defined whether a PC1 value is high, medium, or low do not appear to exist based on our research. Generally speaking, 22.24% would seem moderate.
...
> The top contributing variables are Communication.about.medicines, Doctor.communication, Staff.responsiveness, Discharge.information, Quietness, Cleanliness, Left.before.being.seen, and Median.time..minutes..patients.spent.in.ED. Based on the Data Dictionary, many of these variables come from the HCAHPS Patient Survey, indicating that they are the result of patient experiences.
