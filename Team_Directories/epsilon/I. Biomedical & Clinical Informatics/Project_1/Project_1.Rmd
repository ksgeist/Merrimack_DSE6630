...
> For Exploratory Data Analysis purposes, and ease of stakeholder understanding, we opted for <b> Option 1.3.2</b>. As discovered in Demo1, for some of our graphing we had to decode some of our encoding for readability when creating graphs, so we've decided to begin with non-encoded data.
...
> Across each of the potential targets, the hospitals with lower readmissions rate/ratio perform better compared to the national average on pneumonia-related hospital days. The differences between hospitals that perform the same or worse than the national average are more obvious when looking at the observes and predicted readmissions rates, with observed readmission rate offering the clearest difference between the performance groupings. Excess Readmission Ratio appears to show a similar pattern, but seemingly has more overlap of the boxplots than the observed or predicted rates. The boxplots for the Excepted Readmission Rate have much greater overlap than the other plots.
...
> As expected, across all the readmission rates for all the figures, the hospitals that are rated with pneumonia-related readmissions "worse" than the national average have the highest predicted pneumonia related readmissions, followed by the performance groups that are the "same", then "better", suggesting that each readmission rate is performing to some degree as we would expect them to. For each readmission rate, medicare spending increases as readmissions increase. This is most transparent for the hospitals that perform worse with pneumonia-related readmissions. The `Predicted Readmission Rates` and `Expected Readmission Rates` show the greatest increase in medicare spending as readmission rates increase. This could be because these hospitals have sicker patients requiring more resources. 
<p>
For `Excess Readmission Ratio` and `Observed Readmission Rate`, death rate show a mostly flat trend as it relates to readmission rate; however, for `Predicted Readmission Rate` and `Excepted Readmission Rate`, there is a negative relationship, especially evident for the "worse" performing hospitals, where death rate increases as readmissions decrease. This may be because hospitals who perform worse may discharge their patients too soon before they're well enough to fully recover potentially leading to death. It could also reflect the inverse of the medicare spending: where increase medicare spending may indicate a more sickly patient population, and more sickly patient population may have higher likelihood of death. 
<p>
For the `Predicted Readmission Rate` and `Excepted Readmission Rate`, the time spent in the ED for worse and average performing hospitals has a positive relationship with slightly increasing pneumonia readmissions as minutes per patient in ED increases. Better performing hospitals do not seem to have the same positive slope, and have a maximum median minutes in ED at approximately 300 minutes, compared to 450-600 minutes seen for Average and Worse performing hospitals. There also appears to be a slight increase in pneumonia related readmissions as the percent of patients that left the ED before being seen increases as well based on the `Predicted Readmission Rates` and `Expected Readmission Rates` figures. Better performing hospitals seem to cap around 8% of patients leaving before being seen, while average and worse hospitals have 17-19% of pateints leaving before being seen. All of this could indicate staffing issues, especially as the emergency department volumes appear to be similar across performance groups.
<p>
There does not appear to be an obvious trend between Venous Thromboembolism Prophylaxis and predicted readmissions; however, average and worse performing hospitals have a wider spread from 0-100% for providing prophylaxis for DVTs, whereas better performing hospitals have a 75-100% range for providing prophylaxis. Given this, better hospitals have a slightly lower rate of DVT than average or worse hospitals, but for better hospitals there is a very small positive trend visible where increased DVT is related with increase pneumonia readmission -- that positive relationship is not as obvious with average and worse hospitals.

> For most of the figures, `Observered Readmission Rate` and `Excess Readmission Ratio` tend to have slopes closer to 0 making it appear like there are not real relationships between the performance metircs and readmissions. Therefore, between the four readmission rates, `Predicted Readmission Rate` and `Expected Readmission Rate` helped provide the clearest picture of what might be different across these hospitals based on their pneumonia-related readmission. Since `Predicted Readmission Rate` takes into account both historical hospital performance and patient population complexity, and expected readmission rate only standardizes to other hospitals with similar population maixes, `Predicted Readmission Rates` is kept as the selected readmission rate.
...
longScore %>%  
  ggplot(aes(y = PredictedReadmissionRate,  ## <-- CHANGE THIS! PredictedReadmissionRate, ExcessReadmissionRatio, ExpectedReadmissionRate,  observed_readmission_rate
    ...
    labs(title = "Relationship of `Predicted Readmission Rate` pneumonia-related \nhospital readmissions to each performance metric",
        x = "Rate or Score",
...
> Once again, `Excess Readmission Ratio` figures tend to have slopes closer to 0 making it appear like there are not real relationships between the performance metircs and readmissions. `Observered Readmission Rate` aligns very closely with the figures from `Predicted Readmission Rate`. The `Expected Readmission Rate` figures do not show much differentiation between the pneumonia-related readmission performance groups, if any at all. In most figures the average and worse hospitals have overlapping, near identical, relationships between predicted pneumonia readmissions and patient satisfaction metrics. In most figures the high performing hospitals are are very close to those average and worse trends as well. Therefore, we will be focusing on `Observered Readmission Rate` and `Predicted Readmission Rate` as the performance groups have clear hierarchy as before, where worse than average performing hospitals have higher predicted pneumonia readmissions, and better than average hospitals have lower readmission rates.
<p>
Interestingly, while the better perfomring hospitals have lower predicted pneumonia related readmissions, for the majority of the patient satisfaction metric figures the slopes are nearly flat, whereas worse or average performing hospitals see a negative relationship for every figure where a satisfaction metric improves. This may be due to a more consistent standard of care in higher performing hospitals.
<p>
Compared to the other patient satisfaction metrics Cleanliness has the weakest relationship to pneumonia readmissions, followed closely by quietness. The average and worse performing hospitals have slight negative relatioships where increased cleanlieness and quietness leads to decreased pnemonia readmissions. The weaker relationship here may suggest readmissions have more to do with patient's status at the time of discharge (i.e. whether they were discharged too early), or post-discharge "transitional care" as suggested in the CMS "Hospital 30-Day Pneumonia Readmission Measure", 2008. (https://qualitynet.cms.gov/files/5d0d3ae5764be766b0104c37?filename=Pneumo_ReadmMeasMethod.pdf)
<p>
This idea is reinfoced by the strong negative relationship between pnemonia related readmissions and doctor communication, and pnemonia related readmissions and nurse communication. Increased communication may suggest the patient or caregiver is given proper transitional care instructions, corroborated by a nearly identical graph for discharge informaiton and care transition.
<p>
The `Predicted Readmission Rate` figures have two metrics where better performing hospitals have a slight increase in pneumonia related readmissions where there is higher nurse communication and higher staff responsiveness. The trend lines for better performing hospitals cross over with average performing hospitals such that it appears average performing hospitals have lower readmissions than better performing hospitals. These crossovers are not seen in the `Observered Readmission Rate`. For this reason, and because it seems more appropriate to compare actual readmission rates with the actual patient satisfaction survey data, the `Observered Readmission Rate` was selected to represent these patient satisfaction metrics. 
...
longHCAHPS %>%  
  ggplot(aes(y = observed_readmission_rate,  ## <-- CHANGE THIS! PredictedReadmissionRate, ExcessReadmissionRatio, ExpectedReadmissionRate,  observed_readmission_rate
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                ...
                show.legend = T) + 
    ## CHANGE THE TITLE FOR FULL CREDIT!
    labs(title = "Relationship of `Observed Readmission Rate` pneumonia-related \nhospital readmissions to each patient satisfaction metric",
        x = "Rate or Score",
        y = "Observed Pneumonia-related Readmissions", #Updated to reflect feature
        color = "Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
  scale_color_manual(values = c("gold", "cadetblue", "maroon"),
                    labels = c("Better", "Same", "Worse")) + 
...
> Of all of these scatterplots, those representing doctor/nurse communication and the time a patient spends in the ER as the most compelling plots. Together they tell a story that when hospitals are able to give patients more attention and time their prognosis is better.
> One lingering question our team has discussed is related to the Medicare spending. In class we've been discussing that hospitals receiving more Medicare money tend to have better standard of care, but in these figures, and the figures we investigated for Demo1, we're seeing the opposite: increased medicare spending seems tied to more readmissions. We discussed this as a team that this could be because the patients are going back in for readmission and therefore 'of course that hospital is getting more money'. The leading hint in question 4 also suggests another reason: sicker patient population. So, perhaps there is another feature (not in our dataset) that would indicate that relationship where more spending leads to better outcomes?
...
> We do agree with this selection of target in this instance. `Predicted Readmission Rate` accounts for historical hospital performance as well as the general patient population, perhaps more importantly for this project and amount of data, it is not missing ~13% of the data it would be if we were selecting `observed_readmission_rate`. The `observed_readmission_rate` is a snapshot of a hospital's performance and may miss out on accurately represent the patient population as a whole or the hospital's performance in general (perhaps the captured year was one where several providers retired, or a change in upper management).
...
(sum(colSums(is.na(dat2Analyze)) > 0)/ ncol(dat2Analyze)) * 100
  

na_counts <- colSums(is.na(dat2Analyze))
...
> Currently, 31 columns (of 31 columns) have some amount of missing data. This is 100% of features having some level of missingness. Notably, our planned outcome variable `PredictedReadmissionRate` has 2090 missing features. This is roughly 43% of the current dataset. Even with those 2090 rows are removed, we would still be left with 30 columns or 96.8% of columns with some amount of missing data in every predictor column. 
...
> Cell $[6,1]$ of the Missing v. Missing matrix shows 1788 rows where both `Predictited Readmission Rate` and `Postoperative Respiratory Failure Rate` are missing. This is ~37% of our current dataset's observations where both our target variable and `Postoperative Respiratory Failure Rate` are missing. This does feel like a high-degree of co-missingness, but MRSA has an even larger degree of co-missingess at Cell $[1,2]$ where 2042 rows or 42% of the observations are missing MRSA and our target variable.
...
> No, we do not feel any differently about Question 9D based on the new dimensions. We still have 2,726 rows and 31 columns. The curse of dimensionality would be more likely if our number of rows had been reduced so drastically that it was closer to the number of columns.
...
> We got a split ratio of 82% training and 18% testing. This is close to the canonical 80-20 split, with slightly more training and slightly less testing. Based on the code above, finding the ideal split ratio is based on the number of parameters. Therefore, we know the number of parameters in our dataset directly impacted this calculation. If we had a the same dataset with only a different number of parameters, our test/train split would likely be slightly different.
...
> The variable with the highest OOB was Payment for pneumonia patients, which had a MSE of 948,452.73. This was by far the largest MSE, with the second highest being the Healthcare workers given influenza vaccination variable, which had a MSE of 291.21. Notably, only two variables,	Medicare spending per patient and SurveyResponseRate, had a MSE of 0, indicating that there were no imputation errors for these variables.
> The variable with the highest PCF was Emergency department volume, with a PCF of 0.46. The remaining five variables all had signficantly smaller PCF values, but none were 0.
...
```{r}
# EG
# We chose the Doctor communication variable
...
> As seen in the tables, prior to imputation, the Doctor communication variable had 125 NA values. After imputation, there were none, as expected. Additionally, the only metric that changed was the mean, which slightly increased from 89.68 to 89.72.
...
> We also used table() to look at the values prior to imputation and after imputation. An interesting finding is that prior to imputation, all of the values in the Doctor communication field were whole numbers. After imputation, there are now more values, many of which have decimals. 
...
> The p-value is incredibly small (3.085e-14). Due to this, the null hypothesis (H0) is rejected. Therefore, the distribution is very likely not normally distributed. W is close to 1 (0.98).
...
> Prior to the Box-Cox transofrmation, the summary statistics showed that the data ranged from 11.93 - 22.91. After Box-Cox, the data ranged from 1.749 - 2.031. This shows that the Box-Cox transormation squsihed the data, which impacts the skewness and variance. Since the data was not normally distributed prior to this, this will help it achieve a normal distribution. 
...
```{r}
# Verify that the imputedTest dataset includes both PredictedReadmissionRate and bc_PredictedReadmissionRate
"PredictedReadmissionRate" %in% names(imputedTest)
"bc_PredictedReadmissionRate" %in% names(imputedTest)
...
> Instances where there is dark blue in this heatmap indicate areas where there is the highest potential for multicollinearity. Some of the variable combinations that have the potential for multicollinearity include:
ComparedToNational_Death.rate.for.pneumonia.patients and Death.rate.for.pneumonia.patients

> ComparedToNational_Composite.patient.safety and Composite.patient.safety

> MRSA.Bacteremia and ComparedToNational_MRSA.Bacteremia
...
> The variable that seems most correlated with our target, bc_PredictedReadmissionRate, is ComparedToNational_Hospital.return.days.for.pneumonia.patients. This variable has the darkest blue square, indicating that it is highly positively correlated with our target variable.
...
> Between the heatmap and the table of the top 15 Variance Inflation Factors (VIF), there is evidence that multicollinearity is an issue in this dataset. Overall.hospital.rating has the highest VIF of 22.69 and Recommend.hospital has the second highest of 14.77. As mentioned earlier, VIF > 4 (and especially 10) can identify multicollinearity. Following this logic, we have 5 other variables with a VIF > 4: Nurse.communication, Care.transition, Staff.responsiveness, Communication.about.medicines, and Doctor.communication. Many of these variables are surrounding either staff or communication, indicating an overlap in information, which could be contributing to the collinearity. If any of these variables were created from each other, this would also cause collinearity.

> Additionally, Discharge.information has a VIF of 3.48, which is close enough to 4 that it can be noted.

> There can be many variables that are dervied from another. An example of this is Payment Category for pneumonia patients being derived from Payment for pneumonia patients. Keeping both causes high collinearity and doesn't make sense for the model. Generally speaking, the most valuable of the two should be kept, which in some cases can be the original variable, but is ususally the new variable.
...
> ComparedToNational_Hospital.return.days.for.pneumonia has the highest positive correlation with the target variable (bc_PredictedReadmissionRate). This makes sense because if a hospital has higher than national average return days for pneumonia, then they will have a higher readmission rate. Therefore, an individual will be more likely to be readmitted.
...
> We surpass 80% of explained variance at PC11, where the cumulative variance equals 0.83983.
...
> 22.41% of variance is explained by just PC1.
> 1.056% of variance is explained by the final PC (PC18).
> The attribution portion of the variance in PC1 is moderate. Exact thresholds that defined whether a PC1 value is high, medium, or low do not appear to exist based on our research. Generally speaking, 22.41% would seem moderate.
...
> 5 PCs appear optimal based on the scree plot. Although there is a steep drop at 1, this would not be the optimal choice.
...
fviz_pca_biplot(pca, 
             ...
             col.var = "black",
             habillage = returnHospitalComparison,
             select.var = list(contrib = 12),   ## Try changing the number
             repel = T)                        ## Try switching to FALSE
```
...
> The top contributing variables are Communication.about.medicines, Doctor.communication, Composite.patient.safety, Discharge.information, Quietness, Postoperative.respiratory.failure.rate, and Cleanliness. Based on the Data Dictionary, most of these variables come from the HCAHPS Patient Survey, indicating that they are the result of patient experiences.

> The target, PredictedReadmissionRate, does not show up in this plot, but bc_PredictedReadmissionRate does. It is just at the red line, indicating that it contributes to the overall explanation of variance, but is not one of the storngest contributors.
...
> From a previous question up above with the Scree Plot 5 looked to be the best K variable to use. But there is still much overlap which means the clusterings groups aren't strong but really weak correlations at this time.
...
>WSS is a metric of how compact the clusters are. It's based on the total square distance between the data point and the centroid cluster it belongs to. Based on this method it looks to be 2 or 6. It didn't since earlier answer was 5.
Source <https://aiml.com/what-is-the-within-cluster-sum-of-squares-score/>
...
> It isn't surprising since one of my original thoughts on which k cluster numeric was the best shows the result here. It agrees with the elbow method since to find the best "k" numeric is where on the plot the pattern of change is at it's greatest or changes direction compared to the other k numerics.
...
kmeansResult <- kmeans(readyTrain, 	
                         ...
                         nstart = 50, #This tells how many times you want to run the process / algorithm for each center is being chosen.
                         iter.max = 1000, #Max number of updates the algorithm can do.
                         algorithm = "MacQueen") # when the algorithm is running, it'll gradually do updates on the centroids, and not all centroids at once.  

##Sources <https://github.com/valentinasilva8/KMeans-Clustering-Analysis-in-R>
##<https://search.r-project.org/CRAN/refmans/factoextra/html/eigenvalue.html>
...
kmeansTrain <- imputedTrain %>% 
  ...
  ## This will combine and modify columns and results if Pneumonia.Hospital.Return.Days meets a certain threshold.
    ## Cluster is added to the dataset as a column which takes vectors from kmeansResult$cluster observation.
    ## Pneumonia.Hospital.Return.Days is recoded to the observation / cell value equals 1 return "1" or if it equals 0 or -1 input "0".
...
kmeansTrain %>% 
  ...
  ## Will search for any data values within the column "Measure"
  ...
  ## Will plot the every unique cluster a different observations on the x axis, and the values as the response or numerical calculated data to be added into the graph to show results of each observation there is.
    ...
    theme_classic() + ## Sets the theme to ggplot2's classic style.
...
"Cleanliness in Hospitals", "These hospitals had a lower rating on their cleanliness, being that the hospitals may not be the cleanest patients still got better care to get better sooner.",
"Postoperative Respiratory Failure due to pneumonia", "Even though there is a lower amount of readmission rate for hospitals in Cluster 1, there are slightly higher Postoperative Respetory Failure.",
"Average Discharge Information rate", "The amount of information for Cluster 2 hospitals have higher discharge information than Cluster 1 hospitals.",
"The Rate of Patient Safety", "The second cluster of hospitals had a lower rate of Patient safety protocols which could lead to staying in the hospital longer than those going cluster 1 hospitals ",
"Communication about Medicine", "This was more superior in cluster 2 of hopsitals which can help us see why there is a higher average for being on pair with the national average then the other clustered hospitals.",
"Periop Rate of Embolism or Thrombosis", "This stat is lower on the cluster 2 of hospitals potentially due to the lower rate of Postoperative Respitory Failure Rate from blood clots after surgical procedures."
)
...
dict %>% 
  kable(
    ...
    pack_rows("2: Average to Lower Performing Hostials!", 5, 8) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> The results of the OLS indicate with a shapiro-Wilk test, we can see that p > 0.05 which rejects normality which is a good thing in this case. This indicated the residuals are consistant with a normal distrobution. With the Wilk result of 0.99862 this is really close 1, expressing to be strongly supporting normality. These outcomes met our assumptions.
...
>There seems to be a slight result of non-linearity due to some slight curvature in the resulting graphs.
...
> The residual standard error (RMSE) of approximately 0.78 indicates that the model’s predicted values deviate from observed values by about 0.78 units on average. This suggests moderate prediction accuracy, although interpretation is limited by the transformation applied to the dependent variable. Together with the adjusted R² of 0.395, the model demonstrates reasonable explanatory and predictive capability for complex healthcare data.
...
>Top five from Ordinary Linear Spuares (OLS) Results were:
...
>Top five from Elastic Net Results were:
Hospital return Days for pneumonia patients
Medicare spending per patient
Death Rate for Pneumonia Patients.
State
...
>Both of the models had the same most important features in the same order. Thus we can use either or for modeling since they do produce the same importance of feautres.
...
> The apparent inversion between PCA loadings and regression coefficients arises because the two methods capture different aspects of the data. PCA is an unsupervised technique that identifies directions of greatest variance among predictors, while regression is a supervised method that identifies variables associated with the dependent outcome. Additionally, the sign of PCA loadings is arbitrary and can be reversed without affecting the underlying component structure, whereas regression coefficients have a meaningful direction. As a result, variables that appear important in PCA do not necessarily correspond to those that influence the outcome in regression, and apparent sign differences should not be interpreted as true contradictions.
