...
```{r}
## These files are used to represent geographic regions - each file has its own purpose in calculating & rendering plots that utilize spatial data. All files need to be named the same (minus the extension) for them to work together.

## .cpg files are files that define the encoding for creating the associated shapefile
## .dbf files are a database file type used in a number of programs and store attributes about the shapefile
## .prj files contain information on coordinates and map projections
## .shp files contain information on the vectors used to describe the polygons (regions), such as states
## .shp.xml files contain metadata about the information stored in the .shp file, such as how the file was created and any transformations that were applied
## .shx files are indexes of information inside the other files
...
```{r}
cali_sf <- read_sf("C:\\Users\\mike_\\OneDrive\\Documents\\School\\Healthcare Analytics\\Project2\\California_County_Boundaries_8408091426384550881\\cnty19_1.shp") 
...
```{r}
## There were 69 outputs from the validity check because of there are 58 counties in california. An additional 11 records are added to account for individual islands inside each county, where applicable.
```
...
```{r}
## The bounding box for this shapefile is as follows:
## xmin: -13849230 ymin: 3833650 xmax: -12704980 ymax: 5162403

## To zoom in / out, the bounding box values would increase or decrease, respectively. They need to be adjusted in the original aspect ratio to preserve the relative size of the features.
```
...
```{r}
## The projection for the shapefile is as follows:
## WGS 84 / Pseudo-Mercator

## Projections are used to manipulate the 3D information of the surface of the earth to two dimensions seen on a map. To do this, some features of the map are contorted. Changing a projection will change the level & location of these contortions.
```
...
map1 <- cali_sf %>%
  ...
  ggplot() +
    ...
    coord_sf(expand = FALSE) +   ## <- keeps map tight to its bounding box
    ## Give it a title and footnote/caption 
    labs(title = "California Counties bordering Oregon",
         caption = "Source: California State Geoportal") 
...
## create a dataframe, sourced off of the total deaths data (which is number of deaths, per county, per month, per year)
mortality_sf <- totalDeaths %>% 
  ## rename the "County" column to "COUNTY_NAM"
  rename(COUNTY_NAM = County) %>%
  ## group by the county, year & month
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## reset the totalDeaths column to the sum of specific county's deaths per year / month, removing N/As
  ## (this is essentially how the data was originally organized)
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## join (including results from each table) on the COUNTY_NAM common column
  ## "many-to-many" indicates we specifically understand that no relationship checks will occur, suppressing the warning message
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## recreate as a shapefile
  st_as_sf() %>% 
  ## remove records that represent an island
  filter(is.na(ISLAND))
```
...
```{r}
## Beyond the fact that these two joins work on different data types (relational vs spatial), the full join join type isn't implemented in the st_join() method, so although it would be technically possible to use the st_join method (with some rewriting of the above), the end result would not match that of a full_join
```
...
```{r}
## (58 counties * 48 months of data) - nas = 2552
```
...
```{r}
## The data isn't scaled, so of course the areas of high population have highest counts of mortality. As is, this doesn't provide any insight.
```
...
key <- "1acb8f43444a10ef94aa63724f8bfcd3fac09a40"
```
...
```{r}
## DP1_0048C => Count!!SEX AND AGE!!Male population!!Selected Age Categories!!65 years and over
## DP1_0048P => Percent!!SEX AND AGE!!Male population!!Selected Age Categories!!65 years and over
```
...
## This dataframe contains counts of people in each county. This is further broken down by sex & age, and also expressed as percentages.
...
```{r}
## 1: Different APIs accept different ways of requesting & filtering information. In a general sense, the base url (the host name) of the API would need to change to represent whatever service is being utilized (nist.gov, noaa.gov, etc). Based on the API's documentation, different parameter names & values (the querystring) would be different to match what that service expects. Therefore, const values like the "DP1_*" and field names would vary.
## 2: The construction of the URL (and querystring) would remain similar across API clients. Once the URL is built, the HTTP client (in this case, an R library) would make the HTTP request to the resource specified in the URL. Again, the streaming of the response from the resource would also remain the same across aPI clients (reading the stream as JSON). However, each API resource will probably structure their data in different ways, so a function would need to allow flexibility to work with different shapes of data for parsing into a relational set.
...
```{r}
## Don't forget to use st_as_sf() to put back into a shapefile!!
mortality_sf2 <- mortality_sf %>%
  inner_join(census, by = join_by(COUNTY_FIP == COUNTY)) %>%
  st_as_sf()
...
```{r}
## It looks like mortality rates are actually declining in 2020. However, the 2020 data isn't complete - so we're comparing only 8 months of data in 2020 against 12 months of data for the other years. It's hard to tell this as we are grouping only by year in the above data set.
```
...
```{r}
## The issue is the lack of data for 4 months of 2020, so we're not truly comparing apples to apples. One thing we could do is just compare the first 8 months of each year which would allow an accurate comparison of deaths in 2020.

## If we had a full 12 months of data for 2020, we could have accurate maps in 2.7.
...
```{r}
## At this point, the pattern for 2020 looks similar to 2019, except that in most cases, the degree of mortality has increased. This is more apparent in southern counties as opposed to northern counties (at least at the current scale).
```
...
## Not robust! Both the residuals & fitted values indicate not a good model fit. The QQ plot shows a fair amount of non-normality. Additionally, it is suspect that the predicted death rates are decreasing for the predicted months, a reversal in trend from the previous years.
```
...
```{r}
## Since we're trying to generate county neighbor information in this step, we don't need to know about the county's death rates by year & month. The neighbor function doesn't need to know that, and it creates unncessary redundancy. It will definitely cost a lot of computation if the original data set wasn't truncated.
...
```{r}
sarModel <- lm(monthlyPercMortality ~ Month + Year + lagged_monthlyPercMortality,
    data = mortality_sf2)
```
...
## It appears the OLS model is more fitted to the data. The residuals are much more straight, the Q-Q is somewhat more standardized. The sarModel has some shape indicating non-linearity.
```
...
```
```{r}
## The comparison indicates that the OLS model is better fitted.
```
...
```{r}
## The analysis of the OLS model cannot be trusted - the county name is being incorporated into the model, which obviously adds no value. The higher r^2 values appear to be symptomatic of higher terms = higher r^2 results.
```
...
```{r}
train_len <- dim(mortalityTrain)[1]
test_len <- dim(mortalityTest)[1]
total_len <- dim(mortality_sf2)[1]

print(paste("train (count/pct)", train_len, train_len / total_len))
print(paste("test (count/pct)", test_len, test_len / total_len))

## Pretty close to the 80/20! (~82/18)
...
```{r}
## A good portion of the 2020 data is during a pandemic, so the possibility of non-pandemic years predicting pandemic-time data accurately is low.
```
...
```{r}
## OOB error can be helpful in estimating the performance of a model during its training phase. It can also provide suggestions on better values for tuning parameters.
```
...
```{r}
## mtry is the number of variables considered during the splits of the random forest decision tree. Its usage here is probably an attempt to increase randomness given that there are few predictors in our data set the model can utilize.
```
...
```{r}
## Mathematically, r^2 values can be negative - however, it logically indicates an error. These results can't be compared to the previous model as preProcess applies transformations across the dataset, including the target.

## Kuhn, M. (2019, March 27). 3 Pre-Processing. The caret Package. https://topepo.github.io/caret/ 
```
...
```{r}
## It appears to be helping just slightly, in the fact there are no longer negative r^2 values, but the error rates and r^2 values are still poor. The mtry value still indicates that the value of 1 is optimal. We still have an accuracy problem.
```
...
```{r}
## The training model performed about as expected on the test data. The RMSE / MAE values are similar. Since they're so similar, but high, it appears they are suffering from underfitting.
...
```{r}
## The predicted values seem to be higher than the actual mortality rates. The training model is leading to an overprediction of mortality. 
...
```{r}
## CODE COPIED FROM June Lemieux's comment in the announcement!
...
```{r}
## first filter down to the year + strata we're looking for

caliDeathsSummarized <- caliDeaths %>%
  filter(
    Year == 2020
    & Strata == "Total Population"
    & Cause == "ALL"
    ) %>%
  inner_join(census, join_by(County == County)) %>%
  select(County, Year, Month, Count, totalN) %>%
  na.omit() %>%
  rename(
    AllDeathsCount = Count,
    CountyPopulation = totalN
  ) %>%
  group_by(Year, Month) %>%
  summarize(
    StatePopulation = sum(CountyPopulation),
    MonthlyDeathRate = 
      sum(AllDeathsCount, na.rm = TRUE)/mean(StatePopulation)*100
  ) %>%
  ungroup() %>%
  select(Year, Month, MonthlyDeathRate)
```
...
```{r}
# rfPredictions only contains jan - aug 2020 data, create a new one

predicted_actual_comparison <- newdata %>%
  rename(
    PredictedPercentMortality = percMortality
  ) %>%
  filter(Year == 2020 & Month %in% c(9,10,11,12)) %>%
  select(-isPredicted) %>%
  inner_join(caliDeathsSummarized, by=join_by(Year == Year, Month == Month)) %>%
  rename(
    ActualPercentMortality = MonthlyDeathRate
  )
...
predicted_actual_comparison %>%
  ...
  ggplot(aes(x = Month, y = measurement, color = type)) +
    geom_point() +
    geom_smooth(method = "lm")

cor.test(
  predicted_actual_comparison$PredictedPercentMortality, 
  predicted_actual_comparison$ActualPercentMortality,
  method = "pearson")
```
...
```{r}
## The plot shows that there is significant difference between predicted & actual death rates. The coorelation values (specifically the p-value of ~0.16) indicates that we cannot reject the null hypothesis (there is no correlation). 

## The difference in predicted / actual is due to the predictions being based on < 2020 data. Death rates increased abnormally in 2020 (pandemic) so our model performed poorly.
```
...
```{r}
## Temporal autocorrelation is the relationship between successive values (of the same variable). ^1

## There appear to be a number of tools that can help find autocorrelation and account for it. By applying lags that account for peaks and valleys of values in time-series data, its possible to make better predictions as some of the variance is accounted for. ^2


## 1: Abdulhafedh, A. (2017) How to Detect and Remove Temporal Autocorrelation in Vehicular Crash Data. Journal of Transportation Technologies, 7, 133-147. doi: 10.4236/jtts.2017.72010.

## 2: Holbrook, R., & Cook, A. (n.d.). Time Series as Features. Kaggle. https://www.kaggle.com/code/ryanholbrook/time-series-as-features/tutorial 
...
```{r}
## Since the MAE has gotten better (with the addition of the time-lagged variables), it would appear that the new variables are helping the model get better. The difference between the training/testing MAEs are minor, so I'd say its a pretty balanced model.
```
...
```{r}
## Since random forest models need complexity, the goal would be to add more variables. For example, combining data from neighboring states.
```
...
```{r}
## Although counterintuitive, after adjusting for temporal lags, the created random forest model makes good predictions on death rates, even though the training data was sourced from 2017-2019 data and the test data was sourced from 2020 (the pandemic year). This indicates that although COVID itself may have caused a great deal of deaths, its overall impact on the mortality rates over time was minimal.

## Data exists for COVID-specific mortality values within our available data sets. If the goal is to analyze the total impact COVID has had on mortality, the next step would be to compare COVID-specific deaths against total deaths over time.
```
...
```{r}
## Looking specifically at accidental deaths, I think its important to point out the difference in sex. The fact that the medians of the two groups are percentage-points away from each other is significant.
...
```{r}
## There is a significant downturn in May of 2020 in accidental deaths for both sexes. Although a lot was happening in May of 2020, the only thing I can think of as having an effect would be more enforcement of stay-at-home orders.
...
```{r}
## These map plots don't show significant changes due to the stay-at-home orders, however zooming out across the state, there is an obvious change in total accidental deaths. Perhaps accounting for % change versus the previous year (rather than overall deaths) would show this beter in the maps. The maps do not show the impact of sex on the death rates. To show the death rates between sexes on the maps, we would need to dissect the maps information into sex-specific calculations (as we've done above), then facet out the maps to show each sex's rates over time (a total of 8 maps).
...
```{r}
## Its possible that some counties don't report those deaths. Causes of death are public information (as far as I know) so it shouldn't be a data governance issue
```
...
## Show maps of accidental deaths, per sex, per stay-at-home order or not

newInjDeaths <- injDeaths %>%
  mutate(stayAtHome = as.factor(stayAtHome)) %>%
  ## Group by stay-at-home order, year, county, AND sex
  ...
  ## set any N/A to 0
  mutate(
    deathsInjuriesPercent = ifelse(deathsInjuriesPercent == "NaN", 0, deathsInjuriesPercent)
  ) %>%
  ## Coerce to make sure it is seen as a shapefile
  st_as_sf() %>% 
  ## Remove any of the island geometries as you did in the first part
...
summ <- fivenum(newInjDeaths$deathsInjuriesPercent)

newInjDeaths <- newInjDeaths %>% 
  mutate(deathLabels = cut(deathsInjuriesPercent, 
                          breaks = c(0, 2.72641, 15.78947, Inf),
                          labels = c("0-2.72641", "2.72642-15.78947", "15.78948+"),
                          ordered_result = TRUE, right = F)) 
```
...
ggplot(newInjDeaths) +
  ...
  # Add title and caption
  labs(
    title = "California Annual Percent Deaths due to Accident or Injury, by County, Sex & Stay-at-home Order in Effect",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
...
injDeaths_noNA <- na.omit(injDeaths)

cor.test(
  injDeaths_noNA$femaleMaleRatio, 
  injDeaths_noNA$deathsInjuriesProportion,
  method = "pearson")
```
...
```{r}
## Per the test, there is definitely coorelation. However, the impact of this correlation is relatively small and wouldn't make a big change to the visualizations we already have. Incorporating this correlation into the models does make sense, as it will make them more accurate.
...
```{r}
## Yes, I think that plot shows the correlation fairly well. Its not overly strong, but it is apparent.
...
```{r}
## Question: Did the pandemic impact the rate of suicide in California?
## Null hypothesis: The COVID-19 pandemic had no effect on suicide rates in California.
## Alternative hypothesis: The COVID-19 increased suicide rates in California.

## I don't think I'd include spatial lags on the first iteration, but definitely temporal lags. It's possible that there could be some latency of the impacts due to lonliness & depression.
...
```{r}
## Let's use the hypothesis above!
```
...
```{r}
## First capture all suicide deaths from our API (2020+)
years <- 2020:2023
suicideDeathsFromApi <- NULL
allDeathsFromApi <- NULL

for(year in years){
  filters <- URLencode(str_interp('{"Year":"${year}","Cause":"SUI"}'))
  url <- paste0('https://data.chhs.ca.gov/api/3/action/datastore_search?resource_id=078185a9-e3a7-403f-a546-b8c582f0a9d8&limit=50000&filters=', filters)
  
  temp <- fromJSON(url)
  temp <- temp$result$records

  if(is.null(suicideDeathsFromApi)){
    suicideDeathsFromApi <- as.data.frame(temp)
  }
  else{
    suicideDeathsFromApi <- rbind(suicideDeathsFromApi, as.data.frame(temp))
  }
  
  filters <- URLencode(str_interp('{"Year":"${year}","Cause":"ALL"}'))
  url <- paste0('https://data.chhs.ca.gov/api/3/action/datastore_search?resource_id=078185a9-e3a7-403f-a546-b8c582f0a9d8&limit=50000&filters=', filters)
  
  temp <- fromJSON(url)
  temp <- temp$result$records

  if(is.null(allDeathsFromApi)){
    allDeathsFromApi <- as.data.frame(temp)
  }
  else{
    allDeathsFromApi <- rbind(allDeathsFromApi, as.data.frame(temp))
  }
  
}

suicideDeathsFromApi <- suicideDeathsFromApi %>%
  mutate(
    Year = as.numeric(Year),
    Month = as.numeric(Month),
    Count = as.numeric(Count)
...
allDeathsFromApi <- allDeathsFromApi %>%
  mutate(
    Year = as.numeric(Year),
    Month = as.numeric(Month),
    Count = as.numeric(Count)
...
```{r}
## just keep non-island records
cali_sf_no_islands <- cali_sf %>%
  filter(is.na(ISLAND)) %>%
  select(-ISLAND) %>%
  distinct() %>%
  st_as_sf()
...
```{r}
## Combine sets to get the data we need

suicideDeaths2020_2023 <- suicideDeathsFromApi %>%
  filter(Strata == "Gender" & (Strata_Name %in% c("Male", "Female")) & Cause == "SUI" & Year %in% c(2020:2023)) %>%
  group_by(County, Year, Month, Strata_Name) %>%
  summarize(
    totalDeaths = sum(Count, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  rename(
    Gender = Strata_Name,
    SuicideDeaths = totalDeaths
  ) %>%
  mutate(
    Gender = as.factor(Gender)
  )

suicideDeaths2017_2019 <- sexDeaths %>%
  filter(leadingCauseDeath == "SUI" & Year %in% c(2017:2019)) %>%
  group_by(County, Year, Month, biologicalSex) %>%
  summarize(
    totalDeaths = sum(sexDeaths, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  rename(
...
suicideDeaths <- rbind(suicideDeaths2017_2019, suicideDeaths2020_2023)

allDeaths2020_2023 <- allDeathsFromApi %>%
  filter(Strata == "Gender" & (Strata_Name %in% c("Male", "Female")) & Cause == "ALL" & Year %in% c(2020:2023)) %>%
  group_by(County, Year, Month, Strata_Name) %>%
  summarize(
    totalDeaths = sum(Count, na.rm = TRUE)
  ) %>%
  ...
  rename(
    Gender = Strata_Name,
    AllDeaths = totalDeaths
  ) %>%
  mutate(
    Gender = as.factor(Gender)
  )

allDeaths2017_2019 <- sexDeaths %>%
...
allDeaths <- rbind(allDeaths2017_2019, allDeaths2020_2023)
```


```{r}
## Create a spatial data set of suicide rates by county by year.
suicideVsAllDeathsByMonth <- suicideDeaths %>%
  inner_join(allDeaths, 
             by = 
               join_by(County == County, Year == Year, Gender == Gender, Month == Month)
             ) %>%
  mutate(
    SuicideDeaths = ifelse(is.na(SuicideDeaths), 0, SuicideDeaths),
    AllDeaths = ifelse(is.na(AllDeaths), 0, AllDeaths)
  ) %>%
  mutate(
    SuicideProportionOfDeaths = SuicideDeaths / AllDeaths
  ) %>%
  mutate(
    SuicideProportionOfDeaths = ifelse(SuicideProportionOfDeaths == "NaN", 0, SuicideProportionOfDeaths)
  ) %>%
  mutate(
    SuicidePercentOfDeaths = as.numeric(SuicideProportionOfDeaths) * 100
  ) %>%
  inner_join(cali_sf_no_islands, by = join_by(County == "COUNTY_NAM")) %>%
  mutate(
    Year = as.factor(Year)
  ) %>%
  arrange(County, Year) %>%
...
suicideVsAllDeathsByYear <- suicideVsAllDeathsByMonth %>%
  select(-Month) %>%
  group_by(County, Year) %>%
  summarize(
    SuicideDeaths = sum(SuicideDeaths, na.rm = TRUE),
    AllDeaths = sum(AllDeaths, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(
    SuicideProportionOfDeaths = (SuicideDeaths / AllDeaths)
  ...
  mutate(
    SuicidePercentOfDeaths = ifelse(SuicideProportionOfDeaths == "NaN", 0, SuicideProportionOfDeaths) * 100
  ) %>%
  st_as_sf()

suicideVsAllDeathsByYear <- suicideVsAllDeathsByYear %>%
  mutate(
  deathLabels = cut(
    SuicidePercentOfDeaths, 
      breaks = c(0, 0.40, 0.80, 1.20, Inf),
      labels = c("0-0.40","0.41-0.80", "0.81-1.20", "1.20+"),
      ordered_result = TRUE, right = F
    )
  ) %>%
  st_as_sf()
...
```{r}
## Create some plots of each county's suicide rate over time

suicideVsAllDeathsByYear %>%
  ggplot() +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +
  ...
  # Add title and caption
  labs(
    title = "California Annual Percent Deaths due to Suicide, by County, by Year",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Dec 2023",
    caption = "Source: California DPH Vital Records"
  ) +
...
```{r}
## Draw a line graph, stratified by sex
## Facet it as its difficult to read when combined

suicideVsAllDeathsByMonth  %>%
  st_drop_geometry() %>%
  group_by(Month, Gender, Year) %>% 
  summarise(SuicidePercentOfDeaths = mean(SuicidePercentOfDeaths, na.rm = TRUE)) %>% 
  ggplot(aes(x = Month, 
             y = SuicidePercentOfDeaths,
             color = as.factor(Year), 
             linetype = as.factor(Gender))) +
    geom_line(alpha = 0.7, size = 1.1) +
    scale_x_continuous(n.breaks = 10) +
    scale_color_manual(values = skittles[c(4:8, 9:11)]) +
    scale_linetype_manual(values=c("dotdash", "solid")) +
    labs(title = "Proportion all deaths due to suicide, by sex",
         y = "Proportion of All Deaths Due to Suicide", 
         subtitle = "Cumulative for all California counties, Jan 2017 - Dec 2023",
         color = "Year",
         linetype = "Sex of Decedent") +
    theme_minimal() + 
    facet_wrap(~ Gender)
...
```{r}
## Based on the data, it would appear that the proportion of deaths caused by suicide between 2017-2023 were quite different between the sexes. Surprisingly, the suicide rate dropped significantly in 2020, and slightly rose 2021-2023. Higher suicide rates seem to center around higher populated areas. Suicide rates were highest in 2018 & 2019.
```

5. **A data science question you would like to test**
```{r}
## Did stay-at-home orders (due to the COVID-19 pandemic) result in higher suicide rates?
```
...
```{r}
## Null hypothesis: There is no evidence that the COVID-19 stay-at-home order raised suicide rates in proportion to total deaths.
## Alternate hypothesis: There is evidence that the COVID-19 stay-at-home order raised suicide rates in proportion to total deaths.

## Based on the results, it would appear that the lockdown did not increase suicide proportions of total deaths. In fact, it appears to have reduced suicide % of death rates.

## Potential issues / next steps:
## - The sources of data between the 2017-2019 & 2020-2023 data sets are different. Could there be a difference in collection methods or other error causing these dramatic differences?
## - Did the total deaths increase in 2020+ (due to COVID) enough to reduce the proportion of suicide? If so, additional analysis is required.
```
