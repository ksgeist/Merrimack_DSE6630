...
#MH updated include = FALSE to TRUE avoid knitting issues per comment in section 2.6.2
...
Source: https://desktop.arcgis.com/en/arcmap/latest/manage-data/shapefiles/shapefile-file-extensions.htm

> California_County_Boundary.cpg is an optional file, where CPG stands for code page file. It specifically defines the encoding used in the dataset.

> California_County_Boundary.dbf is a database file which stores the attribute information in the dataset. These features are stored in a tabular form.

> California_County_Boundary.prj is a projection file that stores the projection (coordinate system) of the data.

> California_County_Boundary.shp is the shapefile. This file has the geometry of all the features in the dataset. When loaded in to a software like ArcGIS or QGIS, it will show the polygons (or points if applicable) in the dataset.

> California_County_Boundary.shp.xml is a metadata file for the shapefile and contains information about the shapefile. 

> California_County_Boundary.shx indexes the geometry. This is a necessary file and SHX stands for shape index file.

> As mentioned in the lecture, all of these files must have the same file name and be stored in the same directory for them to open and work correctly.
...
## Spark
# cali_sf <- read_sf("C:/Users/Desicious Win/Desktop/California_County_Boundary_view_layer_for_public_use_-7341318997869969624/California_County_Boundary.shp")

# #For MH comp 
# cali_sf <- read_sf("C:/Users/User/Documents/DSE6630/California_County_Boundaries2025/California_County_Boundary.shp")

## Alternatively, you can read in the shapefile using the 'st_read()' function
## EG
 cali_sf <- st_read("/Users/elizabethgreenan/Desktop/California_County_Boundaries2025/California_County_Boundary_view_layer_for_public_use_-7341318997869969624/California_County_Boundary.shp") 
```
...
**Hint 1**: Are all of them counties? Try running `print(calif_sf$COUNTY_NAM)`. What do you see?

EG
Source: https://r-spatial.github.io/sf/reference/valid.html

```{r}
# Print each of the COUNTY_NAM vcalues in cali_sf dataframe
print(cali_sf$COUNTY_NAM)

# Check to see if there are duplicate counties
table(cali_sf$COUNTY_NAM)

```


> According to the above code and the source cited at the beginning of this question, st_is_valid checks whether geometries are valid. In this case, TRUE printed 69 times because there are 69 geometries in this dataset. When I ran print(cali_sf$COUNTY_NAM), the county names were output. However, Colusa county is duplicated, showing up twice in the output. This shows that although there are 59 counties (58 unique), there are more geometry features than counties.
...
Source: https://epirhandbook.com/en/new_pages/gis.html

> The  bounding box (bbox) that come with the shapefile are coordinate boundaries. They provide the minimum and maximum x and y values where the entire map can be seen. In this case, the bounding box is: xmin: -13849230 ymin: 3833650 xmax: -12704990 ymax: 5162402.

> If you wanted to zoom into a geographic feature (ex. Los Angeles county instead of the entire state of CA), you would make the bounding box smaller. If you wanted to zoom out on a geographic feature (ex. look at the whole US instead of CA), you would make the bounding box larger.
...
> The projection is WGS 84/ Pseudo-Mercator. The projection tells us how the data is being displayed on the map. When the projection is changed, so is the coordinate system. Changing the projection can cause the map to look distorted and features such as distance, area, and angles will change.
...
ggplot() +
  ...
  coord_sf(expand = FALSE) +   ## <- keeps map tight to its bounding box
  ## Give it a title and footnote/caption 
  labs(title = "3 Northernmost California Counties",
       caption = "Source: California State Geoportal") 
...
```{r}
### Uncommented the following code, which adds labels
...
# Create a cities data frame with coordinates and add in San Diego
cities <- data.frame(
  city = c("Sacramento", "San Diego", "Los Angeles", "San Francisco", "San Jose"), # Added San Jose to the list of cities
  lon = c(-121.4944, -117.1611, -118.2437, -122.4194, -121.8853), # Added latittude and longitude (added negative sign to San Jose longitude)
  lat = c(38.5816, 32.7157, 34.0522, 37.7749, 37.3387),
  cityType = c("capital", "major", "major", "major", "major") # Added major as a city type for San Jose
)
...
ggplot(data = cali_sf) +
  ...
  ## Define the fill color for the city shapes
  scale_color_manual(values = skittles[1:5], name = "City") + # Updated to 1:5 to account for San Jose
...
# Create morality_sf, which contains the number of total deaths
mortality_sf <- totalDeaths %>% 
  # Rename County to COUNTY_NAM  so it matches the shapefile
  rename(COUNTY_NAM = County) %>% # The syntax for this is (new_name = old_name)
  # Group the data by county, year, and month
  group_by(COUNTY_NAM, Year, Month) %>% 
  # Calculate the summary of the total deaths, which is the sum of total deaths and na.rm = TRUE tells us to ignore the NA values
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  # Join the cali_sf dataframe by COUNTY_NAM. The relationship parameter is many-to-many, indicating that there can be duplicate counties
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  # convert the dataframe into a sf
  st_as_sf()
```
...
Source: https://r-spatial.github.io/sf/reference/st_join.html

> st_join() performs a spatial join on two dataframes based on their geometries. We could not have executed the join with st_join() because the totalDeaths data does not have a spatial aspect to it. cali_sf has a geometry field, indciating that it has spatial data. Since only one of the dataframes has geometry, a spatial join using st_join() would not have worked.
...
```{r}
# check number of rows in morality_sf
nrow(mortality_sf)
```

> There are 2,596 features in the mortality_sf dataframe.
...
```{r}
# Check number of rows in cali_sf
nrow(cali_sf)
```

> The cali_sf dataframe only has 59 features. There are so many more features in the mortality_sf dataframe because in Question 6, we grouped morality_sf by county, year, and month. Therefore, any given county will show up multiple times in morality_df, whereas the cali_sf dataframe just lists the 59 counties in California.
...
```{r}
# Calculate the number of counties in mortality_sf
n_distinct(mortality_sf$COUNTY_NAM)
```

```{r}
# Check the unique number of counties in mortality_sf
unique(mortality_sf$COUNTY_NAM)
```


> There are 58 counties in COUNTY_NAM. In Question 2, there were 59 counties. These numbers are different because in Question 2, we looked at all of the counties in California, regardless of whether or not they were unique, in the cali_sf dataframe. As noted earlier, Colusa county has a duplicate. Now, in Question 7B, we are looking at the unique counties in mortality_sf. In Quesion 6, morality_sf was grouped by county, year, and month and joined to cali_sf on the county name. In this question we are now looking at the unique county names in the mortality_sf dataframe.
...
> This plot is showing the raw numbers of annual deaths by county per year in California. Los Angeles has the highest number of deaths and is the county with the largest population in California. Therefore, when looking at total number of deaths, the county with the largest population will always have the largest number of deaths. There are similar darker colors (5,000-49,999 deaths) in the following counties: Fresno, San Bernardino, Riverside, Orange, and San Diego counties. These trends are consistent in all four graphs. From a quick Google search, all of these counties have anywhere between 1-3 million people. Los Angeles county has over 9 million residents. This issue shows why it is important to use rates or proportions instead of raw numbers.
...
EG
EG Key: 5c232fce69ab41e424284fac644002c1aa02f6b1

```{r}
## Set your API key (put it between the quotation marks):
#key <- "b45feb2ccbeea85f5dbbbd783544f75092df5a90" #DS key
key <- "5c232fce69ab41e424284fac644002c1aa02f6b1" # EG key
#key <- "dfbb9b8a4e5fe8bcf8b1c5615c86e644771a0976" # MH key
```
...
Source: https://api.census.gov/data/2020/dec/dpmp/variables.html

> DP1_0048C is a number, while DP1_0048P is a percent. They both represent general demographic characteristics of the male population age 65 and older. However, since one is a raw number and one is a percentage, they are represented differently. DP1_0048C provides the raw counts while DP1_0048P provides the proportions. The numeric values have C at the end of them, representing count, while the percentages have P at the end of them, representing percent. Looking back at Question 8, this is an example of when you would want to use percentages instead of raw counts.
...
> The census dataframe contains 58 rows and 12 columns. By using names(census), we can see what the 12 columns in the dataframe are. By looking at the table, we can see that each county has its own row and it provides information such as the number of males and females, total population, percentage of people age 65 and over, number and percentage of males and females age 65 and over, and the state and county information. There are also no duplicate county names. The County field gives the actual county name, while the STATE AND COUNTY fields provide the FIPs information. STATE is 06, representing the California FIPs code and each county has a different County FIPs code, which is three digits.
...
> Since the current URL is customized to the Census data, to use a different API, the first step would be to change the URL. In order to get data from an API, the URL must be set up. Since the beginning of the URL has the base API endpoint, it would need to be changed from api.census.gov/data to the new source. Next, the specific dataset would have to be adjusted. This could include the year and what specific dataset you are looking for. An example would be using a weather API (https://openweathermap.org/api) and looking at historical weather data. The variales to retrieve and any county or state data would also be adjusted depending on the use case. Lastly, you would need to add your personal API key. For any new API, a new key will need to be requested.

> Once the data has been obtained through the API, a function can be used or written to pull the data. The steps followed by this function include building the URL for the API request, requesting the data and converting it to a dataframe, cleaning the data, and returning a cleaned dataset. Although the process generally stays the same regardless of what API is used, the URL will change depending on either the dataset or the use case. Even if the same API is being used, there may be instances where you need different data or different years of data. In this case, the base API endpoint would remain the same, but the data request step would change.
...
```{r}
# Check data types in mortality_sf
str(mortality_sf)

# Check data types in census
str(census)

# Check fields in mortality_sf
names(mortality_sf)

# Check fields in census
names(census)
```

> The common field between mortality_sf and census is the county field. In mortality_sf, it is called COUNTY_NAM and in census, it is called County. This is the field we will want to join on when creating mortality_sf2. Hint 1 suggests using the FIPS data to join, but there is not FIPS data in the mortality_sf dataframe.

Source: https://www.rdocumentation.org/packages/joyn/versions/0.3.0/topics/left_join

```{r}
# Create mortality_sf2 by joining the census data to the mortality_sf data on the County field. A left join is used to ensure all data from the mortality_sf dataset remains when joining with census data to make mortality_sf2
...
EG NOTE: I downloaded Professor Geist's file and compared it to mine. Her file had 2,552 observations and 22 variables. The variables she has that we don't are COUNTY_ABB, COUNTY_COD, and ISLAND. I opted to comment her code out for Question 12 and use ours. However, if this causes issues for later questions, feel free to comment my part out for Question 12.

MH NOTE: Testing for issue with section 3.2.1. There are some counties appearing with 0 neighbors which is preventing weights from generating. It works when running with the google drive data. I believe this is likely due to islands associated with some of the counties. **NOTE TO PROF: ALL WRITTEN RESPONSES BEFORE SECTION 3.2.1 WERE BASED ON OUR DATA AND THEN PIVOTED BEGINNING IN 3.2.1 DUE TO ISSUE WITH WEIGHTS**

EG commented r, eval = FALSE
```{r, eval = TRUE}
## Comment me out or change EVAL=FALSE to run your own versions!
#load("C:/Users/Desicious Win/Desktop/mortality_sf2.Rdata") DS data location
#MH path:
# load("C:/Users/User/Downloads/mortality_sf2.Rdata")
load("census.Rdata")
```
...
> Since this plot is now showing the California annual percent mortality by county, as opposed to raw counts, it tells a different story. Previously, the plot was showing death counts, so the most populated counties had the highest number of deaths. In this case, we are now seeing counties like Los Angeles have lower percentages of mortality, as opposed to counties such as Inyo, or more notably, in 2018 especially, Shasta county. We are being told that deaths are going up due to COVID-19, but this is not a graph looking at the the number of deaths, but the percentages. Additionally, since COVID-19 started in 2020, it would be easier to see trends in death rates that included more COVID-19 data if we looked at 2020-2023. Doing this would allow us to focus on years when COVID-19 was rampant and we were seeing many deaths.
...
> The reason that our 2020 data ends in August is likely due to monthly data being filed in a lagged manner so that all prior month's data is included. Since this assignment is taking place in September 2020, we would only expect to see data through August. Depending on when in September we're receiving this data, it's also possible that August's mortality percentages are under represented, but we cannot know this for certain. If we had all 12 months of data for each year, including 2020, the maps in 2.7 would be much more accurate. But, given that we only have data through August, one option might be showing mortality % over the years just from January through August. This would provide a comparison that, although excludes half of 'flu season' in the COVID era, could still present a strong argument for 'higher mortality during COVID'.
...
Helpful Ineractive county map: https://www.counties.org/counties/#info 

> Compared to the 2017 - 2019 maps, most of the southern counties see an increase in monthly mortality percentage including, Kern, Ventura, Los Angeles, San Bernardino, Orange, Riverside and Imperial. San Diego county is higher than 2017 and 2018, but does not seem to be higher than 2019. In the North, Plumas, Mendocino, Sonoma, and San Joaquin counties s seem to have higher mortality rates compared to previous years. The only county with a decreasing death rate that is visible by looking at the map is Mariposa county in central CA.
...
> Comparing the results from the Percent Mortality figure with the predicted 2020 September-December mortality was a strong indicator that this model is not robust. Investigating the results of `plot(olsModel)` supports this: the Q-Q plot is not a straight line suggesting the residuals are not normally distributed. There is also evidence of heteroskedasticity in the Scale-Location plot suggesting the residuals do not have constant variance. Although not evident by these plots, based on our knowledge of disease spread and epidemiololgy, we are likely missing spatial autocorrelation, as well as temporal autocorrelation. 
...
> There are 2552 observations in the `mortality_sf2` dataset, suggesting for the 58 counties, there are 44 observations for each county. Calculating the neighbors on just the **distinct counties** in the mortality shapefile will make calculating the weights less computationally expensive by removing the redundant geometry since each of those 44 observations per county have repeated polygons. Calculating the weights can now just be done once on each of the 58 counties and then can be joined into our `mortality_sf2` dataset.
...
MH
> Note to team/self: unlike Demo2, the model above used `lm()` instead of `lagsarlm()`. What was done here, different from Demo2 was that the spatially lagged predictor was calculated and added as a predictor instead of using the spatial lag of the dependent variable, so the anova is not performing Likelihood Ratio Test as it was in Demo2, it is testing if there is a signficiant difference between `sarModel` and `olsModel`, or more specifically: does the spatial lag feature improve the model. The null hypothesis would be that spaital lag does not improve the model; however the results of an F-stat 97.6 and P-value < 0.001 (much less) lead us to reject that null hypothesis and determine that a county's mortality rate is significantly associated with that of it's neighbors.
...
> A 1 unit increase in the average mortality of neighboring counties is associated with a 0.38 unit increase in a county's mortality, with all other variables constant. Year is not significant, which makes sense given 2020 is behaving very differently. Month is signficiant, but the coefficient is very small at -0.0003. Almost every single county is signficiant suggesting strong structural difference across counties which could be a result of demographics, health care access etc. The $R^2$ is 0.727, meaning the model explains about 73% of the variation. 
...
> While the model does show statistically signficiant results and suggest that spatial lag was an appropriate addition, the results of `plot(sarModel)` are very similar to that of `plot(olsModel)` where the Q-Q plot is not a straight line suggesting the residuals are not normally distributed. There is also evidence of heteroskedasticity in the Scale-Location plot suggesting the residuals do not have constant variance. So there is still work to be done to make a trustworthy model.
...
## Your code here.
#Train
sum(mortality_sf2$Year !=2020)/nrow(mortality_sf2)
#Test
sum(mortality_sf2$Year ==2020)/nrow(mortality_sf2)

```

> Based on the time-aware split, 81.8% of the data is in the training set, and 18.2% is in the test data. Numerically, this is very close to the canonical 80-20 test split! But in practice, the time-aware split will preent data leakage, it also is more close to a real world prediction.

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> Leaving out all of the 2020 data prevents the model from learning 2020-specific patterns. Given the higher mortality rate seen in our line graph, and the known differences happening in 2020 it is reasonable to think  the model may underestimate the 2020 mortality rate.
...
> Fitting a model with OOB error means that we are randomly selecting samples from the original dataset such that some data points are not included in the model, but then the out of bag (OOB) samples are used to test the tree. The errors from these OOB samples is the out-of-bag errors which acts as a form of validation without having to further split the data to have a validation set and can help paint a picture on whether or not the Random Forest model is overfitting. 

Source: https://dataanalyticsbook.info/chapter-5.-learning-i-cross-validation-oob.html
...
> `mtry` is a hyperparameter used to help tune the random forest model. It tells the model to look through `mtry` number of randomly selected predictors in the model to find the best split from those predictors. The default is ncol(x)/3. In our case, this would be mtry 1, which would lead to a large amount of randomness. Instead we will test mtry 1, 2 and 3 to see which best tunes our model.

Source: https://parsnip.tidymodels.org/reference/details_rand_forest_randomForest.html
...
> The best `mtry` is mtry 1. The RMSE is the lowest and the $R^2$ is the highest. A negative $R^2$ is possible since random forest calculates $R^2$ as 1 - (SSE/SST), so this would mean that the predictions are worse than using the average spread of data. We should not directly compare this model to the olsModel or the sarModel, because this model was trained on different data split. The olsModel did not have a test/training split, whereas the random forest oob model did. Furthermore, the target variable was not isolated out of the preprocessing step for centralizing and scaling the data, which was not performed on the olsModel which may also make a direct comparison of RMSEs misleading.
...
> There are no more negative $R^2$ values suggesting cross validation is more stable. `mtry` 1 is still the best based on the lowest MAE and RMSE, and highest $R^2$. The RMSE for the cross validation is almost the same as the OOB RMSE with `mtry` 1 (CV is 0.9833 as opposed to OOB 0.9835) suggesting stable model and tuning. The low $R^2$ however suggests that the model is only able to account for 4.7% of the variance in the data.
...
>The results of the RMSE and MAE do seem to be quite high compared to the training model. So there must be something deeper within the data or preprocessing of the data sets due to potential data leakage, a common error amongst these types of complex computing / problem solving.

>The results of the RMSE and MAE do seem to have an overly optimistic training model which may lead to be overfitting problems and a bit of bias as well.
...
> The scatterplot seems to be under predicting compared to the actual mortality rates. Since the predicted mortality results are more so on the lower end of mortality then what the real results were. The training model compared to the test model did show underprection errors that caused the graph shown on the right histograph to have a bias future outcome to be lower than what the actual results showed us.
...
```{r}
url <- 'https://data.chhs.ca.gov/api/3/action/datastore_search?resource_id=f0979d00-9add-40f3-a51f-1ce1ba039faa&filters={"Cause":"ALL","Geography_Type":"Residence","Strata":"Total%20Population"}&limit=100000'
...
```{r}
caliDeaths <- caliDeaths %>% filter(Year == 2020)

#Shows the unique amount of "Months" * number of unique amount of 'County'(should be 58) which does equal 696
caliDeaths %>% summarise(n_distinct(Month)) #12 months
caliDeaths %>% summarise(n_distinct(County)) #58 counties in cali
caliDeaths %>% summarise(n_distinct(Month, County)) #counties * months

#A double check to make sure the sum we got above does result in the amount of observations we know we should have; 696 
counties = 58
months = 12
counties * months

#Changing the data type of Year and Month to integers from numbers being stored as text
#Changing the data type of Count to numeric to do mathematics equations for future analysis.
...
```{r, echo = TRUE, warning=FALSE, message=FALSE}
# Create morality_sf, which contains the number of total deaths
actual_mortality_sf <- caliDeaths %>% 
  # Rename County to COUNTY_NAM  so it matches the shapefile
  rename(COUNTY_NAM = County) %>% # The syntax for this is (new_name = old_name)
  # Group the data by county, year, and month
  group_by(COUNTY_NAM, Year, Month, Count) %>% 
  # Calculate the summary of the total deaths, which is the sum of total deaths and na.rm = TRUE tells us to ignore the NA values
...
head(actual_mortality_sf, 14)
unique(actual_mortality_sf$COUNTY_NAM)
```
...
EG - I cannot get this to work and commented it out so it will knit. I was getting an errorm which I think is due to the fact that the mortalityTest dataframe has normalized Year and Month values, so I can't filter on 9:12 and 2020.

```{r}

## Filter the data so it shows Sept-Dec 2020 data
#mortality_sept_dec <- mortalityTest %>%
 # mutate(Year = as.numeric(Year),
  #       Month = as.numeric(Month)) %>%
  #filter(Year == 2020, Month %in% 9:12)
## Make a scatterplot of the actual vs. predicted values
#p <- ggplot(mortality_sept_dec, aes(x = monthlyPercMortality, y #= rfPredictions)) +
  ## Scatteplot with geom_point
 # geom_point(alpha = 0.5, color = skittles[4]) +
  ## Add a line to help see the general trend
  #geom_abline(color = skittles[3], linetype = "dashed", size = #1.5) +
 # theme_minimal() +
  #labs(
   # title = "Actual vs. Predicted California Monthly Percent #Mortality",
 #   subtitle = "Random Forest Predictions on Sept - Dec 2020",
  #  x = "Actual Mortality",
   # y = "Predicted Mortality"
#  )
## Add marginal histograms with the ggExtra package
...
Source: https://capone.mtsu.edu/eaeff/462/wpp/temporalAutocorrelation.html

> Temporal autocorrelation is when the residuals are correlated with their values. According to the above source, although autocorrelation does not cause bias in parameter estimates, it does create bias in the standard errors. This causes misleading and untrustworthy t-statistics when running an OLS model.

> Time series lags capture what the Month variable alone may not fully capture because it shows lags and patterns instead of an actual month. Similarly, while County by itself provides some information, it cannot fully provide spatial relationships. For example, if there are two counties next to each other in southern CA that behave similarly, this does not mean they represent the entire state of CA.
...
> This suggests that the time series lagged variables are helping the model, as the MAE dropped significantly. Since there was an improvement in the MAE, I think the model is performing pretty well.
...
> Something else that can be done to improve the model is hyperparameter tuning. There are various parameters that can be adjusted, incuding the number of trees (n_estimators) and tree depth (max_depth). There are additional parameters that can also be adjusted. This testing can help increase the complexity of the model, which can improve its performance.
...
> Since we weren't able to get answers for all of the parts, I'm unable to speak on the correlation test on the Sept to December 2020 predictions relative to the true Sept to December 2020 monthly percent mortality.

> However, from our other results, it is clear from the 10-fold CV Random Forest model, that month is the most important predictor, but also that sptial lag is also highly important. Interestingly, Year does not show up at all as being an important variable. This indicates that analyses with year may not be as beneficial. These results further prove that utilizing lags in analysis can be incredibly benefical. Therefore, it is recommended to use them for analyses going forward.
...
> No, it would not be fair to model the proportion of deaths due to injury or accident without accounting for an effect of sex. As seen in the Proprotion of All Deaths Due to Accident and Injury plot, females have a lower poprortion of deaths due to accident or injury than men do in all four years of the data. Therefore, the analysis may be innacurate if we are not looking at how deaths are impacted by sex.
...
> This is an interesting trend and shows that prior to 2020, the proprotion of deaths due to accident or injury were stable in both sexes, but especailly in females. Regardless of year, it is clear that men have a much higher porportion of deaths due to accident or injury, which was also seen in the previous plot. In 2020, for both men and women, there was a steep drop that continues through the remaining data in August 2020. This was likely due to the fact that most states still had stay-at-home orders in place, so people were less likely to be leaving their homes, traveling, etc. This could reduce accidental injuries and deaths from car accidents or just general risky activities.
...
> Based on the above plot, I think there is some evidence that the percent of deaths due to accidents and injuries changed as a result of the stay-at-home orders. An interesting find is that in 2018, Imperial county had the highest annual percentage of deaths due to accident or injury. Then, in 2019, there was NA data, and in 2020, the percentage decreased. In 2020, there were also a significant number of counties with NA data, but these are not specific to just 2020. Additional filtering on these maps would be helpful. Some ideas would be to use a filter on month, so you could see if there was a decrease in March-May of 2020. Additionlaly, filtering on sex per county would also provide useful information.
...
> NA values do not necessarily mean missing data, but they can reflect a very small or statistically insignificant number of deaths in a county. This could also be a result of underreporting or even no reporting.
...
ggplot(injuryDeathSummary) +
  ...
  ## Updated to look at biological sex and year
  facet_grid(biologicalSex ~ Year)
```

This plot shows the deaths borken down by male and female. Interestingly, there is only one county that had the highest number of deaths due to accident or injury, which was represented by males in 2018. The female plots look comprable across the years, whereas there appears to be some noticable change/decrease in deaths for males in 2020.
...
EG
Source: https://corrr.tidymodels.org/reference/correlate.html
Source: https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/cor

```{r}
# After research, the use = "complete" will drop NA values
cor(injDeaths$femaleMaleRatio, injDeaths$deathsInjuriesProportion, use = "complete")
...
> The code resulted in a correlation coefficient of 0.279791. This is a relatively smaller correlation, indicating that we likely do not need to make a correction on our maps.
...
> Yes, the plot does agree with the correlation coefficient. A correlation coefficient of 0.27 like we calculated represents a weak positive relationship. Therefore, this plot is showing the line going in a positive direction. Additionally, the points are quite scattered, showing a weaker relationship than if they were clustered closer to the line.
...
**Data Science Question:**
 ...
 > How did the COVID-19 pandemic and shutdown impact the porportion of deaths due to accidents and injuries across counties in California?
 
 **Hypothesis**
 > We hypothesize that the proportion of deaths due to accidents and injuries across counties in CA decreased due to the COVID-19 shutdown. This is due to stay-at-home orders and decreased travel. We also hyptohesize that counties with higher populations, such as Los Angeles or Orange county, could see the greatest impacts.
 
 > We would use a Random Forest model that would utilize both spaital and temporal lags. Spatial lags would allow us to look at county data whereas temporal lags would allow us to look at trends over time.
...
> The selected cause of death for this question is: **ALZ**: Alzheimer’s Disease 
...
alzDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ## Here is where you filter for a specific cause of death relative to sex
  filter(leadingCauseDeath == "ALZ") %>% 
  ## Now full join with the first 4 columns of the totalDeaths table to get 
  ## total deaths also
  full_join(totalDeaths[ , 1:4], by = c("County", "Year", "Month")) %>%
  ## Rename the sexDeaths column to something more appropriate
  rename(alzDeaths = sexDeaths) %>% 
  ## Calculate the proportion of all deaths that are from Alzheimer’s 
  ## RELATIVE TO ALL DEATHS (not population!!)
  mutate(deathsALZProportion = alzDeaths/totalDeaths,
         ## The ratio of females to males in the population
         femaleMaleRatio = femalesN/malesN,   
         ## The ratio of 65+ females to males in the population
         femaleMaleRatio65 = females65plus/males65plus,  
         ...
         Date = as.Date(paste(Year, Month, "1", sep = "-"), 
                        format = "%Y-%m-%d")) %>%    
         ## Illegal div by 0 turned to NaN; turn back to 0
  mutate(deathsALZProportion = ifelse(deathsALZProportion == "NaN", 
                                           0, deathsALZProportion))  
```
...
```{r}
##### Calculate the proportion of all deaths due to Alzheimer’s
alzDeathsSummary <- alzDeaths %>% 
  ## Group by stay-at-home order, year, county, AND sex
  group_by(stayAtHome, Year, County, biologicalSex) %>% 
  ## Change the name to make the join easier
  rename(COUNTY_NAM = County) %>% 
  ## Mean proportion of deaths for each month-year-county-sex combo
  summarise(deathsAlzPercent = mean(deathsALZProportion, na.rm = TRUE)*100) %>% 
  ## Join to get the geometries for mapping
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Coerce to make sure it is seen as a shapefile
  st_as_sf() %>% 
...
#### Find the five number summary
summ <- fivenum(injuryDeathSummary$deathsInjuriesPercent)
paste0("The five-number summary of % of deaths that are Alzheimer’s-related is: ")
paste(c("Min", "Lower Quartile", "Median", "Upper Quartile", "Max"), 
      round(summ, 3))
## Notice that, with the minimum, 1st quartile, AND median all equal to 0, we
## really only have three groups

##### Now add on the cuts alzDeathsSummary labels for mapping:
alzDeathsSummary <- alzDeathsSummary %>% 
  mutate(deathLabels = cut(deathsAlzPercent, 
                          breaks = c(0, 3.343, 18.489, Inf),
                          labels = c("0-3.342", "3.343-18.489", "18.489+"),
                          ordered_result = TRUE, right = F)) 
```
...
```{r, echo=FALSE}
## Initialize the plot
ggplot(alzDeathsSummary) +
  ## Chloropeth: draw the polygons of the counties and color by total deaths
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +

  ## Apply manual fill scale for total deaths
  scale_fill_manual(
    name = "% of Deaths Due to Alzheimer’s Disease",
    values = c4a(palette="brewer.or_rd", n=3),
    na.value = "gray70",
    drop = FALSE
  ) +
  ...
  # Add title and caption
  labs(
    title = "California Annual Percent Deaths due to Alzheimer’s Disease, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) + 
...
```
MH
> It is not clear whether the NA data is due to a lack of individuals in counties with Alzheimer's Disease or a lack of reporting in the chloropeth graps. Regardless, the Alzheimer's related deaths appear to be consistent for most counties over the years, with 2020 showing the most `NA` counties. A couple of the counties with higher mortality in 2017-2019 had lower mortality in 2020.

```{r, echo=FALSE}
alzDeaths %>% 
  group_by(Month, biologicalSex, Year) %>% 
  summarise(deathsALZProportion = mean(deathsALZProportion, na.rm = TRUE)) %>% 
ggplot(aes(x = Month, 
           y = deathsALZProportion, 
           color = as.factor(Year), 
           linetype = as.factor(biologicalSex))) +
  geom_line(alpha = 0.7, size = 1.1) +
  scale_x_continuous(n.breaks = 10) +
  scale_color_manual(values = skittles[c(5, 9:11)]) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(title = "Proportion all deaths due to Alzheimer’s Disease, by sex",
       y = "Proportion of All Deaths Due to Alzheimer’s Disease", 
       subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
       color = "Year",
       linetype = "Sex of Decedent") +
  theme_minimal()
...
EG
> It appears that females are at a much higher risk of death due to Alzheimer's Disease than males, which is the opposite of what we saw in the ealier injury graphs.In 2020 it appears that deaths for males plateaued around March and then dropped after May, while the females peaked in April 2020, but also dropped significantly in May. 2017 through 2019 showed similar trends for mobidity in male Alzheimer's patients. 

```{r, echo = FALSE}
## Now plot the numbers of deaths per year due to accident and injury by sex
alzDeaths %>% 
ggplot(aes(y = deathsALZProportion, x = biologicalSex, fill = as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "Biological Sex of Decedent", 
       y = "Proportion of All Deaths Due to Alzheimer's Disease", 
       fill = "Year", 
       title = "Proportion of All Deaths Due to Alzheimer's Disease",
       subtitle = "California Counties, Jan 2017 - Aug 2020") + 
  scale_fill_manual(values = skittles[c(5, 9:11)]) + 
  theme_classic()
```
...
DS
> According to all the years, females have a much higher morbidity for Alzheimer's Disease than males. Amongst the 2020 year, there is one outlier for females. There is one outlier for males in 2019. This could lead to skew of the morbidity rates for the corresponding years. Additionally, the mean mobidity for males appears to be 0 which could be a data error and merits additional investigation. The averages across all years appear to be the same, but should be noted that 2020's data only accounts for deaths through August and the September through December data is expected to alter the 2020 plots. 


 **Data Science Question:**
 
 > Were there less reported deaths due to Alzheimer's Disease in 2020 than in previous years? Is one sex impacted more than another?
 
 **Hypothesis**
 > Morbidity due to Alzheimer's Disease decreases in 2020 due to Alzheimer's patients often living in group homes that were at higher risk of COVID exposure/outbreaks changing the leading cause of death in these patients.