...
```{r}
## I chose the unencoded data set - during the demo project, it seemed like encoding was possibly grouping some hospitals together that shouldn't be.
...
```{r}
## Observed readmission rates - it has the least amount of overlap compared to the others
```
...
```{r}
## Considering up to this point, it seems like the observed variable shows the most distinction, so I'm choosing that metric.
## I find it interesting that readmission rates increase as the average medicaid spending increases. Medicare is primarily for older adults. This implies that older patients have a higher chance of readmission.
```
...
longScore %>%  
  ggplot(aes(y = observed_readmission_rate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of observed pneumonia-related hospital \nreadmissions to each performance metric",
        ...
        color = "observed_readmission_rate Compared to National Average") +
...
```{r}
## Using the same variable as above for comparison purposes.
## In almost all metrics, as hospital rating / scores increase, the chance of readmission decreases. The "low-performing compared to natl average" hospitals have a more significant decrease as scores increase than "high-performing compared to the natl average" hospitals. This is most likely due to the standards of "high performing" hospitals having a higher baseline score than "low performing" hospitals, so the change isn't as drastic.
...
longHCAHPS %>%  
  ggplot(aes(y = observed_readmission_rate,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of observed pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
        ...
        color = "Observed Pneumonia-related Readmission Compared to National Average") +
  scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
  scale_color_manual(values = c("gold", "cadetblue", "maroon"),
                    labels = c("Better", "Same", "Worse")) + 
  guides(fill = "none", color = guide_legend(override.aes = list(fill=NA))) +    ## To override the filled SE boxes from geom_smooth() in the legend
...
Your answer here.
```{r}
## The metric I find the most interesting is that as the staff responsiveness for "high-performing" hospitals gets higher ratings, there is a slight increase in readmissions. 
## I'd be curious to know why that is - whether it is a data issue (small sample / high-impact outliers) or something deeper. Could good staff attentiveness result in patients wanting to return?? 
...
```{r}
## Nope, that makes sense! Losing the 600 data points is pretty significant. My intuition is to always go for the crude value, but I can see how adjusted values may be more "accurate" by capturing extenuating circumstances.
```
...
Your answer here.
```{r}
## In terms of bias, I think some potential issues could be:
##  - Was any confirmation bias introduced? Did any data collection agents know of the hospital's performance compared with the national average that could have impacted results?
## In terms of limitations:
##  - Misdiagnosis (was the original pneumonia diagnosis missed, so when the patient readmitted was it considered an initial admission?)
##  - Death / recoveries occurring outside the hospital (should a readmission have taken place, but didn't?)
```
...
```{r}
## A low variance indicates that the values of a variable are very similar (across observations). 
```
...
```{r}
## Unique identifiers (such as FacilityId) don't have any statistical value. Since it would have an extremely high variance (each value is unique), `nearZeroVar()` would not remove it from the list of variables. If kept in during analysis, it will lead to unnecessary computation or invalid relationships being incorporated into the model.
...
Your answer here.
```{r}
## The curse of dimensionality refers to the result models end up with when their training data has many variables (compared to the number of observations). In this case, we have 4,800 observations with 31 variables which is most likely not suffering from this effect.
...
## spot check
# dat2Analyze %>%
#   filter(is.na(`MRSA Bacteremia`))

```

```{r}
## All columns have some level of missingness
```
...
## No observation has a complete set of variables
...
```{r}
## Why => It's a character vector rather than numeric
```
...
Your answer here.
```{r}
## For 1788 observations, both `Postoperative respiratory failure rate` and `PredictedReadmissionRate` were missing. Yes - that's more than 1/4 of the total observations.
```
...
```{r}
## MRSA statistics, respiratory failure rate and the others listed in the plot. Many are significantly missing, with values >50%.
## This is an issue as this makes the data sparse and potentially flaw any models we make with it.
...
Your answer here.
```{r}
(length(dat2Analyze) / nrow(dat2Analyze))
## Yes - it definitely increases dimensionality as we've dropped about half the observations.
...
Your answer here.
```{r}
## Data is partioned into training & test sets - it is important that this splitting takes place prior to building a model as we cannot test the effectiveness of a model on data it has already seen (trained on). Data leakage is the term used to describe how that training data impacts the end model, symptomatically in things like overfitting.
...
## its close! it matches the 82/18 split as calculated in calcSplitRatio()
```
...
## MSE measures the error continuous numerical variables, while PCF measures the error of categorical variables. The closer to zero, the better.
## `Payment for pneumonia patients` (continuous) and `Emergency department volume` (categorical) had the highest OOB errors 
```
...
summary(train$`Medicare spending per patient`)
summary(imputedTrain$`Medicare spending per patient`)


table(train$`ComparedToNational_Composite patient safety`)
table(imputedTrain$`ComparedToNational_Composite patient safety`)

## Nope - it looks like the imputation was effective in keeping the data statistically similar
...
Your answer here. 
```{r}
## Having classes that exist in training but not in test (or vice versa) will be a problem as the model would be trained with/out them. The same frequency map needs to be applied to both the train and test sets, accounting for the frequency of the categories between both.
...
Your answer here.
```{r}
## A histogram / q-q plot is insufficient for determining whether a distribution is approximately normal because it is subjective (looking at the graph rather than calculating it). Additionally, depending on the bin size, variations in true distribution could be hidden.
```
...
Your answer here.
```{r}
## The extremely low p-value of the SW test indicates the null hyp can be rejected, e.g there is significant evidence showing the data did not come from a normal distribution.
```
...
Your answer here.
```{r}
## The test data must also be transformed. The reason for this is similar to the answer of question 18 - the scale of these new variables must be equivalent between train & test so the model can correlate between them. If we use a different lambda value, we have changed the scale and any insights found in the training data are now meaningless.
...
Your answer here.
```{r}
## There is a high correlation between columns that measure comparison to national average and the raw values, which makes sense. If a raw value is high, its probably going to be higher than the national average.
```
...
Your answer here.
```{r}
## `Compared to National - hospital return days
...
Your answer here.
```{r}
## Yes - the overall hospital rating & recommend hospital variables have a very high VIF. 
```
...
Your answer here.
```{r}
## This may be expected since something like 'overall' is a value computed from other variables. In this case, we should drop variables like 'overall.hospital.rating' and 'recommend.hospital' as those appear to be aggregations
```
...
Your answer here.
```{r}
## PC1: 22% of the variance
## PC18: 1% of the variance
## PC1 is definitely not enough to build a model off of. The common standard is getting a number of components together to explain 80-90% of the variance. In this case, its only 22%. This would put it in the low category.
...
Your answer here. 
```{r}
## A scree is a pile of rocks and other fallen material at the base of a cliff like on a mountain. It has a similar shape to this plot.
```
...
fviz_pca_biplot(pca, 
             ...
             select.var = list(contrib = 4),   ## Try changing the number
...
Your answer here.
```{r}
## The variables that have the most explanation of variance are those that are patient-subjective responses, rather than statistical measures (like specific illness rates)
...
Your answer here:
```{r}
## To me, it looks like k=5 has the most explanatory power, but it is hard to tell from these graphs which elipses cover the most points
```
...
Your answer here:
```{r}
## The within sum of squares method (WSS) measures how tightly packed data points are around a centroid. To me - it still looks like 5 is a good k-value.
```
...
Your answer here:
```{r}
## It doesn't line up with my original take, but that's expected since those methods were heuristic-based.
```
...
"Communication about Medicines", "These hospitals provide information about medicines for patients, suggesting this cluster may inform patients better about their illness, including how to treat themselves",
"Quietness", "Hospitals with quieter settings may increase relaxation for a patient, decreasing recovery times",
"Doctor Communication", "Hospitals with lower doctor communication scores may cause patient confusion, hindering recovery",
"Discharge Information", "Hospitals with lower discharge information scores may lack patient guidance on how to recover effectively",
"Cleanliness", "Hospitals with lower cleanliness scores may increase patient stress, hindering recovery",
"Post-Operative Respiratory Failure Rate", "Hospitals encountering higher rates of respiratory failure may indicate poor methods of stabilizing patients"
)
...
Your answer here:
```{r}
## Almost met, the Cook's distance plot indicates some high-influence outliers
```
...
Your answer here.
```{r}
## Yes - the residuals vs fitted plot shows linearity
```
...
Your answer here:
```{r}
## The r^2 value will always increase as predictors are added to the model, which can make comparisons between models difficult. Adjusted r^2 penalizes the value when predictors are included that don't impact the target variable.
...
Your answer here.
```{r}
## The search grid is being constructed using a two-range matrix which is being used to provide different alpha/lambda values to the train method so it can do multiple variations at once
## Cross validation is being performed to prevent overfitting - using multiple samples ensures the model can handle new data correctly
## lambda = 0 means that no shrinkage is performed
## alpha = 0 means that full shrinkage / regularization is performed (same as OLS regression)
```
...
Your answer here.
```{r}
## The values of alpha & lambda are both zero, indicating that the optimal regularization is equivalent to an OLS regression.
...
Your answer here.
```{r}
## This aligns with the OLS regression - both indicate that "Hospital return days for pneumonia patients" is the most important feature.
...
```{r}
## The difference between regression and PCA is that they are two different tools - PCA attempts to reduce dimensionality while regression attempts to predict values. The sign of the loadings indicate positive or negative correlation.
...
Your answer here.
```{r}
## Making predictions using the training set result in NA RMSE, 0 RSquared and NA MAE values. This is most likely due to making predictions using the exact data the model was trained on.
...
Your answer here.
```{r}
## The data appears to be linear based off of the scatterplots, but the r^2 value remains low (< 0.4). This typically means that the relationship between the target variable and the predictors is weak. A few recommendations:

## - Regenerate a model utilizing the "important" factors we found earlier. The current model includes all the factors, regardless of determined importance.
## - Investigate regenerating the model using transformations
## - Its possible the amount of data provided isn't enough to determine a proper prediction model - more should be collected.
