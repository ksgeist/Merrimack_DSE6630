...
p2 <- ggplot(pneumoniaAnalyze2, aes(x = rating_cat, y = observed_readmission_rate)) +
  ...
  theme_minimal() + theme(plot.title = element_text(size = 9), axis.title.x = element_text(size = 9), axis.title.y = element_text(size = 9))
...
p3 <- ggplot(pneumoniaAnalyze2, aes(x = death_rate_cat, y = observed_readmission_rate, fill = death_rate_cat)) +
  geom_violin() +
  labs( # Define title and axis labels
    title = "Death Rate v. Observed Readmission Rate",
...
# Optionally, add extra left margin to p2 and p3
p2 <- p2 + theme(plot.margin = unit(c(5.5, 5.5, 5.5, 20), "pt"))
p3 <- p3 + theme(plot.margin = unit(c(5.5, 5.5, 5.5, 20), "pt"))

# Stack p2 and p3 vertically
right_side <- plot_grid(p2, p3, ncol = 1, align = "v", rel_heights = c(1, 1))

# Combine p1 and right_side horizontally
final_plot <- plot_grid(
  p1, right_side,
  ncol = 2,
  rel_widths = c(1, 1) # adjust as needed for your preferred proportions
)
