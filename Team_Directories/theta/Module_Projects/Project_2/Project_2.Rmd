...
> cnty19_1.shp: This is the main shapefile which includes the actual geometries
> of our data.

> cnty19_1.shx: This is the shape index file, which includes an index that allows
> for easy linking between the .shp and .dbf files. 

> cnty19_1.dbf: This is the attribute file, which includes attributes for each of
> the features of the data. 

> cnty19_1.prj: This is the projection file, which includes information about the
> map projection for the objects.

> cnty19_1.cpg: This is the character encoding file, which includes encodings
> for special characters within the dbf file. 

> cnty19_1.shp.xml: This is the metadata file, which includes metadata about
> the shapefile. This metadata includes information such as date created and
> field descriptions. 
...
**Hint 1**: Are all of them counties? If not, what are they? Try running `print(cali_sf$COUNTY_NAM)`. What do you see?
...
> TRUE printed 69 times, since st_is_valid(cali_sf) is checking to see if each
> geometry is valid. Since we have 69 features, we are seeing that all of the 
> geometries are valid. 

> Each feature represents a single geometry. One might expect each geometry to
> be a county, however it is seen that some counties are listed twice.
> It appears that some counties require multiple geometries.
> Looking at the case of islands, it appears that a separate geometry is
> stored for an island, yet is given the same county name as its mainland
> counterpart. 
...
> The bounding box is defined by the line:

> Bounding box:  xmin: -13849230 ymin: 3833650 xmax: -12704980 ymax: 5162403

> The bounding box defines a rectangle area with a minimum and maximum 
> longitude (x) and latitude (y). 
> This controls the extent of our data's spatial boundaries.

> To zoom in on a feature, you would adjust the minimum and maximum values
> to be closer together, focusing in on the desired area.
> Likewise, you would zoom out by increasing the distance of the minimum and
> maximum values. 
...
> The projection is defined by the line:

> Projected CRS: WGS 84 / Pseudo-Mercator

> From this, it is seen that we are using the WGS 84 projection.
> The projection defines how the 3D geometry of the Earth is mapped onto
> a 2D plane. 

> Changing the projection will result in the geographical geometries to be
> distorted in different ways, appearing differently between projections. 
...
# Create mortality_sf from totalDeaths.
# totalDeaths contains monthly death counts by county, year, and month.
# This makes it appropriate data for mapping mortality at the county level.
mortality_sf <- totalDeaths %>% 
  # Rename County as COUNTY_NAM so that we can join the data with cali_sf
  rename(COUNTY_NAM = County) %>%
  # Group the data by COUNTY_NAM, Year, and Month so that death counts can be aggregated.
  group_by(COUNTY_NAM, Year, Month) %>% 
  # Count the deaths by group, removing NA values
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  # Perform a full join with cali_sf using COUNTY_NAM as the key. 
  # The many-to-many relationship allows us to retain all rows in the case that a county has multiple rows.
  # This is true with our geometries, as discussed earlier. 
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  # Convert the joined data into a spatial object
  st_as_sf() %>% 
  # Filter out islands
  filter(is.na(ISLAND))
```
...
> st_join() performs a join on two spatial datasets based on the spatial
> relations between geometries.

> We could not use st_join here, because we were joining a non-spatial
> dataset to a spatial dataset.
> Therefore, we needed to use full_join().
...
> The full_join() brought in the features from totalDeaths.
...
n_distinct(cali_sf$COUNTY_NAM) # Counting counties in cali_sf to prove a point
```

> There are 58 counties in COUNTY_NAM.

> As discussed earlier, question 2 listed certain counties multiple times
> when a county had multiple geometries.

> If we use n_distinct on the county data in cali_sf, we can see that it also
> equals 58 distinct counties. 
...
> This map shows the counties with the highest deaths to be counties with large cities.
> This is because our map is only looking at raw counts of deaths, which will
> naturally be higher in places with more people.
> Therefore, this map does not help much in understanding mortality rates
> across counties. 
...
key <- "4d6b8285971b0ee1a0bd207088b6c263abcd2475"
```
...
> DP1_0048C contains the count of males 65 years and older.
> The data is stored as an integer value.

> DP1_0048P contains the percent of males 65 years and older.
> The data is stored as a float value.
...
head(census) # Display first 6 rows

names(census) # List variable names

n_distinct(census$STATE) # Check that only California is included in the data
```


> This dataframe contains census data for the state of California (State 06).
> The data contains the population counts and percentages of each county,
> and also the counts and percentages divided by male and female.
> In addition, it contains the counts and percentages per county of those
> who are 65 and older, as well as such data divided by male and female. 
...
> There are several key components of this function that would need to be
> adapted to work with a different API.

> First, the URL needs to be adjusted for the API being used.
> Fields such as get=, for=, in=, and key= were defined in a way specific for
> our census API and our purpose with it.
> Thus, they would need to be corrected for the specific API.

> Second, the state, vars, and header sections are also set up for our specific use
> case, and would need to be altered based on the API being used.

> Third, our data cleaning targets columns and values that are specific to our
> census data.
> This would need to be removed or altered to fit the data of the API of choice. 

> Despite these specific changes, several general structural choices would
> remain the same, regardless of the API.

> One necessary component is constructing the URL.
> While some of the parameters need to change, we will still be constructing
> a URL in the same general format to locate the data we are trying to pull.

> Likewise, presuming the data is still JSON, we will be able to still use the 
> fromJSON() to pull the data from the URL. 

> Lastly, cleaning and returning the data will still be required, but will
> need to be adjusted as discussed above. 
...
# Alter census COUNTY as a 3-digit code
census2 <- census %>%
  mutate(COUNTY = sprintf("%03d", as.numeric(COUNTY)))

# Full Join
mortality_sf2 <- mortality_sf %>%
  full_join(census, by = c("COUNTY_FIP" = "COUNTY"))

# Return to shape file
mortality_sf2 <- st_as_sf(mortality_sf2)
```
...
> Counter-intuitively, it appears as if mortality rates see a peak in 2018 and
> then decrease as the years progress. 
> This pattern goes against the idea that deaths go up due to COVID-19.
> However, this is likely due to the fact that our 2020 data does not include
...
> The COVID-19 pandemic made census taking challenging, and thus left us with
> incomplete data.
> One potential idea for correction is to sample artificial values for the remaining
> 4 months.
> This could be done using averages or by imputation with a prediction model.
> However, these values may not be accurate and could distort the true picture.

> Another idea could be to cut off each year at the 8 month mark, but
> this would also limit the scope of our analysis and potentially paint an 
> incorrect picture. 

> Thirdly, we could use a measure such as the mean or median of death rates
> for each year.

> If we had a full 12 months of data for each year, then we'd likely be able
> to use our prior method for creating maps. 
...
> Now, there does appear to be an increase in mortality from the prior years. 
> Many of the southern counties has gone up from the 0.572-0.689 range to the
> 0.690-0.861 range. 

> However, for many of the counties in the middle and northern parts of the
> state, there is not a great amount of change from 2019. 
...
> The model does not appear to be robust. 

> The Residuals vs Fitted plot is the only plot that might suggest linearity
> and homoscedasticity. 
> Most of the points follow a roughly horizontal line.
> There does appear to be a trail of certain points in a descending slope,
> however the rest of the points follow the expected distribution. 

> The Q-Q plot shows significant tails on both ends of the diagonal line.
> These deviations suggest that the data does not follow a normal distribution.

> The Scale-Location plot demonstrates a bizarre curve in the red line.
> It is hard to argue that the line shows a mostly horizontal pattern,
> and therefore suggests that there is heteroscedasticity. 

> Many points fall outside of the guidelines of the Residuals vs Leverage plot.
> This suggests that the data contains a great deal of points that 
> disproportionately affect the model.
> That would suggest that the model is unstable. 

> From all of these plots, it can be assumed that the model violates OLS assumptions.
...
> mortality_sf2 has 2552 observations, which is a multiple of 58 (58 x 44).
> Calculating weights on redundant data would lead to a higher computational cost.
> Furthermore, neighbors could become confused, as it will not recognize that
> the data is redundant and would consider each observation as a different neighbor.
...
> Overall, the OLS model appears to be a better fit for the data.
> The model has a lower RSS than the sarModel (0.52334 vs 1.86374),
> which tells us that it leaves less unexplained error.
> In addition, the F statistic has a very low p-value,
> which tells us that it is a better fit than the sarModel. 
...
> The most significant predictors are COUNTY_NAMAlpine,
> COUNTY_NAMMono, and COUNTY_NAMSierra which all have coefficients of
> -0.05112698. 
> This suggests that these counties have significantly lower mortality rates.

> The adjusted R-squared value is 0.7165. 
> This suggests that the model is fairly strong.
> However, this number is likely inflated due to the large
> number of predictors within the model. 

#### Question 17E [0.25 points]
Can you trust the results of the model? Why or why not?

```{r}
# Checking conditions (as we did before)
...
> As discussed earlier, the data does not appear to meet the required
> expectations for an OLS model. 

> Refering back to question 16, we determined that the results of the model
> are likely not trustworthy. 

> However, this model seemed to be a better fit than the sarModel,
> which suggests that neither model does a great job at predicting our target. 
...
# Needed in Question 19
test_keys <- mortalityTest %>% select(COUNTY_NAM, Year, Month)
```
...
```{r}
# Determining percentage of data in each set

# Total number of rows in the original data
total_rows <- nrow(st_drop_geometry(mortality_sf2))

# Number of rows in the training set
train_rows <- nrow(mortalityTrain)

# Number of rows in the testing set
test_rows <- nrow(mortalityTest)

# Percentages
train_percent <- (train_rows / total_rows) * 100
test_percent <- (test_rows / total_rows) * 100

# Print results
train_percent # Aprox. 82%
test_percent # Aprox. 18%
```

> There is aproximately 82% of the data in the train set and
> aproximately 18% of the data in the test set.
> These numbers are very close to the canonical 80-20 split.

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> Since 2020 was a year with an unusual mortality pattern,
> the model will not learn those patterns during training.
> This might mean that the model will struggle to generalize onto the
> new data from 2020 during testing. 
...
> Fitting a model with out-of-bag error is the practice of using 
> bootstrap aggregation, or bagging, to train ensemble models, such as random
> forests, and then estimate performance using data samples not included in the 
> bootstrap sample.

> This method of model validation provides a few benefits.
> One benefit is that a separate validation set is not required, conserving
> data.
> Another benefit is that the error estimates are unbiased, as they are 
> tested with data that was entirely unseen during training. 
...
> mtry, in the context of a random forest, is the number of variables chosen
> at random as candidates at each split.
> This is defined, because a random forest will only consider a certain number
> of predictors at each split.

> The default mtry is one-third of the total number of predictors. 
> We are overriding this here to try and tune the model.
> By testing several values of mtry, we are attempting to find the
> value that results in the best performing model.
> In addition, our limited number of features would result in
> only one predictor being used if we did not manually configure these
> choices. 

> Source: https://afit-r.github.io/random_forests
...
> The value of mtry that gave the best results was 1.
> It had a RMSE of 0.98, which was lower than the other two mtry values.
> In addition, it had an R-squared value of 0.03. 
> The other R-squared values were negative, suggesting significantly worse
> performance. 

> We should be able to compare the RMSE of this model to the one fitted in 
> question 16. 
> Our preprocessing conducted with preProcess() is applying the preprocessing
> to the predictors, not the target. 
> Therefore, it should be reasonable to compare scores, as the targets
> should be the same. 
...
> Unlike the OOB model, we see no negative R-squared values.
> Once, again, mtry 1 produced the highest R-squared and lowest RMSE.
> The RMSE of the CV model is 0.9858, which is almost identical to the OOB
> RMSE of 0.9834.
...
> The RMSE of 1.0092 tells us that on average, the predicted values are 
> about 1 standard deviation away from the actual values of the target. 

> The MAE of 0.7446 tells us that on average, the absolute difference between
> the predicted and actual values of the target is 0.7437 standard deviations.

> Our training error (0.9858) is not that greatly different from our testing
> error (1.0092).
> In addition, our R-squared value is only about 0.02.
> These observations together suggest that the model is underfitting. 
...
> The scatterplot shows us that as actual mortality rates get higher,
> so do our predicted values.
> The histogram for the predicted mortality shows a mean further to the right
> of the histogram than we see in our actual mortality histogram.
> This suggests that our model is overpredicting mortality. 
> Even if the model is underperforming, this would suggest that the 2020
> data demonstrates a higher mortality trend. 
...
```{r}
# Credit to June for the fix as posted on Canvas
...
caliDeaths_clean <- caliDeaths_clean %>%
  mutate(
...
county_pop <- caliDeaths_clean %>%
  group_by(County) %>%
  summarise(Population = first(Count))
```

```{r}
caliDeaths_monthly <- caliDeaths_clean %>%
  left_join(county_pop, by = "County") %>%
  mutate(
    monthlyPercMortality_actual = (Count / Population) * 100
  ) %>%
  select(County, Year, Month, monthlyPercMortality_actual)
```
...
```{r}
# Predictions for Months 9-12 (Random Forest)

latest_predictors <- mortality_sf2 %>%
  filter(Year == 2020, Month == 8) %>%
  select(COUNTY_NAM, everything())

COUNTY_NAM <- rep(latest_predictors$COUNTY_NAM, each = 4)
Month <- rep(9:12, times = nrow(latest_predictors))
Year <- rep(2020, length(COUNTY_NAM))

newdata_rf <- latest_predictors[rep(1:nrow(latest_predictors), each = 4), ]
newdata_rf$Month <- Month
newdata_rf$Year <- Year

newdata_rf$monthlyPercMortality_pred <- predict(rfCV, newdata = newdata_rf)
```


```{r}
caliDeaths_monthly2 <- caliDeaths_monthly %>%
  rename(COUNTY_NAM = County)

newdata_rf2 <- newdata_rf %>% mutate(Year = as.character(Year))
caliDeaths_monthly <- caliDeaths_monthly %>% mutate(Year = as.character(Year))

comparison_rf <- newdata_rf2 %>%
  rename(County = COUNTY_NAM) %>%
  left_join(caliDeaths_monthly, by = c("County", "Month")) %>%
  drop_na(monthlyPercMortality_pred, monthlyPercMortality_actual)
```
...
p <- ggplot(comparison_rf, aes(x = monthlyPercMortality_actual, y = monthlyPercMortality_pred)) +
  geom_point(alpha = 0.5, color = skittles[4]) +
  geom_abline(color = skittles[3], linetype = "dashed", size = 1.5) +  # Reference line
  geom_smooth(method = "lm", se = TRUE, color = "blue") +              # Trend line
  theme_minimal() +
  labs(
    title = "Actual vs. Predicted California Monthly Percent Mortality",
...
# Correlation test
cor_test_rf <- cor.test(comparison_rf$monthlyPercMortality_actual, comparison_rf$monthlyPercMortality_pred)
print(cor_test_rf)
```

__For full credit__ include a brief interpretation of your findings.

> The plot shows a slow linear increase.
> It suggests that as actual mortality rises, so does the predicted, however
> not to a great extent.

> Once again, the histograms display a higher predicted mean than actual.
> This suggests that the model overpredicts mortality.

> The correlation test produces a p-value of 0.4618, which is greater than
> 0.05.
> We fail to reject the null hypothesis.
> There does not appear to be a statistically significant linear relationship
...
> Temporal autocorrelation is the concept of when a variable value at a certain
> time interval is correlated with itself at other time intervals.
> In our case, that would mean that deaths in one month are not independent of
> deaths in other months. 

> Seasonal momentum refers to cyclical temporal autocorrelation.
> In our case, this coulld refer to a pattern where mortality rates
> rose or declined in certain months every year. 

> Month is a categorical variable that can compare average differences 
> between months, but cannot take into account these types of dynamic
> relationships. 
> This is similar to the categorical nature of County, which
> cannot capture the impact of neighboring counties like a spatially
> lagged variable could. 

> Sources:
> https://www.sciencedirect.com/science/article/pii/S0304405X11002613
> https://www.numberanalytics.com/blog/mastering-autocorrelation-techniques
...
> Such results would suggest a strong improvement over our previous model. 
> A lower MAE suggests a better performing model.
> In addition, the testing and training MAEs being close in value suggests that
> the model is neither overfitted nor underfitted.
> From this, it would be concluded that the addition of time series lagged
> variables are helping the model. 
...
> In depth model tuning could help improve the model by adjusting complexity.
> Random forests are a type of enseble model that creates several decision trees.
> We can perform hyperparameter tuning to adjust the number of trees that the
> model produces. 
> Increasing this number might lead to more stability and accuracy within 
> the model, as it is comparing more decision trees when making a prediction.
> In addition, we could tune for the tree depth to ensure that each tree
> created is more complex.
...
> So far, we have created several random forest models to try and predict mortality
> rates during 2020 at a county level.
> Our data was limited due to our 2020 data not having the months of September
> through December. 
> Our initial attempt using out-of-bag error led to low performing results.
> With three versions tested, each using a different number of predictors at
> each split, the best performing random forest model received an R-squared
> value of about 3%.
> Our next attempt utilized 10-fold cross-validation as opposed to out-of-bag
> error.
> Of the highest performing CV model, RMSE was similar to that of the OOB model,
> and had a lower R-squared value of about 2%. 

> Overall, our models appear to be limited in their ability to capture
> the variation in mortality rates.
> One key identified weakness is the lack of temporally lagged variables.
> The data appears to have a strong temporal autocorrelation component, which
> is not captured by the Month predictor.

> Moving forward, it is recommended that time-series predictors are added
> to the model to capture the identified temporal autocorrelation.
> In addition, a greater extent of hyperparameter tuning is recommended
> to experiment with model complexity to try to identify the optimal model. 
...
> No, as the plot shows a clear difference between each sex.
> Across all years (except 2020), the entire IQR for Male is higher than
> that of Female.
> In every case, the median proportion of deaths is higher for Male than Female.
> The plot clearly demonstrates that males have a higher proportion of deaths 
> due to injury or accident than females. 
...
> Once again, we see that the trend in every year is higher for Male than it
> is for Female.
> In 2020, we see a sharp decrease in deaths when we hit May.
> One idea to explain this might be the lockdown due to COVID.
> If more people were staying home, then there might be less deaths due to
> accident or injury.
> However, the lockdown started in March, which we see as an increase in this
> proportion until we hit May.
> Another potential reason is that around this point, COVID related deaths
> began to increase, overtaking accident or injury related deaths. 
...
> We see in southern California a decrease in deaths due to accident and injury
> in 2020 compared to prior years.
> However, outside of the south part of the state, it is harder to tell
> if there is change.
> Many of the counties report as NA, which hinders our ability to compare
> across the years. 
> Furthermore, these maps lack any information about two of the potentially
> important factors we previously discusssed. 

> One recommendation for change would be to focus in more on 2020 by creating
> two maps. One which looks at the months before the lockdown and one that
> looks at the months after.
> Such a comparison could show us if the lockdown had an affect. 

> Similarly, you could create different maps that showed the death percentages
> in each county split by sex. 
...
> The gray counties could be explained for a couple of reasons.
> One reason is that those counties are not reporting deaths due to accident
> or injury.
> However, a more likely reason is that if the counties have very small
> numbers of such deaths, then the results are suppressed on our map. 
...
ggplot(injuryDeathSummary) +
  # Draw county polygons colored by percent deaths due to accident/injury
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +
  
  # Manual fill scale for percent deaths
  scale_fill_manual(
    name = "% of Deaths Due to Accident or Injury",
    values = c4a(palette = "brewer.or_rd", n = 3),
    na.value = "gray70",
  ...
  labs(
    title = "California Annual Percent Deaths due to Accident or Injury, by County and Sex",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) +
  
  # Facet by year and biological sex
  facet_wrap(~ Year + biologicalSex)
```
...
> The correlation test produced a coefficient about 0.28.
> This values is not really high, suggesting a weak positive relationship.
> The p-value is greatly lower than 0.05, which suggests that the relationship
> is statistically significant. 
> However, taking the low coefficient into account, it is questionable if
> the correlation, while significant, is great enough to warrant adjusting
> the maps. 
...
> The plot demonstrates a large skew in the data.
> This would suggest that adjusting the map would be beneficial.
> The direction of the slope is postivie, which agrees with what was 
> determined from the correlation coefficient. 
> Furthermore, the skew of the plot provides extra support for the idea
> that the maps should be adjusted, which the correlation coefficient did 
> not fully convey. 
...
> How does the adult female-to-male ratio affect the proportion of deaths due to accident or injury in California counties over time, and how do temporal and spatial dependencies influence these relationships?

> Counties with higher adult female-to-male ratios will have significantly different proportions of deaths due to accident or injury.

> Once again, I would utilize a random forest.
> Ideally, the model will incorporate spatial and temporal lags.
> As we have witnessed, there appears to be a spatial component to our target.
> Southern California demonstrates higher death proportions across all years.
> In addition, as we have previously discussed, death rates seem to fluxuate 
> between seasons.
> While this may be mostly attributed to illness, one might expect working
> patterns or weather conditions to affect deaths due to accident or injury. 

> Looking at senior population might also be of interest.
> It may be the case that those in their working years are more prone
> to death from accident or injury.
> On the other hand, it may be the case that older individuals
> struggle to recover from injury more than younger individuals.
...
```{r}
clrdDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  filter(leadingCauseDeath == "CLD") %>% 
  full_join(totalDeaths[ , 1:4], by = c("County", "Year", "Month")) %>%
  rename(clrdDeaths = sexDeaths) %>% 
  mutate(
    deathsCLRDProportion = clrdDeaths / totalDeaths,
    femaleMaleRatio = femalesN / malesN,
    femaleMaleRatio65 = females65plus / males65plus,
    retireeRatio = over65 / totalN,
    Date = as.Date(paste(Year, Month, "1", sep = "-"), format = "%Y-%m-%d")
  ) %>%
  mutate(deathsCLRDProportion = ifelse(is.nan(deathsCLRDProportion), 0, deathsCLRDProportion))
```


```{r}
# Annual Choropleth Maps

clrdSummary <- clrdDeaths %>%
  group_by(Year, County) %>%
  summarise(deathsCLRDPercent = mean(deathsCLRDProportion, na.rm = TRUE) * 100) %>%
  rename(COUNTY_NAM = County) %>%
  full_join(cali_sf, by = "COUNTY_NAM") %>%
  st_as_sf() %>%
  filter(is.na(ISLAND)) %>% 
  ungroup()
...
ggplot(clrdSummary) +
  geom_sf(aes(fill = cut(deathsCLRDPercent, breaks = c(0, 1, 3, Inf), labels = c("<1%", "1-3%", ">3%"))), 
          color = "gray20", size = 0.2) +
  scale_fill_manual(
    name = "% of Deaths Due to CLRD",
    values = c4a(palette = "brewer.or_rd", n = 3),
    na.value = "gray70",
    drop = FALSE
  ) +
  ...
  labs(
    title = "California Annual Percent Deaths due to CLRD, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) +
  facet_wrap(~ Year, ncol = 4)
...
> We see overall a large amount of NA counties. 
> Southern California shows a great amount of red counties.
> In addition, we do see a bit of a drop off in 2020. 
> This might potentially be due to the rise in COVID cases overtaking CLRD.
> Overall, it is hard to get a good sense of the differences between counties
> and years due to the vast number of NA counties. 
...
clrdDeaths %>% 
  group_by(Month, biologicalSex, Year) %>% 
  summarise(deathsCLRDProportion = mean(deathsCLRDProportion, na.rm = TRUE)) %>% 
  ggplot(aes(x = Month, 
             y = deathsCLRDProportion, 
             color = as.factor(Year), 
             linetype = as.factor(biologicalSex))) +
    geom_line(alpha = 0.7, size = 1.1) +
    scale_x_continuous(n.breaks = 10) +
    ...
    scale_linetype_manual(values = c("dotdash", "solid")) +
    labs(
      title = "Proportion of All Deaths Due to CLRD, by Sex",
      y = "Proportion of All Deaths Due to CLRD", 
      subtitle = "Cumulative for all California counties, Jan 2017 - Aug 2020",
      color = "Year",
      linetype = "Sex of Decedent"
    ) +
...
> While many of the trends overlap,
> it does appear that females have a higher proportion of CLRD related deaths
> than males. 
> There appears to be a sharp increase between April and May for most years,
> and a lesser increase between the fall and winter. 
> The year 2020 shows a great downward trend for males, 
> and a lower proportion than ususal for females.
> However, this might be a consequence of our incomplete data for 2020. 

```{r}
# Correlation Test

cor.test(
  clrdDeaths$femaleMaleRatio,
  clrdDeaths$deathsCLRDProportion,
  use = "complete.obs"
)
```

> The correlation coefficient of 0.426 suggests a moderately strong positive
> linear relationship between femaleMaleRatio and the proportion of CLRD related
> deaths. 

> The p-value is greatly below 0.05,
> suggesting that the correlation is statistically significant. 


> Data Science Question: How does the adult female-to-male ratio affect the 
> proportion of deaths due to Chronic Lower Respiratory Disease (CLRD)
> in California counties over time?

> Hypothesis: Counties with higher adult female-to-male ratios will exhibit higher proportions of deaths due to CLRD than counties with lower adult female-to-male ratios. Furthermore,
> the proportions of deaths due to CLRD will be lower in 2020 than previously
