...
author: "Stephanie Garofalo, Sharon Singh, & Jason Cicero"
date: "28 May 2025"
output:
  html_document:
    toc: true
...
```{r}
#team, i had to direct R as to where my source code was. You may not need this line 
source("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/Team_Directories/kappa/Module_Projects/Project_1/reRunDemoData.R")
## Uncomment if this is your choice:
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/Team_Directories/kappa/Module_Projects/Project_1/FY2024_data_files/dataFull2024.Rdata")
dat2Analyze <- dataFull
...
## Uncomment if this is your choice:
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/Team_Directories/kappa/Module_Projects/Project_1/FY2024_data_files/dataAnalyzeNoEncoding2024.Rdata")
...
> We chose the heart failure dataset because ti is one of the most common and critical conditions for hospitals when monitoring readmissions. It has significant cost implications, along with impact to the life of the patient - therefore reducing readmission rates is important for both hospital and patient success. Moreover, the dataset offers multidimensional hospital-level data tied to a high-impact clinical outcome. The dataset includes many assets, including operational, clinical, financial, and patient experience variables which ultimately allow for comprehensive analysis. Heart failure readmissions are a critical policy-relevant outcome, making the findings valuable for improving hospital care and reducing costs. The dataset’s size, diversity, and inclusion of both adjusted and unadjusted measures make it well-suited for robust predictive or explanatory modeling.
...
sliceData <- dat2Analyze %>%
  ...
  mutate(`National Comparison Return Hospital Days` = factor(`National Comparison Return Hospital Days`, 
                                                             levels = c("Better than average", "Same as average", "Worse than average", "Unknown")))
...
longTargets <- dat2Analyze %>%
  rename(NatlComparisonHospitalDays = `ComparedToNational_Hospital return days for pneumonia patients`,
         ...
         `Expected Readmission Rate` = ExpectedReadmissionRate,
         `Number of Readmissions` = NumberOfReadmissions) %>%
  mutate(NatlComparisonHospitalDays = case_when(
    NatlComparisonHospitalDays == 1  ~ "Better",
    NatlComparisonHospitalDays == -1 ~ "Worse",
    NatlComparisonHospitalDays == 0  ~ "Same",
    TRUE ~ "Unknown"
  )) %>%
  mutate(NatlComparisonHospitalDays = factor(NatlComparisonHospitalDays,
                                             levels = c("Better", "Same", "Worse", "Unknown"))) %>%
  select(contains("readmission"), NatlComparisonHospitalDays) %>%
...
longTargets %>% 
  ...
  labs(title = "Which target to use?",
        ...
        x = "",
        y = "Rate or Score",
        fill = "HF-related hospital days (compared to national average):") +
...
Now that you can compare these groups more easily, what we're looking for is a balance between **clear difference** between hospital ordinality relative to the national average (i.e., "Are better hospitals really better in HF readmission rates?").

Which target(s) appear the most distinct to you?
> The excess readmission ratio stands out as the most distinct as here we can most clearly see a seperate between 'Better', 'Same', and 'Worse' groups. Alongside, the boxplots show non-overlapping ranges and  noticeable shifts, meaning hospitals rated “Better” really tend to have lower ratios, while “Worse” hospitals show higher. Meanwhile, the Predicted Readmission Rate and Expected Readmission Rate are fairly consistent, but their group differences are smaller, with lots of overlap. Lastly, te Number of Readmissions shows a lot of spread and overlap, especially in the “Worse” category, making it noisy.
...
> Based on both the boxplot and density plot comparisons, the Excess Readmission Ratio appears as the most distinct and meaningful target for modeling. Unlike the Expected or Predicted Readmission Rates, which show substantial overlap between “Better,” “Same,” and “Worse” hospitals, the Excess Readmission Ratio shows distinct separation between these categories, ultimately making it easier to distinguish high- and low-performing hospitals.
Additionally, the Number of Readmissions is heavily skewed, reducing its usefulness as a balanced target, whereas the Excess Readmission Ratio is a standardized measure that directly compares observed readmissions to expectations, offering a clear and interpretable indicator of hospital performance relative to national benchmarks. We can conlude that it is the most appropriate target for modeling heart failure-related hospital readmission patterns.



### 2.4 What is the relationship of HF-related hospital readmissions to the hospital performance metrics?
...
ggplot(longScore, aes(x = Value, y = observed_readmission_rate, color = NatlComparisonHospitalDays)) +
  ...
  labs(title = "ExcessReadmissionRatio vs. Hospital Performance Metrics")

```

> After changint the y-axis for each of the four different readmission rates (Excess Readmission Ratio, Expected Readmission Ratio, Number of Readmissions, Predicted Readmission Rate) I concluded that the excess readmission ratio is the one I feel most strongly about. This is because it provides a distinct seperation between "Better", "Same", and "Worse" hospitals, primariyl for the key hospital eprformance metrics. “Better” hospitals generally cluster at lower excess ratios while “Worse” hospitals spread higher. It provides a normalized and interpretable performance measure that best reflects true quality differences.
...
longScore %>%  
  ggplot(aes(y = ExcessReadmissionRatio,  ## <-- CHANGE THIS!
    ...
    labs(title = "Relationship of Observed Readmission Rate and HF-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
        y = "Predicted HF-related Readmissions",
        color = "HF-related Readmission Compared to National Average") +
...
```{r, echo = FALSE}
load("~/Documents/GitHub/Merrimack_DSE6630/Team_Directories/kappa/Module_Projects/Project_1/FY2024_data_files/pneumoniaAnalyzeNoEncoding2024.Rdata")
...
> After comparing all four readmission rate options, I decided to use the Observed Readmission Rate as the target variable. The scatterplots reveal a stronger relationship between observed readmissions and patient satisfaction metrics like doctor and nurse communication, where higher communication scores are associated with lower readmissions. The patterns across categories (Better, Same, Worse) also remain clearer in observed readmissions compared to other targets, helping us identify actionable insights. This choice aligns with the goal of capturing real-world performance and how it relates to patient care.
...
```{r}
longHCAHPS %>%
  ggplot(aes(y = observed_readmission_rate,  ## <-- change here
             x = Value)) +
    geom_point(alpha = 0.25) + 
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)),
                method = "lm", se = T, show.legend = T) + 
    labs(title = "Relationship of Observed Readmission Rate and pneumonia-related hospital \nreadmissions to each patient satisfaction metric",
         x = "Rate or Score",
         y = "Observed Pneumonia-related Readmissions",
         color = "Pneumonia-related Readmission Compared to National Average") +
    scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
    scale_color_manual(values = c("gold", "cadetblue", "maroon"),
                       labels = c("Better", "Same", "Worse")) + 
...
> From the scatterplots, the most compelling relationships are between Observed Readmission Rate and metrics like doctor communication and nurse communication—higher communication ratings tend to align with lower readmission rates, suggesting that strong provider-patient communication may help prevent unnecessary readmissions. Conversely, cleanliness and quietness show minimal impact, highlighting that while these factors are important for overall patient experience, they may not directly influence pneumonia-related readmissions.

One outstanding question I have is: How is the national average for pneumonia-related hospital days calculated, and is it adjusted for hospital size, geographic location, or patient demographics?
...
dat2Analyze <- dat2Analyze %>% 
  select(-any_of(c("ExpectedReadmissionRate", 
                   "ExcessReadmissionRatio",
  ...
  rename(
    ...
    `Composite patient safety` = 
      `Score_CMS Medicare PSI 90: Patient safety and adverse events composite`,
...
# Clean up remaining column name prefixes
...
p[!is.finite(p)] <- 0  # Replace Inf, -Inf, NaN with 0
p[is.na(p)] <- 0       # Replace NA with 0
...
## Set a color palette
prettyPurples <- colorRampPalette(brewer.pal(9, "PuRd"))(75)
...
Citation:
DataScience+ (2015). Imputing Missing Data with R MICE Package. Retrieved from https://datascienceplus.com/imputing-missing-data-with-r-mice-package/
...
> Based on the scree plot, the optimal number of principal components appears to be around 4 to 5 PCs.
...
> The variable 'Composite.patient.safety' contrasts with 'Communication.about.medicines' because it points in nearly the opposite direction (near Quadrant III), whereas Communication.about.medicines points more toward Quadrant I, in the top right.
This suggests that as hospitals score higher on communication about medicines (where there is a positive association), they tend to score lower on composite patient safety, indicating these two variables may capture different or even opposing aspects of hospital performance.
...
fviz_pca_biplot(pca,
    ...
    col.var = "black",
    habillage = returnHospitalComparison,
    select.var = list(contrib = 5),   ## <- updated to show top 5 contributors
    repel = TRUE)
...
> The within sum of squares is a measure of how tightly the data points within each cluster are grouped around their cluster’s centroid. It sums up the squared distances between each point and its assigned cluster center across all the clusters. In this case, it is measuring the compactness of clusters and the elbow method gives us clarity to which point adding more clusters only yields minimial improvements. 
Looking at the WSS elbow plot, the optimal number of clusters is k=2, since that’s where we see the first major elbow or bend — after this point, adding more clusters leads to only minor improvements in WSS.
This does not agree with my earlier choice but this one is more accurate and precise. 
...
dict %>%
  kable(
    format = "html",
    caption = "Table 9. Hospital segmentation analysis: two types of broad groupings identified."
  ) %>%
  pack_rows("1: Highest Performing Hospitals", 1, 4) %>%
  pack_rows("2: Underperforming / At-Risk Hospitals", 5, 8) %>%
  kable_styling(bootstrap_options = c("striped", full_width = F))
...
We are using repeated k-fold cross-validation because it averages eperformance over multiple random splits. This way we able to evaluate the model more robustly because we test it on multiple subsets of data, thereby reducing overfitting and giving us a better understanding on how the model will perform on unseen data. 
Lamba=0 means there is no shrinkage penality and alpha=0 means there is pure ridge resgression (only an L2 penalty). 
...
In PCA, importance refers to how much a variable contributes to explaining the variance in the principal components. In regression, importance to how strongly a predictor explains or predicts the outcome. 
The signs of PCA loadings is technically aribtrary - it depends on the direction of the axis. In regression, the sign of the coefficent matters because it tells us the direction of the relationship between the predictor and the outcome (positive or negative). 
...
This tells us that neither method has meaningfully captured more predective power - either because the underlying relationships in the data are too weak/noisy/non-linear. 
I think we should avoid relying solely on these linear models for making predictions about pneumonia-related hospital readmissions because the predictive power is relatively low (R^2 around 0.35-0.40). This could lead to unreliable predictions in practice. 
We should explore non-linear models such as random forests/gradient boosting/support vecto machines. These can capture complex interactions, along with non-linear effects that linear models cannot. These models could uncover non-linear patterns.
We could also consider adding additional data sources, along with more predective feartures. Important clinical, demographic, or socioeconomic variables were not included so adding them could meaningfully boost predective performance. We could review whether important interactions are currently being missed. 
Overall, the reccomendation is to use a more flexible machine learning approach (Random Forest) and enhance the feature set to achieve practical and reliable models for decision making. While OLS and Elastic Net gives a baseline they would be insufficent for robust predictions. 
...
1. Switch to classification-based random forest using `ComparedToNational_Hospital return days for heart failure patients`
