...
```{r}
cali_sf <- read_sf("/Users/stephgarofalo/Desktop/California_County_Boundaries/cnty19_1.shp")
...
key <- "f1111fae4aac8c242aa84cf496bb6e57c99fe7dc"
```
...
```{r}
library(sf)
cali_sf <- st_read("/Users/stephgarofalo/Desktop/California_County_Boundaries/cnty19_1.shp")
...
# Re-import if needed and clean
caliDeaths <- read_csv("/Users/stephgarofalo/Desktop/California_County_Boundaries/caliDeaths.csv", col_types = cols(Count = col_character()))


# Filter for Alzheimer's disease
alz_df <- caliDeaths %>%
  filter(Cause == "ALZ", Geography_Type == "Occurrence", Strata == "Total Population") %>%
  mutate(Count = as.numeric(Count)) %>%  # <-- force conversion here
  group_by(Year, County) %>%
  summarise(alz_deaths = sum(Count, na.rm = TRUE), .groups = "drop")

# Ensure Count is numeric — force conversion with warning suppression
caliDeaths$Count <- suppressWarnings(as.numeric(caliDeaths$Count))


# Get total deaths for same group to calculate proportions
total_deaths <- caliDeaths %>%
  filter(Cause == "ALL", Geography_Type == "Occurrence", Strata == "Total Population") %>%
  group_by(Year, County) %>%
  summarise(total_deaths = sum(Count, na.rm = TRUE), .groups = "drop")

# Combine and compute Alzheimer's death % per county-year
alz_prop <- left_join(alz_df, total_deaths, by = c("Year", "County")) %>%
  mutate(percent_alz = 100 * alz_deaths / total_deaths,
         COUNTY_NAM = County)

# Join spatial data
alz_map_data <- left_join(cali_sf, alz_prop, by = "COUNTY_NAM") %>%
  st_as_sf()

# Bin proportions into three categories
alz_map_data <- alz_map_data %>%
  mutate(death_category = cut(percent_alz,
                              breaks = c(0, 5, 10, Inf),
...
# Plot
ggplot(alz_map_data) +
  geom_sf(aes(fill = death_category), color = "gray40", size = 0.2) +
  scale_fill_brewer(palette = "OrRd", na.value = "gray90", name = "% of Deaths from Alzheimer's") +
  coord_sf() +
  facet_wrap(~Year, ncol = 4) +
  theme_minimal() +
  theme(
  ...
  labs(
    title = "Proportion of Total Deaths Attributed to Alzheimer's Disease in California",
    subtitle = "By County and Year",
    caption = "Source: California DPH Mortality Data"
  )
...
```{r}
# Filter and summarize Alzheimer’s and total deaths by year and sex
alz_yearly <- caliDeaths %>%
  filter(Geography_Type == "Occurrence", Strata == "Gender", Cause == "ALZ") %>%
  group_by(Year, biologicalSex = Strata_Name) %>%
  summarise(alz_deaths = sum(as.numeric(Count), na.rm = TRUE), .groups = "drop")

total_yearly <- caliDeaths %>%
  filter(Geography_Type == "Occurrence", Strata == "Gender", Cause == "ALL") %>%
  group_by(Year, biologicalSex = Strata_Name) %>%
  summarise(total_deaths = sum(as.numeric(Count), na.rm = TRUE), .groups = "drop")

# Join and calculate proportions
alz_year_prop <- left_join(alz_yearly, total_yearly,
                           by = c("Year", "biologicalSex")) %>%
  mutate(alz_death_prop = alz_deaths / total_deaths)

ggplot(alz_year_prop, aes(x = as.numeric(Year),
                          y = alz_death_prop,
                          linetype = biologicalSex)) +
  geom_line(alpha = 0.8, size = 1.2, color = "steelblue") +
  geom_point(size = 2, color = "steelblue") +
  scale_x_continuous(breaks = unique(alz_year_prop$Year)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "Proportion of Total Deaths Attributed to Alzheimer's Disease",
    subtitle = "Annual trend by biological sex across California counties",
    x = "Year",
    y = "% of All Deaths from Alzheimer's",
    linetype = "Sex of Decedent"
  ) +
  theme_minimal()
...
```{r}
# Alzheimer’s Death Proportion by Race/Ethnicity

# Alzheimer's deaths by Race-Ethnicity
alz_race <- caliDeaths %>%
  filter(Geography_Type == "Occurrence", Strata == "Race-Ethnicity", Cause == "ALZ") %>%
  group_by(Year, County, RaceEth = Strata_Name) %>%
  summarise(alz_deaths = sum(as.numeric(Count), na.rm = TRUE), .groups = "drop")

# All deaths by Race-Ethnicity
total_race <- caliDeaths %>%
  filter(Geography_Type == "Occurrence", Strata == "Race-Ethnicity", Cause == "ALL") %>%
  group_by(Year, County, RaceEth = Strata_Name) %>%
  summarise(total_deaths = sum(as.numeric(Count), na.rm = TRUE), .groups = "drop")

# Join and compute proportion
race_prop <- left_join(alz_race, total_race, by = c("Year", "County", "RaceEth")) %>%
  mutate(prop_alz = alz_deaths / total_deaths) %>%
  filter(!is.na(prop_alz) & prop_alz < 1)  # filter unreasonable values

ggplot(race_prop, aes(x = RaceEth, y = prop_alz)) +
  geom_boxplot(fill = "mediumpurple", color = "black") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title = "Proportion of Alzheimer's Deaths by Race/Ethnicity",
    subtitle = "California counties across all years",
    x = "Race/Ethnicity",
    y = "% of Total Deaths from Alzheimer's"
  ) +
  theme_minimal() +
...
> The visualizations provide a comprehensive view of Alzheimer’s disease mortality patterns across California. The annual choropleth maps reveal a consistent presence of Alzheimer’s-related deaths across counties, with most counties reporting that 5–10% of total deaths are due to Alzheimer’s. Spatial variation appears modest, suggesting widespread impact rather than strong geographic clustering. The line graph shows that Alzheimer’s accounted for a consistently higher proportion of total deaths among females compared to males from 2014 to 2023. This gender gap has persisted across years, with female percentages hovering around 8% and male percentages around 3–4%, indicating a potential need for gender-specific healthcare resources or further study into underlying biological and social factors. Lastly, the boxplot stratified by race/ethnicity illustrates clear disparities: White individuals experience the highest median percentage of Alzheimer’s-related deaths, while other groups like Black, Hispanic, and Asian populations show lower and more variable rates. These disparities highlight potential inequities in diagnosis, reporting, or access to care, suggesting a need for further investigation into systemic and cultural contributors to Alzheimer’s mortality outcomes.

5. **A data science question you would like to test**

> Has the racial/ethnic disparity in Alzheimer’s-related death proportions increased, decreased, or remained stable over time in California counties?
...
> Hypothesis: At least one racial/ethnic group has a significantly different annual proportion of Alzheimer’s-related deaths compared to others.
