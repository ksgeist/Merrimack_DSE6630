...
filepath <- "/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/Team_Directories/kappa/Module_Demos/Demo_1/hospitals_current_data"
```
...
```{r}
# List all CSV files in the folder and assign to 'files'
files <- list.files(filepath, pattern = "\\.csv$")

# Loop through each file and read it in with cleaned column names
for (f in 1:length(files)) {
  dat <- clean_names(
    read_csv(file.path(filepath, files[f]), show_col_types = FALSE),
    case = "upper_camel"
  )
...
# Create a summary table of file names
...
> These dataframes are in long format.
...
> These columns have quite a large amount of NA.

**Hint: **Columns 5 & 7:10 have problems with the `NA` class.

##### **Question 4**: [1 point] SG
...
FY_2025_Hospital_Readmissions_Reduction_Program <- replace_with_na_all(
  FY_2025_Hospital_Readmissions_Reduction_Program,
  condition = ~ .x %in% c("Not Available", "N/A", "", "NA")
)
...
> The other issue that is coercing it to the character type is that it contains "Too Few to Report" as entries as well as numerical values. In the Data Dictionary it also states that the data type for 'NumberOfReadmissions' is character. 


##### **Question 6**: [1 point] SG
...
```{r}
# Replace 'Too Few to Report' in 'NumberOfReadmissions' column with a **5**.
...
> 5 is a conservative estimate that keeps the data usable without falsely implying zero or losing the row as NA. It acknowledges there were readmissions, just not enough to report precisely. I agree with this choice.


##### **Question 8**: [1 point] SG
...
> You’d need a loop, lapply(), or ifelse() with sample(), because gsub() only applies a single fixed replacement, not random values. This introduces variability but may reduce consistency.

##### **Question 9**: [3 points] SS
...
```{r}
#re read data 
FY_2025_Hospital_Readmissions_Reduction_Program <- read_csv(
  file.path(filepath, "FY_2025_Hospital_Readmissions_Reduction_Program_Hospital.csv"),
  show_col_types = FALSE
) %>% clean_names(case = "upper_camel")

#replace with integer
FY_2025_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions <- 
  lapply(FY_2025_Hospital_Readmissions_Reduction_Program$NumberOfReadmissions, function(x) {
    if (grepl("Too Few to Report", x)) {
      sample(1:10, 1)  # Random integer from 1 to 10
    } else {
      x
    }
  }) %>% unlist()
...
FY_2025_Hospital_Readmissions_Reduction_Program <- FY_2025_Hospital_Readmissions_Reduction_Program %>%
  mutate_at(vars(NumberOfDischarges, NumberOfReadmissions, ExcessReadmissionRatio, PredictedReadmissionRate, ExpectedReadmissionRate),
            ~ as.numeric(.))
```
...
```{r}
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/I. Biomedical & Clinical Informatics/Demo_1/readmissionsClean.Rdata")
```

##### **Question 11**: [1 point] SS
...
#spread MeasureName into columns - values are the readmission rates 
wideDF <- FY_2025_Hospital_Readmissions_Reduction_Program %>%
  pivot_wider(
    names_from = MeasureName,                  # Each unique measure (e.g., PN, AMI) becomes a column
    values_from = NumberOfReadmissions         # Fill those columns with readmission counts
  )
...
library(naniar)
library(purrr)
tidyNjoin <- function(datList, hospitalInfo) {
  clean_df <- function(df) {
    df %>%
      janitor::clean_names(case = "upper_camel") %>%
      mutate(across(where(is.character), ~ ifelse(.x %in% c("Not Available", "N/A", "", "NA"), NA, .x))) %>%
      mutate(across(where(is.character), trimws)) %>%
      distinct()
  }
  cleaned_list <- purrr::map(datList, clean_df)
  merged <- reduce(cleaned_list, function(x, y) {
    join_keys <- intersect(names(x), names(y))
    join_keys <- intersect(join_keys, c("FacilityId", "MeasureName"))
  ...
  hospitalInfo <- hospitalInfo %>% distinct(FacilityId, .keep_all = TRUE)
  final <- left_join(merged, hospitalInfo, by = "FacilityId")
  return(final)
}
...
tidyNjoin <- function(datList, filterList, hospitalInfo) {
  ...
  filter_by_measure <- function(df, measures) {
    if ("MeasureName" %in% names(df)) {
      df <- df %>% filter(MeasureName %in% measures)
    }
    return(df)
  }

  cleaned_filtered_list <- map2(datList, filterList, function(df, fl) {
    df %>%
      clean_df() %>%
      filter_by_measure(fl) %>%
      select(-any_of(c("StartDate", "EndDate", "Footnote", "Footnote1", "Footnote2", "MeasureId")))
  })
  
  merged_df <- reduce(cleaned_filtered_list, function(x, y) {
    join_keys <- intersect(names(x), names(y))
    join_cols <- intersect(join_keys, c("FacilityId", "MeasureName"))
    
    if (length(join_cols) == 0) {
      full_join(x, y, by = "FacilityId")
    } else {
      full_join(x, y, by = join_cols)
    }
  })
    final_df <- left_join(merged_df, hospitalInfo, by = "FacilityId")
...
>We worked through this function to prepare us for our team project, where we need to independently clean, filter, and join multiple large hospital datasets based on a selected health condition, like we did with pneumonia in this case. The goal, like with any data set is to build a clean and analysis-ready dataframe that reflects only the most relevant variables for our research question. By writing a reusable function, we can help to streamline this process, adjust inputs easily, and enable preprocessing — all critical skills for real-world data analysis in healthcare informatics.
...
> Using the contains() function, I dropped all columns that included "LowerEstimate", "HigherEstimate", "Denominator", and "HcahpsAnswerPercent" in their names. A total of 7 features were removed from the dataset. These columns were not needed for modeling since they either represented duplicate values or unhelpful categorical breakdowns.

```{r}
# Store original number of features
original_ncol <- ncol(pneumoniaFull)
...
# Store new number of features
new_ncol <- ncol(pneumoniaFull)

# Calculate how many features were dropped
features_dropped <- original_ncol - new_ncol
features_dropped
```
...
# Check dimensions after dropping
dim(pneumoniaFull)
```
...
> I believe should perform ordinal encoding on the ComparedToNational_ columns because the values represent ordered categories (examples are "Worse than the National Benchmark", "No Different than National Benchmark", "Better than the National Benchmark"). These categories have a meaningful rank or order but are not numerical by default, so encoding them as ordinal integers (such as 1 = Worse, 2 = No Different, 3 = Better) preserves that order and allows models to use them effectively.




##### **Question 16**: [1 point] SS
What other column do we also need to perform this kind of encoding on?

> We also need to perform ordinal encoding on the Score columns from the HCAHPS dataset (ex. "Score_Nurse communication", "Score_Doctor communication"). These scores are typically linear mean scores or ordinal ratings, and should be treated as ordered numerical values with reflecting increasing quality. If they are stored as character or factor, they must be encoded appropriately to reflect their ordinal nature.

##### **Question 17**: [3 points] JC
...
```{r}
#Define the ordinal scale mapping
ordinal_map <- c(
  "Worse than the National Benchmark" = 1,
  "No Different than the National Benchmark" = 2,
  "Better than the National Benchmark" = 3
)

#Identify all relevant columns
compared_cols <- names(pneumoniaFull)[grepl("^ComparedToNational_", names(pneumoniaFull))]

#Create a temp df, apply mapping
temp_compared <- pneumoniaFull[, compared_cols] %>%
  lapply(function(col) as.integer(ordinal_map[col])) %>%
  as.data.frame()

#Replace the original columns
pneumoniaFull[, compared_cols] <- temp_compared

#Double-check encoding worked
lapply(pneumoniaFull[, compared_cols], table, useNA = "ifany")

#Score columns comparison
# Step 1: Identify Score columns
score_cols <- names(pneumoniaFull)[grepl("^Score_", names(pneumoniaFull))]

#Convert Score columns to numeric
temp_scores <- pneumoniaFull[, score_cols] %>%
  lapply(function(col) as.numeric(gsub("[^0-9.]", "", col))) %>%
  as.data.frame()

#Replace original columns
pneumoniaFull[, score_cols] <- temp_scores

#Double-check Score columns are now numeric
sapply(pneumoniaFull[, score_cols], class)

#Check some distributions
lapply(pneumoniaFull[, score_cols], table, useNA = "ifany")

str(pneumoniaFull[, c(compared_cols, score_cols)])
...
```{r}
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/I. Biomedical & Clinical Informatics/Demo_1/pneumoniaFullEncoded.Rdata")
```
...
```{r}
#Define the categorical columns that we want to frequency encode
cols2encode <- c("State", 
                 "CountyParish")  
#These are high-cardinality geographic identifiers. 

#Extract only the columns we want to encode from the full dataset
temp <- pneumoniaFullEncoded[, names(pneumoniaFullEncoded) %in% cols2encode]
#This holds only the selected columns to be transformed,
...
add_freq <- function(data, column_name) {
  ...
  #Create a frequency table counting how many times each category appears (including NAs).
  
  data[[column_name]] <- frequency_map[match(data[[column_name]], names(frequency_map))]
  #For each value in the column, replace it with its frequency count from the table.
  #match() ensures the correct value is mapped from the frequency table.
  
  return(data)  #Return the modified dataframe
}

#Loop through each column we want to frequency encode
for (col in names(temp)) {
  temp <- add_freq(temp, col)
  #add_freq() function to replace category values with their frequency counts
}

#Replace the original columns in pneumoniaFullEncoded with the frequency-encoded versions
for (c in 1:length(cols2encode)) {
  pneumoniaFullEncoded[, cols2encode[c]] <- temp[, cols2encode[c]]
  #This line writes the frequency-encoded values back into the main dataset
}

```             


##### **Question 19**: [1 point] JC
...
```{r}
state_freqs <- pneumoniaFullEncoded$State
county_freqs <- pneumoniaFullEncoded$CountyParish

#Count number of unique frequency values
length(unique(state_freqs))  
length(unique(county_freqs)) 

# Count how many times each frequency value is repeated
table(table(state_freqs))      
table(table(county_freqs))     

```

> The state feature has 57 unique frequency values so this would be worth keeping for frequency encoding.
CountyParish has 1549 unique values and 791 counties share the same frequency of 1. It makes sense to drop CountyParish from frequency encoding.
...
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/I. Biomedical & Clinical Informatics/Demo_1/pneumoniaAnalyze.Rdata")
...
> ExcessReadmissionRatio is likely PredictedReadmissionRate/ExpectedReadmissionRate. There is likely a high correlation between Predicted and observed readmission rate.
The best target is probably PredictedReadmissionRate though the main disadvantage of using it is that it may incorporate variables or assumptions that are not fully transparent in our analysis. Also, since it is already a model-derived estimate, using it as a target could amplify any biases or limitations.

##### **Question 21**: [1 points] JC

Why did I have you drop the two columns used to make `observed_readmission_rate`?

> To prevent redundancy and data leakage.
...
```{r}
# Recode NurseComm into bins again
pneumoniaAnalyze <- pneumoniaAnalyze %>%
  mutate(NurseCommGroup = case_when(
    is.na(`HcahpsLinearMeanValue_Nurse communication`) ~ "NA",
    `HcahpsLinearMeanValue_Nurse communication` >= 91 ~ "91–100",
    `HcahpsLinearMeanValue_Nurse communication` >= 81 ~ "81–90",
    `HcahpsLinearMeanValue_Nurse communication` >= 71 ~ "71–80",
    TRUE ~ "Below 71"
  )) %>%
  mutate(NurseCommGroup = factor(NurseCommGroup,
                                 levels = c("Below 71", "71–80", "81–90", "91–100", "NA")))

# Density plot
ggplot(pneumoniaAnalyze, aes(x = PredictedReadmissionRate, fill = NurseCommGroup)) +
  geom_density(alpha = 0.6) +
  labs(
    title = "Density of Predicted Readmission Rate by Nurse Communication Score Group",
    x = "Predicted Readmission Rate (%)",
    y = "Density",
    fill = "Nurse Communication"
  ) +
...
> This plot shows that hospitals with higher nurse communication scores tend to have lower predicted readmission rates. Specifically, facilities in the 91–100 score range cluster around 15–17% readmissions, while lower-scoring groups, like 71–80, show higher rates often above 20%. The NA group has a wider spread but generally aligns with poorer-performing groups. Overall, better nurse communication appears to be linked to fewer hospital readmissions.
