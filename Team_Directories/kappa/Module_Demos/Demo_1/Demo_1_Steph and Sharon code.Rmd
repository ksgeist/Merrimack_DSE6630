...
#Rename column for convenience and plot
pneumoniaAnalyze <- pneumoniaAnalyze %>%
  rename(Hcahps_NurseComm = `HcahpsLinearMeanValue_Nurse communication`)

pneumoniaAnalyze <- pneumoniaAnalyze %>%
  mutate(
    NurseCommGroup = cut(Hcahps_NurseComm,
                         breaks = c(0, 60, 70, 80, 90, 100),
                         labels = c("0–60", "61–70", "71–80", "81–90", "91–100"))
  )

ggplot(pneumoniaAnalyze, aes(x = NurseCommGroup, y = PredictedReadmissionRate)) +
  geom_violin(fill = "steelblue", color = "gray30") +
  geom_boxplot(width = 0.1, outlier.shape = NA, alpha = 0.5) +
  facet_wrap(~ State, ncol = 4) +
  labs(
    title = "Predicted Readmission Rate by Nurse Communication (HCAHPS), Faceted by State",
    x = "Nurse Communication Score Group",
