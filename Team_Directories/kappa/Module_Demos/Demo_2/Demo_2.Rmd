...
```{r}
load("/Users/stephgarofalo/Documents/GitHub/Merrimack_DSE6630/I. Biomedical & Clinical Informatics/Demo_1/pneumoniaFull.Rdata")
...
> The reason we did not bring the fully-processed data from Project 1, is because we made multiple changes and transformations, to the point where it no longer reflects the original data/visuals. 
...
#### **Question 2A**: [0.25 points] SS

```{r}
# Inspect how states are represented in the pneumoniaFull dataset
pneumoniaFull %>% 
  select(State) %>% 
  distinct() %>% 
  head()

# Inspect how states are represented in the states map data
states %>% 
  select(region) %>% 
  distinct() %>% 
  head()
...
> The State column in the pneumoniaFull dataset contains two-letter uppercase abbreviations like "AL", "AZ", and "CO", while the region column in the states map dataset contains full lowercase state names like "california", "arizona", and "colorado". These differences mean the state names do not match directly, and we will need to convert the abbreviations to full lowercase names in order to merge the datasets successfully.
...
```{r}
# Create distinct lookup of state abbreviations to full lowercase names
state_lookup <- state.fips %>%
  select(abb, polyname) %>%
  distinct() %>%
  # Remove duplicates just in case (e.g., WA appears twice)
  group_by(abb) %>%
  slice(1) %>%
  ungroup()

# Join with pneumoniaFull
pneumoniaFull <- pneumoniaFull %>%
  left_join(state_lookup, by = c("State" = "abb")) %>%
  # Rename 'polyname' to 'region' to match the map data
  rename(region = polyname)

# Check result
head(pneumoniaFull %>% select(State, region))
```
...
> A potential fix would be to modify the polyname column in state.fips to remove everything after a colon (ex. change washington:san juan island to washington). This ensures we only use the base state name for matching. However, this solution would become tedious if we were working at a more granular level such as counties, where subregions are meaningful and removing the suffix would result in loss of precision. In such cases, we’d need more detailed geographic information or use a separate reference file for counties altogether.
...
#### **Question 6**: [1 point] SS
Are there any missing data? If so, what data are missing? Did you make a mistake?

> There is missing data in the stateAggPneumonia dataset, specifically, there is 1 missing value in both meanMedicareSpending and medianMedicareSpending. However, this was not a mistake - the missing data likely reflects  gaps in the original dataset, where at least one state in the original dataset was missing all values for Score_Medicare spending per patient, so when the mean and median were calculated using 'na.rm = TRUE', the result was still NA.
...
> Showing someone a mean when we should have shown a median, or vice versa, can create serious interpretation problems because these two measures respond very differently to skewness along with outliers in the data. The mean is sensitive to extreme values and can be pulled in the direction of outliers, potentially putting it at risk for misrepresenting a usual case. This would make a hospital or state appear better or worse than it really is. 
The median is more robust to skew and outliers, but it may oversimplify the data. If we choose the wrong measure, stakeholders might draw incorrect conclusions, such as overestimating typical readmission rates or underestimating variation in care quality across states, leading to misguided decisions in public health planning and so on. 

\   

#### 1.2.5.1 Using boxplots embedded in violin plots to assess distribution symmetry

#### **Question 8A**: [1 points] SS
...
> Based on Figure 1, I believe the ones that would clearly benefit from using the median over the mean are:
Pneumonia Hospital Return Days as this measure is strongly right-skewed with many high outliers. The median would be more appropriate here because the mean would be distorted by those extreme values.
Medicare Spending per Patient is the closest to symmetric, with a more balanced boxplot and fewer extreme outliers. In this case, using the mean would be acceptable. 
Pneumonia Readmission Rate also shows right skewness, though less extreme than the previous variable. The boxplot is off-center within the violin shape, and there are notable outliers. The median would provide a more stable central measure.
Pneumonia Death Rate is moderately skewed right and has a long tail with outliers at the upper end. The median would likely be more representative of the typical death rate across states.
...
#### 1.2.5.4 Final selection of mean vs. median {#median}
...
#### **Question 9A**: [0.25 points] {#table4} SS
...
```
> One trend worth testing is that many of the states with the highest median pneumonia-related readmission rates—such as District of Columbia, New Jersey, West Virginia, Michigan, and Illinois—are highly urbanized or densely populated areas. 
This could suggest there is a relationship between urbanization or population density and higher readmission rates. 
Another pattern is the inclusion of states like California and Nevada, which may experience higher hospital strain or resource variation across regions- factors such as healthcare system burden, socioeconomic disparities, or access to follow-up care could be contributing to these elevated readmission rates. 
...
```{r}
# Count the number of unique states (regions) in the training set
length(unique(stateAggTrain$region))
```

> Yes, all 52

#### **Question 10B**: [0.25 points] SS

Why am I not asking you to check the states in the testing data?

> This is because the testing set is only a subset of the full dataset, it may not contain all 52 states, this is due to the way data was split. The training set is what we care about for modeling and learning the relationships, so it must include the full geographic variation.


#### **Question 10C**: [0.25 points] SS
...
> This is because the stateAggTrain and stateAggTest datasets were created from raw data and we had not yet scaled or centered these values. When data is unscaled and may be skewed, the median is a more robust choice to avoid the influence of outliers.

Once data is scaled and centered (as we did in Project 1), the choice between mean or median becomes less important because transformations reduce skewness and make the distribution more symmetric. Therefore, using the median for aggregation here is a safe and consistent choice before merging with geometry and applying further modeling steps.
...
> The states object contains geographic information such as longitude and latitude coordinates. This allows it to be used with geom_polygon() for basic mapping. However, it lacks coordinate reference system (CRS) data and does not  support spatial joins or spatial statistical modeling. The st_as_sf() function from the sf package converts geographic data into a proper shapefile (sf) object. This format includes full polygon geometries, retains CRS data, fully supports spatial joins, and can be used with geom_sf() for mapping as well as with spatial statistics and modeling functions.
...
> 1) Rook continuity defines neighboring polygons based on shared edges only. It's named after the rook in chess, which moves only vertically and horizontally. In spatial terms, two regions are considered neighbors under rook continuity only if they share a common border.
2) Under rook continuity, Colorado and Arizano would no longer be considered neighbors because they do not share an actual edge, just a corner.
3) To switch to rook contiguity in poly2nb() we set the argument queen = FALSE.
...
> Yes, this feels completely consistent because Alabama has 4 neighbors, and the output shows four weights: [1] 0.25 0.25 0.25 0.25. This confirms that each of its neighbors is being considered equally.
Row-standardization means the total weight assigned to all neighbors equals 1. Since Alabama has 4 neighbors, each one gets a weight of 1/4=0.25, which is what we see in the results above.
...
> The results tells us that MA has 5 neighbors, each with a spatial weight of 0.2. This adds up as MA has 5 neighbors (New Hampshire, Vermont, New York, Connecticut, and Rhode Island). 
...
> We are performing all spatial pre-processing only on the training data in order to avoid data leakage. Data leakage happens when information from the test set influences the model training process.
If we had used the testing data set as well to compute spatial relationships or weights, we would be allowing information from the test data to indirectly influence the model’s understanding of the training set. This would result in overly optimistic performance metrics and a model that doesn’t generalize well.
By restricting spatial pre-processing to the training set only, we ensure that our model is trained on only the information it should have access to and evaluation on the test set remains unbiased.
...
#### **Question 13**: [1 point] SS
Identify each of the outliers from the graph and interpret the type of outlier (high-high, low-high, etc.) that they are. 

>High-High Outliers Quadrant I
These states have high pneumonia readmission rates and are surrounded by neighbors that also have high rates (positive reinforcement of high rates) New Jersey, District of Columbia, and West Virginia. 
These suggest potential regional clustering of high readmission rates in the Northeast/Mid-Atlantic.
Low-High Outlier Quadrant II
Delaware has a low readmission rate, but is surrounded by neighbors with high rates (unusual relative to its neighbors). It stands out because although its neighbors have high readmission rates, it does not, potentially a sign of successful state-specific interventions or policies.
Low-Low Outliers Quadrant III
These states have low readmission rates and are surrounded by low-rate neighbors. There are no particular outliers identified here.Utah has a low readmission rate and is surrounded by neighbors with low rates. 
High-Low Outliers Quadrant IV
This states have high readmission rates but are surrounding by states with low rates. The graph shows no states that are outliers in this category.


#### **Question 14A**: [0.5 points] SS
...
> We use the centered version of the raw median readmission rate rather than the transformed and scaled version because centering ensures comparability between the variable and its spatially lagged counterpart.Centering shifts the distribution so that its mean is 0, which allows us to interpret values above or below average consistently. Scaling is unnecessary here and could distort quadrant interpretation because it changes the unit of measurement and variance, which isn't helpful when we're interested in directional relationships rather than standardized magnitudes.
In fact, using the raw median readmission rates allows for easier interpretation of the actual readmission values.The transformed version is then helpful for regression assumptions but makes interpretation in a spatial context harder.
...
> The conclusion does not change when we use the transformed bc_PredictedReadmissionRate instead of the centered raw median. Moran’s I statistic remains high at approximately 0.289, indicating moderate positive spatial autocorrelation. This means that states with similar transformed pneumonia readmission rates still tend to cluster geographically. The p-value of 0.0007 is still significant, so we reject the null hypothesis that readmission rates are randomly distributed across space. This Moran’s I value is very similar to the earlier result using the centered raw median (about 0.268), which reinforces that the observed spatial clustering is robust regardless of which version of the readmission rate we use.
...
> Coldspot states are those with low pneumonia-related readmission rates that are near other states with similarly low rates. To be a coldspot, the state must have a significantly low rate compared to the national average, and its nearby states must also have low rates. This shows a clear cluster of low readmission rates.
...
> Before calculating spatial weights and performing spatial lagging, the raw pneumonia readmission rates were centered — that means the mean value was subtracted from each state's rate. This transformation shifts the data to have a mean of 0, turning the raw values (like 16.7%) into deviations from the national average. So when you calculate the spatial lag — which is just a weighted average of neighboring states' centered values — the result is not a percentage anymore. It’s how much higher or lower than average Massachusetts’ neighbors are on average.

#### **Question 15C**: [0.5 points] SG
...
```{r}
# Extract and center the Median_RawPredictedReadmissionRate
y_centered <- scale(states_sf_AggTrain$Median_RawPredictedReadmissionRate,
                    center = TRUE, scale = FALSE)

# Show Massachusetts' value (row 20)
y_centered[20]
```

**QUESTION**: Is this a lot closer to the spatially-lagged value?

> Yes. The centered value is on the same scale as the spatially-lagged value, which is why it looks much closer. That’s why centering was necessary — so both values could be meaningfully compared and used to determine spatial patterns like clusters and outliers.
...
```{r}
## Add a new column to classify each state into a spatial quadrant category
states_sf_AggTrain <- states_sf_AggTrain %>%
  mutate(
    ## Begin defining the 'quadrant' column using conditional logic
    quadrant = case_when(
      ## If the p-value is greater than 0.05, there is no significant spatial autocorrelation → mark as "N.S."
      pValue > 0.05 ~ "N.S.",
      ## If p-value is significant (<= 0.05), no need to re-check pValue — all remaining rows already passed that condition above
      
      ## High-High: Centered median readmission (y) > 0 AND lagged neighbor average > 0 → Hotspot (Quadrant I)
      y > 0 & lag_medianReadmissions > 0 ~ "Hotspot",
      
      ## Low-Low: Centered median readmission (y) < 0 AND lagged neighbor average < 0 → Coldspot (Quadrant III)
      y < 0 & lag_medianReadmissions < 0 ~ "Coldspot",

      ## High-Low: State has high readmissions (y > 0) but neighbors have low values (lag < 0) → High-Low Outlier (Quadrant IV)
      y > 0 & lag_medianReadmissions < 0 ~ "High-Low Outlier",

      ## Low-High: State has low readmissions (y < 0) but neighbors have high values (lag > 0) → Low-High Outlier (Quadrant II)
      y < 0 & lag_medianReadmissions > 0 ~ "Low-High Outlier",

      ## Catch-all in case none of the conditions are met (shouldn’t happen, but good to include for safety)
      TRUE ~ NA
    )
  )
```
...
#### **Question 15E**: [0.5 points] SG
Interpret what it means if Indiana is a "low-high" outlier.

> This suggests that Indiana stands out in a positive way compared to its neighbors. While the region around it struggles with higher hospital readmissions, Indiana is doing significantly better. This could point to more effective hospital practices, state-level healthcare policies, or patient outcomes within Indiana that are worth investigating or replicating elsewhere. It may also flag Indiana as a potential best-practice model for surrounding states.

#### **Question 15F**: [1 point] SG
...
> Yes, the hot and coldspots identified in the LISA map make sense when compared to graph p1. For example, states like West Virginia and Pennsylvania appear dark in p1 and PA is correctly labeled as a hotspot, while states like Montana and Wyoming are light and form a coldspot cluster. However, hot/coldspots do not always match individual states’ high or low readmission rates because LISA considers both the state's value and the values of its neighboring states. A state with a high rate surrounded by low-rate neighbors might be labeled an outlier rather than a hotspot, and vice versa. Spatial context matters more than the individual value alone.
...
> If we were starting from the raw data, we would first need to clean and wrangle it—this includes removing or imputing missing values, correcting data types, and filtering out any irrelevant observations. Next, we would need to engineer features (e.g., converting categorical variables, creating summary measures), followed by normalizing or transforming variables to ensure comparability, especially if they’re on different scales or skewed. After that, we would split the data into training and testing sets to prevent data leakage. Then, we would perform spatial joins or aggregations to attach geographic identifiers and calculate spatial weights based on the appropriate geometries. Finally, we’d be ready to fit a spatial regression model like using the cleaned, transformed, and geographically-aware training data.
...
> You're fitting an OLS regression again because after aggregating to the state level, the sample size has changed—we now have fewer observations (just one per state). This means the earlier OLS model fitted on hospital-level data isn't directly comparable. Additionally, running OLS here gives us a baseline model to compare against the spatial regression model (SAR), helping us evaluate how much the spatial structure improves predictive power.
...
#### **Question 17B**: [1.5 points] SG
- What would we conclude from the results of the Moran's $I$ test on the __residuals__? 

> The Moran’s I test checks whether the leftover errors from our OLS model (the residuals) still show a spatial pattern. Since the p-value is just above 0.05, we can’t say for sure that the errors are clustered by location. That means the OLS model did a decent job, and there’s no strong evidence that the errors are influenced by nearby states.
...
> Yes, if we had started by looking at the residuals before calculating Moran’s I and LISA on the original readmission data, we could have missed important spatial patterns. That’s because once a model (like OLS) is fit, it might already account for some spatial structure through the predictors. So if we only looked at the residuals, we might wrongly conclude that there isn’t much spatial clustering — even though the original data did have strong spatial relationships. By starting with Moran’s I and LISA on the raw data, we confirmed upfront that spatial clustering exists, which helps justify the need for spatial models like SAR.

- Why might these results disagree with what we found from the Moran's $I$ and LISA on the scaled target?

> These results might differ because we’re now looking at the residuals from a regression, not the raw readmission rates. Earlier, Moran’s I and LISA showed clear spatial clustering in the raw data. But after fitting a model with predictors, some of that spatial pattern was explained — so there’s less spatial autocorrelation left in the residuals, which is why the test result is weaker.
...
> 1. Global Moran's I: The Global Moran’s I value was positive and statistically significant, indicating that pneumonia-related readmissions tend to cluster geographically—states with high values are near others with high values, and the same for low values. This suggests clear spatial autocorrelation in the target variable.
> 2. Local Moran's I (LISA): LISA results identified specific hot and coldspots. For example, West Virginia and surrounding states formed a hotspot, while several northwestern states formed a coldspot. These results confirm the presence of localized clusters, strengthening the case for spatial dependence in the data.
> 3. Moran’s I on OLS residuals: The Moran’s I test on the OLS model residuals gave a p-value slightly above 0.05, suggesting that the residuals do not have statistically significant spatial autocorrelation. This result might misleadingly imply that OLS is sufficient, even though earlier tests showed otherwise.
> 4. Test for spatial autoregression using ρ (rho): The SAR model showed a significant spatial autoregressive coefficient (ρ), with a p-value < 0.05. This indicates that spatial lag (influence from neighboring states) contributes meaningfully to the model, and spatial dependence should not be ignored.
> 5. Likelihood Ratio Test (LRT): The LRT comparing the SAR and OLS models was significant (p = 0.0286), indicating that the SAR model fits the data better than OLS. This supports the idea that including spatial structure improves the model’s performance.
> 6. LM test for residual spatial autocorrelation: The LM test on the SAR model residuals showed a high p-value (0.818), indicating that there is no significant leftover spatial autocorrelation. This suggests the SAR model has successfully accounted for spatial dependencies in the data.

What is your **final takeaway**? Do we need to account for spatial autocorrelation in our regression model?

> Yes, we do need to account for spatial autocorrelation in our regression model. The Global and Local Moran’s I tests showed clear evidence of spatial clustering, and the SAR model significantly improved model fit over OLS based on the Likelihood Ratio Test. Most importantly, the SAR model successfully removed residual spatial autocorrelation, meaning it better captures the structure of the data. Ignoring spatial effects would likely underestimate or misattribute relationships in the data.1
...
> After accounting for spatial effects in the SAR model, only two predictors stayed significant: Medicare spending per patient and embolism/thrombosis rate. The spatial lag term (rho) was also significant, meaning nearby states' readmission rates do matter. Some variables that were important before, like hospital return days and death rate, are no longer significant. This shows that once we adjust for spatial patterns, only a few predictors truly stand out.
...
> It’s challenging to interpret this RMSE because the model was trained on the Box-Cox transformed version of the target, not the original raw readmission rates. That means the RMSE is on the transformed scale, so it doesn’t directly reflect the percentage of readmissions in real-world terms without converting it back.
...
> The SAR (Spatial Lag) model has a lower RMSE (1.15) and MAE (0.79) compared to the OLS model (RMSE = 1.33, MAE = 0.84), which means it produces more accurate predictions overall.
...
> Since the p-value (0.81762) is much greater than 0.05, we fail to reject the null hypothesis. There is no significant spatial autocorrelation remaining in the residuals after fitting the SAR model. This means the SAR model successfully accounted for spatial effects in the data.
...
> A significant SEM but non-significant LM test in the SAR model means the SAR already handled the spatial autocorrelation well. Since there’s no leftover spatial pattern in the residuals, we don’t need an SEM. This shows the SAR model was a good fit for our data.
...
> To analyze spatial data, we start with Moran’s I and LISA to check for spatial autocorrelation in the outcome variable, which tells us if spatial models are necessary. If there is spatial dependence, we fit a Spatial Autoregressive Model (SAR) to account for how neighboring regions influence each other. If residuals from the SAR still show spatial autocorrelation, we then try a Spatial Error Model (SEM), which captures unmeasured spatial effects. If we suspect that both the outcome and the predictors have spatial patterns, we use a Spatial Durbin Model (SDM), which accounts for spatial effects in both. This workflow helps determine the best model for understanding or predicting spatial patterns in the data.
...
> Yes, data scientists do incorporate spatially lagged variables into machine learning models like Elastic Net, Random Forests, and Neural Networks to enhance predictive performance, especially when dealing with spatial autocorrelation. Researchers are exploring more ways to incorporate spatially lagged variables into machine learning models to improve prediction performance for spatial autocorrelation. Integrating spatial lag parameters into Random Forest models has been shown to modestly enhance their predictive accuracy by capturing spatial dependencies between observations. Neural networks can be adapted to account for spatial effects by including spatial lag vectors as input features, allowing the network to learn from both the original and neighboring values. Another approach, known as Spatially Explicit Machine Learning, combines techniques like spatial lagging or Eigenvector Spatial Filtering (ESF) with models such as Random Forests to reduce prediction error and minimize spatial autocorrelation in model residuals.
A study titled "Spatial Models or Random Forest? Evaluating the Use of Spatially Explicit Machine Learning Methods to Predict Employment Density around New Transit Stations in Los Angeles" demonstrates the application of spatially explicit Random Forest models. The research found that incorporating spatial lag parameters modestly improved the accuracy of Random Forest models in predicting employment density.
Credit, Kevin. (2021). Spatial Models or Random Forest? Evaluating the Use of Spatially Explicit Machine Learning Methods to Predict Employment Density around New Transit Stations in Los Angeles. Geographical Analysis. 54. 10.1111/gean.12273. 
...
> The state-level spatial dataset is inappropriate for further machine learning because it is too small—we are working with only 49 observations. Most machine learning algorithms, including Random Forest, Elastic Net, and Neural Networks, require substantially larger datasets to train models effectively, avoid overfitting, and generalize well. Machine learning models benefit from complexity in the data which our aggregated dataset lacks. Because we’ve aggregated to the state level, we’ve lost the individual hospital-level granularity, which limits the amount of variation the models can learn from.&#x20;
...
> A more suitable spatial structure for applying machine learning to this dataset would be hospital-level or county-level data, rather than state-level. Hospital-level data has a much larger sample size with over 2,600 observations, making it more appropriate. County-level or ZIP code-level data also provides finer geographic resolution and helps preserve spatial variation. This enables more localized analysis and the integration of spatial lag features or neighborhood characteristics. These levels of spatial structure allow for the incorporation of spatial relationships to high-readmission areas, into machine learning models.
...
> A social determinant of health (SDOH) refers to the non-medical factors that influence health outcomes. These include the conditions in which people are born, grow, live, work, and age, and they shape the distribution of resources, opportunities, and power. Common examples include income level, educational attainment, employment status, housing, food security, neighborhood safety, and access to healthcare. The phrase comes from the public health and epidemiology fields and was popularized globally by the World Health Organization.
...
## Remove D.C. because it's an outlier in density that can skew the color scale
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
ggplot() +
  ## use sf geometry with population density as fill color
  ...
  ## Add map title and data source caption
  labs(title = "Adult population density by state",
    caption = "Source: U.S. Census Bureau, 2023") +
  ## Clean up background and axes for map visualization
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
    plot.background = element_rect(fill = "white"),
  ...
  ## Make sure map is proportionally accurate
  coord_sf()
...
## Remove D.C. again
  ...
  ## Take top 10 most dense states
  head(10) %>% 
  ## Create a bar plot where states are reordered by population density (most dense at top)
ggplot(aes(x = fct_reorder(ID, popDensity), y = Median_RawPredictedReadmissionRate)) +
  ## Create horizontal bar chart
  geom_col(fill = skittles[3]) +
  theme_minimal() + 
  ## ## Flip the coordinates so states are on the y-axis and readmission rates on the x-axis
  coord_flip() +
...
> Yes, there does appear to be a correlation between population density and median pneumonia-related hospital readmissions when comparing the two maps (p2 and p1). High-density states in p2, such as New Jersey, Massachusetts, Maryland, and New York, also show higher readmission rates in p1. Lower-density states like Wyoming, Montana, and the Dakotas generally show lower readmission rates. The presence of darker shades (indicating higher values) in similar regions on both maps suggests a positive spatial correlation.&#x20;
...
> The scatterplot (p3) shows a positive linear relationship between population density and median pneumonia-related hospital readmissions. As population density increases, the predicted readmission rate also rises. The best-fit line and its confidence interval reinforce this. This suggests that population density may be a meaningful predictor of hospital readmission rates. The bar plot (p4) complements this by displaying the top 10 most densely populated states, such as New Jersey, Rhode Island, Massachusetts, and New York many of which also have among the highest median readmission rates. 
...
> Moran’s $I$ = 0.105, p = 0.098. There's weak, non-significant spatial autocorrelation in the residuals. So, the model is not clearly misspecified spatially, and a standard OLS model may be sufficient for now.
...
> The value of the spatial autoregressive coefficient, ρ, is 0.354 with a p-value of 0.00705, which indicates that there is statistically significant spatial dependence in the target variable.  This confirms that pneumonia-related hospital readmission rates in one state are influenced by those in neighboring states. This result is not surprising given the earlier findings from the Global Moran’s I and LISA tests, which showed clear spatial clustering. Even after accounting for factors like population density, the spatial signal in the target remains. However, the LM test has a p-value of 0.684, which is not significant. This tells us that the SAR model has done a good job of accounting for spatial effects, as there is no strong evidence of spatial patterning left in the residuals.&#x20;
...
> The SAR (Spatial Autoregressive) model is the better fitting model because it has a lower AIC (61.513 vs. 65.221) and a higher log-likelihood (−10.757 vs. −13.611) compared to the OLS model. The likelihood ratio test yields a p-value of 0.0169, which is less than the standard significance level of 0.05. This means we reject the null hypothesis that the simpler OLS model is sufficient, and conclude that the SAR model, which accounts for spatial dependence in the target variable, provides a significantly better fit to the data.
...
> Yes, there were some notable changes in predictor significance between the first SAR model and the updated SAR model that includes population density. In the earlier SAR model, variables such as composite patient safety and postoperative respiratory failure rate were close to statistical significance, but their p-values increased slightly in the updated model. Perioperative pulmonary embolism or deep vein thrombosis rate and Medicare spending per patient remained statistically significant in both models, suggesting they are consistently influential predictors. Population density became statistically significant in the updated SAR model, with a p-value of 0.0415 and a positive coefficient of approximately 0.0092. This means that even after accounting for spatial dependence, states with higher population density tend to have higher median pneumonia-related readmission rates.

&#x20;Interpreting the coefficient, for every increase of 100 people per square mile, the predicted readmission rate increases by about 0.009 on the transformed scale. This supports the earlier hypothesis that higher population density is associated with more readmissions.
...
> The three spatial models (SAR, SEM, and SDM) all outperform OLS in terms of prediction error. Each spatial model has a lower RMSE (1.09 vs. 1.30) and MAE (0.74 vs. 0.82) than the OLS model, indicating better predictive accuracy. However, there is essentially no difference in predictive performance among the SAR, SEM, and SDM models, as they share identical RMSE and MAE values. The SAR model still appears to be “good enough” for practical use, especially considering its relative simplicity compared to the more complex SDM. The fact that all three spatial models perform similarly shows that spatial dependence is clearly present but perhaps not fully accounted for even after including a variable like population density. This suggests we are likely still omitting other spatially structured variables.
