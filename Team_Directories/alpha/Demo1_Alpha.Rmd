...
output:
  html_document:
    ...
    toc_float: true
    theme: flatly
bibliography: ../../I.Bioinformatics/Demo/references.bib   
nocite: '@*'
---
...
> It appears that the most appropriate fit type for the runVST would be the rlog fitType. I'm a little unsure as to why this may be, but I believe this could be the case since our data is working with the counts of the gene expression and this works well for rlog = Malek Sabri
...
> An adjusted p-value is something that I had to do some research on even with my background in data science. The adjusted p-value is new to me because I've never really needed it for the types of hypothesis testing I've used. It is useful in this experiment because we are working with multiple genes in this differential gene expression experiment. When working with these multiple genes there may be an increased chance of having some false positive results. Thus, the adjusted p value comes in to hopefully correct the false discovery rate as much as possible. FDR will control the false positive rates helping the data be as accurate as possible = Malek Sabri

##### **Question 13** (MS): Why is it critical we employ a p-value adjustment here, like the FDR?

> So employing p-value adjustment is critical here for a number of different reasons, but most obviously to maintain the validity of the experiment. If these findings aren't adjusted later on down the road this can lead to a lot of problems including the fact that we may have some interesting discoveries, but they could be totally invalid when tested = Malek Sabri

##### **Question 14** (MS): How many differentially expressed genes did we find with the __Wald test__?

> There are 2 low counts and 1 outlier = Malek Sabri

##### **Question 15** (MS): Looking at the Verble et al. (2023) study, how do our results differ from their `DESeq2` results and why? 

> This question was kind of tricky for me so I'm going to try my best at it. At first, I saw that we used the Benjamini-Hochberg method for FDR and I didn't think the original study would do that but they did as well. So the next thing I was looking for in the paper was to see the difference in "Time". My best guess is the way in which we used "design" with the code: design = ~ Colony + Treatment + Time. I believe that this code is more accurate than the one that the original study did and we are taking more into account. = Malek Sabri
...
> It appears that lowExpression is defined as lowExpression <- length(totalCounts[totalCounts < 10*ncol(df)*0.9]) - The function looks like it is multiplying the columns in the data frame by .9 and 10 and if the count is then less then length totalCounts we are labeling it as lowExpression = Malek Sabri
...
> Based on what I've learned so far I would say that it is likely ill-advised to do the filtering for low expression. The main reason for me believing that this is ill-advised is that there is still valuable info in what we could be potentially filtering out. Comparing the low expressed genes with high ones could also provide some valuable insight = Malek Sabri
...
> The thing that I'm noticing about the heat map is that on the left hand side I'm seeing a "lack therof" in terms of the labeling of the heat map. I don't see anything there which makes it kind of hard to interpret. The next thing I notice is the way in which everything is grouped by correlation. The gene expression gets lower as it moves down and towards the right = Malek Sabri
...
> I wouldn't say that my assessment on the arrangement of the treatments and time points change and this is because the distances don't get calculated by the control or not control. Rather, it is calculated by the correlation and how they're expressed = Malek Sabri
...
# sampleDistances <- dist(t(exprs(vstObject)))     ## Hint: what did we do to extract a matrix from the VST object earlier?
...
> I do think that both of these could still be used. Obviously as noted above we are using plotPCA because of it's tailored ability to work with gene expression data. But, if needed i do believe proncomp and prcomp could provide meaningful insights = Malek Sabri
