...
> These results suggest that OLS performs worse than all three spatial models, which all perform equally well. SAR is good enough as it performs just as well as the other two spatial options we explored. We could still be omitting spatial variables, even with this improved performance, as so many Social Determinants of Health are inherently spatially correlated (i.e., socioeconomic status is linked to place of residence, educational opportunities are linked to property taxes which is linked to where you live, also linked to public school districts, etc.).
