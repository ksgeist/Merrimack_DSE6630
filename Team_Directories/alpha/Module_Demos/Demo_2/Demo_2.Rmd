...
```
>  When we take the variable group out, it makes the boundaries all out of place, with the state boundary lines going in all directions. We need a group variable here because it tells ggplot which points belong to the same shape/boundary. Without it, ggplot2 tries to connect all the coordinates in the dataset as one continuous line, which gives us that big jumble of lines and an incorrect map.
...
> It looks like the "pneumoniaAnalyzeFullyEncoded" dataset form Project 1 had dropped alot of important spatial data including 	
CountyParish, Address, CityTown, and ZipCode. The dataset we are using here "pneumoniaFull", contains all these spatial data. This suggests that using the unmodified dataset which contains this geographic context is important in order to get variables that are essential for mapping and spatial interpretation. These would have been lost or distorted in a machine learning optimized datasets, which on the other hand see these variables as noise and have stronger predicting cabablilties with out them. 
...
#### **Question 2A**: [0.25 points] - KB

```{r}
# checking the unique state names in the `states` dataset
unique(states$region)

# checking the unique state names in the pneumonia dataset
unique(pneumoniaFull$State)
```

> By looking at the unique state names in both the datasets, it looks like the States in the `states` dataset are shown with there full name spelled out and no capital letters. On the other hand, when looking at the Staes in the `pneumoniaFull` dataset, we can see that the State names are shown with abriviations with capital letters.  
...
```{r}
## KSG: THIS IS WHERE THE ADDED ROWS ARE HAPPENING. So, you were very close - but there is an issue in your join.
## Your state lookup table is right, but the issue is in your left_join. Now, what you did here isn't technically wrong - it's a byproduct of the fact that state.fips has all the extra regions for islands! It isn't your fault. So, you're left with two options:

## 1 - Don't ACTUALLY overwrite pneumoniaFull; just show that your code works
## 2 - Rerun this to start pneumoniaFull from fresh:
# load("../../../../I. Biomedical & Clinical Informatics/Demo_1/pneumoniaFull.Rdata")
# 
# ## Let's change some key data types as well:
# pneumoniaFull <- pneumoniaFull %>% 
...
## So it isn't that your copy of pneumoniaFull is bad, it's that you overwrote it with a "bad" version

# Here is a lookup table that contains both the State abbreviations and full names
state_lookup <- state.fips %>%
  select(State = abb, region = polyname) %>%
  distinct()

# Now we use left join to join the lookup table to pneumoniaFull and replace at the abbreviation with the full names
pneumoniaFull <- pneumoniaFull %>%
  left_join(state_lookup, by = "State") %>%
  mutate(State = region) %>%  # Replace abbreviation with full name
  select(-region)             # Remove region
...
> In order to fix this problem, we could remove everything after the ":" in the polyname column and keep only one row per state abbreviation. This will make the joining clean with a single, lowercase state name. However, this would be alot of work and allow for more errros if we were to do it at the county level, due to subregions. 
...
# KB: what KB did before 
# showing how `getStateName` works using the `mutate()` function 
# pneumoniaFull <- pneumoniaFull %>%
#   mutate(region = gsub(":.*", "", State))

## KSG: Not quite --> I may have worded this confusingly, but I really just meant to run the function. For example, look at what this does!
#getStateName(pneumoniaFull$State)

# KB: This returns a named character vector where Names are the abbreviations and Values are the full state names

## KSG: Now, what could you do to replace "region" in pneumoniaFull using this code? It was simpler than you thought!

# KB: showing how using unname() removes abbreviations and only keeps statenames in new feature column 
pneumoniaFull <- pneumoniaFull %>%
  mutate(region = getStateName(State))

# shows only the statenames in the new feature column
str(pneumoniaFull$region)
```
...
```{r}
#Checking for the missing values and counting them by the columns
colSums(is.na(stateAggPneumonia))
```

> It looks like there still are missing data. meanMedicareSpending has 1 data missing, meaning that one state has all values missing and medianMedicareSpending	had 1	data missing, which probaly means that the same state has no valid values to calculate the median. Therefore there was no mistake made. All region values are present and all the meterics exept for that MedicareSpending one are present. It is totally  normal with real world data sets to have missing values!
...
> Showing a mean instead of a median or vice versa, when it is not appropriate can definetly lead to misleading interpretations. For example, if the data is skewed or contain outliers, it can pull the mean away from the true center of the distribution. In this case, the stakeholder would be assuming that the mean represents the typical value, when in actually is being distorted by the extreme values. This will cause the stakeholders to misjudge the outcomes. In our sutuation, if one or a few hospitals in a state has extremely high readmission rates, the mean would overstate the performance, making the state look worse than it actually is. On the other hand, using the median could hide extreme but maybe important information, so it could understate serious problems in a state with some very high readmission hospitals.

#### 1.2.5.1 Using boxplots embedded in violin plots to assess distribution symmetry

#### **Question 8A**: [1 points] - KB
...
> Looking at the boxplots below, it looks like Medicare Spending per Patient is the most symetrical, but still has a slight right skew. Pneumonia Death Rate is also mostly symetrical, but also has a small right skew. Pneumonia Hospital Return Days has a strong right skew. Pneumonia Readmission Rate also has a mildly strong right skew. Overall, using the median over the mean would benifit Pneumonia Hospital Return Days, Pneumonia Death Rate, and Pneumonia Readmission Rate. This is true because these measures all have a skewed distribution and contain high outliers that can cause the mean to me misrepresented. Also, using meidan for consitancy for Pneumonia Readmission Rate would be good. 
...
___Interpretation___: Because p < 0.05, we reject the hypothesis of symmetry and conclude that death rates for pneumonia patients are significantly skewed. We should use the median to aggregate this variable.
...
___Interpretation___: Because p < 0.05, we reject the hypothesis of symmetry and conclude that hospital return days for pneumonia patients are significantly skewed. We should use the median to aggregate this variable.
...
___Interpretation___: Because p < 0.05, we reject the hypothesis of symmetry and conclude that Medicare spending per patient is significantly skewed. We should use the median to aggregate this variable.
...
> Looking at Figure 2, there is no cause for concern with the choice of using the median for all four variables as determined previously from 8A and 8B. It looks like Figure 2 actually confimrs that the median will provide more of a stable estimate, expecially with the skewness and outliers that were seen well in the violin plots from Figure 1.
...
#### **Question 8D**: [1 points] - KB
```{r}
# Your code here

# Removing all the columns that have "mean" from the original stateAggPneumonia
...
#### **Question 9A**: [0.25 points] {#table4} - KB
...
> A noticeable trend from both the map and Table 4 is that states with higher median pneumonia-related readmission rates tend to be more densely populated areas, for example I know that New Jersey, California, New York, Illinois, and Florida have larger populations.
...
p2 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name = "Median deaths per pneumonia cases in 30 days", 
...
p3 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name = "Median Medicare spending per patient", 
                        low = "white", high = skittles[7]) +       ## !!Update color here   
                        ## Leave low as white; only !!update high!
  
  ## !!Update the title to reflect a better map title
  labs(title = "Median Medicare spending for pneumonia patients by state",
       ## Leave the caption as-is
       caption = "Source: Centers for Medicare & Medicaid Services (CMS), 2024") +
...
p4 <- ggplot(mergedStateAggPneumonia, aes(x = long, y = lat, group = group, 
  ...
  geom_polygon(color = "grey20", size = 0.2, alpha = 1) +
  ## !!Update this change the legend and color scale
  scale_fill_continuous(name = "Median pneumonia hospital return days within 30 days", 
...
# Counting the number of states that made it into the training set
length(unique(pneumoniaFull$region))
```

> Looks like there are 52 states that made it!

#### **Question 10B**: [0.25 points] - KB

Why am I not asking you to check the states in the testing data?

> The reason you are not asking us to check the states in the testing data is because it is okay if not all the states are included there, since the goal of the testing data is to see how well the model handles unseen data. As long as the training data has all the states, the model is able learn from the full diversity of data. Then we can use the testing data, even if its different or smaller, to check how well the model actually performs.

#### **Question 10C**: [0.25 points] - KB
...
> The reason the median was used when aggregating stateAggTrain and `stateAggTest is because we were working with the raw and unscaled data. Before, when we were deciding between using the mean and median, we were using data that had already been scaled, which helps to reduce the impact of any outliers, so using either the mean or median was fine in that case.
However, now that we are back to using the original data, the outliers and skewed values can really affect the mean alot more. That is why it is important to use the median. Overall, it is a better choice here, giving us a more reliable summary of the data without being thrown off by any extreme values.
...
> The st_as_sf() function from the sf package is convertting the map data into a standardized spatial format called a "simple features" or sf object. This format then stores both the shapes (geometry) and related data so that the spatial analysis tools in R can use them. You didnt use just the states object we already used for mapping because it is mainly for plotting, where as st_as_sf() prepares the data for advanced spatial calculations like finding neighbors or measuring spatial autocorrelation. Overall, the states object lacks the detailed structure and metadata needed for these types of analyses, so converting to an sf object works better for proper spatial statistics.
Source:
Pebesma, E. (2018). Simple Features for R: Standardized Support for Spatial Vector Data. The R Journal, 10(1), 439-446. https://doi.org/10.32614/RJ-2018-009
(For more info on sf package: https://r-spatial.github.io/sf/)
...
> Your answer here. 
For this contiguity, the states are considered neighbors if they share a full border (excludes states connected by corners). In the case of Colorado and Arizona, they would not be considered neighbors because they are only connected by a corner. How we would change this continuity in the 'poly2nb()' is by changing the line to 'queen' = FALSE.
...
> Your answer here.
After accounting for spatial autocorrelation, the significant predictors (with p-values < 0.05) in the SAR model are those that remain consistently important even after adjusting for spatial spillover effects. Compared to Table 10, some variables that were significant before are still significant and some variables that were not significant before have become significant. Compared to Figure 20 (Elastic Net feature importance), we see the most important variables identified by the Elastic Net overlap with the SAR model's significant predictors. By including spatial autocorrelation, we get a clearer picture of which predictors truly matter. Some variables gained significance, revealing that what once seemed like noise may actually be meaningful information.
...
> Your answer here.
It is challenging because they are standardized to different scales. The changes in the data mean the data will represent relationships differently. 
...
> Your answer here.
MAE tells us how far off our model's predictions are from the actual values. The OLS MAE is 0.84 while the SAR MAE is 0.79, indicating that the SAR perfomed better (due to the error being less). RMSE also measures prediction error, squaring the errors before averaging. The OLS RMSE is 1.33 while the SAR RMSE is 1.15, indicating that the SAR performed better in that aspect. Therefore, the SAR model is better due to its lower MAE and RMSE.
...
where $y$ is the target variable (median pneumonia-related readmissions), $W$ is the spatial weights matrix, which defines the neighborhoods (stored in `states_sw`!), and $\lambda$ is the _spatial error coefficient_. This means that the first term of the equation, $\lambda W \epsilon$, measures the spatial correlation between our errors. So, if $\lambda = 0$, the spatial correlation between the errors isn't enough to bias the standard errors. As with SAR, the remaining terms, $\beta_1 X_1 + \beta_2 X_2 + ... \beta_n X_n$ are just the standard regression coefficients for each of the $n$ predictors. $E$ is an unmeasured error term, which is now the overall error leftover after fitting the SEM regression. 
...
#### **Question 21A**: [1 point] - NL
Test the null hypothesis that $\lambda = 0$, i.e., that there is no spatial autocorrelation in the residuals. What p-value do you get and what conclusions do you make?

> We get a p-value well below 0.05 for both the LR test value (p = 0.015) and the Wald test value (p < 0.001), both of which suggest that we reject the null hypothesis that states there is no spatial autocorrelation in the residuals. Combined with the lambda value of 0.61, the p-value suggests that the residuals are relatively strongly and statistically significantly correlated.
...
> We may have a significant SEM outcome but the LM in our SAR was not significant because they are testing different outcomes. The SEM shows that there is spatial correlation between residuals. The SAR however, is testing whether or not there is spatial correlation that impacts our dependent variable. Essentially, SEM is telling us there is spatial correlation, but SAR is testing and telling us that the spatial correlation lag term isn't important enough to our results to consider further.
...
> 1. Calculate Moran's I to examine global spatial autocorrelation. Can use LISA to identify more local indicators/clusters. If this shows evidence of spatial autocorrelation then continue with spatial modeling
2. Fit a SAR model based on output of Moran's I/LISA if there is autocorrelation in the dependent variable.
3. Fit an SEM model if Moran's I shows autocorrelation on the residuals
4. use SDM to examine both dependent and independent variable spatial autocorrelation holistically if indicated by the SAR and SEM models.
...
> Yes, data scientists have extracted spatially lagged targets/predictors and plugged them into machine learning models. In the paper "Leveraging Spectral Neighborhood Information for Corn Yield Prediction with Spatial-Lagged Machine Learning Modeling: Can Neighborhood Information Outperform Vegetation Indices?" (Noa-Yarasca, 2025; https://www.mdpi.com/2673-2688/6/3/58), the authors describe a spatially-lagged machine-learning model they created by expanding upon the Spatial Lag X model, which seems to only incorporate spatial autocorrelation on the predictor variables, sort of the inverse of SAR. The authors use their spatially-lagged machine-learning to capture non-linear relatioships in a way that SLX and SAR are not able to do. They computed spatially-lagged neighbors and appended that as a new feature, then plugged that into their machine learning models, including random forest, XG-boosting, extremely randomized tree regression, and gradient boosting regression. They saw that, when including the spatially-lagged neighbors, the models had dramatically improved prediction performance.

#### **Question 23**: [2 points] - NL
...
> 50 states is simply too small of a sample size to do any meaningful machine learning modeling with. Even using techniques like bootstrapping would be difficult to get machine learning models to work on this.

#### **Question 24**: [1 point] - NL
...
> There are multiple spatial structures we could use. Zip codes may be the easiest to implement (no duplicates, easy to "plug-and-play" as a result). The CountyParish feature could also be used, though they may be more difficult to map as there are duplicate counties between states, so we would have to concatenate them with the State then go from there. Zip codes are probably a better option as there are tens of thousands of them compared to, I believe, only thousands of counties. An N in the thousands is workable but machine learning loves big data.
...
> Social Determinants of Health are factors that are do not directly define (as opposed to something like genetics, which does directly define) an individuals health but still contribute to it. These tend to be environmental factors like where people live, their socioeconomic status, air and water quality near them, access to nutrition, etc. 
...
## Beginning the ggplot
ggplot() +
  ## Filling states based on population density
  geom_sf(aes(fill = popDensity), color = "grey20", alpha = 1, size = 0.2) +
  ## Setting up custom color gradient for population density fill, ranging from white to skittles[3] (red)
  scale_fill_gradient(
    name = "Hundreds per sq. mi.",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +
  ## Labeling density plot with appropriate labels
  labs(title = "Adult population density by state",
    caption = "Source: U.S. Census Bureau, 2023") +
  ## Inputting a clean/minimal map theme
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
    plot.background = element_rect(fill = "white"),
  ...
  ## This code projects the map properly
  coord_sf()
...
## Filtering out D.C. again
  #filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
## Beginning a ggplot scatterplot with population density as the independent variable and predicted pneumonia readmission rate as the dependent variable.
ggplot(aes(x = popDensity, y = Median_RawPredictedReadmissionRate)) +
## Plotting points that are semitransparent, plotting a linear model prediction line in red, and setting the theme to minimalist.
  geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm", se = T, color = skittles[3]) +
  theme_minimal() +
  ## Labeling the plot appropriately with a title, x-axis label, and y-axis label.
...
## Filtering out D.C.... again. Poor thing.
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
  ## Arranging states in descending order of population density
  arrange(desc(popDensity)) %>% 
  ## Showing only the top 10, after ordering them. This will show the top 10 most dense states.
  head(10) %>% 
  ## Plotting a horizontal bar chart after ordering the states, with a dependent variable (bar size) of pneumonia readmission rates.
ggplot(aes(x = fct_reorder(ID, popDensity), y = Median_RawPredictedReadmissionRate)) +
  ## Changing the fill color to skittles red and changing the theme to minimalistic.
  geom_col(fill = skittles[3]) +
  theme_minimal() + 
  ## Inverting the coordinates of the plot
  coord_flip() +
  ## Adding appropriate labels: a title and a label for the dependent variable. The independent variable (state) is inherently obvious just based on the values (state names) so the label is blank to reduce clutter. Adding a caption to include the source.
...
> There is a correlation between population density and pneumonia-related admissions, yes. There seem to be more readmissions in areas with higher population density. It could be that a more dense population means that resources are more tight or that disease is able to physically spread more easily. It is hard to conclude much (if anything) just from this correlation.

#### **Question 27B**: [0.5 points] - NL
...
> Predicted pneumonia-related hospital readmissions increases as population density increases, according to `p3`. `p4` shows that there is nuance to this, where it is not a guarantee that a more dense state on average has a higher average rate of readmissions, though the overall trend suggests the same as `p3`. 
...
> Moran's I results in a p-value of 0.09, which is > 0.05. This fails to reject the null hypothesis, suggesting that there is not spatial autocorrelation in the residual terms. 
...
> The Rho value of 0.35443 suggests that there is some spatial correlation in the dependent variable: states with high predicted readmission rates tend to be nearby other states with high values. The p-value (0.016887 < 0.05) suggests that this is a statistically significant result. The LM test (p = 0.68448) fails to reject the null hypothesis, suggesting that there is no spatial autocorrelation in the residuals. 
...
> The null hypothesis for this LRT states that the simpler model, OLS, is sufficient for modeling the data. The p-value of 0.016887 rejects this null hypothesis, suggesting that OLS is not sufficient, compared to SAR. SAR also exhibits a better AIC value suggesting a better fit of the data.
...
> Compared to our original SAR we ran, the only predictor that changed significance is `popDensity`, which was not included in the original SAR but is significant in the second SAR. The beta coefficient suggests that, ceteris paribus, an increase in population density of one unit correlates to an increase in pneumonia-related hospital readmission rate of about 0.0091 units.
...
> These results suggest that OLS performs worse than all three spatial models, which all perform equally well. SAR is good enough as it performs just as well as the other two spatial options we explored. There is no statistical indicator that we are missing any spatial variables based on the analysis we have done (SAR Lm was insignificant, SAR performs well). Realistically, We could still be omitting spatial variables, even with this improved performance, as so many Social Determinants of Health are inherently spatially correlated (i.e., socioeconomic status is linked to place of residence, educational opportunities are linked to property taxes which is linked to where you live, also linked to public school districts, etc.).


## 3.3 Testing Another Hypothesis: Readmissions by Poverty
...
p2 <- states_sf_AggTrain %>% 
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
ggplot() +
  geom_sf(aes(fill = percBelowPoverty), color = "grey20", alpha = 1, size = 0.2) +
  scale_fill_gradient(
    name = "(percantage below poverty line)",
    low = "white", high = skittles[3],
    na.value = "grey90"
  ) +

  labs(title = "Percentage of Population Below Poverty Line by state",
    caption = "Source: U.S. Census Bureau, 2023") +
...
p3 <- states_sf_AggTrain %>% 
  filter(ID != "district of columbia") %>%  ## <- Comment this out to see why!
ggplot(aes(x = percBelowPoverty, y = Median_RawPredictedReadmissionRate)) +
  geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm", se = T, color = skittles[3]) +
  theme_minimal() +
  labs(title = "Hospital readmissions by population poverty",
...
p4 <- states_sf_AggTrain %>% 
  ...
  head(10) %>% 
ggplot(aes(x = fct_reorder(ID, percBelowPoverty), y = Median_RawPredictedReadmissionRate)) +
  geom_col(fill = skittles[3]) +
  theme_minimal() + 
  coord_flip() +
  labs(title = "Top 10 Most Impoverished States",
     y = "Median % hospital readmissions",
     x = "",
     caption = "Source: CMS, 2024 & U.S. Census, 2023; n.b. Wash D.C. excluded")
...
grid.arrange(p3, p4, ncol = 2)
```
> There seems to be some correlation between the percentage of the population below the poverty line and the percentage of pneumonia related hospital readmissions, though this is more difficult to discern from the map than in the previous exercise with population density.

##### Creating a new SAR for poverty
```{r}
sarTrain <- states_sf_AggTrain %>% 
  as_data_frame() %>% 
  select(-Median_RawPredictedReadmissionRate, 
...
moran.test(residuals(OLS), states_sw)
```
> The output of Moran's I (p = 0.02641) suggests that there is statistically significant spatial autocorrelation in the residual terms, when accounting for the percentage of the population under the poverty line.

##### Fitting the SAR for poverty
```{r}
SAR <- lagsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain, 
                listw = states_sw)
summary(SAR)
...
> The SAR model when accounting for the percentage of the population under the poverty line shows that there is some statistically significant spatial autocorrelation in the dependent variable (Rho = 0.34846). The LM test suggests that there is no spatial autocorrelation in the residuals of the model (p = 0.38383), suggesting that this model does an apt job at capturing those spatial relationships.

##### Compare the SAR poverty model to the OLS model with a Likelihood Ratio Test (LRT)
```{r}
anova(SAR, OLS)
```
> The results of the LRT suggest that this SAR model accounting for poverty rates performs significantly better than the simpler OLS model.

##### Inferential Statistics for poverty model
```{r}
tbl_regression(SAR) %>% 
  modify_caption("Table 7. Spatial Lag (SAR) Regression results.") %>% 
  modify_header(label = "**Coefficient**") %>% 
...
> The summary table suggests that the percentage of the population below poverty is not a statistically significant determinant of pneumonia-related hospital readmission rates, though it is close to significant and does have a very high beta coefficient of 3.6. This is one of those cases where you might be tempted to think "biologically significant, not statistically significant." Postoperative respiratory failure rate is significant in this model whereas in the non-spatial model it was insignificant. All other predictors remain the same in terms of statistical significance.

##### Fitting SEM and SDM models with poverty rates
```{r}
## Fit the SEM (spatial error model) using the training data and weights matrix
SEM <- errorsarlm(bc_PredictedReadmissionRate ~ ., data = sarTrain, 
                  listw = states_sw)
...
summary(SDM)
```
##### Predictions for all four models accounting for poverty rates
```{r}
olsPredictions <- predict(OLS, newdata = sarTest)
...
> Again we see that the spatial models all perform similarly, while performing as a group much better than the simpler OLS model. Overall, the models accounting for the percentage of the population below the poverty line perform less well than the models tested above that account for population density. The improved performance of the spatial models does suggest that poverty rate has some relationship with pneumonia-related hospital readmission rates, though it was shown in the SAR model to be statistically insignificant when directly predicting the target variable. The SEM model returns statistical significance for poverty rate as a predictor of errors, which suggests that poverty rate may be a proxy for other, unexplained spatial variables, as it relates to the unexplained variation. The SDM model also returns statistical significance for poverty rate as a predictor of pneumonia-related hospital readmission rates. This suggests that poverty rate has spatial spillover effects that SAR is not able to fully capture, as it only targets the spatial lag of the dependent variable, while SDM targets the spatial lag of the dependent and independent variables. These indirect effects that SDM captures may be contributin to poverty rate's influence on pneumonia-related hospital readmission rates.
