...
author: "Krisha Bugajski, Nicholas Lagoni, and Trinity Tobin"
date: "21 May 2025"
output:
  html_document:
    toc: true
...
> There were 25 features dropped

```{r}
# Identifying features/columns to drop
columns_to_drop <- tidyOutput %>%
  select(
...
tidyOutput <- tidyOutput %>% select(-any_of(cols_to_drop))
```
...
#unique(tidyOutput$ComparedToNational)
```
...
cols_to_encode <- names(tidyOutput)[grepl("MeasureName|ComparedToNational", names(tidyOutput))]


#2. print original value counts before encoding
for (col in cols_to_encode) {
  cat("Original value counts for column:", col, "\n")
  print(table(tidyOutput[[col]]))
  cat("\n")
}

# 3. create a temporary dataframe for encoding
temp_df <- tidyOutput[, cols_to_encode]
...
tidyOutput <- tidyOutput %>%
  filter('comparedToNational_' %in% c(
...
tidyOutput[, cols_to_encode] <- temp_df
