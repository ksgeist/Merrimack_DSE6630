...
# Function: join_clean_datasets

# Purpose: Clean, filter, pivot, and merge a list of hospital-related dataframes

# Inputs:
#   - df_list: A list of dataframes to clean and join
#   - condition: An vector of MeasureName values to filter on, for example in this case "PN" for pneumonia)

# Output: A fully merged and cleaned dataframe


join_clean_datasets <- function(df_list, condition = NULL) {
  
  # Defining the columns that need to be dropped if they exist
  drop_cols <- c("StartDate", "EndDate", "Footnote", "MeasureId")
  
  # Cleaning, filtering, and pivoting each dataframe in the list
  cleaned_list <- lapply(df_list, function(df) {
    
    # Dropping columns that are not necessary if they exist
    df <- df %>% select(-any_of(drop_cols))
    
    # Filtering for MeasureName if present
    if (!is.null(condition) && "MeasureName" %in% names(df)) {
      df <- df %>% filter(str_detect(MeasureName, paste(condition, collapse = "|")))
    }
    
    # Pivoting wider using "MeasureName"
    if ("MeasureName" %in% names(df)) { 
  
      value_col <- setdiff(names(df), c("FacilityId", "MeasureName"))
      if (length(value_col) == 1 && is.character(df[[value_col]])) { # if there is exactly one value column and it’s character type
        df[[value_col]] <- suppressWarnings(as.numeric(gsub("[\\$,]", "", df[[value_col]]))) # remove $ and commas from the value column and convert it to numeric
      }
      df <- df %>%
        pivot_wider(names_from = MeasureName, values_from = all_of(value_col))
    }
    
    return(df)
  })
  
  # Starting with hospitalInfo
  merged_df <- hospitalInfo
  
  # Joining all cleaned datasets on FacilityId
  for (df in cleaned_list) {
    merged_df <- full_join(merged_df, df, by = "FacilityId")
  }
  
  # Joining with readmissionsClean 
  merged_df <- full_join(merged_df, readmissionsClean, by = "FacilityId")
  
 # Merging duplicate columns for .x and .y versions
base_names <- unique(gsub("\\.(x|y)$", "", grep("\\.(x|y)$", names(merged_df), value = TRUE)))
for (name in base_names) {
  x_col <- paste0(name, ".x")
  y_col <- paste0(name, ".y")
  if (x_col %in% names(merged_df) && y_col %in% names(merged_df)) {
    merged_df[[name]] <- coalesce(merged_df[[x_col]], merged_df[[y_col]])
    merged_df[[x_col]] <- NULL
    merged_df[[y_col]] <- NULL
  }
}
  ...
  # Removing columns that need to be dropped again after joining
  merged_df <- merged_df %>% select(-any_of(c("StartDate", "EndDate", "Footnote", "MeasureId")))
...
```{r}
# Using the new function to join datasets for pneumonia
my_merged_df <- join_clean_datasets(
  df_list = list(Maternal_Health, Complications_and_Deaths, Unplanned_Hospital_Visits),
  condition = c("PN")  # merging for pneumonia
)

# Since after merging, columns contained '_Pneumonia (PN) 30-Day Readmission Rate' I created another function to clean called clean_columns_and_remove_duplicates

clean_columns_and_remove_duplicates <- function(df, pattern = "_Pneumonia \\(PN\\) 30-Day Readmission Rate") {
  # Removing the pattern from column names
  new_names <- gsub(pattern, "", names(df))
  
  # Finding duplicates created by this renaming
  duplicated_names <- new_names[duplicated(new_names)]
  
  # Keepinf only the first occurrence of each duplicated name, remove others
  cols_to_keep <- !duplicated(new_names)
  
  # Applying the cleaned column names and subset columns
  df_clean <- df[, cols_to_keep]
  names(df_clean) <- new_names[cols_to_keep]
  
  return(df_clean)
}

# Applying the function to
my_merged_df_clean <- clean_columns_and_remove_duplicates(my_merged_df)

# Checking the dimensions
dim(my_merged_df_clean)
```
