...
tidyNjoin <- function(datList, filterList, hospitalInfo, readmissionsClean) {
  ...
  # Define columns to drop across datasets, case-insensitive
  drop_cols <- c("startDate", "endDate", "footnote", "MeasureId")
  
  # Extract all unique MeasureName values from all dataframes, helps ensure all possible measures are present for consistency
  all_measures <- unique(unlist(lapply(datList, function(df) unique(df$MeasureName))))
  
  # Identify all columns that hold scores/values, not identifiers or metadata
  # this ensures consistent column structure across datasets
  all_valuecols <- unique(unlist(lapply(datList, function(df) setdiff(
    names(df),
    c("FacilityId", "FacilityName", "Address", "CityTown", "State", "ZipCode", 
      "CountyParish", "TelephoneNumber", "MeasureName", drop_cols)
  ))))
  
  # Process each dataframe individually to clean, filter, and pivot it
  wide_list <- purrr::imap(datList, function(df, i) {
    
    # Get corresponding filters for this dataframe, if present
    filters <- if (i <= length(filterList)) filterList[[i]] else NULL
    
    # Drop non-essential columns that vary across datasets, case-insensitive
    cols_to_drop <- names(df)[tolower(names(df)) %in% tolower(drop_cols)]
    df <- df %>% select(-any_of(cols_to_drop))
    
    # Filter for relevant measures only
    ...
    # Ensure all expected value columns are present in the current dataframe
    # Missing ones are created with NA to preserve consistent structure for joining
    for (col in all_valuecols) {
      if (!(col %in% names(df))) df[[col]] <- NA
    }

    # Ensure all measures are represented — even if they're missing in the current df
    # This prevents problems during pivoting and helps ensure uniformity across datasets
    missing_measures <- setdiff(all_measures, unique(df$MeasureName))
    if (length(missing_measures) > 0 && nrow(df) > 0) {
      add_df <- expand.grid(FacilityId = unique(df$FacilityId), MeasureName = missing_measures, stringsAsFactors = FALSE)
      for (col in all_valuecols) add_df[[col]] <- NA
      df <- bind_rows(df, add_df)
    }
    
    # Pivot the dataframe from long to wide format
    # Each combination of value column and MeasureName becomes a unique column
    df %>%
      pivot_wider(
        id_cols = c("FacilityId"),
        names_from = "MeasureName",
        values_from = all_of(all_valuecols),
        names_glue = "{.value}_{MeasureName}"  # E.g., Score_Death rate, etc.
      )
  })
  
  # Merge all pivoted dataframes together using a full join on FacilityId
  mergedDF <- purrr::reduce(wide_list, function(x, y) {
    full_join(x, y, by = "FacilityId")
  })
  
  # Join with hospitalInfo 
  mergedDF <- mergedDF %>% left_join(hospitalInfo, by = "FacilityId")
  
  # Join with readmissionsClean last
  mergedDF <- mergedDF %>% left_join(readmissionsClean, by = "FacilityId")
  
  # Remove any duplicated rows that might have come in during joins
  mergedDF <- mergedDF %>% distinct()

  # Remove duplicate columns from merges (for example: col.x, col.y...)
  mergedDF <- mergedDF %>% select(-matches("\\.y$|\\.x$|\\.x\\.x$|\\.y\\.y$"))

  # Drop any leftover unnecessary columns (case-insensitive again)
  # Includes rechecking for MeasureName
  drop_cols_final <- c(drop_cols, "MeasureName")
  cols_to_drop_final <- names(mergedDF)[tolower(names(mergedDF)) %in% tolower(drop_cols_final)]
  mergedDF <- mergedDF %>% select(-any_of(cols_to_drop_final))
  
  # Return the cleaned, joined, and wide-format dataset
  return(mergedDF)
}
```

```{r}
finalDF <- tidyNjoin(datList, filterList, hospitalInfo, readmissionsClean)
dim(finalDF)
```
