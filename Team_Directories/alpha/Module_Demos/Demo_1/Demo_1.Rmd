...
```{r}
# This function takes a list of dataframes, a filter list, and a hospital info dataframe, and performs 
# a series of data cleaning and merging operations to produce a tidy, merged dataset.
tidyNjoin <- function(datList, filterList, hospitalInfo) {
  ...
  # Loop through the remaining dataframes in the cleaned list and merge them one by one
  # Use a full join to retain all records from both dataframes
  ...
  # Here we filter the dataframe to only include rows where the 'MeasureName' matches
  # the values in the 'filterList'. This helps us focus on specific measures of interest.
  merged_df <- merged_df %>%
  ...
  # Step 5: Clean up column names by removing the .x, .y, etc.
  # When we merge dataframes, R adds suffixes (like .x, .y) to column names to differentiate
  # between similarly-named columns from the different dataframes. We clean up these column names.
  colnames(merged_df) <- gsub("\\.x$|\\.y$|\\.x\\.x$", "", colnames(merged_df))
  
  # Step 6: Merge with hospitalInfo to get hospital-related information
  ...
  # Step 7: Merge with the readmissionsClean dataset
...
The main reason you're working through this function is to build the skills and techniques necessary to efficiently manage, clean, and prepare multiple datasets for analysis in Project 1. The function you just worked on helps practice tasks like merging data, removing unnecessary columns, and preparing datasets in a structured and scalable way—skills you'll need to carry out complex analyses in your project.
