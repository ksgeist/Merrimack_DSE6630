...
filepath <- file.path(Sys.getenv("HOME"), "Desktop", "hospitals_current_data") #Creates "filepath," a vector object linking to the folder of hospital data placed outside of the local repo as it is too large to push to github. 
...
for(f in 1:length(files)) { #Sets up a for loop looping from 1 to the length of the files object, representing a count of the number of data files.
    dat <- clean_names(read_csv(paste0(filepath, files[f]), #within the loop, cleans the name of the selected file, standardizing special characters to underscores.
