...
```{r}
# Defining the breakpoints and labels
...
# making sure target is a factor
readyTrain$ReturnDaysCategory <- as.factor(readyTrain$ReturnDaysCategory)

# Defining the target
target_variable <- "ReturnDaysCategory"

# Exclude numeric version of the return days from predictors
predictors <- setdiff(names(readyTrain), c("ReturnDaysCategory", "Hospital.return.days.for.pneumonia.patients"))
...
varImpPlot(rf_model_classification)
```
...
train_preds <- predict(rf_model_classification, newdata = readyTest)

# Confusion matrix
confusion_matrix <- table(Predicted = train_preds, Actual = readyTest$ReturnDaysCategory)
print(confusion_matrix)

# Accuracy
accuracy <- mean(train_preds == readyTest$ReturnDaysCategory)
