...
I choose the data set Option 2 (to load the data without the encoding) because loading this data will eliminate any changes that resulted from the previous encoding, while ensuring the data is still cleaned and available to be encoded by our new code from this project. 
...
longTargets %>% 
  ggplot(aes(x = Value, y = NatlComparisonHospitalDays, fill = NatlComparisonHospitalDays)) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.1)           ## <--------- THIS IS WHERE I ADDED THE LINE 
  labs(title = "Which target to use?",
       subtitle = "By readmitted LOS in hospital compared to national average",
        x = "",
        y = "Rate or Score",
...
> Your answer here.
It changes my decision, as the Excess Readmission Ratio is the one graph that has a clear delineation between the 'Better', 'Same', and 'Worse' variables. 
...
> Your answer here.
When comparing the graphs from the four variables, only the PredictedRatio and the ExcessReadmissionRatio have slopes that appear orderly and consistent enough to view a clear relationship. From these two, these graphs reiterate that ExcessReadmissionRatio has the best separation between the variables.

```{r, fig.width=7, fig.height=8, echo = FALSE, message=TRUE, fig.cap="Figure 4. Readmission rate by hospital performance and readmitted patient LOS"}

## This block has my altered code!!!!!!!

longScore %>%  
  ggplot(aes(y = ExcessReadmissionRatio,  ## <-- CHANGE THIS! done
             x = Value)) +
    ...
    labs(title = "Relationship of ExcessReadmissionRatio to pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
> Your answer here.
Hospitals with higher patient satisfaction, especially in nurse/doctor communication, tend to have lower excess readmission ratios. The contact kept with the doctor ensures that any issues that arise are quikcly addressed without the need for readmission.  On the other hand, cleanliness might not show a strong link to pneumonia-related readmissions because these readmissions often relate more to a personal decision to enter and stay at the hospital, as opposed to an infection, which keeps you there.  I still feel most strongly about 'Excess Readmission Ratio' due to how the data fits the plots well and due to the same reasons as above showinf similar patterns in these scatterplots.
...
> Your answer here.
One compelling relationship in teh scatterplots was the relationsihp between 'Excess Readmission Ratio' and recommened hospital because all of the slopes for each of the comparison variabsles is negative. Another compelling relationship from these scatterplots is the relationship between the 'Excess Readmission Ratio' and quietness because of the lack of slope for all these comparison variables. This lack of relationship makes me question why the variable was included in the original dataset and how they measured quietness. 
...
> Your answer here.
Near zero variance is the statistical measure that identifies variables in a dataset as having few or only one output, creating an extremely low variance for the variable. In terms of looking at the data, these features do not show any relationships and are not very useful in answering larger questions. 
...
> Your answer here.
The curse of dimensionality is a fun way of naming the problems that arise when you are working with an extremely large data set and a lot of information. One such problem within this curse umbrellanis the 'big-P, little-N (n >> P) problem' , which occurs when there is a significanyly larger amount of predictor variables when compared to the observations. We likely don't have this problem because hospitals collect such large amounts of data that we have large amounts of information (and observations). We lso pre-processed the data, so there are already less predictors than we started with.
...
> Your answer here.
All of the columns (100%) havesome level of missingness.
...
> Your answer here.
The variable that seems to be most correlated to missingness with 'PredictedReadmissionRate' is 'MRSA Bacteria'. This does give me cause for concern, as a bacterial infection would likely see readmission. If this data is missing, it's hard to devise a plan that treats and controls form the spread of MRSA bacteria. 
...
dict %>% 
  kable(
    ...
    pack_rows("2:  Struggling Hospitals", 5, 8) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> We are using repeated 5-fold cross-validation with grid search to train an Elastic Net regression model, simultaneously tuning both:
$\alpha$, which balances L1 (LASSO) and L2 (Ridge) penalties, and
$\lambda$, which controls the overall strength of the regularization.
This approach helps mitigate overfitting, addresses multicollinearity, and selects the most predictive features, making the final model more robust and interpretable.
...
>  PCA and regression measure importance differently because they have different overall goals. Regression will find features that best predict the outcome (Y), so importance is based on the size of the effect on Y.
PCA finds features that explain the most variance in the predictors (X), while ignoring the outcome, so importance is based on how much a feature varies across samples. Also, the signs of PCA loadings are arbitrary and don’t indicate direction like regression coefficients do. Because of this, a feature can be very important for predicting the outcome (high regression coefficient) but not vary much (low PCA loading), or the other way around, overall making importance seem inverted between the two methods (Jolliffe, 2002). 
...
>  We can see that both the OLS and Elastic Net models are performing very similarly. They both show consistent results between the training and testing data, which is a good sign. It means they are not overfitting or underfitting. However, they are only explaining about 35–40% of the variation in readmission rates. That means that while the models are stable, they’re not strong predictors. Between the two models, Elastic Net may still be the better option because it can handle data that is more complex and also reduce noise from the variables that are less useful. Overall, neither model is giving us the level of accuracy we would want to have to have confident predictions.
There might be a few posibilities for that. One possibility is that the relationship between the predictors and readmission rates isn’t actually linear, and both OLS and Elastic Net assume a linear relationship. As we saw earlier with the OLS Regression Assumption Diagnostic Plots, linearity was mostly met, with some minor nonlinear patterns. There was a slight upward curve in the Residuals vs. Fitted graph. To help with this, using more flexible models like random forests, boosted trees, or other nonlinear models might help to see patterns that are missing. 
Also, looking further at the data to see if ther might have been important factors that are not be captured in the current model. Expanding the dataset with additional variables that could impact readmission rates could lead to stronger, more accurate predictions.
...
1. Switch to classification-based random forest using `ComparedToNational_Hospital return days for pneumonia patientsload(file = "FY2024_data_files/readyTrain.Rdata")
...
### Justifying the Ridge Regression Model: 
...
We implemented Ridge and Lasso Regression models to predict the number of days until a patient is readmitted to the hospital following a pneumonia-related discharge. We selected these regression approaches as natural next steps to compare to the elastic net described in the project, while still working on continuous variables.

For the Ridge Regression (L2 regularization), we used 5-fold cross-validation to identify the optimal lambda value for model performance. The model performed best at a lambda of 0.0501, and the corresponding mean squared error (MSE) on the test data was 0.651, which represents a moderate amount of error as our values range from about -10 to 14. 

For the Lasso Regression (L1 regularization), cross-validation determined the optimal lambda to be 0.0101, and the resulting test MSE was slightly higher at 0.652. This difference in MSE is very minor, but could be indicative of

Our rationale for using Ridge and Lasso regression was to handle multicollinearity among the predictors. Additionally, Lasso helps with feature selection as it reduces some features to complete 0.

While both Ridge and Lasso models yielded decent test errors, they do not capture class imbalances or interpret group-level differences (Low, Medium, High). Additionally, substantial feature selection and multicollinearity reduction work was done prior to running these models, which may diminish their benefits slightly compared to other models. Other models that are more sensitive to multicollinearity should be examined to see if the work done was sufficient in preempting that concern, or if the regularization of these two models is still beneficial.


# Random Forest

### Justifying the Random Forest Model: 
