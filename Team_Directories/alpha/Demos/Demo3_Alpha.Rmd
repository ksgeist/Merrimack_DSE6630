...
> #Malek
Based on the visual evidence, Hypothesis 1, which links higher population density to increased pneumonia-related readmissions, might be supported. After evaluating all of them this is probably the hypothesis I'd learn towards the most. There appears to be a positive correlation between the two variables. Hypothesis 2, suggesting that a higher proportion of elderly populations correlates with increased readmissions, shows some potential but requires statistical testing to confirm significance. Hypothesis 3, which associates higher poverty rates with higher readmissions, also needs statistical validation despite some visual correlation. Follow-up actions should involve detailed statistical analyses to confirm these relationships and further exploration into specific factors like urban vs. rural disparities or healthcare policies in states with higher poverty rates.
...
countySummary <- pneumoniaFull %>%
  group_by(region, subregion) %>%
  summarise(mean_predicted_readmissions = mean(PredictedReadmissionRate, na.rm = TRUE), .groups = "drop")
...
```{r}
## FILL IN THE BLANKS
mergedData <- merge(counties, countySummary, by = c("region", "subregion"), all.x = TRUE) %>% arrange(order)
...
```{r}
colnames(stateInfo)[colnames(stateInfo) == "lng"] <- "long"
FinalData <- merge(mergedData, stateInfo, by = c("lat", "long"), all.x = TRUE) %>% arrange(order)

g1 <- ggplot(FinalData, aes(x = long, y = lat, group = group, fill = mean_predicted_readmissions)) + 
  geom_polygon(color = "black", alpha = 1) +
  scale_fill_continuous(name = "Mean Predicted Readmissions", low = "lightgrey", high = "#28B463", na.value = "white") +
  theme(
    plot.margin = margin(b = 5 * unit(1, "lines")),
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "white"),
    legend.position = "top") +
  ggtitle("Average Predicted Pneumonia-Related Hospital Readmissions by County")
...
> Your answer here.Highest readmission  rate are in the southern and eastern US, while the lowest in rates are in the Western part of the country. Not necesesarily some counties are lack of hospital. 
...
```{r}
## Couldn't figure out how to get this chunk to work
#library(sf)
#names(FinalData)

#FinalData <- FinalData %>%
#  mutate(admission_rate = mean_predicted_readmissions / population * 100)  
#ggplot(FinalData, aes(x = median_household_income, y = admission_rate)) +
# geom_point() +
# geom_smooth(method = "lm", col = "red") +
#  labs(title = "Relationship between Median Income and Hospital Admission Rate",
#       x = "Median Income",
#       y = "Hospital Admission Rate (per 1000 people)") +
#  theme_minimal()
#
#counties_sf <- st_as_sf(counties, FinalData = c("longitude", "latitude"), crs = 4326)
#
#merged_sf <- merge(counties_sf, merged_data, by = "county_id")
#
## Plot the map with admission rates
#ggplot(merged_sf) +
#  geom_sf(aes(fill = admission_rate)) +
#  scale_fill_viridis_c() +
#  labs(title = "Map of Hospital Admission Rates by County",
#       fill = "Admission Rate (per 1000 people)") +
#  theme_minimal()
