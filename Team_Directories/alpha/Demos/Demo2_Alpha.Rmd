...
author: 'Ryan Canfield, Malek Sabri & Tiffany Michel Acosta '
date: "24 April 2024"
output:
  word_document:
    toc: true
...
bibliography: "../../II. Biomedical & Clinical Informatics/Demo/references.bib"
nocite: '@*'
---
...
> Answer is in code chunk! = Malek


```{r, message=FALSE, warning=FALSE}
# filepath <- "~/Desktop/hospitals_current_data/" #Sets path directory
filepath <- "C:\\Users\\RRC\\Downloads\\hospitals_current_data\\" #Sets path directory
files <- list.files(path = filepath, pattern = "Hospital.csv") #Takes the files in the directory and makes sure they are the pattern of "Hospital.csv"

for(f in 1:length(files)) { #Creates a loop going through each file
    dat <- clean_names(read_csv(paste0(filepath, files[f]), #Reads in the file, changes the column names to something more efficient, and sets the case to "upper_camel"
                                show_col_types = FALSE), 
                       case = "upper_camel")
    filename <- gsub(".Hospital\\.csv", "", files[f]) #Substitutes the ".Hospital\\.csv" to a blank
    assign(filename, dat) #Assigns the data frame
}

files <- gsub(".Hospital\\.csv", "", files) %>% data.frame() #Gets rid of ".Hospital\\.csv" from the files and then makes it a data frame
names(files) <- "File Name" #Changes column name to "File Name"
```
...
> To me it is looking like the data was not read in properly! and in fact almost looks like it was read in inversely. The way the data looks right now is not showing that each row is a single line of data, rather the rows are column names such as "FY_2024_Hospital_Readmissions_Reduction_Program" = Malek
...
> So obviously there are some concerns with the NA values in the specified columns. I'm not really sure as to what the plan is to correct it, but I believe that will be discussed as we will likely run into further problems down the line if we don't fix the NA's. One thing that I'll be curious to see is how we handle this. In the past, I've simply gotten rid of NA's but in this case I don't think it'd be wise to do that. With that said, in class we learned about some other methods of possibly repplacing the NA values with something else. = Malek
...
summary(FY_2024_Hospital_Readmissions_Reduction_Program[, c(5, 7:10)])
#So it turns out I didn't really fully understand what was happening. I had automatically assumed that just because there was missing values that R interpreted it as NA. But in reality we need to do some more cleaning and actually change it to NA
# = Malek
```
...
> I would say that the thing that is standing out to me, by looking at the data dictionary, is that there are some values which are not numeric but the type of character in the column isn't what it should be. I think it'd be useful to make sure everything that is numeric should be numeric and everything that is a character or string be exactly that. = Malek
...
```{r}
# ?gsub
# gsub is a function that is new to me and I'm glad I am familiarizing myself with it. It's beneficial to use if there is a pattern in a string and I want to replace that pattern with a given value. Here, the pattern flaw is getting replaced with the number 5 = Malek
FY_2024_Hospital_Readmissions_Reduction_Program <- FY_2024_Hospital_Readmissions_Reduction_Program %>%
  mutate(NumberOfReadmissions = gsub("N/A|Not Available|[^0-9]", "5", NumberOfReadmissions))
...
> Your answer here.Acknowledging that some readmissions did occur.
Providing a practical estimate that avoids introducing too much bias.
Keeping the analysis straightforward without adding complications.
...
* **Hint 6: **If you are really stuck, move ahead and I give you a cleaned, merged data set to work with.
 ...
 pivot wider we would decide if we were going to filter for any specific hospital readmission conditions. For your __Project 2__, you will be choosing which medical condition(s) you want to focus on for predicting hospital readmission. However, for the remainder of this demo, we're going to focus on just __pneumonia__. 
...
```{r}
# Define the function to clean and join dataframes
clean_and_join_dataframes <- function(dataframes, filter_conditions = NULL) {
  cleaned_dataframes <- list()
  
  for (df in dataframes) {
    # Drop unnecessary columns
    df <- df %>% select(-starts_with("Start Date"), -starts_with("End Date"), -starts_with("Footnote"), -starts_with("MeasureId"))
    
    # Filter the dataframe if filter conditions are provided
    if (!is.null(filter_conditions)) {
      for (condition in names(filter_conditions)) {
        df <- df %>% filter(get(condition) == filter_conditions[[condition]])
      }
    }
    ...
    cleaned_dataframes <- append(cleaned_dataframes, list(df))
  }
  
  # Join dataframes on 'Facility ID' and 'Measure Name'
  merged_df <- cleaned_dataframes[[1]]
  for (i in 2:length(cleaned_dataframes)) {
    merged_df <- full_join(merged_df, cleaned_dataframes[[i]], by = c("Facility ID", "Measure Name"))
  }
