...
HEART_FAILURE_Full <- tidyNjoin(datList, filterList, hospitalInfo)
# table(Unplanned_Hospital_Visits$ComparedToNational)

## Save file for students:
save(HEART_FAILURE_Full, file = "HEART_FAILURE_Full.Rdata")
...
HEART_FAILURE_Full <- HEART_FAILURE_Full %>% 
...
for (col in 1:length(cols2encode)) {
  HEART_FAILURE_Full[, cols2encode[col]] <- temp[, cols2encode[col]]
}
...
HEART_FAILURE_FullEncoded <- HEART_FAILURE_Full %>% 
...
## Save file for students:
save(HEART_FAILURE_FullEncoded, file = "HEART_FAILURE_FullEncoded.Rdata")
...
temp <- HEART_FAILURE_FullEncoded[, names(HEART_FAILURE_FullEncoded) %in% cols2encode]
...
for (c in 1:length(cols2encode)) {
  HEART_FAILURE_FullEncoded[, cols2encode[c]] <- temp[, cols2encode[c]]
}
################# End my answer to Question 15

#########################################
## Now, let's make HEART_FAILURE_Analyze:
HEART_FAILURE_Analyze <- HEART_FAILURE_FullEncoded %>% 
  select(-CountyParish)

## Save file for students:
save(HEART_FAILURE_Analyze, file = "HEART_FAILURE_Analyze.Rdata")

HEART_FAILURE_Analyze <- HEART_FAILURE_Analyze %>% 
...
HEART_FAILURE_AnalyzeDemo <- HEART_FAILURE_Analyze %>% 
...
## Save file for students:
save(HEART_FAILURE_AnalyzeDemo, file = "HEART_FAILURE_AnalyzefromDemo.Rdata")
