...
```{r}
```{R}
# Compare distributions for numeric variable ''
ggplot() +
  geom_density(aes(x = HEART_FAILURE_Full$FacilityId), color = "blue", fill = "blue", alpha = 0.3) +
  geom_density(aes(x = HEART_FAILURE_Full$FacilityId), color = "red", fill = "red", alpha = 0.3) +
  labs(title = "Comparison of facilty ID Distribution",
       x = "FacilityId",
       y = "Density",
       fill = "Data") +
  scale_fill_manual(values = c("blue" = "Original Data", "red" = "Imputed Data"))

# Compare frequencies for categorical variable 'sex'
original_sex_freq <- HEART_FAILURE_Full %>% count(CountyParish)
imputed_sex_freq <- HEART_FAILURE_Full %>% count(CountyParish)

comparison_sex <- merge(original_country_freq, imputed_country_freq, by = "CountryParish", suffixes = c("_original", "_imputed"))

ggplot(comparison_country, aes(x = country)) +
  geom_bar(aes(y = n_original, fill = "Original Data"), stat = "identity", position = "dodge") +
  geom_bar(aes(y = n_imputed, fill = "Imputed Data"), stat = "identity", position = "dodge") +
  labs(title = "Comparison of country Frequency",
       x = "country",
       y = "Frequency",
       fill = "Data") +
  scale_fill_manual(values = c("Original Data" = "blue", "Imputed Data" = "red"))
```
yes it did change 
...
```
```{r}
## Renames some of the rows in the Cluser column
kmeansTrain %>% 
