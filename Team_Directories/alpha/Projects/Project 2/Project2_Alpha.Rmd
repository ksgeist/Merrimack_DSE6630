...
# #Malek
hcahps_data <- longDataScore %>% 
  filter(str_detect(Variable, "HcahpsLinear"))
...
```{r}
# > #Malek
imputedTest <- missForest::impute.newdata(object = missForest_model, newdata = test)

imputedTest <- imputedTest$ximp %>% 
  mutate(State = test$State,
         PredictedReadmissionRate = test$PredictedReadmissionRate) %>% 
  mutate_at(vars(Emergency.department.volume,
                 ComparedToNational_MRSA.Bacteremia,
                 ComparedToNational_Composite.patient.safety,
                 ComparedToNational_Death.rate.for.pneumonia.patients,
                 ComparedToNational_Hospital.return.days.for.pneumonia.patients),
...
> Your answer here.# Apply the Box-Cox transformation to the imputedTest data
...
> Your answer here  # Perform PCA on the scaled training dataset
...
> Your answer here# Extract the first two principal components
