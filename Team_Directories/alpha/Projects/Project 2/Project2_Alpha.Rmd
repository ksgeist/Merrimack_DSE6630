...
# Plot distribution before imputation (using imputed test data as a proxy)
ggplot(imputedTest, aes(x = `Payment for pneumonia patients`)) +
  geom_histogram(binwidth = 1000, fill = "blue", alpha = 0.7) +
  ggtitle("Payment for pneumonia patients Before Imputation") +
  xlab("Payment Amount") +
  ylab("Frequency")

# Plot distribution after imputation (using imputed train data)
ggplot(imputedTrain, aes(x = `Payment for pneumonia patients`)) +
  geom_histogram(binwidth = 1000, fill = "green", alpha = 0.7) +
  ggtitle("Payment for pneumonia patients After Imputation") +
  xlab("Payment Amount") +
  ylab("Frequency")

# Categorical variable: Emergency department volume
# Plot frequencies before imputation (using imputed test data as a proxy)
ggplot(imputedTest, aes(x = factor(`Emergency department volume`))) +
  geom_bar(fill = "blue", alpha = 0.7) +
  ggtitle("Emergency department volume Before Imputation") +
  xlab("Volume") +
  ylab("Frequency")

# Plot frequencies after imputation (using imputed train data)
ggplot(imputedTrain, aes(x = factor(`Emergency department volume`))) +
  geom_bar(fill = "green", alpha = 0.7) +
  ggtitle("Emergency department volume After Imputation") +
  xlab("Volume") +
  ylab("Frequency")

```

> Your answer here.It looks like it did not change. 
...
```{r}
shapiro_test_result <- shapiro.test(imputedTrain$PredictedReadmissionRate)

# Print the test result
print(shapiro_test_result)
```

>  Your answer here.The p-value is much smaller than the common alpha level of 0.05, which means we reject the null hypothesis. The null hypothesis of the Shapiro-Wilk test is that the data is normally distributed. Since we reject the null hypothesis, we conclude that the PredictedReadmissionRate does not follow a normal distribution.
...
Can you tell what it did to the distribution? __Look closely__, if not. It's subtle but discernible.It looks like a normal QQ plot for the normilised quantile residuals, In other words distribution looks normalized 
...
> Your answer here.To get more acurate results

```{r}
# Define colors (if needed)
prettyPurples <- c("#E0BBE4", "#957DAD", "#D291BC", "#FEC8D8", "#FFDFD3", "#B19CD9", "#DA70D6")

# Box-Cox Transformation on imputedTest data
bc_test_data <- BoxCoxTrans(imputedTest$PredictedReadmissionRate)
print(bc_test_data)

# Apply Box-Cox transformation
imputedTest$bc_PredictedReadmissionRate <- predict(bc_test_data, imputedTest$PredictedReadmissionRate)

# Plot the histogram of Box-Cox transformed data
ggplot(imputedTest, aes(x = bc_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 fill = prettyPurples[6],
                 color = prettyPurples[2]) +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Predicted Readmission Ratio (Test Data), N = 2,726 hospitals",
       y = "Frequency",
       x = "Predicted Readmission Ratio")

# QQ plot for Box-Cox transformed data
...
```{r}
# Box-Cox Transformation on imputedTrain data
bc_data <- BoxCoxTrans(imputedTrain$PredictedReadmissionRate)
imputedTrain$bc_PredictedReadmissionRate <- predict(bc_data, imputedTrain$PredictedReadmissionRate)
...
# Create a copy of the imputed training set
readyTrain <- imputedTrain

# Drop the original, non-Box-Cox transformed target variable
readyTrain <- readyTrain %>%
  select(-PredictedReadmissionRate)

# Center and scale the training set
readyTrain_scaled <- as.data.frame(scale(readyTrain))

# Create a copy of the imputed test set
...
readyTest_scaled <- as.data.frame(scale(readyTest))

# Print the first few rows of the scaled datasets
head(readyTrain_scaled)
head(readyTest_scaled)
...
> Your answer here the elbow looks like is  around dimession 6 
...
> this indicates that there are underlying patterns in the data that are captured by the first two principal components. There is clustering relative to the hospital return days for pneumonia patients 
...
> your anwser here The within sum of squares (WSS) measures the compactness of clusters in a clustering algorithm like k-means. It is the sum of squared distances between each point and its cluster centroid. Lower WSS values indicate more compact clusters.
...
```{r}
```{r}
# Perform k-means clustering on the readyTrain dataset
kmeansResult <- kmeans(readyTrain, 	
...
# Add a column called Cluster to the imputedTrain dataset based on the clustering result
kmeansTrain <- imputedTrain %>% 
  mutate(Cluster = kmeansResult$cluster) %>% 
  clean_names() # Clean the column names
# Specify which variables to import for PCA
...
# Rename some of the rows in the Cluster column
kmeansTrain %>% 
  # Select relevant variables and 'Cluster' column
  ...
  # Clean column names with title case
  clean_names(case = "title") %>% 
  # Group data by cluster
  group_by(Cluster) %>% 
  # Summarize numeric variables by calculating the mean, removing NAs
  ...
  ggplot(aes(x = Cluster, y = Value, fill = as.factor(Cluster))) +
    ...
    # Use a minimal theme
    theme_minimal() +
    # Customize legend and title appearance
    theme(legend.position = "bottom",
          strip.text = element_text(size=6),
          legend.text = element_text(size = 8),
          legend.title = element_text(size = 10),
          title = element_text(size = 12, color = "maroon")) + 
    # Create facets for each measure with free y-axis scaling
    facet_wrap(~Measure, 
               scales = "free_y",
               ncol = 3)
```
