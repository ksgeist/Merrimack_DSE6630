...
Your object is to work through this Project 2. You will have 31 Questions to answer as you work through this, just as with Project 1. Then, you will be able to choose your own adventure again to perform your own __clinical informatics__ analysis:
...
source(file = "answers_demo2_Alpha.R", echo = FALSE)
```
...
# #Malek
longDataScore2 <- pneumoniaAnalyzeDemo %>%
  rename(NatlComparisonHospitalDays = `ComparedToNational_Hospital return days for pneumonia patients`) %>% 
  select(contains(c("NatlComparisonHospitalDays", "PredictedReadmissionRate", "HcahpsLinear"))) %>% 
...
longDataScore2 %>%  
  ggplot(aes(y = PredictedReadmissionRate, 
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), 
                ...
                se = TRUE, 
                show.legend = TRUE) + 
    labs(title = "Relationship of pneumonia-related hospital readmissions to HcahpsLinear variables",
         x = "Rate or Score",
         y = "Predicted Pneumonia-related Readmissions",
         color = "Pneumonia-related Readmission Compared to National Average") +
    scale_fill_manual(values = c("gold", "cadetblue", "maroon")) + 
...
> #Malek
The relationship that stands out to me the most would be that of the hospital rating as well as the cleanliness. For both of these graphics we see similar shapes. One thing that I am curious of is if the cleanliness or rating actually could potentially play a role in the outcome of patients. 
...
> #Malek
I would say that I do agree with your choice... Probably always best to agree with you in these situations as there is likely a reason why you did what you did haha. Anyways, I like keeping both of them in there as it gives some kind of standard for us to use and compare with. As long as things aren.t becoming redundant, which I don't believe they are in this case, then we should be in good shape.
...
> #Malek
So, zero or near-zero variance deals with data that is pretty much exactly the same and thus the information is redundant and not useful. The problem arises when trying to build our models and all this data is doing is creating noise. Ultimately making our models less helpful. When these are removed the models are better able to focus on the important things. 
...
> #Malek
SurveyResponseRate, healthcare workers given influenza vaccination, and median time patients spent in ED are all variables that are highly correlated missingness to the target variable. I would say that I am not concerned as there are plenty of other variables for the models to build off of that aren't at high levels of missingness. 
...
```{r}
ind <- createDataPartition(pneumoniaAnalyzeDemo$PredictedReadmissionRate,                  ## Remember to put the target here!
                            p = train_prop,                                   ## Remember to use the object you just made from your function!
                            list = FALSE)
...
# Impute missing values using missForest for the test dataset
imputedTest <- missForest(temp_test, 
                          variablewise = TRUE,
                          verbose = FALSE)

# Convert factors back to numeric type in the imputedTest dataset
...
p1 <- ggplot(hf_data, aes(x = NumberOfDischarges)) + 
  geom_histogram(binwidth = 50, fill = "blue", alpha = 0.7) + 
  ggtitle("Number of Discharges Before Imputation")

p2 <- ggplot(hf_data, aes(x = State)) + 
  geom_bar(fill = "blue", alpha = 0.7) + 
  ggtitle("State Frequency Before Imputation")

# Impute missing values
hf_data$NumberOfDischarges[is.na(hf_data$NumberOfDischarges)] <- median(hf_data$NumberOfDischarges, na.rm = TRUE)
hf_data$ExcessReadmissionRatio[is.na(hf_data$ExcessReadmissionRatio)] <- median(hf_data$ExcessReadmissionRatio, na.rm = TRUE)
hf_data$PredictedReadmissionRate[is.na(hf_data$PredictedReadmissionRate)] <- median(hf_data$PredictedReadmissionRate, na.rm = TRUE)
hf_data$ExpectedReadmissionRate[is.na(hf_data$ExpectedReadmissionRate)] <- median(hf_data$ExpectedReadmissionRate, na.rm = TRUE)
hf_data$NumberOfReadmissions[is.na(hf_data$NumberOfReadmissions)] <- median(hf_data$NumberOfReadmissions, na.rm = TRUE)
...
hf_data$State[is.na(hf_data$State)] <- mode_impute(hf_data$State)

# Visualize distributions after imputation
p3 <- ggplot(hf_data, aes(x = NumberOfDischarges)) + 
  geom_histogram(binwidth = 50, fill = "green", alpha = 0.7) + 
  ggtitle("Number of Discharges After Imputation")

p4 <- ggplot(hf_data, aes(x = State)) + 
  geom_bar(fill = "green", alpha = 0.7) + 
  ggtitle("State Frequency After Imputation")
...
> Your answer here. yes it did change 
...
> Your answer here.hile histograms and Q-Q plots are useful tools for visualizing data distributions and checking for normality, they have limitations that make them insufficient on their own to conclusively determine whether a distribution is approximately normal.
...
```{r}
shapiro_test_result <- shapiro.test(heart_failure_filtered$NumberOfDischarges)
shapiro_test_result
```
...
```{r}
# Apply the Box-Cox transformation to the imputedTest data
bc_test_data <- predict(bc_data, imputedTest$PredictedReadmissionRate)
imputedTest$bc_PredictedReadmissionRate <- bc_test_data

# Visualize the Box-Cox transformed data
library(ggplot2)

# Histogram of the transformed PredictedReadmissionRate
ggplot(imputedTest, aes(x = bc_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, fill = "purple", color = "black") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Predicted Readmission Ratio (Test Data)",
       y = "Frequency",
       x = "Predicted Readmission Ratio")

# Q-Q plot of the transformed PredictedReadmissionRate
qqnorm(imputedTest$bc_PredictedReadmissionRate)
qqline(imputedTest$bc_PredictedReadmissionRate, col = "hotpink", lty = 2, lwd = 2)
#Why Transform the Test Data?
...
```{r}
##Make a copy of the imputed training set
readyTrain <- hf_data_train

# Drop the original, non-Box-Cox transformed target from the dataset
readyTrain <- readyTrain %>% select(-ExpectedReadmissionRate)

# Center and scale the whole training set
readyTrain <- as.data.frame(scale(readyTrain))

# Add back the target variable (scaled)
readyTrain$ExpectedReadmissionRate <- hf_data_train$ExpectedReadmissionRate

# Do the same thing for the test data
readyTest <- hf_data_test %>% select(-ExpectedReadmissionRate)
readyTest <- as.data.frame(scale(readyTest))
readyTest$ExpectedReadmissionRate <- hf_data_test$ExpectedReadmissionRate
...
```{r}
# Perform PCA on the scaled training dataset
pca_result <- prcomp(readyTrain[, -ncol(readyTrain)], center = TRUE, scale. = TRUE)

# Calculate the variance explained by each principal component
explained_variance <- pca_result$sdev^2 / sum(pca_result$sdev^2)

# Create a scree plot
scree_plot <- qplot(1:length(explained_variance), cumsum(explained_variance), geom = "line") +
  geom_point() +
...
Perform PCA: Using the prcomp function on the scaled training dataset, excluding the target variable.
Calculate Explained Variance: Compute the proportion of variance explained by each principal component.
Plot Scree Plot: Plot the cumulative proportion of variance explained by each principal component to visualize the 'elbow'.
Run this code in your R environment to generate the scree plot. The point at which the line starts to level off indicates the optimal number of principal components to retain.

Elbow Point: The number of principal components at which the cumulative proportion of variance explained starts to level off is considered optimal. This indicates the number of components that capture most of the variation in the data.
...
> this indicates that there are underlying patterns in the data that are captured by the first two principal components. 
...
pca_data$ExcessReadmissionRatio <- readyTrain$ExcessReadmissionRatio

# Calculate WCSS for different values of k
set.seed(123)
wcss <- sapply(1:10, function(k) {
  kmeans(pca_data[, 1:2], centers = k, nstart = 20)$tot.withinss
})

# Plot the WCSS against the number of clusters
elbow_plot <- qplot(1:10, wcss, geom = "line") +
  geom_point() +
...
kmeansTrain <- imputedTrain %>% 
  mutate(Cluster = kmeansResult$cluster) %>% ## Add the cluster assignment to the dataset.
  clean_names() ## Clean the column names to ensure consistency in naming.

```

```{r}
## Specifies which variables to import for PCA.
pcaImportantVars <- c("overall_hospital_rating",            ## Overall hospital rating
                      "care_transition",                    ## Care transition score
                      "nurse_communication",                ## Nurse communication score
                      "recommend_hospital",                 ## Recommendation score
                      "staff_responsiveness",               ## Staff responsiveness score
                      "communication_about_medicines",      ## Communication about medicines score
                      "doctor_communication",               ## Doctor communication score
                      "discharge_information",              ## Discharge information score
                      "predicted_readmission_rate",         ## Predicted readmission rate
                      "quietness",                          ## Quietness score
                      "median_time_minutes_patients_spent_in_ed",  ## Median time patients spent in ED
                      "hospital_return_days_for_pneumonia_patients",  ## Hospital return days for pneumonia patients
                      "emergency_department_volume",        ## Emergency department volume
                      "compared_to_national_hospital_return_days_for_pneumonia_patients", ## Comparison to national hospital return days for pneumonia patients
                      "cleanliness")                        ## Cleanliness score
## Renames some of the rows in the Cluster column
kmeansTrain %>% 
  ## Selects relevant variables and 'Cluster' column
  select(all_of(pcaImportantVars), cluster) %>%  ## Selects the specified variables and the cluster column.
  rename(`Hospital Return Days Score` = hospital_return_days_for_pneumonia_patients,  ## Renames the column for better readability.
         `Hospital Return Days Nat'l Comparison` = compared_to_national_hospital_return_days_for_pneumonia_patients,  ## Renames the column for better readability.
         `Median Minutes spent in Emergency Dept` = median_time_minutes_patients_spent_in_ed) %>%  ## Renames the column for better readability.
  ## Cleans column names
  clean_names(case = "title") %>%  ## Cleans the column names to title case.
  group_by(Cluster) %>%  ## Groups data by cluster.
  ## Groups data by cluster
  summarise_if(is.numeric, mean, na.rm = TRUE) %>%  ## Summarises numeric variables by their mean within each cluster.
  pivot_longer(-1, names_to = "Measure", values_to = "Value") %>%  ## Converts the data from wide format to long format for plotting.
  ## Creates a plot with the given data
  ggplot(aes(x = Cluster, y = Value, fill = as.factor(Cluster))) +  ## Creates a ggplot object for visualization.
    geom_col(alpha = 0.75) +  ## Adds bar plots with transparency.
    scale_fill_manual(values = myPal) +  ## Manually sets fill colors for the clusters.
    labs(title = "Cluster Means",  ## Adds a title to the plot.
         x = "",  ## No label for the x-axis.
         fill = "Cluster") +  ## Adds a label for the fill legend.
    theme_minimal() + 
  theme(legend.position = "bottom",
...
"Emergency Department (ED) Volume", "Medium ED volume compared to other clusters",
"Shortest Median Time in ED", "Patients spend the least amount of time in the ED",
"High Patient Satisfaction", "Highest scores across all communication aspects",
"Low Readmission Rate", "Well below national average for predicted readmission rate",
...
"High Overall Hospital Rating", "Highest overall rating compared to other clusters",
"Excellent Care Transition", "Highest rating for care transition aspect",

"Strong Nurse Communication", "Highest nurse communication scores",
"Strong Doctor Communication", "Highest score in doctor communication aspect",
"High Care Transition", "Highest rate of Care while transitioning",
"High Queitness", "Reported to have the lowest noise level",
"Cleanliness", "Reported to get the highest cleanliness score",
)
...
dict %>% 
  kable(
    ...
    pack_rows("1: Highest Ranked Emergency Department", 1, 4) %>%
    pack_rows("2: Highest Performing Hospitals", 5, 8) %>%
    pack_rows("3: Best Hospitals for Patient Care ", 9, 12) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> After reading this article https://corporatefinanceinstitute.com/resources/data-science/variance-inflation-factor-vif/#:~:text=Generally%2C%20a%20VIF%20above%204,that%20needs%20to%20be%20corrected. I found that multiple variables have high multicollinearity mainly all the varaibles above 5 have strong multicollinearity so tese 5 need to be addressed Overall.hospital.rating	24.65
...
maxvalues <- max(readyTrain$bc_PredictedReadmissionRate)
maxvalues
minvalues <- min(readyTrain$bc_PredictedReadmissionRate)
minvalues
...
> The values $\alpha$ and $\lambda$ are 1 and 0 which suggest that the model is performing lasso regression without regularization. The model attempts to select features without a penalty. This could lead to overfitting and requires more evaluation to potentially tune for optimal performance.
...
> The most important features wre Recommend hospital, Hospital return days for pneumonia patients, payment for pneumonia patients and Overall hospital rating. This compares to the important feature for regression because overall hospital rating and recommend hospital were high on the VIF list making then have high multicolinearity. For PCA the same two variables contributed the most in Dim 1 and 2.
...
>  After comparing all the numbers in the table below we can see that they are all very similar and close to each other. When comparing the RMSE the R^2 and MAE for the training and testing sets we can see that OLS regression might overfitting and the Elastic Net is underfitting. Below is the break down.

> For OLS regression the RMSE is lower on training data compared to testing data, R^2 is higher on training data compared to testing data, and the MAE is lower on training data compared to testing data. All of these point towards the model starting to overfit.

> For Elastic Net RMSE is higher on training data compared to testing data, R^2 is lower on training data compared to testing data, and MAE is higher on training data compared to testing data. These are all the oppisite of the OLS regression and so they point to the model beginning to underfit.

> Even thought the model is slightly overfitting the I think the OLS regression preforms better because the values of the RMSE, R-squared, and MAE are closer then the values for the Elastic Net model. Becasue of this when given more data that it has not seen it will do better and be stronger at making predictions.

```{r}
prOLS_train <- postResample(pred = predict(mod, newdata = readyTrain), 
                            obs = readyTrain$bc_PredictedReadmissionRate)
prElastic_train <- postResample(pred = predict(elasticMod, newdata = readyTrain), 
                                obs = readyTrain$bc_PredictedReadmissionRate)
...
> To conclude, the OLS regression model is showing signs of overfitting which can be seen in the table above where the metrics are lower on the testing data compared to the training data. However, it still generally outperforms the Elastic Net model in terms of predictive accuracy on unseen data. Despite the overfitting, the OLS regression model can still provide valuable insights and predictions on paitents with pneumonia-related illnesses. To the clients, I would recommend other regularization techniques like Lasso to try to fix the overfitting problem. By fixing this, this would also improve the performance of the model. We could also explore other optons like trying different ensemble methods like boosting and bagging. Another thing could be looking at how the data was collected so we can improve the model by giving it different or better data. We can update the datasets to add any new or relevant data. We can look at domain-specific variables to see if demographics, medical history, and treatment protocols could provide valuable insights into readmission rates. Finally we can check back in with the paitents at a later time to see if any new patterns emerge that were not there intially. Even though the OLS regression model still has some limitations, it still provideding valuable predictions and insights. If we employ the recommendations above I am confident that the models prediction accuracy will further improve and do a better job at meeting our objectives.
