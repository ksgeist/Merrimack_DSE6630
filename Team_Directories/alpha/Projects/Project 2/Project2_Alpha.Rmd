...
```{r}
load("../../../../II. Biomedical & Clinical Informatics/Project/pneumoniaAnalyzefromDemo.Rdata")
...
kmeansTrain %>% 
  ...
  group_by(Cluster) %>% 
  ## Groups data by 'Cluster'  summarise_if(is.numeric, mean, na.rm = TRUE) %>% 
...
dict %>% 
  kable(
    ...
    pack_rows("1: NAME ME!", 1, 4) %>%
    pack_rows("2: Highest Performing Hospitals", 5, 8) %>%
    pack_rows("3: NAME ME!", 9, 12) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
