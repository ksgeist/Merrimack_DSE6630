...
>  After comparing all the numbers in the table below we can see that they are all the exact same. When comparing the RMSE the R^2 and MAE for the training and testing sets we can see that OLS regression and the Elastic net are the same due to the best alpha and lambda values for the Eleastic Net are both 0 making it an OLS regression

> The model performs equally well on both the training and testing datasets, it suggests that the model has found  the best parameters to capture any  underlying patterns the data might have without overfitting or underfitting. This is the best case scenario and indicates that the model well preform well to unseen data. Even though this is what we want, it doesn't necessarily mean that the model is optimal. We should stillconsider other evaluation metrics and perform more cross-validation to ensure the reliability and robustness of the model.
