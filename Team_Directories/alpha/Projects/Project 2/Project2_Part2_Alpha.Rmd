...
train <- HEART_FAILURE_AnalyzeDemo[ind, ]
test <- HEART_FAILURE_AnalyzeDemo[-c(ind), ]
```
...
```{r}
# > #Malek
imputedTest <- imputedTrain$ximp %>% 
  ## Put the original State and target variables back on
  mutate(State = test$State,
         ExpectedReadmissionRate = test$ExpectedReadmissionRate) %>% 
  ## Convert the factors back to character type...
  mutate_at(c("Emergency.department.volume",
              "ComparedToNational_MRSA.Bacteremia",
              "ComparedToNational_Composite.patient.safety",
...
```{r, echo = FALSE, warning=FALSE, message=FALSE}
ggplot(imputedTrain, aes(x = ExpectedReadmissionRate)) +
...
```{r}
bc_data <- BoxCoxTrans(imputedTrain$ExpectedReadmissionRate)
bc_data
```
...
```{r, echo = TRUE}
imputedTrain$bc_ExpectedReadmissionRate <- predict(bc_data, imputedTrain$ExpectedReadmissionRate)
```

```{r, echo = FALSE, message = FALSE, warning=FALSE}
ggplot(imputedTrain, aes(x = bc_ExpectedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
...
qqnorm(imputedTrain$bc_ExpectedReadmissionRate)
qqline(imputedTrain$bc_ExpectedReadmissionRate, 
...
returnHospitalComparison <- factor(readyTrain$ComparedToNational_Hospital.return.days.for.heart.failure.patients)
...
"Return hospital days due to Heart Failure", "Below national average and lowest return days overall", 
...
mod <- lm(bc_ExpectedReadmissionRate ~ ., data = readyTrain)
par(mfrow=c(2,2))
plot(mod, 1)
plot(mod, 2)
plot(mod, 3)
...
```{r, echo = FALSE}
## Fit a linear model
mod <- lm(bc_ExpectedReadmissionRate ~ ., data = regressionTrain)
...
## Calculate the RMSE
RMSE(predictions, regressionTest$bc_ExpectedReadmissionRate)

maxvalues <- max(readyTrain$bc_ExpectedReadmissionRate)
maxvalues
minvalues <- min(readyTrain$bc_ExpectedReadmissionRate)
minvalues
