...
```{r}
# #Malek
filtered_mortality_sf2 <- mortality_sf2 %>% 
  filter(totalN > 1000)

filtered_mortalityMonthly_sf2 <- filtered_mortality_sf2 %>% 
  group_by(COUNTY_NAM, Year, Month) %>% 
  summarize(percMortality = 100 * (sum(totalDeaths, na.rm = TRUE) / mean(totalN, na.rm = TRUE)), .groups = 'drop')

breaks <- pretty(range(filtered_mortalityMonthly_sf2$percMortality, na.rm = TRUE), n = 10)

tm_shape(filtered_mortalityMonthly_sf2) + 
  tm_polygons(
    col = "percMortality", 
    palette = get_brewer_pal(palette = "BuPu", n = 10, plot = FALSE),
    breaks = breaks,
...
> #Malek - So essentially this new map should be much cleaner than the first one. By only having counties with a population greater than 1000 I'm definitely getting rid of a lot of unecessary noise. With the decrease in noise, we should hopefully experience an increase in accuracy. The mortality rates are now more stable and thus less impacted by the smaller population sizes. Personally, I feel as though the map is much easier to read now and we can come to many more definitive conclusions.
...
> #Malek - In order to make sure the prediction is indeed robust it's important to evaluate a few things including linearity, normality, and independence. For the most part, this is not really a model I would like to put my name beside. It appears that there is room to improve and I definitely think it can be done. 
...
> #Malek - The first model was too simple and did not meet the necessary criteria I was looking for. The following random forest is obviously more complex while also diving much deeper than a linear regression can.

```{r, error=TRUE}
##Malek

deaths2020 <- fromJSON("https://data.chhs.ca.gov/datastore/odata3.0/579cc04a-52d6-4c4c-b2df-ad901c9049b7?$format=json")
deaths2020 <- deaths2020$value

deaths2020 <- deaths2020 %>%
  mutate(Month = as.numeric(substr(month, 6, 7)),
         Year = as.numeric(substr(month, 1, 4)),
         totalDeaths = as.numeric(total_deaths)) %>%
  select(County, Year, Month, totalDeaths) %>%
  rename(COUNTY_NAM = County)

merged_data <- mortalityMonthly_sf2 %>%
  left_join(deaths2020, by = c("COUNTY_NAM", "Year", "Month"))

set.seed(15)
rf_model <- randomForest(percMortality ~ COUNTY_NAM + Year + Month + totalDeaths, data = merged_data, na.action = na.omit)

newdata$propDeaths <- predict(rf_model, newdata = newdata)

predictions <- predict(rf_model, newdata = merged_data)
rmse <- sqrt(mean((merged_data$percMortality - predictions)^2, na.rm = TRUE))
print(paste("RMSE of the Random Forest model:", round(rmse, 2)))

newdata$isPredicted <- "1"

temp <- deaths2020 %>%
  group_by(Year, Month, COUNTY_NAM) %>%
  summarise(propDeaths = sum(totalDeaths, na.rm = TRUE) / sum(totalN, na.rm = TRUE) * 100) %>%
  mutate(isPredicted = "0")
...
temp %>%
  ...
  scale_size_manual(values = c(0, 3)) +
  labs(
    title = "Percent Deaths per Capita",
    y = "% of deaths per capita", 
    subtitle = "Cumulative for all California counties",
    color = "Year",
    linetype = "Year"
...
> #Malek - The random forest caused a significant increase in accuracy and overall improval. This is something that I now have much more confidence in as it is inherently more reliable than the linear regression. The map also provides a way to visualize as well as analyze through a different lens.
...
> #Malek - I would probably say that it doesn't make the most sense to consider this a fair model without accounting for sex. I feel as though sex could provide some valuable insight which is due to some of the inherent biological differences between sex. Additionally, there may be different behaviors for the different sexes which could also definitely have an impact. 
...
> #Malek - So this trend where the female population on average has a significantly lower proportion of deaths due to accident or injury is quite compelling and shows exactly why it is important to map these out seperated by sex. Another trend that I notice is that both male and female tend to increase/decrease at the same time which could be indicating environmental or societal factors. The next thing I would like to point out is how there is a very big drop off in May 2020 for males. My best assumption for this would be due to covid. I'm assuming that the male population had more jobs that could cause death and injury but during covid these jobs slowed down and people were just outside getting injured less in general and thus, less male workers were exhibiting death due to injury or accident which caused a steep drop off in the graph.
...
> #Malek - I would say that there is moderate evidence for proportion of deaths due to accident and injury being changed as a result of the stay-at-home order. However, if I want to have stronger evidence some of the things I could do would be to do some statistical analysis like a t test or time series analysis. 
...
correlation_data <- injDeaths %>%
  select(femaleMaleRatio, deathsInjuriesProportion) %>%
  filter(!is.na(femaleMaleRatio) & !is.na(deathsInjuriesProportion))

correlation <- cor(correlation_data$femaleMaleRatio, correlation_data$deathsInjuriesProportion)

print(paste("Correlation between femaleMaleRatio and deathsInjuriesProportion:", round(correlation, 4)))
```
...
> #Malek - I would say that this is probably sufficient enough just to make a correction in the models. The question I would ask though is... what is a correlation coefficient relative to to consider it "high". I do think though that with some tweaking to the models this would be good enough for shareholders
...
> #Malek - I would say that the scatterplot does not agree since it has an upward slope and positive but the CC I got was negative
...
```{r}
source("../../../../III. Public Health & Epidemiology/Project/calcSplitRatio.R")
...
>  #Malek - So my understanding of what is happening is you start with an initial model, find the residuals, then build a model for the residuals, and update the model based on the residuals model. In each iteration this is done until the model gets better and better. The link that helped me understand GBM more in depth can be found here: https://www.frontiersin.org/journals/neurorobotics/articles/10.3389/fnbot.2013.00021/full

#### Question 20 [2 points]:

What hyperparameters can one tune in a GBM? **Hint**: Make sure to explain at least 4! 

> #Malek - This type of question is very fresh in my mind as I am currently taking the artificial intelligence and machine learning course. 4 hyperparameters that can be tuned in a GBM include the number of trees to make sure we are getting the best tuning possible and not overfitting, shrinkage which defines how much each tree has an impact on the final model, tree depth to control the depth of each tree with deeper trees being more complex but more at risk for overfitting, and lastly minimum number of observations which determines minimal number of leaves in trees. 
...
> Your answer here.mapping the geographic effect is valuable for understanding relationships and making informed decisions.
...
> Your answer here.GBM predicts mortality rates for all counties.
Choropleth maps are created, visualizing predicted mortality rates across the entire region.
Interpretation:
We interpret the model coefficients and feature importances in the context of geography.
Regions with high or low predicted mortality rates are identified, helping inform public health policies and resource allocation.
...
> Your answer here.o see a stronger variable importance, we might need:
More Data: Collecting additional data over a longer time frame could reveal more nuanced patterns.
Contextual Factors: Consider environmental context (e.g., urban vs. rural) and specific age or sex groups.
Behavioral Changes: Analyze changes in behavior due to pandemic-related restrictions (e.g., reduced mobility, altered work patterns).
...
> Your answer here.This implementation reduced Covid cases? This article support this question 
https://www.cdc.gov/mmwr/volumes/69/wr/mm6935a2.htm?s_cid=mm6935a2_w

#### Question 27 [1 point]:
Formulate a hypothesis based on your question.

> Your answer here.The implementation of stay-at-home orders in California significantly reduced the spread of COVID-19, leading to decreased hospitalizations and deaths.

We can test this hypothesis by analyzing relevant data, comparing infection rates before and after the orders, and assessing the impact on public health outcomes. 
...
> Your answer here.We collect daily COVID-19 data for California, including confirmed cases, hospitalizations, and deaths.
Information on stay-at-home order implementation dates is included.
We handle missing values and normalize features.

>We divide the data into pre-stay-at-home order and post-stay-at-home order periods.
Daily averages for cases, hospitalizations, and deaths are calculated.
Line charts show trends over time, with stay-at-home order dates overlaid.
...
```{r, echo = TRUE}
 # Your code here. # Your code here. Add more chunks if needed / desired.
 # Example code to create a heat map
#library(ggplot2)
#library(dplyr)


 # Filter data for pre and post stay-at-home order
#pre_lockdown <- Covid_cases %>% filter(date < as.Date("2020-03-19"))
#post_lockdown <- Covid_cases %>% filter(date >= as.Date("2020-03-19"))

 # Aggregate data by county
#pre_lockdown_agg <- pre_lockdown %>% group_by(county) %>% summarize(total_injuries = sum(injuries), total_fatalities = sum(fatalities))
#post_lockdown_agg <- post_lockdown %>% group_by(county) %>% summarize(total_injuries = sum(injuries), total_fatalities = sum(fatalities))

 # Create heat maps
#ggplot(pre_lockdown_agg, aes(x = county, y = total_injuries)) +
#  geom_tile(aes(fill = total_injuries), color = "white") +
#  scale_fill_gradient(low = "yellow", high = "red") +
#  labs(title = "Traffic-Related Injuries Pre-Lockdown", x = "County", y = "Total Injuries")

#ggplot(post_lockdown_agg, aes(x = county, y = total_injuries)) +
#  geom_tile(aes(fill = total_injuries), color = "white") +
#  scale_fill_gradient(low = "yellow", high = "red") +
#  labs(title = "Traffic-Related Injuries Post-Lockdown", x = "County", y = "Total Injuries")
...
> Your answer here.After visualizing the data, there appears to be a noticeable reduction in traffic-related injuries and fatalities post-lockdown compared to pre-lockdown. This conclusion is drawn from the heat maps which show lower intensity colors (representing fewer incidents) in most counties after the stay-at-home order was implemented, indicating a possible effect of reduced vehicular traffic due to the lockdown.
