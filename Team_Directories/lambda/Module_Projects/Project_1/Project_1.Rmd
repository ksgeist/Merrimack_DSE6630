...
> The reason for this choice is that I want to have full control over the encoding process. This allows me to make my own encoding decisions depending on the specific analysis/modeling
...
```{r, fig.cap = "Figure 1. Readmission Rates vs. readmitted hospital LOS", fig.width=8, fig.height=6, echo = FALSE, include = FALSE}
longTargets %>% 
...
longTargets %>% 
  ggplot(aes(x = Value, y = NatlComparisonHospitalDays, fill = NatlComparisonHospitalDays)) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.1) +  #  boxplot layer
  labs(title = "Which target to use?",
       subtitle = "By readmitted LOS in hospital compared to national average",
        x = "",
        y = "Rate or Score",
...
> The Excess Readmission Ratio and the Predicted Readmission Rate appear the most distinct to me
...
> The observed rate can be more noisy in terms on the data, the excess readmission ratio accounts for expect vs predicted readmissions making it more fair for hospital comparisons. I still would choose exess readmission ratio here.
...
longScore %>%  
  ggplot(aes(y = ExcessReadmissionRatio,  ## changed line
             x = Value)) +
    ...
    geom_smooth(aes(fill = as.factor(NatlComparisonHospitalDays), color = as.factor(NatlComparisonHospitalDays)), 
                ...
                show.legend = T) + 
    ## title changed
    labs(title = "Relationship of Excess Readmission Ratio for pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
...
> Higher communications scores are clearly associated with lower excess readmission rations. This makes sense logically, better communication results in better patient understanding and better outcomes/reduced readmissions. PredictedReadmissionsRate shows these relationships quite well.
...
> A couple of the most compelling relationships here are definitely doctor/nurse communication and discharge information plots. They all show relatively strong negative relationships for readmissions especially for hospitals with rated "worse" compared to the national average.
One of the outstanding questions I have is a bit more theoretical. To what extent could patient satisfaction metrics hold deeper meaning for less observable aspects of the hospital, such as accountability, ethics, a commitment to patient centered care? And, could these more cultural hospital traits be linked to these measurable outcomes like readmission rates.
...
> Yes, I generally agree with the target variable choice. Since the prompt revolves around readmission rates, choosing a target variable that directly models those outcomes ensures that we are anchoring our research to properaly answer the right questions. We can count on reliable comparisons with this target variable as it is more wholistic for our goal. 
...
> A potential bias that could be present is existing disparities outside of the hospital. Meaning, certain hospitals could be underfunded or those serving underserved populations could already have structurally higher rates due to factors outside of the collected data.
...
dat2Analyze <- dat2Analyze %>% 
  ## droppping any unused/not needed columns
  select(-ExpectedReadmissionRate, 
         -ExcessReadmissionRatio,
         -observed_readmission_rate, 
         -contains(c("Sample_", "NumberOfPatients")),
         -NumberSurveysCompleted) %>% 
  ## renaming long/unclear cariable names for easier reference in code
  ...
  ## moving the target variable to the first column for convenience
  select(PredictedReadmissionRate, everything())

## removing column prefixes/suffixes to simplify the names
...
> Features with zero or non zero variance are variables that dont change much across hospitals in the dataset. Meaning, these kinds of metrics cant help our model tell hospitals apart at all. Keeping these kinds of features can add noise to the model because they carry less useful information.
...
> Unique identifiers like that dont hold predictive value for the model, they are labels. If you were to keep it and run the near zero function it wouldnt be flagged because it would technically have high variance because its a unique ID.
...
> This problem arises when the number of features (P) is greater than or close to the number of obervations, N. This can make it easy for the model to over fit because its learning patterns with a smaller amount of data compared to features, AKA the patterns arent generalized properly.Here we've removed redundant and unimportant features and have been properly cautious with preprocessing, leaving behind a managable amount of variables with the many thousands of data entries in the data set, so we are not risking dimensionality issues.
...
missing_counts <- colSums(is.na(dat2Analyze))

# how many columns have at least one NA
num_missing_cols <- sum(missing_counts > 0)
...
paste0(round(percent_missing_cols, 2), "% of features have SOME missing data.")
```

> All columns have at least one NA
...
```{r}
# Count the number of complete observations (rows with no missing values)
num_complete_obs <- sum(complete.cases(dat2Analyze))
...
> It tells us the number of observations where both variables Score_Perioperative pulmonary embolism or deep vein thrombosis rate	and Score_MRSA Bacteremia are missing. Yes, it feels like a high number of missingness. 
...
> MRSA Bacteremia, ComparedToNational_MRSA Bacteremia, Postoperative respiratory failure rate, medicare spending per patient are the top four highest correlations of missingness
...
> Yes, the drop making the missingness issue feel more important, this reduction shrinks our dataset by a good amount. It is definitely something to keep in mind as we move forward
...
> We split our data into train and test sets so we can build our model on the training data set, and test how well it works on unseen data for the model, the test data. Data leakage happens when the model gets access to the testing data during training, which can lead to unreliably high performance because the model is tested on data it was already come into contacted with.
...
calcSplitRatio <- function(p = NA, df) {
  ...
  ## if p is null then estimate p as the num of columns in the set minus 1
  if(is.na(p)) {
    p <- ncol(df) -1   ## COMMENT HERE
  }
  
  ## calculate the recommended num of testing samples
  test_N <- (1/sqrt(p))*nrow(dat2Analyze)
  ## convert the count into a proportion of the total dataset
  test_prop <- round((1/sqrt(p))*nrow(dat2Analyze)/nrow(dat2Analyze), 2)
  ## calculate the training proportion
  train_prop <- 1-test_prop
  
  ## print the ideal ratio
  print(paste0("The ideal split ratio is ", train_prop, ":", test_prop, " training:testing"))
  
  ## return the actual value proportion to use
  return(train_prop)
}
...
> 0.82:0.18, Yes, it is close. This is the general split for a well proportioned data set. 
...
> MSE is used for continuous variables, it measures how far the imputed values are from the true values. PCF is used for categorical variables, it shows the proportion of incorrectly imputed values out of the values that were imputed.
...
> MICE is an iterative method used for handling missing data. It works by modeling each variable with missing values as a function of the other variables in the dataset. Each variable is imputed in turn, using the most recently imputed values of the others, hence why its called chained equations. It incorporates uncertainty into the imputation process by generating multiple imputed datasets.
...
```{r}
# numeric variable before and after imputation
summary(train$`Healthcare workers given influenza vaccination`)  # Before
summary(imputedTrain$`Healthcare workers given influenza vaccination`)  # After

# check a categorical variable before and after imputation
table(train$`ComparedToNational_Healthcare workers given influenza vaccination`, useNA = "always")  # Before
table(imputedTrain$`ComparedToNational_Healthcare workers given influenza vaccination`)  # After
```

> The numeric variable showed minimal change after imputation. The mean shifted slightly and the median decreased by just 1 point. The range and quartiles were nearly identical. The categorical variable the NAs went down and no new categories were introduced, meaning all the NAs were imputed
...
> If a category exists in the test but not the train, we should assign it a default value such as 0 or the mean/mode. If a category exists in the train but not in the test thats fine it just will not appear during testing.
...
> They are subjective, meaning they could look normal to someone and not to another, its all about interpretation here. They dont provide a numeric threshold, and could appear normal but actually still be skewed.
...
> since the p value is well below 0.05, we would reject the null hypothesis that the data come from a normal distribution.
...
> We are building a model that will learn patterns from the transformed training data, so if we didnt apply the same transformation to the test data we would be giving it unscaled and differently distributed values. The value of lambda determines how the transformation reshapes the data, so using a different lambda for the test set would apply a different transformation function, leading to inconsistent model interpretation.
...
ggplot(readyTest, aes(x = bc_PredictedReadmissionRate)) +
  geom_histogram(alpha = 0.6, 
                 ...
                 color = "skyblue") +
  theme_minimal() +
  labs(title = "Box-Cox Transformed Predicted Readmission Rate (Test Set)",
       y = "Frequency",
       x = "Predicted Readmission Ratio")

qqnorm(readyTest$bc_PredictedReadmissionRate)
qqline(readyTest$bc_PredictedReadmissionRate, 
       col = "skyblue", lty = 2, lwd = 2)
```
...
> The compared to national_ variables have the highest potential for this because they can be built off of some of the same data

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> Nurse communication, staff responsiveness, and hospital return days for pneumonia patients
...
> Yes, especially in the variables overall hospital rating and recommend hospital, as they are both well over 10 VIF.
...
> Nurse communication, staff responsiveness, and hospital return days for pneumonia patients. They are all more positive correlations, there are no negatively correlated variables for our target.
...
> The percent of variance explained by PC1 is 22.41%, The percent of variance explained by the final principal component (PC18) is 1.06%. The variance explained by PC1can generally be considered moderate
...
> the optimal number of principal components appears to be around 4 or 5.
...
> its a geological term, refering to debris that accumulates at the bottom of a hill/cliff/slope. The plot resembles a steep cliff that tapers off quickly into a more gentle slope.
...
> composite patient safety, Perioperative.pulmonary.embolism.or.deep.vein.thrombosis.rate, doctor communication
...
> the top contributers are mostly derviced from the satisfaction survery. the largest variation among hosdpitals in the data is driven by patient experience.
...
> k=6 seems to have the best explanatory power. There is a decent separation between clusters with the least amount of overlap. Yes, it is subjective and tough to tell by just visuals.
...
> it measures the total variance within each cluster. here we are seeing how the wss decreases as we increase k. the optimal is still roughly k=4, where the rate of decrease flattens out.
...
> yes k=2 is suprising compared to the elbow method which looked to be around 3 or 4.
...
"Communication About Medicines", "Hospitals in this group have better scores in patient communication about medicines, which can lead to improved adherence and outcomes",
"Doctor Communication", "Hospitals in this group consistently have better doctor communication scores, contributing to better patient understanding and care coordination",
"Composite Patient Safety Score", "This cluster shows slightly higher safety scores, suggesting better prevention of hospital-acquired complications",
"Cleanliness", "Scores for cleanliness are consistently higher in these hospitals, which may reflect better infection control practices",
"Quietness", "Patients at these hospitals report a quieter environment, often associated with better rest and recovery",
"Postoperative Respiratory Failure Rate", "This group has significantly lower postoperative respiratory failure rates, suggesting stronger procedural care and recovery protocols"
)
...
dict %>% 
  kable(
    format = "html",
    caption = "Table 9. Hospital segmentation analysis: two broad groupings identified.") %>%
    pack_rows("1: Highest Performing Hospitals", 1, 4) %>%
    pack_rows("2: Hospitals Needing Improvement", 5, 8) %>%
    kable_styling(bootstrap_options = c("striped", full_width = F)
  )
```
...
> random forest and multinomial logistic regression. to know whether or not my predictions were robust i would look for consistent performance in cross validation, low generalization error, and alignment with domain knowledge
...
> the OLS regression model seems to meet its assumptions. there are no glaring violations of linearity of normality. this model is stable enough for preliminary interpretation
...
> yes i do, the relationship between predictors and the transformed target appears linear which is suffiicient for elastic net to proceed.
...
> we used adjusted instea because it accounts for the number of predictors in the model, penalizing addition of irrelevant variables.
...
> the rmse tells us how far off our models predictions are from the actual values. the r^2 and adjusted r^2 still give us useful info, our model explains roughly 40% of the variance in predicted readmission rates.
...
> we are doing a 5 fold cross validation repeated five times using grid search to tune the elastic net model. Meaning, we are evaluating many combos of alpha which control the mix of l1/l2 penalties and lambada which controls the degree of shrinkage to fine the best performing model. cross validation helps prevent overfitting and ensures the model generalizes well.
...
> they are both 0 meaning that no regularization was applied in the optimal model. the elastic net essentially default to an ols regression.
...
> the most important feature in the elastic net model is hospital return days for pneumonia patients and medicare spending per patient. this aligns well with the ols resultswhere these same variables had the largest absolute coefficients.
...
> pca and regression measure importance in different ways. pca reflects variance explained not predictive power. thats why some variables like communications about medicines rank higher in pca but lower in regression models
...
> both elastic net and ols regression models have identical test set performance and similar training performance. since the elastic net applied regularization and still performed same as ols, it is the better model because it has less risk of overfitting.
...
> elastic net is the better choice over ols, it offers equal performance while reducing overfitting. but, the low r^2 suggests missing predictors or non linearity. I would recommend testing out non linear models to improve predictability.
...
```{r}
# cross-validation
lasso_ctrl <- trainControl(method = "repeatedcv", number = 5, repeats = 5)

lasso_grid <- expand.grid(alpha = 1, lambda = seq(0, 5, length.out = 50))

# train
lasso_mod <- train(bc_PredictedReadmissionRate ~ .,
                   data = readyTrain,
                   method = "glmnet",
                   tuneGrid = lasso_grid,
...
```
```{r}
# coefficients from the trained LASSO model
lasso_coeffs <- coef(lasso_mod$finalModel, lasso_mod$bestTune$lambda)
lasso_coeffs_df <- as.data.frame(as.matrix(lasso_coeffs))
names(lasso_coeffs_df) <- "Coefficient"
lasso_coeffs_df$Feature <- rownames(lasso_coeffs_df)

# Filter out zero coefficients and intercept
lasso_plot_data <- lasso_coeffs_df %>%
  filter(Coefficient != 0, Feature != "(Intercept)") %>%
  arrange(desc(abs(Coefficient))) %>%
  mutate(Feature = fct_reorder(Feature, abs(Coefficient)))

ggplot(lasso_plot_data, aes(x = Feature, y = Coefficient, fill = Coefficient)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  scale_fill_gradient2(low = "darkblue", mid = "white", high = "darkred", midpoint = 0) +
  labs(title = "Top Coefficients from LASSO Model",
       x = "Feature",
       y = "Coefficient Value")
```

The bar plot shows the top coefficients selected by the LASSO model (with alpha = 1 and lambda = 0), ranked by importance.
Notes:
- Hospital return days for pneumonia patients is by far the most important predictor.
- The most important predictors match those found in both the elastic net and old models, though LASSO tends to zero out weaker predictors, improving interpretability.
- Since the LASSO used lambda = 0, it  acted as an unregularized regression. so its performance is nearly identical to ols and elastic net in the prior results.
...
# alpha = 0 (Ridge), lambda varies
ridge_grid <- expand.grid(.alpha = 0, 
                          .lambda = seq(0, 5, length.out = 25))
...
ridge_importance <- varImp(ridgeMod)$importance
ridge_importance$Feature <- rownames(ridge_importance)
...
ridge_importance %>%
  ...
  ggplot(aes(x = Feature, y = Overall, fill = Overall)) +
  geom_col() +
  coord_flip() +
  scale_fill_gradient(low = "#DCCAE3", high = "#C00E50") +
  theme_minimal() +
  labs(title = "Top Features from Ridge Regression",
       x = "",
       y = "Importance (Overall)")
```

- Hospital.return.days.for.pneumonia.patients is the most influential predictor with the highest importance score, just as it did in the elastic net and ols models
- Medicare.spending.per.patient and Death.rate.for.pneumonia.patients follow as strong predictors, suggesting financial and outcome-related variables are critical in predicting readmissions.
- State, Venous Thromboembolism Prophylaxis, and Quietness have moderate impact, these possibly capture systemic quality-of-care indicators

The Ridge model reinforces previous findings but distributes weight more evenly across a larger set of variables (no zeroing out), emphasizing its strength in controlling multicollinearity while keeping all predictors



## Explanations and Conclusions

I chose to extend the analysis using LASSO and Ridge regression because they each address critical limitations of the OLS model in different but complementary ways.

### LASSO (Least Absolute Shrinkage and Selection Operator)

LASSO was selected to

- Perform feature selection by shrinking some coefficients exactly to zero, simplifying the model.
- Help identify the most influential predictors of pneumonia readmission rates.
- Be more interpretable for stakeholders by filtering out noise and focusing on high-impact variables.

This is especially helpful in healthcare datasets where many predictors may be weakly informative, and we want to isolate those that can drive a real data driven change

### Ridge Regression

Ridge was selected to

- Control multicollinearity more aggressively than OLS or Elastic Net by shrinking all coefficients but keeping them in the model
- Preserve all predictors which is ideal if we want a comprehensive view of all contributing factors without discarding any potentially important signal.
- Offer better generalization to unseen data, especially if many predictors are correlated.

Using both of these allowed us to compare the below

- Sparsity vs. shrinkage: LASSO zeroes out features, while Ridge distributes weight across correlated predictors.
- Interpretability vs. robustness: LASSO helps explain results clearly; Ridge ensures stable predictions.

Both of these together give a holistic view of the trade offs between model complexity, interpretability, and predictive power, which leads to better data backed decisions
