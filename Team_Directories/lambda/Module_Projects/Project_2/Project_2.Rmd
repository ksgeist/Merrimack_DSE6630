...
> * `cnty19_1.cpg`-- character encoding used for the .dbf file
* `cnty19_1.dbf` -- the attribute table dataframe
* `cnty19_1.prj` -- the projection file
* `cnty19_1.shp` -- the main geometry file with the spatial shapes
* `cnty19_1.shp.xml` -- metadata file
* `cnty19_1.shx` -- shape index file to finf the geometry in the shp file
...
> it prints 69 times because the shapefile contains 69 geographioc features, not just the california counties. so the 69 features include the 58 counties and 11 additional geographic parts
...
> the bounding box defines the spatial range that encloses the 69 features in the shp file. to zoom in you would narrow the bounding box by adjusting the values to a smaller range to focus the area you watn to focus. to zoom out you would expand the range of the bounding box by making the values higher to show a broader area
...
> WGS 84 / pseudo-mercator. its a projected crs that converts the earths surface into a flat map using a mercator projection. if you switched to a different projection it will reshape the map potentially preserving area instead of shape. this could affect visual appearance and analytical accuracy depending on what the task/goal is
...
```{r}
# Filter to just the 3 counties that border Oregon and store as map1
map1 <- cali_sf %>%
  filter(COUNTY_NAM %in% c("Del Norte", "Siskiyou", "Modoc")) %>%
  ggplot() +
  geom_sf(fill = "lightblue", color = "gray20", linewidth = 0.25, alpha = 0.8) +
  coord_sf(expand = FALSE) +
  labs(
    title = "Northernmost California Counties Bordering Oregon",
    caption = "Source: California State Geoportal"
  )
...
```{r}
## updated cities data frame with San Jose included
cities <- data.frame(
...
ggplot(data = cali_sf) +
  ...
  scale_size_manual(values = c(capital = 6, major = 3), guide = "none") +
  scale_color_manual(
    values = skittles[1:5],  ## 5 colors for 5 cities
    name = "City"
  ) +
  labs(
...
## Start with the `totalDeaths` table, which contains monthly death counts per county
mortality_sf <- totalDeaths %>% 
  ## Rename "County" column to "COUNTY_NAM" so it matches the column name in the shapefile
  rename(COUNTY_NAM = County) %>%
  ## Group data by county, year, and month so we can get monthly death totals by county
  group_by(COUNTY_NAM, Year, Month) %>% 
  ## Sum death counts within each group to get the total deaths per county-month-year
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  ## Join the mortality data with the California counties shapefile using COUNTY_NAM as the key
  ## "many-to-many" accounts for counties that may appear in both tables multiple times 
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  ## Convert the joined result back into a spatial object so it can be used in mapping and spatial analysis
  st_as_sf() %>% 
  ## Remove rows associated with islands to declutter the map
  filter(is.na(ISLAND))
```
...
> it does a spatial join, meaning it joins two sf object based on their geographic relationships. you couldnt use st_join instead of full_join because the mortality data isnt a spatial object and because st_join requires both inputs to be sf object w/ valiud geometries.
...
> Because mortality_sf includes repeated counties over multiple time periods
...
> 58. after filtering out islands and duplicates only the standard 58 counties remain
...
> the total number of deaths looks highest in LA county but this is likely due to population size. the san diego area and around la county also have a darker shade showing more deaths
...
key <- "5e0e7fb1b9bd20f7bdd91d51623eb89f1a5b78e8"
```
...
> `DP1_0048C` -- count of people aged 65 years and older in a county
`DP1_0048P` -- percentage of the total population that is aged 65 years and older in that county
...
> the census df includes demographic data from the 2020 census for all 58 counties in california
...
> to use a different api in the function you would change the base url and query parameters to match whatever the new APIs structure is as well as update variable codes and col names as needed.
...
mortality_sf <- mortality_sf %>%
  mutate(FIPS = paste0("06", COUNTY_FIP))  # California FIPS prefix is 06

census <- census %>%
  mutate(FIPS = paste0(STATE, COUNTY))  # These are already in your census df

# Now join on FIPS
mortality_sf2 <- left_join(mortality_sf, census, by = "FIPS")
```
...
> the percent mortality showws some counties had high per capita death rates in 2017-19, not just in 2020. this could suggest localized mortality issues unrelated to covid
...
> the 2020 mortality data includes 8 months, making annual percent mortality seem lower than previous years. to fix this you could use the average percentage to fill in the gaps
...
> this updated map shows a more expected pattern of death increase during the early parts of the covid pandemic.the southern counties show a slight increase as expected and shows the percentage more accurately.
...
> no the regression prediction isnt robust. the q-q plot shows residuals that deviate significantly from normal especially in the tails. these violations suggest that predictions from the model could be unreliable
...
> youre calculating neighbors on just the distinct counties because the spatial relationships dont change over time. calculating neighbors on repeated rows would bring in redundant and unnecessary computations which is also a resource waster.
...
> the sarmodel is a better fit to the data. the anova test shows a statistically significant improvement in model fit when adding the spatial lag term.
...
> sarmodel shows that mortality in one county is influenced by the mortality in its neighboring counties because the spatial lag coefficient is highly significant. the summary statistics tell us that the model explains around 73% of the variation in monthly percent mortality
...
> this model shows improvement over the ols model. even though the residuals seem more evenly distributed there is still some deviation in the tails, meaning the model is generally reliable but should be cautiously interpreted.
...
# Percentages
train_pct <- round((train_n / total_n) * 100, 2)
test_pct <- round((test_n / total_n) * 100, 2)

cat("Training set percentage:", train_pct, "%\n")
cat("Testing set percentage:", test_pct, "%\n")
```

> the calculated split is very close to the usual 80 20 split, at 81.82/18.18

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> by excluding the 2020 data from the training set the model could fail to account for any unexpected events or changes like the covid pandemic, meaning this limits the models ability to generalize 2020 conditions due to the extenuating circumstance
...
> fitting a model with OOB error means that during random forest training each tree is built using a bootstrap sample, and the not used or OOB data serve as a validation set for that tree. we do this to get an estimate of model performance without having a separate validation set.
...
> mtry is the number of predictors randomly selected at each split in a decision tree within a random forest, the default is usually 1/3 of the number of predictors. here i believe its overridden to search values 1 through 3 to explore different performance variations across splits.
...
> the best mtry value is 1 because it gave the lowest RMSE and the highest r squared (though thats a relatively low value). Yes r squared can be negative when the model performs worse than just predicting the mean itself. we cant directly compare the rmse of this model with the one in 16c because the predictors were centered and scaled but the target variable was not.
...
> No there are no negative r squared values in these results, suggesting the model generalizes better and avoids overfitting. the best performing mtry value is again 1, with the lowest rmse and the highest r squared
...
> the model seems to be underfitting a bit, the test rmse and test mae are higher than the training rmse. the low r squared and high errors indicate that the model explains little variance in mortality.
...
> the plot shows the model consistently underpredicts mortality with predicted values tightly clustered around zero regardless of actual values. this suggest that the model fails to capture the true variablility in mortality 
...
```{r}
# read in as downloaded csv because was having access errors even with the updated github push
caliDeaths <- read.csv("~/Desktop/deathCount.csv")
...
caliDeaths_clean <- caliDeaths %>%
  mutate(
    Data_Revision_Date = mdy(Data_Revision_Date),
    Month = month(Data_Revision_Date)
  ) %>%
  # Filter for Sept–Dec 2020, Total Population, All Causes
  filter(
    Year == 2020,
    Month %in% 9:12,
...
```{r}
# monthly percent mortality
monthlyMortality <- caliDeaths_clean %>%
  group_by(County) %>%
  mutate(total_deaths_2020 = sum(Count, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(monthlyPercMortality = (Count / total_deaths_2020) * 100)
```
...
comparison_df <- data.frame(
  Actual = monthlyMortality$monthlyPercMortality,
...
ggplot(comparison_df, aes(x = Actual, y = Predicted)) +
  ...
  theme_minimal() +
  labs(
    title = "Predicted vs. Actual Mortality (Sept–Dec 2020)",
...
```{r}
# Correlation test
cor_test_result <- cor.test(comparison_df$Predicted, comparison_df$Actual)
print(cor_test_result)
```

__For full credit__ include a brief interpretation of your findings.

> the scatterplot and correlation test show that the models predictions for sept to december of 2020 do not strongly align with the actual mortality values. the correlation is weak and slightly negative and not statistically significant, meaning the model struggled to generalize to real 2020 outcomes.
...
> time series lags capture temporal autocorrelation by using past values of the target variable. this allows the model to the learns and momentum over time. so, while "month" identifies seasonal trends, lagges variables show recent shifts or spikes that a static alendar label cannot capture.
...
> the addition of time series lagged variables seemingly significantly improved the model then. it reduced the test mae by roughly 0.6. And, because the training mae is close to the test mae, this means the model is not overfitting and is generalizing well.
...
> you could add interaction features or engineered predictors that capture relationship between variables. for example you could capture interactions between month and region to model seasonal effect that vary by location.
...
> our analysis above aimed to predict monthly percent mortality across valifornia counties using death record and random forest models. the initial models showed modest/low predictive power, and the performance improved significantly after incorporating time series features like temporal lags. this highlights how important it is to capture seasonal patterns and more recent mortality trends. Even so, the model still showed low correlation with actual mortality numbers in 2020, suggesting limitations in the model accounting for external factors/population health strain. Going forward, I would recommend incorporating additional relevant features such as environmental and/or healthcare system data, using time aware validation methods, and updating the model with current data to improve its accuracy for better results and/or decision making
...
> no it wouldnt be fair. the boxplot shows pretty consistent differences between sexes, with males having a higher proportion of injury related deaths across all years. Ignoring sex would overlook a seemingly key factor here.
...
> males consistently have a higher proportion of deaths due to accident/injury compared to females.in may of 2020 there was a steep drop in injury related deaths, going off track from past patterns. covid could explain this effect, due to the stay at home orders which in turn could lead to reduced exposure to any kind of accidents or injuries.
...
> this map shows some variation across years but it doesnt CLEARLY indicated that the stay at home order influenced this result. to show stronger evidence of covids shut down impact, we would need to have some kind of stay at home status by county before and during the order itself. if we wanted to include biological sex, filtering by sex would be necessary to assess whether there is an interaction between sex and the proportion of deaths due to accident/injury
...
> the counties filled gray are counties with missing or suppressed data. it is probably a data maturity/suppression issue, when the number of deaths in a county is very small, the data could be suppressed to protect privacy or could be insufficient to calculate an actually reliable proportion
...
```{r}
# recalc summarized data by stay-at-home period and county
injuryDeathSummary_sa <- injDeaths %>% 
  group_by(stayAtHome, County) %>%
  rename(COUNTY_NAM = County) %>%
  summarise(deathsInjuriesPercent = mean(deathsInjuriesProportion, na.rm = TRUE) * 100) %>%
  ungroup() %>%
  # join w shapefile geometries
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>%
  st_as_sf() %>%
  filter(is.na(ISLAND)) %>%
  # labeled bins
...
# before vs. during stay-at-home
ggplot(injuryDeathSummary_sa) +
  geom_sf(aes(fill = deathLabels), color = "gray20", size = 0.2) +
  scale_fill_manual(
    name = "% of Deaths Due to Accident or Injury",
    values = c4a(palette = "brewer.or_rd", n = 3),
  ...
  labs(
    title = "California Percent Deaths Due to Accident or Injury by County",
    subtitle = "Comparison Before and During Stay-at-Home Orders",
    caption = "Source: California DPH Vital Records"
  ) +
  facet_wrap(~stayAtHome)
```
...
> this coefficient of 0.28 shows a moderately weak positive relationship between female to male population ratio and the proportion of deaths due to accidents/injury. I do not think the relationship is strong enough to warrant any complex correction in maps intended for stakeholders
...
> yes, it does. the slightly positive slope of the best fit line reflects a mildly positive correlation like we just observed. the observation that the relationship is positive but still pretty weak still remainds true
...
> I would use a random forest regression model because it can handle nonlinear relationships well, and performs effectively with a mix of continuous and categorical predictors. Yes, I would ideally include spatial lags to account for regional clustering and temporal lags to capture seasonality and recent trends in injury related mortality
...
### plotting
```{r}
diabDeaths <- caliDeaths %>%
  filter(str_detect(Cause_Desc, regex("diabetes", ignore_case = TRUE)),
         Strata == "Total Population") %>%
  group_by(County, Year) %>%
  summarise(diabDeaths = sum(Count, na.rm = TRUE), .groups = "drop")

totalDeaths_annual <- caliDeaths %>%
  filter(Cause == "ALL", Strata == "Total Population") %>%
  group_by(County, Year) %>%
  summarise(totalDeaths = sum(Count, na.rm = TRUE), .groups = "drop")

diabProportions <- diabDeaths %>%
  full_join(totalDeaths_annual, by = c("County", "Year")) %>%
  mutate(deathsDiabProportion = diabDeaths / totalDeaths)
```

```{r}
# filter/clean
diabMapData <- diabProportions %>%
  filter(Year >= 2017 & Year <= 2020) %>%
  rename(COUNTY_NAM = County) %>%
  mutate(deathsDiabPercent = deathsDiabProportion * 100) %>%
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>%
  st_as_sf() %>%
  filter(is.na(ISLAND)) %>%
  mutate(deathLabels = cut(deathsDiabPercent,
                           breaks = c(0, 1.5, 3, Inf),
                           labels = c("0–1.5%", "1.5–3%", "3%+"),
                           ordered_result = TRUE,
                           right = FALSE))

# plot
ggplot(diabMapData) +
  geom_sf(aes(fill = deathLabels), color = "gray30", size = 0.2) +
  scale_fill_manual(
    name = "% of Deaths Due to Diabetes",
    values = c4a(palette = "brewer.pu_rd", n = 3),
    na.value = "gray80",
    drop = FALSE
  ) +
  coord_sf(expand = FALSE) +
  ...
  labs(
    title = "California Annual Percent of Deaths Attributed to Diabetes",
    subtitle = "Proportion of Total Deaths per County, 2017–2020",
    caption = "Source: California DPH Vital Records"
  ) +
  facet_wrap(~ Year, ncol = 4)
```
...
#   ) %>%
#   group_by(Year, Month) %>%
#   summarise(deaths_diab = sum(Count, na.rm = TRUE), .groups = "drop")
# 
# #  total deaths for proportion
# total_monthly <- caliDeaths %>%
#   mutate(
#     Data_Revision_Date = mdy(Data_Revision_Date),
#     Month = month(Data_Revision_Date)
#   ) %>%
...
# 
# 
# diab_monthly_prop <- left_join(diab_monthly, total_monthly, by = c("Year", "Month")) %>%
#   mutate(prop_diab = deaths_diab / total_deaths)
# 
# # Plot
# ggplot(diab_monthly_prop, aes(x = Month, y = prop_diab, color = as.factor(Year))) +
#   geom_line(size = 1.1) +
#   geom_point(size = 2) +
#   scale_color_brewer(palette = "Dark2") +
#   labs(
#     title = "Monthly Proportion of Deaths Due to Diabetes in California",
#     subtitle = "Based on Vital Records, 2017–2020",
#     x = "Month",
...
```{r}
sex_diab_data <- caliDeaths %>%
  filter(
    Year %in% 2017:2020,
    Strata == "Gender",
    Strata_Name %in% c("Male", "Female"),
    Cause %in% c("DIA", "ALL")
  ) %>%
  group_by(County, Year, Strata_Name, Cause) %>%
  summarise(Deaths = sum(Count, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = Cause, values_from = Deaths) %>%
  mutate(diabetes_percent = DIA / ALL) %>%
  group_by(County, Strata_Name) %>%
  summarise(mean_diabetes_pct = mean(diabetes_percent, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = Strata_Name, values_from = mean_diabetes_pct)

# plot
ggplot(sex_diab_data, aes(x = Male, y = Female)) +
  geom_point(alpha = 0.7, color = "steelblue") +
  
  # ref line
  geom_abline(aes(slope = 1, intercept = 0, color = "Reference: y = x"), linetype = "dashed", size = 1) +
  
  # best fit for actual data
  geom_smooth(method = "lm", se = FALSE, aes(color = "Best-Fit Line"), size = 1) +
  
  # color scale to label each line
  scale_color_manual(
    name = "Line Type",
    values = c("Reference: y = x" = "gray40", "Best-Fit Line" = "darkred")
  ) +
  
  labs(
    title = "Diabetes-Related Deaths by Sex (2017–2020)",
    subtitle = "Mean % of total deaths due to diabetes, by county",
    x = "Male Diabetes Mortality (%)",
    y = "Female Diabetes Mortality (%)",
    caption = "Blue dots represent counties"
  ) +
  theme_minimal()
```
...
> The choropleth maps for 2017 through 2020 show some spatial variation in diabetes-related mortality across California counties. While the overall spatial pattern remains relatively stable across the years, some counties especially in the Central Valley and Southern California show persistently higher proportions of deaths attributed to diabetes, often exceeding 3%. This suggests a potential geographic clustering of diabetes burden that could warrant some more digging into localized social determinants of health, access to care, and chronic disease prevention efforts.

> The scatterplot of diabetes related deaths by sex further illustrates this burden by stratifying the mean percent of total deaths due to diabetes by males and females across counties. The best fit line in red falls below the ref line indicating that across most counties male mortality from diabetes slightly exceeds female mortality. However, the strong linear association still suggests that patterns of diabetes mortality are highly correlated between sexes, potentially driven by shared environmental or health system factors. The distance of many points from the identity line may also signal counties where sex specific interventions could be explored.

### Questions and Hypothesis

##### Data specific question
> Does the proportion of diabetes related deaths vary significantly by sex across California counties, and are these differences associated with any underlying demographic or geographic factors such as rurality or access to healthcare?


##### Possible Hypothesis
> Counties with higher male-to-female ratios in diabetes-related mortality are more likely to be rural and have limited access to preventative care or endocrinology services, contributing to disproportionate chronic disease outcomes among men.

> This hypothesis builds on the consistent observation that male diabetes mortality rates tend to slightly exceed female rates in many counties, as shown in the scatterplot. It proposes a potential structural explanation that can be tested using spatial, demographic, and healthcare infrastructure data in future modeling work.
