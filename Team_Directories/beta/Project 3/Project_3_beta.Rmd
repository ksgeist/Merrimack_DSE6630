...
## Make the new data:
## Get the names of the counties; repeat four times for the 4 months
COUNTY_NAM_rf <- rep(unique(mortalityMonthly_sf2$COUNTY_NAM), 4)
## Make a set of months for each of the 58 counties for Sept, Oct, Nov, Dec
Month_rf <- c(rep(9, 58),
            rep(10, 58),
            rep(11, 58),
            rep(12, 58))
Year_rf <- rep(2020, length(Month))
newdata_rf <- cbind.data.frame(COUNTY_NAM_rf, Year_rf, Month_rf)
```

```{r}
class(deaths2020)
head(deaths2020)
#This data is not broken out by month
#So we will instead compare totals for 2020
```
...
#filter to only use total_population
  filter(stata == "Total Population") %>%
# Rename the county column to match sf
  rename(COUNTY_NAM_rf = County) %>%
# Group by county name, year, and month
  group_by(COUNTY_NAM_rf, Year) %>% 
# Summarize totalDeaths by summing up deaths for each group
  summarise(totalDeaths = sum(Count, na.rm = TRUE)) %>%
# Perform full join with the California sf by COUNTY_NAM
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
# Convert the resulting data to an sf object
  st_as_sf() %>% 
...
## Attach it to the dataframe and then call it the same name as the original Y
newdata_rf$propDeaths <- predict(mod_rf, newdata_rf = newdata_rf)
newdata$isPredicted <- "1"
```
