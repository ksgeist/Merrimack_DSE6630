...
> Each of the six files is a different component of the shapefile. 

> The dbf file is the database file which stores attribute data. It includes information such as names, populations, and other attributes for each spatial feature. 

>The cpg file specifies the character encoding for the attribute data in the dbf file. It ensures that text is correctly interpreted by the software.

> The prj file contains the projection information. It specifies the coordinate system and map projection that the spatial data is using. 

> The shp file is the main shapefile that contains the geometric data, like shapes, points, lines, and polygons. 

> The shp.xml file contains the metadata for the shapefile. Source, creation date, description, etc. 

> The shx file is an index file that provides indexing for the geometric data in the shp file. 
...
> The bounding box provides spatial metadata about the geometries, in this case polygons. It defines the outermost limits of the geographic area covered by the shapefile. Essentially it tells us the spatial extent and geographic boundaries of the data in the shapefile. 
...
> This gives us information about the coordinate system. How latitude and longitude are transformed into a two-dimensional system. In this case, spherical projection (Mercator) is used. Also, units of distance is indicated, which in this case is meters. Datum and ellipsoid information is also included (WGS 84). Projection matters because it ensures accurate spatial measurements and relationships. 
...
```{r}
# Filter for three counties of interest
cali_northern_counties <- cali_sf %>%
  filter(COUNTY_NAM %in% c("Del Norte", "Siskiyou", "Modoc"))

# Plot with ggplot2
ggplot() +
  # Use geom_sf to plot spatial data
  geom_sf(data = cali_northern_counties) +
  # Add labels for county names
  geom_sf_text(data = cali_northern_counties, aes(label = COUNTY_NAM),
               size = 3, nudge_y = 0.1) +
  theme_classic() +
  ggtitle("The 3 California counties that border Oregon") +
...
# Rename the county column to match sf
  rename(COUNTY_NAM = County) %>%
# Group by county name, year, and month
  group_by(COUNTY_NAM, Year, Month) %>% 
# Summarize totalDeaths by summing up deaths for each group
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
# Perform full join with the California sf by COUNTY_NAM
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
# Convert the resulting data to an sf object
  st_as_sf() %>% 
# Filter out islands
  filter(is.na(ISLAND))
```
...
> st_join is a function from the sf package in R that is designed for spatial joins between spatial dataframes. It allows for the joining of two sf objects based on the spatial relationships between their geometries. st_join also transfers attributes from both datasets to the joined dataset. st_join could be used, however, in this case, full_join was used to include all counties regardless of whether or not mortality data is available. This ensures that the joined dataset includes all counties for mapping purposes, even if some counties may have missing mortality data. 
...
key <- "6ae60dee7d6b96ec10c86517cbc9f554d12e0781"
```
...
```{r}
# Rename COUNTY column in census df to COUNTY_FIP
census <- census %>%
  rename(COUNTY_FIP = COUNTY)

# Ensure COUNTY_FIP column is of the same type in both dfs
census <- census %>%
...
# Perform left join and call combined df mortality_sf
mortality_sf2 <- left_join(mortality_sf, census, by = "COUNTY_FIP")
```
...
> It seems as though Annual % of All Cause Mortality is decreasing from 2017 to 2020. I don't think this is a true pattern and warrants further investigation.
...
Let's now look at annual and monthly trends in total numbers of deaths
...
> The data for the year 2020 is incomplete. 2017-2019 include 12 months of data, where 2020 only includes 8 months of data. There are a couple ways to fix this. We could compare only the first 8 months of each year. We could project the 8 month data to a full year, however this would only be an estimate. As mentioned in the next section, we could take the average of monthly $ mortality. Yes, if we had full 12 months of data for each year, making a map using annual percentage mortality would be a more accurate representation.
...
```{r}
# Filter for January through August for all years
mortalityJanAug_sf2 <- mortality_sf2 %>% 
  filter(Month <= 8) %>%
  group_by(COUNTY_NAM, Year) %>% 
  summarize(percMortality = 100 * sum(totalDeaths, na.rm = TRUE) / mean(totalN)) %>%
  ungroup()
...
summ <- fivenum(mortalityJanAug_sf2$percMortality)

# Print summary
cat("The five number summary of % mortality for January to August 2017-2020:\n")
print(summ)

# Plot map
tm_shape(mortalityJanAug_sf2) + 
  tm_polygons(col = "percMortality", 
              palette = get_brewer_pal(palette="BuPu", n=10, plot=FALSE),
              breaks = summ,
              alpha = 1,
              title = "Jan-Aug % Mortality, All Causes") +
  tm_borders() + 
  tm_text("COUNTY_NAM", size=0.25, col = "darkgrey") +
  tm_facets(by = "Year", ncol = 2) +
  tm_layout(legend.position = c("right", "bottom"), 
...
> When filtering for just the months of January through August for all years, there is a substantial increase in % mortality of the lower quartile, median, upper quartile, and maximum. This can also be seen visually as the years progress, with 2020 having the heaviest 'purple' throughout the state, especially in the southern counties. 
...
> This regression does not appear to be robust, in my opinion. It shows a decline in % deaths per capita in the latter part of the year. All other years show an increase near the end of the year, and conventional wisdom would assume that during the winter months sickness and deaths as a result would increase. This is mostly likely due to this regression being too simple to capture more of the complex relationships in the data. This regression assumes linear relationship between month and mortality and doesn't account for potential seasonal patterns. Upon checking with 'plot(mod)', it is also apparent by studying the diagnostic plots, that the regression is not robust.
...
#This is giving me a list that contains a data frame, but that dataframe only has 5 rows??
```

```{r}
deaths2020 <- read_sf("~/Desktop/20231206_deaths_final_2014_2022_county_year_sup.csv")
```

**Remember** to filter for the proper condition, Year, and Months to get the data you need for your testing set.

#### Question 13 (SEH) [15 points]:

> I propose to use a Random Forest model to predict the deaths for 2020. The linear model was not a great predictor, and RF is a non-linear model. First, we can create training and test data using the deaths2020 data from above. Then we can create the new model and predict 2020 values. From there we can create a new map and a new line graph. 

```{r}
#First create a training and test set using deaths2020 as 'correct' value
#mortalityMonthly_sf2 is our training data, like above
## Make the new data:
## Get the names of the counties; repeat four times for the 4 months
COUNTY_NAM <- rep(unique(mortalityMonthly_sf2$COUNTY_NAM), 4)
## Make a set of months for each of the 58 counties for Sept, Oct, Nov, Dec
Month <- c(rep(9, 58),
            ...
            rep(12, 58))
Year <- rep(2020, length(Month))
newdata_rf <- cbind.data.frame(COUNTY_NAM, Year, Month)
```


```{r, echo = TRUE, warning=FALSE, message=FALSE}
deaths_rf <- deaths2020 %>% 
#filter to only use total_population
...
```{r}
#Then train the model on the data like we did for the lm
mod_rf <- randomForest::randomForest(
  percMortality ~ COUNTY_NAM + Year + Month, 
  data = mortalityMonthly_sf2, mtry = 3)
#mod_rf
```
...
#Make predictions
## Attach it to the dataframe and then call it the same name as the original Y
newdata_rf$propDeaths <- predict(mod_rf, newdata = newdata_rf)
newdata_rf$isPredicted <- "1"
```

```{r}
#To get the monthly average, we can divide our total in deaths_rf by 12
#Then compare with our summary
## Remove the extra characters from the name of the county
census <- census %>% 
  rename(COUNTY_NAM = County)
```
...
## Join with the total deaths
deaths_rf <- deaths_rf %>% 
  full_join(census, by = "COUNTY_NAM")

deaths_rf$propDeath = deaths_rf$Count/deaths_rf$totalN

deaths_rf_20 <- deaths_rf %>%
  filter(Year == 2020) %>%
  group_by(COUNTY_NAM) %>%
  summarize(monthly_death = sum(as.numeric(propDeath), na.rm = TRUE)/12)

new_means <- newdata_rf %>%
  group_by(COUNTY_NAM) %>%
  summarize(avg_death = mean(propDeaths))

mean((new_means$avg_death - deaths_rf_20$monthly_death)^2)
```
...
> This model seems much better at predicting the increase in deaths. 
...
#### Question 14 (SEH) [1 point]:

Is it fair to model the values without accounting for an effect of sex, do you think? Why or why not?

> There is a very interesting discrepency between sex in relation to deaths from accident and injury. This may be due to job risks in fields that skew heavily male and skewed gender ratios in certain higher risk activities and sports. 'Fair' is a subjective term. There are definitely things we can learn when we split by sex, but there are also things that might be revaled with aggregated data. It is also worth considering how many deaths of each gender there are. My guess would be that California has a fairly even sex ratio, but I'm not sure. 
...
> The stay at home order in California started in March and May was when a few things opened. I'm not sure why there is a huge turndown at this point. It does seem that the decreased activity lead to fewer accident and injury deaths. One possible explaination is that accident and injury deaths can be delayed from the initial event, leading to a decrease slightly after the stay at home order. 
...
> There is not strong evidence that the stay at home order impacted deaths from accident and injury. We could get stronger evidence if more than a few counties decreased in deaths during the stay at home order. I do continue to see an interaction between sex and death from injury and accident, but I'm not sure if it's causally linked from the map. 
...
```{r}
  injDeaths_na <- na.omit(injDeaths)
  cor(injDeaths_na$femaleMaleRatio, injDeaths_na$deathsInjuriesProportion)
```
...
> There is a fairly weak relationship between the male/female ratio and the deaths from injuries proportion, so it's probably not a big deal for our visualizations. 


#### Question 18 (SEH) [1 point]:
...
> Yes. The slow is slightly positive, which corresponds to the slightly positive correlation coefficient, signfying a weak positive relationship. 
...
> Gradiant Boosting Machine (GBM) starts with initializing the model with a constant value, which is typically the mean of the target values for regression tasks such as this. Next, a weak learner (for example, a decision tree) is trained on the residuals (errors) of the current model, aiming to reduce the error. The algorithm utilizes gradient descent to minimize the loss function by adjusting the model in the direction that reduces the error. The predictions from the new model are scaled by a learning rate (shrinkage factor) and then added to the previous ensemble of models to update the model. This step helps to prevent overfitting. Finally, this process is repeated for a specified number of iterations or until the model performance meets a certain stopping criteria. 
Reference: https://www.datacamp.com/tutorial/guide-to-the-gradient-boosting-algorithm

#### Question 20 (AC) [2 points]:

What hyperparameters can one tune in a GBM? **Hint**: Make sure to explain at least 4! 

> 1) Learning rate (shrinkage) can be tuned. The learning rate scales the contribution of each model, where a smaller learning rate makes the model more robust to overfitting by requiring more trees to be added to the ensemble. 
> 2) The number of models (ie number of trees, if using Random Forest) can be tuned, dictating the total number of boosting stages to be run. 
> 3) The maximum depth of the decision trees (max_depth) can also be tuned. Deeper trees capture more complex patterns, at the risk of overfitting. 
> 4) Subsample can be tuned, which is the fraction of samples used for fitting each inidividual base learner (tree). 
...
#### Question 21 (AC) [1 point]:

Which model is superior and why? Do you trust these results?

> While both models seem to have relatively high accuracy, I would lean towards the Random Forest model. This is because although the two models have very comparable RMSE and MAE values, the Rsquared value of the Random Forest model is higher. However, I do think that either would be a good fit. 

#### Question 22 (AC) [1 point]:

Do you trust the variable importance results? Is there any difference in the variable importance between the two models?

> I would trust the variable importance results, as they both are very similar. The strongest difference between the two model's variable importance's is that the retireeRatio is ranked slightly higher than the femaleMaleRatio in the Random Forest model, whereas in the GBM the femaleMaleRatio is ranked slightly higher than the retireeRatio. However, they both agree on the top 8 most important variables, and on the top variable by far, which is totalN. 

#### Question 23 (AC) [1 point]:
...
> The population size of the county is the highest ranking variable in both models. This could be worth mapping as population density is an important geographic factor. 

#### Question 24 (AC) [1 point]:
...
> If we were to proceed with mapping, we may want to scale based on population size like we did eariler in the project to avoid the bias of strongly populated areas. 

#### Question 25 (AC) [1 point]:
...
> Based on the variable importance graphs above, there is not a particularly strong indication that the pandemic is reducing the number of injury-related mortalities. This may be because the population density of counties has such an overwhelmingly large importance that it overshadows other variables. We could remove or scale the totalN to deal with this, and/or include more data as it becomes available. 
...
> Research question: What effect has County, Year, Month, stayAtHome, biologicalSex, femaleMaleRatio, totalN, and retireeRatio had on the proportion of all deaths that have occurred due to Intentional Self Harm (Suicide)?
> The study linked below investigated suicide levels through the COVID-19 pandemic and stay-at-home orders in Chile, and found that numbers actually declined approximately 13%. 
> Reference: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9356570/#:~:text=In%20relation%20to%20non%2Dfatal,et%20al.%2C%202020).

#### Question 27 (AC) [1 point]:
Formulate a hypothesis based on your question.

> The stay-at-home orders in California have led to a significant change in the proportion of deaths due to Intentional Self Harm (Suicide). 

#### Question 28 (AC) [5 points]:
...
> In order to investigate whether suicide rates changed significantly during the stay-at-home orders in California, I would first do exploratory data analysis to examine the rates of death of all causes, and suicide specifically, over time and year-to-year. I would also utilize a Gradient Boosting Machine (GBM) algorithm to predict the proportion of deaths due to suicide. I would choose GBM for its robustness in handling complex relationships between variables, and its high accuracy in our use of it above. Additionally, I would make a few visualizations: 1) a baseline map showing suicide rates before the stay-at-home orders in California, 2) A visualization showing the change in suicide rates over time, particularly focusing on periods before, during, and after the implementation of stay-at-home orders, 3) a visualization comparing demographic variables like female-to-male ratio and retiree ratio to see if there are spatial patterns that correlate with suicide rates, and 4) a map showing the predicted suicide rates for different scenarios, such as with and without stay-at-home orders. 

#### Question 29 (AC) [5 points]:
...
```{r, echo = TRUE}
## Join the sex deaths table with the census data; then filter for SUI
suiDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ...
  ## Calculate the proportion of all deaths that are from suicide relative to all deaths
  mutate(deathsSuicideProportion = suicideDeaths/totalDeaths,
         femaleMaleRatio = femalesN/malesN,   ## The ratio of females to males in the population
         femaleMaleRatio65 = `females65+`/`males65+`,  ## The ratio of 65+ females to males in the population
         retireeRatio = over65/totalN,  ## Ratio of retirees to non-retirees
         Date = as.Date(paste(Year, Month, "1", sep = "-"), format = "%Y-%m-%d"),   ## Convert the date to a date format for plotting
         deathsSuicideProportion = ifelse(deathsSuicideProportion == "NaN", 0, deathsSuicideProportion))  ## Illegal div by 0 turned to NaN; turn back to 0
```
...
```{r, echo = TRUE}
## Calculate the proportion of deaths and prepare the data for mapping
suiDeaths <- suiDeaths %>% 
  group_by(stayAtHome, Year, County) %>% 
  rename(COUNTY_NAM = County) %>% 
  summarise(propDeaths = mean(deathsSuicideProportion, na.rm = TRUE) * 100) %>% 
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  st_as_sf() %>% 
  filter(is.na(ISLAND))

## Visualize the distribution of the proportion of deaths
ggplot(data = suiDeaths, aes(x = propDeaths)) +
  geom_histogram(fill = "#2b8cbe", color = "#000000") +
  labs(x = "% Mortality due to Intentional Self Harm (Suicide)", y = "Frequency", 
       title = "Histogram of % Mortality due to Intentional Self Harm (Suicide)")
...
tm_shape(suiDeaths) + 
  ...
  tm_polygons(col = "propDeaths", 
              ...
              alpha = 1,
              title = "% Mortality due to Intentional Self Harm (Suicide)",
              colorNA = "#e1e2e3") +
  tm_borders() + 
  tm_text("COUNTY_NAM", size = 0.25, col = "#000444") +
  tm_facets(by = "stayAtHome", nrow = 1) +
  tm_layout(legend.position = c("right", "bottom"), 
            title.position = c('right', 'top'))

```

#### Question 30 (AC) [1 point]:
What trends, if any, do you notice? Why do you come to this conclusion?

> It does seem that there may have been a decrease in the proportion of deaths due to suicide during the stay-at-home order in California, and it should certainly be investigated further. Although there seems to be an increase in certain counties in northern California, southern California appears to have seen a decrease in the proportion of deaths due to suicide. I would be curious to see how rural/urban distribution impacts the proportion of deaths due to suicide as well. 
