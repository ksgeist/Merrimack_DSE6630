...
load("readyTrain.Rdata") 
load("readyTest.Rdata") 
...
## Guard against duplicate FacilityIds before they become rownames later
  distinct(FacilityId, .keep_all = TRUE) %>%    # ADD THIS LINE
  ...
  stateTrain <- readyTrain %>% 
    ...
    left_join((pneumoniaFull %>%
              ## Don't want to merge with all features from pneumoniaFull
               ...
               distinct(FacilityId, .keep_all = TRUE)), by = "FacilityId") %>% # ADD the %>% distinct line
