...
filepath <- "/Users/seugley/Desktop/Data Science/DSE 6630/Assignment_2/" 
...
```
> The data appears to be in long format, which is not ideal in this scenario. Each row does not represent a single line of data. The problem here, is that each hospital should be its own row. We're trying to predict disease transfer within hospitals based on characteristics of that hospital, using the other variables. 
...
> We had GitHub issues so I couldn't get access to the updated dataset with Scott and Adeline's updates. My edcoding is based on the original dataset. 

```{r}
#data_science_index <- str_which(comp_df$job_title, "Data Scien")
#comp_df$job_category[data_science_index] <- "Data Science"

pfe <- pneumoniaFull

to_encode <- select(pfe, starts_with("ComparedToNational_"))
#unique(to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`)
col_names <- names(to_encode)
#col_names
#I wanted to do a for loop here, but I couldn't get it to work

worse_index <- str_which(to_encode$`ComparedToNational_MRSA Bacteremia`, "Worse than the National Benchmark")
to_encode$`ComparedToNational_MRSA Bacteremia`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_MRSA Bacteremia`, "No Different than National Benchmark")
to_encode$`ComparedToNational_MRSA Bacteremia`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_MRSA Bacteremia`, "Better than the National Benchmark")
to_encode$`ComparedToNational_MRSA Bacteremia`[better_index] <- 1

worse_index <- str_which(to_encode$`ComparedToNational_Death rate for pneumonia patients`, "Worse Than the National Rate")
to_encode$`ComparedToNational_Death rate for pneumonia patients`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_Death rate for pneumonia patients`, "No Different Than the National Rate")
to_encode$`ComparedToNational_Death rate for pneumonia patients`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_Death rate for pneumonia patients`, "Better Than the National Rate")
to_encode$`ComparedToNational_Death rate for pneumonia patients`[better_index] <- 1

worse_index <- str_which(to_encode$`ComparedToNational_Postoperative respiratory failure rate`, "Worse Than the National Rate")
to_encode$`ComparedToNational_Postoperative respiratory failure rate`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_Postoperative respiratory failure rate`, "No Different Than the National Rate")
to_encode$`ComparedToNational_Postoperative respiratory failure rate`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_Postoperative respiratory failure rate`, "Better Than the National Rate")
to_encode$`ComparedToNational_Postoperative respiratory failure rate`[better_index] <- 1

worse_index <- str_which(to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`, "Worse Than the National Rate")
to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`, "No Different Than the National Rate")
to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`, "Better Than the National Rate")
to_encode$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`[better_index] <- 1

worse_index <- str_which(to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`, "Worse Than the National Value")
to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`, "No Different Than the National Value")
to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`, "Better Than the National Value")
to_encode$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`[better_index] <- 1

worse_index <- str_which(to_encode$`ComparedToNational_Hospital return days for pneumonia patients`, "Fewer Days Than Average per 100 Discharges")
to_encode$`ComparedToNational_Hospital return days for pneumonia patients`[worse_index] <- -1
no_diff_index <- str_which(to_encode$`ComparedToNational_Hospital return days for pneumonia patients`, "Average Days per 100 Discharges")
to_encode$`ComparedToNational_Hospital return days for pneumonia patients`[no_diff_index] <- 0
better_index <- str_which(to_encode$`ComparedToNational_Hospital return days for pneumonia patients`, "More Days Than Average per 100 Discharges")
to_encode$`ComparedToNational_Hospital return days for pneumonia patients`[better_index] <- 1
...
```{r}
load("~/Documents/GitHub/Merrimack_DSE6630/II. Biomedical & Clinical Informatics/Demo/pneumoniaFullEncoded.Rdata")
```
...
```{r}
#concatenate the two columns we want to encode
cols2encode <- c("State", 
               "CountyParish")

#Create a new dataset with only 2 variables
#Done by comparing the 'names' for our original dataset with the elements in 'cols2encode'
#This allows for changing columns later if we wanted and improves usability
temp <- pneumoniaFullEncoded[, names(pneumoniaFullEncoded) %in% cols2encode]

#Create a function 'add_freq'
#Takes two parameters: data set and column_names
add_freq <- function(data, column_name) {
#Create a table 'frequency map'
#Select a columns for the dataset including NA values
  frequency_map <- table(data[[column_name]], useNA = "always")
#Replace the column data with frequency of the occurrence 
...
#Loop through the columns of 'temp' dataset
for (col in names(temp)) {
#apply the function above to the temp dataset
#Each column in temp will be replaced with the frequencies of each occurrence 
  temp <- add_freq(temp, col)
}

#Loop through the length of cols2encode (based on above, len = 2, but could change with updates to the columns)
for (c in 1:length(cols2encode)) {
#Replace the designated columns in the full dataset with the frequencies from the the temp dataset
  pneumoniaFullEncoded[, cols2encode[c]] <- temp[, cols2encode[c]]
}
```             


##### **Question 17**: [1 point] (SH)
...
load("~/Documents/GitHub/Merrimack_DSE6630/II. Biomedical & Clinical Informatics/Demo/pneumoniaAnalyze.Rdata")
...
pneumoniaAnalyze <- subset(pneumoniaAnalyze, select = -c(NumberOfReadmissions, NumberOfDischarges))
```
