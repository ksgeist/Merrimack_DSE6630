...
> Since alpha is one, the model chose a Ridge Regression model. 
Since lambda is one, the model performed no shrinkage and we have a OLS regression. 
...
> The top features for the elastic net were Recommended Hospital, Hospital return days and payment. 
For PCA the top features were overall hospital rating, Care transition, nurse communication, and recommeded hospital. 
For regression the top features were Payment for Pneumonia patients, Death rate for pneumonia patients, median time in the EDD, Hospital return days, Quietness, and excess readmission. 
...
> They are very similar! OLS perfomred better with RMSE, but the Elastic net performed better with Rsquared. OLS also did better with MAE. 

```{r}
prOLS <- postResample(pred = predict(mod, newdata = regressionTrain), 
                      obs = regressionTrain$bc_PredictedReadmissionRate)
...
> The accuracy on both OLS regression and Elastic Net is still fairly low. If they would like to go with one of these models, I would suggest further testing to see how sample size impacts the results as well as the specificity and sensitivity. Ideally, an IRB or ethics body would participate in adjusting the model so that the error has the least negative impact. I would also suggest that further work be performed to see if another model might before with a higher accuracy. The parameter tuning from the Elastic Net suggests that either LASSO or Ridge regression might be models worth considering. Finally, I might suggest a non-linear model to see how it compares in accuarcy for data that seems non-linear from our testing so far. 
...
## Display the output
rbind(prRidge, prLasso) %>% 
  kable(digits = 2,
    format = "html",
    caption = "Table 8. Comparison of the Ridge and Lasso") %>%
    kable_styling(bootstrap_options = c("hover", full_width = F)
)
```

From the RMSE and Rsquare values above (.76 and .77), it is clear that Lasso and Ridge regression perform very similarly. In fact, they also perform very similarly to the elastic net model above. (.77). These are still not very accurate models. However, the graphs above also demonstrate the that data is somewhat non linear. Ridge regression, lasso, and elastic net all assume a somewhat linear relationship between data. Further models could explore models that focus on non-linear relationships. 
