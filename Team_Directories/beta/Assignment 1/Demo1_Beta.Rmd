...
counts <- read_csv(file = "./23524047/Complete_sampleinfo.csv") %>% as.data.frame()
head(counts[,1:6]) 
```
...
39% of the variance is attributable to PC1 and 11% is attributable to PC2. I see the most separation between the treatmeants at 30 minutes. Verble et al. reported "Significant differences from control bees were only detectable within 30 minutes after the treatment, with most occurring at the 10-minute recovery time point." I don't think this graph shows that 10 minutes was the strongest, but I agree that 10 and 30 minute periods both show separation. 
...
Since Support Vector Machines are able to produce non-linear boundaries, they would be able to create a boundary for data such as the graph shown for 30 minutes, that can separate data. They do this by using additional dimensions, which again could help create boundaries for when that data isn't linear. 
...
A volcano plot compares the fold change and significance of the genes. In this particular volcano plot, almost all the genes show no fold change, and there are only two genes that are significant and have greater than 2^2 times more expression in one condition over the other. A vast majority of the data are not significant and also don't have large fold changes. We get much less information from a graph that is clustered together in the
...
There are 12927 features (genes) in each set. The test set has time periods with only one or two colonies represented. This will make it hard to determine if our model is working accurately if we only have 1 batch for each time period to work with. The test set also doesn't always have a control and a cold sample represented, so we can't compare the two. Adjusting the split to 60% does seem to help, but there is less data to train on. Adjusting to 70% is helpful, but less so, and probably a good compromise. 
...
No, the error rate never reaches 0, which means there is not a complete separation. The OOB error however, is not always an accurate prediction of error on a test set. 
...
The training set had a much higher accuracy of 100% (which suggests that it was able to completely separate the cold-express and control samples). The test set had an accuracy of 65.5%. The model may have overfit the data since it performed very well on the training data, but much less well on the test data. 

##### **Question 27**: Why is it not a good idea to look at just the accuracy?

Especially in classification, there can be a different between false positive values and false negative values. Looking at only the accuracy does not tell us anything about these factors. Other measurements like precision or recall are able convey other factors about how well the model is able to predict labels for test data. 
...
Kernel = Sigmoid
Training accuracy: .533
Test accuracy: .517

The training error is lowest for the linear kernel, with an accuracy of 100%. 
The test error is lowest for linear as well with an accuracy of 65.5%. 
...
```{r, echo = FALSE}
rownames(importantsvm_1) <- importantsvm_1[,1]
findOverlappingGenes <- function(cutoff, RFresults) {
  FoldChangeCutoff <- row.names(resultsDESeq)[which(resultsDESeq$log2FoldChange > cutoff)]
  rf <- row.names(RFresults)
  importantgenes <- intersect(FoldChangeCutoff, rf)
...
findOverlappingGenes(2, importantsvm_1)
```
Yes! There are 28 overlapping genes.

##### **Question 31**: At this point, which model is performing better on the gene expression data and why?

There is more overlap between the DESeq results and the random forest model. The accuracy is also higher for the random forest model with 6.55% accuracy. 
