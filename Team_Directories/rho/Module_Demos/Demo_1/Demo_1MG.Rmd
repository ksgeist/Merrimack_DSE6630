...
> When replacing all of the too few instances with a 5 we were able to use gsub. When trying to generate a random number per instance, we aren't able to use gsub because it will generate just one random number and then replace it with just that one instace rather than making a new random number each time. If lappy was used then a new number will be generated everytime few too to report is located. There is another way to do this such as a for loop where every time, that phrase is encountered then it will create a new number to replace it with. The one main differences is that the for loop is going to required a bigger draw from the computing power where when using lappy it will be a lower draw.  
...
>All of the columns that are going to be removed is going to have higher, lower, denominator, or hcahpsanswer percent in the name of it 

```{r}
pneumoniaFull <- pneumoniaFull %>% select(-`Denominator_Perioperative pulmonary embolism or deep vein thrombosis rate`,-`Denominator_Hospital return days for pneumonia patients`,-`Denominator_Postoperative respiratory failure rate`,-`Denominator_Death rate for pneumonia patients`,-`Denominator_CMS Medicare PSI 90: Patient safety and adverse events composite`)

pneumoniaFull <- pneumoniaFull %>% select(-`HigherEstimate_Perioperative pulmonary embolism or deep vein thrombosis rate`,-`HigherEstimate_Death rate for pneumonia patients`,-`HigherEstimate_Hospital return days for pneumonia patients`,-`HigherEstimate_CMS Medicare PSI 90: Patient safety and adverse events composite`,-`HigherEstimate_Postoperative respiratory failure rate`)

pneumoniaFull <- pneumoniaFull %>% select(-`LowerEstimate_Perioperative pulmonary embolism or deep vein thrombosis rate`,-`LowerEstimate_Hospital return days for pneumonia patients`,-`LowerEstimate_Postoperative respiratory failure rate`,-`LowerEstimate_CMS Medicare PSI 90: Patient safety and adverse events composite`,-`LowerEstimate_Death rate for pneumonia patients`)

pneumoniaFull <- pneumoniaFull %>% select(-`HcahpsLinearMeanValue_Nurse communication`,-`HcahpsLinearMeanValue_Recommend hospital`,-`HcahpsLinearMeanValue_Doctor communication`,-`HcahpsLinearMeanValue_Communication about medicines`,-`HcahpsLinearMeanValue_Discharge information`,-`HcahpsAnswerPercent_Recommend hospital`,-`HcahpsAnswerPercent_Doctor communication`,-`HcahpsAnswerPercent_Nurse communication`,-`HcahpsAnswerPercent_Communication about medicines`,-`HcahpsAnswerPercent_Overall hospital rating`,-HcahpsAnswerPercent_Cleanliness,-`HcahpsAnswerPercent_Staff responsiveness`,-`HcahpsAnswerPercent_Care transition`,-HcahpsAnswerPercent_Quietness,-`HcahpsLinearMeanValue_Overall hospital rating`,-HcahpsLinearMeanValue_Quietness,-HcahpsLinearMeanValue_Cleanliness,-`HcahpsLinearMeanValue_Care transition`,-`HcahpsLinearMeanValue_Staff responsiveness`)

#had to brute force it because contains waasn't working, tried to convert to characters or numeric and it still gave reject. 
...
pneumoniaFull <- pneumoniaFull %>% select(-FacilityName,-TelephoneNumber,-Address,-CityTown)
...
> There are a couple columns that need this same kind of encoding, such as payment category for pneumonia patients, they also have the same kind saying that there is more or less that pneumonia patients are paying or if it was around the national average. This also includes any of the comlumns that have compared to the national average in the name of it. This creates a total of 7 columns that need to be encoded. 


##### **Question 17**: [3 points] 
You probably expected this - attempt to encode those columns the method you selected. It's okay if you can only think of a way to brute-force this for now; I will show you code to you do it faster after the assignment. :)
...
```{r}
tryEncoding <- pneumoniaFull 

#compared to medicare MSI 90 
tryEncoding$ComparedToNational_CMS_Medicare_PSI_90_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite` =="Worse than National Benchmark", 1, 0)

tryEncoding$ComparedToNational_CMS_Medicare_PSI_90_better_than_national <- ifelse(pneumoniaFull$`ComparedToNational_CMS Medicare PSI 90: Patient safety and adverse events composite`=="Better than the National Benchmark", 1, 0)

#compared pulmonary embolism
tryEncoding$ComparedToNational_Perioperative_pulmonary_embolism_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate` =="Worse than National Benchmark", 1, 0)

tryEncoding$ComparedToNational_Periopertive_pulmonary_embolism_better_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Perioperative pulmonary embolism or deep vein thrombosis rate`=="Better than the National Benchmark", 1, 0)

#compared death rate of pneummonia 
tryEncoding$ComparedToNational_Death_rate_pneumonia_patients_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Death rate for pneumonia patients` =="Worse than National Benchmark", 1, 0)

tryEncoding$ComparedToNational_Death_rate_for_pneumonia_patients_better_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Death rate for pneumonia patients`=="Better than the National Benchmark", 1, 0)

#compared to postoperative respiratory failure rate
tryEncoding$ComparedToNational_Postoperative_respiratory_failure_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Postoperative respiratory failure rate`== "Worse than National Benchmark",1,0)

tryEncoding$ComparedToNational_Postoperative_respiratory_failure_rate_better_then_national <- ifelse(pneumoniaFull$`ComparedToNational_Postoperative respiratory failure rate`=="Better than the National Benchmark",1,0)

#compared to hospital return days  
tryEncoding$ComparedToNational_Hospital_return_days_for_pneumonia_patients_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_Hospital return days for pneumonia patients` == "Worse than the National Benchmark",1,0)

tryEncoding$ComparedToNational_Hospital_return_days_for_pneumonia_patients <- ifelse(pneumoniaFull$`ComparedToNational_Hospital return days for pneumonia patients`== "Better than the National Benchmark", 1,0)

#compared to MRSA 
tryEncoding$ComparedToNational_MRSA_Bacteremia_worse_than_national <- ifelse(pneumoniaFull$`ComparedToNational_MRSA Bacteremia` == "Worse than the National Benchmark",1,0)

tryEncoding$ComparedToNational_MRSA_Bacteremia_better_than_national <- ifelse(pneumoniaFull$`ComparedToNational_MRSA Bacteremia`=="Better than the National Benchmark",1,0)

#compared to payment
tryEncoding$PaymentCategory_Payment_for_pneumonia_patients_worse_than_national <- ifelse(pneumoniaFull$`PaymentCategory_Payment for pneumonia patients`=="Less than the National Average Payment",1,0)

tryEncoding$PaymentCategory_Payment_for_pneumonia_patients_better_than_national <- ifelse(pneumoniaFull$`PaymentCategory_Payment for pneumonia patients`=="Greater Than the National Average Payment",1,0)

#score emergency department 
tryEncoding$Score_Emergency_department_volume_low <- ifelse(pneumoniaFull$`Score_Emergency department volume`=="low",1,0)

tryEncoding$Score_Emergency_department_volume_medium <- ifelse(pneumoniaFull$`Score_Emergency department volume`=="medium",1,0)

tryEncoding$Score_Emergency_department_volume_high <- ifelse(pneumoniaFull$`Score_Emergency department volume`=="high",1,0)

tryEncoding$Score_Emergency_department_volume_very_high <- ifelse(pneumoniaFull$`Score_Emergency department volume`=="very high",1,0)
...
```{r}
cols2encode <- c("State", 
               "CountyParish") #creating a variable which is selecting both state as well as county parish
...
> There are usually disadvatages and advantages to using the predicted values rather than the obserced readmission rate. There are advantages of using predicted values, these values help to focus on improving decision making and optimizing resources to the best of their abilities. There is the disadvantages of using predicted values which means that there could be overpredicting or underpredicting values. These are some of the things that need to be careful when also calculating the predicted values. The appropriate variable to choose for the most accurate would be the observed readmission rate because these are using the data that was actually gathered and then apply basic math to be able for it to be shown in the best way. In these sitaution there are also moments when to use these as well as not, we should use these when trying to accuratly show data that was being collected. 

##### **Question 21**: [1 points] 

Why did I have you drop the two columns used to make `observed_readmission_rate`?

> The two coloumns were a form of placeholder for columns, because they were just showing data in a form in which wasn't needed. If these two columns were to stay in use then it would make the dataset itself more extented and convoluted and wouldn't be used in any other reason, so having them still in the database it would cause confusion and more storage space to the datasets.  
...
densityplot(pneumoniaAnalyze$NumberOfReadmissions,pneumoniaAnalyze)
...
> When looking at the density distribution above, we can see that there is a skew right. This shows that most of the desity for readmission rates is between 0 to 100 readmissions. This though is a good thing because this means that all of hospitals that were observed has lower reamissions rates. THere are though a few that have high levels of reamissions rates. These outliers are the facilities that need to be examined, since there are some that are close to 500 readmssions. After 200 there are very few points which again is a good sign since we are hoping for less readmissions as the hospitals come along. 
