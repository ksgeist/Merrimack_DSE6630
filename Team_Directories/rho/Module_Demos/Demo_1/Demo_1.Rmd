...
cat("Structure of FY 2025_Hospital Readmissions Reduction Program after replacing values with NA:\n")
...
```{r}
cat("FY 2025 Hospital Readmissions Reduction Program: Number of Readmissions:\n")
...
cat("Structure of FY 2025 Hospital Readmissions Reduction Program after numeric conversion:\n")
str(FY_2025_Hospital_Readmissions_Reduction_Program)
cat("\n")
...
tidyNjoin <- function(datList, filterList, hospitalInfo, readmissionsClean) {
  ...
  # "HospitalOverallRating" is EXCLUDED from this list to ensure it's kept.
  cols_to_drop_from_individual_dfs <- c(
    ...
    "HcahpsMeasureId", # This line is corrected from previous typos.
    "PatientSurveyStarRatingFootnote", "HcahpsAnswerPercentFootnote",
  ...
  for (i in seq_along(datList)) {
    ...
    # FIX: Ensure FacilityId is character in ALL dataframes for consistent joining
    df_current_processed <- df_current_processed %>%
      mutate(FacilityId = as.character(FacilityId))

    # Step 2: Handle NA class issues and convert to numeric (THIS BLOCK IS NO LONGER DUPLICATED)
    df_current_processed <- df_current_processed %>%
      replace_with_na_all(condition = ~.x %in% missing_values_to_replace)

    # FIX: Exclude FacilityId from being converted back to numeric (and MeasureName)
    df_current_processed <- df_current_processed %>%
    ...
    } else { # This 'else' belongs to the outer 'if' (MeasureName check)
      # If no MeasureName column (like Hospital_General_Information), it's probably already wide.
      # Explicitly select HospitalOverallRating to ensure it's not missed.
      pivoted_df <- df_current_processed %>% 
        select(FacilityId, any_of("HospitalOverallRating"), everything()) %>% # <<< CRITICAL CHANGE HERE <<<
        distinct() 
  ...
  return(merged_df)
} # <--- This final brace closes the 'tidyNjoin' function
```
...
> *In Project 1, I will write my own research question, manage the cleaning, pre-processing, and exploratory data analysis of merged datasets. This project allows me to create a custom data preparation process that I can use this to develop a clean, merged dataset that addresses my research question and explore insights related to my topic effectively.*
...
> *I didn't have to drop any columns from the dataset because the pivot_wider function had already addressed their transformation and organization. This function effectively restructured the data, ensuring that all necessary information was retained and presented in a more suitable format for analysis. As a result, there was no need to further modify the dataset by removing columns.*
...
> *Ordinal encoding is a way to organize categories that have a clear order. For example, "Better Than National" is better than "No Different Than National," which is in turn better than "Worse Than National." In this method, we assign numbers to each category: 1 for "Better," 2 for "No Different," and 3 for "Worse."*
>
> *This ranking keeps the important order intact, which helps machine learning models. They can recognize that "Worse" is further down the negative scale than "No Different." This understanding is better than treating each category as separate, like One-Hot Encoding does.*
...
# Corrected: Using pneumoniaFullEncoded instead of temp_pneumonia_df
...
> *We began with 1,549 unique CountyParishes, but after applying frequency encoding, we're left with only 36 distinct categories. This represents a significant reduction in the number of unique entries, meaning that each of the 36 frequency values now, on average, corresponds to about 43 different original counties (1,549/36 ≈ 43).*
>
> *This dramatic decrease in uniqueness leads to a considerable loss of information. Many distinct geographical entities, such as counties, become indistinguishable based solely on this encoded feature. For instance, if County A, County B, and County C each have 50 hospitals, they are all represented by the same encoded value of '50'. Consequently, a machine learning model would mistakenly treat these three diverse counties as if they were identical, overlooking any unique characteristics that might impact factors like readmission rates. This loss of granularity can hinder the model's ability to accurately analyze and predict outcomes based on the recorded data.*
>
> ------------------------------------------------------------------------
>
...
> *To identify areas or hospitals with unexpectedly high readmission rates, we should use the Excess Readmission Ratio (ERR) as our target measure.
>
> [Reasons for This Choice:]{.underline}*
>
> *The term "unexpectedly high" means we need to compare rates while considering patient differences. The Excess Readmission Ratio does this clearly. A ratio higher than 1 shows that a hospital has more re-admissions than expected for similar hospitals, highlighting the issues. A ratio of 1 means the hospital is performing as expected. A ratio below 1 indicates better performance.
>
> The ERR adjusts for risk, which helps compare hospitals fairly. It accounts for different levels of patient needs, which is essential. Using a simple readmission rate could unfairly disadvantage hospitals that care for sicker or more vulnerable patients.
>
> In summary, the Excess Readmission Ratio fits our goal of finding hospitals with high readmission rates compared to what we expect based on their patient populations. This will help us make informed recommendations for patient advocacy.*
...
> It makes sense to drop NumberOfReadmissions and NumberOfDischarges after calculating observed_readmission_rate for several key reasons:
>
> 1.  **Avoid Multicollinearity (Redundancy):** Since observed_readmission_rate is directly derived from the other two metrics, keeping all three introduces redundancy. This can lead to multicollinearity, where highly correlated predictors confuse the model, resulting in unstable estimates and reduced interpretability.
> 2.  **Prevent Data Leakage:** If observed_readmission_rate were treated as a target variable, retaining its components would lead to data leakage, effectively giving the model the answer in the predictors. Even if ExcessReadmissionRatio is the final target, keeping these variables could still create indirect leakage or highly correlated features.
> 3.  **Focus on the Derived Metric:** The observed_readmission_rate is a more meaningful measure for analysis compared to raw counts. Dropping the counts allows the focus to remain on this rate, which is the relevant abstraction for modeling, unless there's a specific hypothesis regarding the counts.
>
> In summary, removing these columns streamlines the dataset by eliminating redundancy and potential issues, enabling a clearer focus on the most significant derived metric for analysis.
...
```{r}
# --- DIAGNOSTIC STEP FOR QUESTION 22 ---
cat("--- Columns in pneumoniaAnalyze before HospitalOverallRating_Label creation ---\n")
print(colnames(pneumoniaAnalyze))
cat("------------------------------------------------------------------\n\n")

# If 'HospitalOverallRating' is in the list above, let's see its unique values.
# This line will only run if the column is present.
if ("HospitalOverallRating" %in% colnames(pneumoniaAnalyze)) {
    cat("--- Unique values in HospitalOverallRating before encoding ---\n")
    print(unique(pneumoniaAnalyze$HospitalOverallRating))
    cat("------------------------------------------------------------\n\n")
} else {
    cat("HospitalOverallRating column is NOT found in pneumoniaAnalyze at this point.\n\n")
}
# --- END DIAGNOSTIC STEP ---

# Your original code for Question 22 starts here:
pneumoniaAnalyze_plot <- pneumoniaAnalyze %>%
  mutate(HospitalOverallRating_Label = case_when(
    HospitalOverallRating == 1 ~ "1 Star",
    # ... (rest of your mutate code) ...
# Ensure ggplot2 is loaded (it should be from your initial setup with tidyverse)
library(ggplot2)
library(dplyr) # For data manipulation (mutate, case_when)

# Re-label HospitalOverallRating for better plot readability
# The HospitalOverallRating column should already be numeric (1-5) from Question 17.
pneumoniaAnalyze_plot <- pneumoniaAnalyze %>%
  mutate(HospitalOverallRating_Label = case_when(
    HospitalOverallRating == 1 ~ "1 Star",
    HospitalOverallRating == 2 ~ "2 Stars",
    HospitalOverallRating == 3 ~ "3 Stars",
    HospitalOverallRating == 4 ~ "4 Stars",
    HospitalOverallRating == 5 ~ "5 Stars",
    TRUE ~ NA_character_ # Assign NA for any unmapped values (e.g., if original was NA)
  )) %>%
  # Convert the new label column to a factor with specific levels for correct ordering on the x-axis
  mutate(HospitalOverallRating_Label = factor(HospitalOverallRating_Label,
                                              levels = c("1 Star", "2 Stars", "3 Stars", "4 Stars", "5 Stars")))

# Create a Violin Plot to explore ExcessReadmissionRatio (target) vs. HospitalOverallRating
# A violin plot shows the distribution shape (density) of a continuous variable for different categories.
# It's excellent for visualizing how the distribution of ExcessReadmissionRatio varies across star ratings.

ggplot(pneumoniaAnalyze_plot, aes(x = HospitalOverallRating_Label, y = ExcessReadmissionRatio, fill = HospitalOverallRating_Label)) +
  geom_violin(trim = TRUE, alpha = 0.7) + # trim=TRUE truncates violins at data extremes, alpha for transparency
  geom_boxplot(width = 0.1, outlier.shape = NA, fill = "white", alpha = 0.5) + # Add small boxplots inside for quartiles, hide outliers
  
  # Add labels and title for professionalism
  labs(
    title = "Distribution of Hospital Excess Readmission Ratio by Overall Star Rating",
    subtitle = "Higher ratios indicate pneumonia readmissions are higher than expected given patient mix",
    x = "Hospital Overall Rating (Stars)",
    y = "Excess Readmission Ratio",
    fill = "Star Rating" # Label for the fill legend
  ) +
  
  # Set y-axis limits to focus on the most relevant range around 1.0 (expected performance)
  # Excess Readmission Ratio values near 1.0 mean performance is as expected.
  # Values > 1.0 mean higher than expected, < 1.0 mean lower than expected.
  scale_y_continuous(limits = c(0.7, 1.3)) + 
  
  # Use a clean theme
  theme_minimal() + 
  
  # Adjust title and subtitle positioning
  theme(plot.title = element_text(hjust = 0.5, face = "bold"), # Center and bold title
        plot.subtitle = element_text(hjust = 0.5))              # Center subtitle

# For a bonus point, you could try faceting by another categorical variable, e.g., 'State':
# + facet_wrap(~ State) # Add this line after theme() if you want to facet by State (might make many small plots)
```
