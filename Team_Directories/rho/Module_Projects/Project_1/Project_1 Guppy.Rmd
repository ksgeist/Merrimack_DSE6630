...
> The dataset that I am going to choose is the pneumonia analyze with no encoding. This dataset has all of the scores of the alternative conditions. This is where we are going to be able change which condition we are going to use other wise. The other dataset that could be choosen is the pneumonia fully encoded dataset, this dataset though is all already encoded for pneumonia meaning that trying to change it to any of the other conditions would be hard since some of those columns were removed from the database. The other thing that the pneumonia not encoding data is that they still have all of the average and median values in the database. 
...
longTargets %>% 
  ggplot(aes(x = Value, y = NatlComparisonHospitalDays, fill = NatlComparisonHospitalDays)) +
  geom_violin(alpha = 0.7) +
    geom_boxplot(width = 0.1) #this is the line of code directed to put in to show quartile in the plot
  labs(title = "Which target to use?",
       subtitle = "By readmitted LOS in hospital compared to national average",
        x = "",
        y = "Rate or Score",
...
> When looking at the graphs, the main thing that looks the best when compared to the national average is actually the better group. This is saying that these hospitals have better than the national average of pneumonia readmission rates. In the graph above the better group shows the most normal distribution and also doesn't have a tail going in either direction. When looking at the other 2 groups being worse and same as the national average. With the other 2 groups they both have long tails showing first shows some great variety of information. With both of these having long tails it caused both of them to be skewed right. The other group that is shown in the graph is the unknown group which is all of the missing data and how it is distrbuted. This is telling us more about just how much is missing. If this group was showing a regular distribution this would be concerning since that means we have a bigger problem of informaiton not getting reported. After looking at this I think this shows that for some groups I think there is some standing that this gives because the better group also has the lowest distribution which is how it show compare to the other groups. For this I think there is a conclusion that we are able to draw since the data is being pictured as predicted. 
...
> When looking at the different graphs that was produced we can see that there is the predcted group which shows what is expect meaning that on y axis it should have the best score but should be on the low side of the x axis. This actually holds true for all the graphs produced where we see that the best is the on the left side of all of them and the worse group is to the right in all the groups which shows that they have higher amount of pneumonia reamissions which is what is expected from having being worse than the national average. The one graph that doesn't really follow the conclusions that were created is the observed, where the highest score is the same as the national average group. In this group the better group and the worse group have close to the same score which isn't what was predicted. 
...
> When looking at the differences between the median time in the hospital, and the potential relationship that it has with postoperative respiratory failure rate. When thinking of good hositals they like to say that there are less waits and there is less overall time. this is what we are seeing when looking at the median time that is being spent at the hosptals. The graph that is pictured below, shows that the worse hospitals have some of the highest median time spent. This again is something that is expected. When looking at the graph for time it doesn't show anything is out of the ordinary, since these time difference are expected. We are now going to look at the postoperative respiratory failure which we are expecting for better hospitals to have lower levels of this rather where worse hospitals should have more of these postoperative respirtory failure. This though isn't the conclusons that we are seeing. We are seeing that this isn't what is being shown by the graphs below. We are actually seeing the reverse of this, there is actally alot more cases of this happening in better hospitals rather than in worse hospitals. These two groups do have a connection with one another. Postoperative respiratory failure is something that comes on after 48 hours after surgery. This can be a connected to the amount of time that is spent in the hospital. When a person spends more time in the hospital more of these complications are able to be caught. Knowing that some of these diseases have long incubation times, meaning that the longer these patients are in the hospital they are able to be monitored continuously which helps to prevent some these conditions happening. When looking at the difference between the graph that was created originally compared to the graph that was produced below was first the data that was used was different. Predicted was the first data used where it was changed to observed data, when looking at all of the outputs for all of the data. The difference between all of them were few. I think though when looking at all the data. I think using the observed data is the best to use though because it allows us to predict thing that are happening currently and then it can be looked upon further. 
...
longScore %>%  
  ggplot(aes(y = observed_readmission_rate,  ## <-- CHANGE THIS!
             x = Value)) +
    ...
    labs(title = "Relationship of observed pneumonia-related hospital \nreadmissions to each performance metric",
        x = "Rate or Score",
        y = "Observed Pneumonia Readmission",
...
> The graph that I feel the most strongly about is again the observed data. I ran the data from the expected and one of the reasons that I didn't like the expected data was because it showed a large range in all of the groups rather than just a few like in the observed data. Which is the one I am choosing. With such a wide range showing in the expected data it seems like it is trying to cover too much and tryingt to cover everything rather than trying to understand more specific groups. 
Comparing the two graphs, we can see that there are still similarities between the two but the trends of the the better hospitals having better scores and less pneumonia patients being readmitted continues into this data set as well. I think one of the big things to note on is the discharge information that is given to the people. I think this is a great thing to note on becuase you can see the decrease at all the levels that as discharge informaition score increase pneumonia readmissions patients decrease. This is a great way to be able to help to decrese readmission rates. I think that one of the other places to note this same kind of reaction is in the doctors communication section. This section is huge because it is sometimes a big conversation in healthcare where doctors don't effectively communicate with their staff and this sometimes can be detremental to the patient rather than anything else. I think this data is a better representation because I think this shows more light on some of the trends that is currently happening. 
...
> The relationship that I find the most compelling is the relationship between the median time spent in the hospital compared to some of the conditions that come on at a later time, such as postoperative respiratory failure and venous thromboembolism prophylaxis. Both of these conditions are ones that take a while to show. For the respirtory failure it usually comes on anywhere from 24 to 72 hours after surgery. There is then the venous thromboembolism prophylaxis but this comes on months after being in the hospital. When looking at the comparison between these scatterplots, the one that is definetly more concerning is the postoperative respirtory failure since it comes on a day or two after surgery. When seeing that the better hospitals have a larger rate of postoperative respiratory failure compared to the others where they have a large range of these happening, this isn't something that you would expect from a highly rated hospital. Especially with the respitory failure because it comes on so soon after surgery that if a hospital kept them around a little while longer then it could have been prevented or caught. One of the other scatterplots that brings quite a bit of attention is how the pneumonia rate goes down with both discharge information as well as doctor communication increasing. I think these are all interesting connections and should be explored more. I think one of the biggest questions that I still have is how to improve these overall standing. How do you improve doctor communications, doctors are busy people and trying to have them spend more time with patients can be difficult. The other thing to try to look to improve is discharge communication, which again is how to improve it more. Is it about improving paperwork to patients or last communications with them. I think one of the other questions that I have is how to lower the rate of some of these conditions tha come on after surgery or a hospital stay.  
...
I do agree because first it was also the data set because I also agree with you that it showed the best results when it came the different scatter plots. One of the things that I noticed was also how well it was able to show active trends that were happening at that moment with the current data collected from the hospitals. One of the other points that I agree you with is that, the predicted and observed data help to specifically target certain hospitals which is crucial for them to be able to improve their own setting. I think both of these sets give a lot more access to understing what is happneing and how it should be changing. 
...
> In any kind of analysation there is going to be some sort of bias when it comes to the bias that is being shown here. The thing I think would be the biggest problem with bias is confirmation bias which is where we are trying to use the data to confirm opions that we already had. This can just include our own person opinions. Most people want to go to a good hospital to get treated so that they get good care. If this data is limited, people would know some of the hospitals that are listened and they would hopefully see the hospital in which they attend. This again would be confirming something that the person already has in mind.  
...
dat2Analyze <- dat2Analyze %>% 
  # this is going to be focusing on working with the dat2analyze table, and this is filtering that for the next changes
  select(-ExpectedReadmissionRate, 
         -ExcessReadmissionRatio,
         -observed_readmission_rate, 
         -contains(c("Sample_", "NumberOfPatients")),
         -NumberSurveysCompleted) %>% 
  ## this is the process of dropping all of the columns that have that name but it is also dropping any columns that have the name Sameple or Number of Patients in it when using contain
  ...
  ## This section above is renaming the name of the columns so that they make a little bit more sense when using them to create models 
  select(PredictedReadmissionRate, everything())

## this is going to choose all of the predicted rates and all of the variables that are associate with that column
...
> The near zero variance function is a function which locates any places where the value is close to zero and doesn't show variation when it comes to the data. This is also an important function to run especially in this work because it locates places where there is a value close to zero which could allow for us to find places where patient privacy could be compromised. This function wil also help to locate some constants in the data that maybe wasn't originally seen. After finding these columns these are going to be dropped so that there can still be variance in the data. 
...
> Having FacilityID in the actual database it would get included in all of hte models that are created. Since these are saved as integers in the database they would be included in the model creation and they don't have significants in the way of creating models. These ID columns are just for that, to help ID which facilities have what scoring. These models are also trying to predict more unbrella trend across myltiple facilities if we were creating models for just one facility then the ID column wouldn't be needed.
...
> The big P little N problem speaks on dataframes that have more variables then it does observations. This isn't somehting that is a problem with this database since it has 370229 observations and 31 variables. If these was an idea to do this kind of anlyzation with hospitals in one local are then this may cause a problem since there wouldn't be a lot of hospitals in such a small area. In changing the amount of observations to so few it could cause a lot of problems since there will be more variable. This concept can also cause data to do some stange things, like points moving far apart, and the mode that is created makes the model and noise approach 100% accuracy. This all can be avoided though by having more observations compared to variables. 
...
```{r}
print(colSums(is.na(dat2Analyze)))


```

> There is some level of missingness to all of the columns that are in the database have some level of emptiness to them. Above is the amount of NAs that are in each column and unfortunetly there is some level of missing in each column.  

**Which column(s) are not missing any data?**

> There are acutally no columns that has all data in it. This is unfortunete because there is a state column which should be an easy column to fill out completely since we should have access to all of the locations of the facilities. This could potentially be fixed and allow us to not have all columns with some missing informaiton   

**How many complete observations do we have in the dataset?** You may find `nrow()` with `complete.cases()` or `drop_na()` useful!
```{r}
nrow(complete.cases(dat2Analyze))
...
> The number that they are reffering to is the 1788 which in the table tells of the predicted readmission rates of postoperative respiratory failure rates. When looking at the missingness from the previous table, this condition is one of the biggest columns that has the most missingness. This amount of missingness can definetly lead to the high level of predictiveness, since the missingness isn't included in predicting areas. This feels like a combination of high missingness as well as high predicted level. 
...
> The most correlation between missingness and our target is, MRSA Bacteremia, Postoperative respiratory failure rate and medicare spending per patient. All of these are very high on the realtionship between predictedt readmissions rates and missingness. Even when looking at these these there is even a big jump between the variables. The variable that has the biggest relationship even above the others is MRSA. I think that this has the biggest correlation becauase MRSA is first not very common, there is a very small amount of people that carry MRSA. The first probelem is already that it is very uncommon, the follow up thing is that MRSA is also one of the bacterias that can cause pneumonia. In these situations where when a patient comes in and the yget tested for pneumonia, then most hospitals will start treating the pneumonia rather than continuing testing. Once treatments stop working and the person is still sick then there is a reason to continue testing. 
...
> I don't feel any differently about the dimensions than previously in question 9D. Even with us dropping the variables there are still so many observations that even if we dropped these variables it still would cause a concern with big p little N. If we were starring to drop observations because of missingness, this would then cause the problem to maybe come into the situation. If we decided to drop observations becuase of missingness, then this could cause problems. In this situation we are just dropping variables which in the long run we can drop variables and it can actually help to create a more accurate model when predicting. In this case it would be predicting of penumonia readmission. 
...
> When we starting to create a model we need to create a training set and a test set. In these situations we split the data as evenly as possible. We are going to be splitting data for a couple reasons, the first being that we first need to train the model. The analogy that I like to use is to a pet, where you need to teach a pet to follow orders but slowly it starts to do those things without having to say the command out loud. There are some simulatrities between this and training a machine. The data sets that we are going to create are the commands, we are separating it into a test set and triaing set. The training set we create are now going to be given to the machine, where the machine will use this data to create an initial model of the data. The more training sets that you give to the machine the better it will be able to predict the variable better. Back to the analogy the more you train the pet the better they will do at it. We know what we are doing with the training set, and we now have a trained model. Now we have a trained model we are going to use the test set, and we are going to put it into the trained model. After putting them into the model we hope that it will create the most accurate predictions of some of the predicted variable. Back to our analolgy, this is like showing your friends the pet's trick and them following it, and not just suddenly ignorning you. When looking at some problems that could occur is data leakage, this can happen when that machine has access to information that normally wouldn't be with there when the time of predictions. This means that if there are variables that are uncounted for in the training set and then in the test set it could cause problems such as overestimating and unreliable predictions. This is why in the beginning we are going to make sure that the test set and training set are very similar so that when making predictions the model is encountering anything it hasn't seeen already. 
...
calcSplitRatio <- function(p = NA, df) {
  ...
  ## this is the function that if a parameter is found to be NA then it is going to count the number of columns and subtract by 1 and making the p variable equal to that. 
  if(is.na(p)) {
    p <- ncol(df) -1   ## this is where p is being assinged as the number of columns minus 1 from the database 
  }
  
  ## This is defining test_N and this is going to be calculated from doing 1 over the square root of the variable p which we defined earlier. It is then multiplied by the number of rows in the array dat2Analyze
  test_N <- (1/sqrt(p))*nrow(dat2Analyze)
  ## In this portion of code we are defining test_prop, where we are doing some similar calculations as previous. We are doing 1 over the square root of p multiplied buy the rows of the dataset then divide that but hte number of rows in the dataset. We have an additional function which is the round function and this is going to take the value of the calculations and its going to round it to the nearest 2 digits. 
  test_prop <- round((1/sqrt(p))*nrow(dat2Analyze)/nrow(dat2Analyze), 2)
  ## This function below is defining train_prop and we are doing 1 minus the test_prop 
  train_prop <- 1-test_prop
  
  ## This is a print function where it is going to paste together the words as well as the train_prop and test_prop 
  print(paste0("The ideal split ratio is ", train_prop, ":", test_prop, " training:testing"))
  ## this is going to return the train_prop
  return(train_prop)
}
...
> We got a split of the training set of 2238 observation and a test set has 488 observations, this is very close to the 80-20 split originally thought. The split shoule be that the training set should be around 80 percent where the test should be 20 percent. In reality the split that we have is an 82-18 split which is as close to the 80-20 split as we can get. This is a great split that we can use for making an accurate model. 
...
> MSE is the mean squared error which means that it is the difference between the predicted variable and the actual variable value. This means that a larger number means that the model isn't close to predicting the true value. As the number gets lower it means that the model is getting closer to the true values that are happening. The variable that has the best MSE is medicare spending per patient. The variable with the highest MSE is the payment for pneumonia patients 952651.45. This shows us that it is a very bad predictor in the model, since the model and actual values are so far apart. The second value is PCF which is proportion of falsely classified this variable is to tell of the accuracy of the predicted values. This means that the lower the number is it means it is more accuarte to the true values. When looking at the values above the one that has the most accurate value is survey response rate. when looking at all of these values though it is shown that all of them are quite low which means that they all show good accuray when guessing the values between the true value and the predicted value. The survey response rate is the best value for PCF but all of them are very low making them good predictors. There isn't a wide variety like seen with hte MSE.    
...
> MICE is a package that can be used to deal with different missing data. This pckage will take the missing values and instead of putting all of the data into another model rather than all of the data going into one model since missing data can have a big impact on the accuracy of the model that is being created. The great thing about this is that this system is able to uphold both models, in predictions. The other thing that MICE is going to do is add in random values. These random values are then determined to be accurate compared to the linear regression. The advantages of MICE is that it uses other information and variables to predidct missingness. This helps for missingness to be based a number of different things rather than trying to predict based on one thing. 
van Buuren, Stef. “Multivariate Imputation by Chained Equations.” Multivariate Imputation by Chained Equations •, amices.org/mice/#:~:text=The%20mice%20package%20implements%20a,the%20quality%20of%20the%20imputations. Accessed 20 June 2025. 
...
> It would be a problem if the test set had different columns than the test set because the modle was trained on a certain set of varibles. If this variable is only in the testing set the model will braeak and it will not complete or it will start to over estimate since this wasn't something it waas taught. To prevent this we need to add it into the training set. This will allow the model to add it into consderation and adjust the model as needed. If there are variables in the training set and not the test set then it would cause mismatch data, and again it would be trying to make predictions on lack of information. Thsi would do that same thing and start to over predict from the informaiton that it already is having.  
...
> When looking at a histogram there is a chance that it can be misleading and it may look like a normal ditribution but it isnt't really. One of the biggest problem with histogram is that even though the same graph would be showed to multiple people there are some people who would think that the data is skewed while another doesn't think it isn't which now leads to different comclusions. When looking at what a Q Q plot it has similar problems as the historgram meaning that persepctive also plays a role in determining if this shows a normal distribution. There are some other options to be able to test for normality such as the Kolmogorov-Smirnov test which we learned about earlier.  
...
shapiro.test(imputedTest$PredictedReadmissionRate)
shapiro.test(imputedTrain$PredictedReadmissionRate)
shapiro.test(dat2Analyze$PredictedReadmissionRate)

```

> When looking at both the test and training set we see that both of the test and training sets have a very low p-value. If the p-vlaue is below .05 then we are reject the null meaning that we reject the data being normally distributed. The test data set had a p-value of .0001174 and the training set had a p-value of 1.986e-13, both of these are going to be rejected because of the low p-value. This isn't good since all of these aren't normally distrubted. Since the test and training group both show not normally distributed. I wanted to see if there is any normality distribution with the predicted readmission pneumonia patients. When running the shapiro-wilk test on the predicted readmission for pneumonia patients from the dat2Analyze and this created a 6.067e-15 which means that it is significantly less than than .05 which again means that we are rejecting the null and this again means that the data isn't normal. 
...
> As previously explored we know that we need to do everything to both sets so that it will stay consistent and not skew the model that is going to be created. So even if there is two different lambda for the test and training set we need to stay consitent so that we don't throw off the model when it is trying to predict. Below is us adjusting the predicted pneumonia readmissions rates, it created a lambda -0.3 with this we are calculating lambda which will then be applied to the predicted pneumonia readmission rate. Below is also the creation of Q-Q plot and histogram which both show a normal distribution since that lambda has been applied. 

```{r}
bc_data2 <- BoxCoxTrans(imputedTrain$PredictedReadmissionRate)
print(paste0("Box-Cox lambda =  ", bc_data2$lambda)) #this will show which lambda is the best to help to make the training set more normally distributed. 

imputedTrain$bc_PredictedReadmissionRate <- predict(bc_data2, imputedTrain$PredictedReadmissionRate) #this is creating a new column which has the readmission rate adjust with the lambda that was created. 
...
ggplot(imputedTrain, aes(x = bc_PredictedReadmissionRate)) +
  ...
  labs(title = "Box-Cox Transformed Predicted Readmission Ratio, N = 2,726 hospitals",
       y = "Frequency",
       x = "Predicted Readmission Ratio") #creation of the histogram to display the data that go changed with the lambda that was calculated


qqnorm(imputedTrain$bc_PredictedReadmissionRate)
qqline(imputedTrain$bc_PredictedReadmissionRate, 
       col = "hotpink", lty = 2, lwd = 2) #this is the creation of the q-q plot with the data that was changed from the lambda calculated. 


# Make sure to check that your transform looks successful!
#graphs below shows that the transform looks successful and is showing normal distribution
...
> The highest potential for multicollinearity would be at the places where darker blues where there is a positive correclation between the two variables, there is also a relationship between the dark reds but this just means that there is a negative relationship between the two variables. The thing that we are looking for is darker colors but not ones that shows a 1 relationship which just means that the variable is being compared to itself, this isn't the relationship we are looking for. The thing that we are trying to avoid is the diagonal that is going across the heatmap because this is where the variables are compared to itself. The other color that we are trying to avoid is the yellow because is shows no relationship between the two varaibles. 

Which variable(s) seem to be most correlated with our target, `bc_PredictedReadmissionRate`?

> Some of the relationships that has the best relationship is between the bc_predictedreadmissionrate is with the nationally compared hospital return dates for pneumonia rates. This has a positive relationship which is an expected relationship, since they are pretty closely related. There also seems to be more of a relationship between the predicted pnumonia readmissions rates to the communication and hospital rating overall. 
