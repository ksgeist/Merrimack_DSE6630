...
> When looking at both the test and training set we see that both of the test and training sets have a very low p-value. If the p-vlaue is below .05 then we are reject the null meaning that we reject the data being normally distributed. The test data set had a p-value of .0001174 and the training set had a p-value of 1.986e-13, both of these are going to be rejected because of the low p-value. This isn't good since all of these aren't normally distrubted. Since the test and training group both show not normally distributed. I wanted to see if there is any normality distribution with the predicted readmission pneumonia patients. When running the shapiro-wilk test on the predicted readmission for pneumonia patients from the dat2Analyze and this created 
