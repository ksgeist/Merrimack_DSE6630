...
> The `.shp` file contains the actual geographic boundary information for each California county. The `.shx` file is an index file that helps GIS software locate the geographic features stored in the shapefile. The `.dbf` file contains the attribute data associated with each county, such as names and identifiers.

The `.prj` file stores the map projection and coordinate system information. The `.cpg` file specifies the character encoding used by the attribute table. Finally, the `.shp.xml` file contains metadata describing the dataset, including information about its source and contents.
...
```{r}
cali_sf <- read_sf("./__California_County_Boundaries2025__/California_County_Boundary.shp")
...
> `TRUE` printed 69 times because there are 69 geographic features in the shapefile that were validated. California only has 58 counties, so not every feature is a unique county. When I printed the county names, some counties appeared more than once. This suggests that some counties are made up of multiple polygons or separate geographic areas, so the 69 features represent individual map geometries rather than 69 different counties.
...
> The bounding box shows the outer limits of the map data, basically acting as a tight frame around all 59 features. It tells us the exact spatial footprint of the data on a grid, which is measured in meters here. If you want to zoom in on a specific feature, you just shrink that frame by raising the minimum coordinates ($xmin$, $ymin$) and lowering the maximums ($xmax$, $ymax$). To zoom out, you do the opposite and expand the frame by lowering the minimums and raising the maximums.
...
> The projection is WGS 84 / Pseudo-Mercator. It tells us how the 3D globe is flattened into a 2D map, measuring the coordinates in meters instead of degrees. Projection matters because flattening a sphere always distorts either shape, area, distance, or direction. If you change the projection, the visual shape of your map will shift, and your calculations for distances or land area will change based on the new system's math.
...
map1 <- cali_sf %>% 
  ...
  ## Initiate the plot using the filtered data
  ggplot() +
  
  ## Add the geometry layers with a standard color
  geom_sf(fill = "cadetblue3", 
          color = "gray20", 
          linewidth = 0.25, 
          alpha = 0.65) +
          
  ## Keep the map tight to its bounding box
  coord_sf(expand = FALSE) + 
  
  ## Add the updated title and caption
  labs(title = "Northern California Counties Bordering Oregon",
       caption = "Source: California State Geoportal")

## Now explicitly call it to force R to draw it in your plot window
map1
...
ggplot(data = cali_sf) +
  ...
  ## Different marker for Sacramento (capital)
  scale_shape_manual(
    values = c(capital = 18, major = 16),
    guide = "none"
  ) +
  
  scale_size_manual(
    values = c(capital = 6, major = 3),
    guide = "none"
  ) +
  
  ## One color for each city
  scale_color_manual(
    values = skittles[1:5],
    name = "City"
  ) +
...
## Start with the mortality dataset containing the total number of deaths by county, year, and month
mortality_sf <- totalDeaths %>% 
  
## Rename the County variable so it matches the county name field in the California shapefile
  rename(COUNTY_NAM = County) %>%
  
## Group the data by county, year, and month so deaths can be summarized for each county-month combination
  group_by(COUNTY_NAM, Year, Month) %>% 
  
## Calculate the total number of deaths within each county, year, and month group
  summarise(totalDeaths = sum(totalDeaths, na.rm = TRUE)) %>%
  
## Join the mortality data to the California county shapefile using county names;
## relationship = "many-to-many" allows multiple mortality records and multiple spatial records
## to be matched when the join key appears more than once in either dataset
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  
## Convert the joined dataset back into an sf object so it can be used for mapping and spatial analysis
  st_as_sf()
```
...
> st_join() joins data based on geographic location, while full_join() joins data using a matching column. In this case, we used full_join() because both datasets have county names that can be matched. st_join() wouldn't really make sense here since the mortality data doesn't have any geographic features to join on.
...
ANSWER:There are 2596 features in mortality_sf
```{r}
...
> There are many more features in mortality_sf because each county appears multiple times for different months and years. In cali_sf, each county only appears once, but in mortality_sf there is a separate record for each county-month-year combination.
...
> Yes. The darker counties seem to be the ones with bigger populations, especially Los Angeles County. You can see similar patterns around other large cities too. This suggests that total deaths are being driven by population size, not necessarily by a higher risk of death.
...
> DP1_0048C contains the actual number of people in that demographic group, while DP1_0048P contains the percentage of the population that belongs to that group. The "C" stands for count and the "P" stands for percent
...
> The `census` dataframe has 2020 Census population and demographic data for California counties. It includes total population, male and female counts, and the number and percent of people age 65 and older. Each row is one county.
...
> To use a different API, I would need to change the URL, API key, and the variables I want to pull. The basic process would stay the same though: connect to the API, get the data, turn it into a dataframe, clean it up, and then use it for whatever analysis I'm doing.
...
> The pattern is less obvious now. Some smaller counties, like Shasta, have higher mortality percentages than larger counties. I don't think this is necessarily a true pattern because smaller populations can make the percentages look higher. Even though deaths increased during COVID-19, that increase doesn't stand out as much after adjusting for population.
...
> This happened because the 2020 data only included 8 months, while the other years had all 12 months. That makes the comparison unfair and can make 2020 look lower than it really was. One way to fix it would be to only compare January–August for all years, or get the missing 2020 data. If we had all 12 months for every year, then the maps from Section 2.7 would have been a more accurate comparison.
...
> Yes, 2020 looks more consistent with the idea that deaths increased during COVID-19. Some Southern California counties, like Los Angeles, Riverside, and Imperial, appear to have higher mortality than in previous years. Other counties do not show much change, so the increase does not seem to be happening everywhere at the same level.
...
> No, I don't think the regression is very robust. The diagnostic plots show several outliers and the residuals do not appear to be normally distributed. The Scale-Location plot also suggests the variance is not constant. Because of this, I think the predictions are rough estimates rather than reliable forecasts.
...
> I only used the distinct counties because there are only 58 unique county boundaries in California, even though mortality_sf2 contains many more rows from different months and years. Calculating neighbors on all of those duplicate county records would repeat the same spatial calculations many times and make the analysis slower without adding any new information. The output of 58 confirms that the neighbor list was created for the 58 unique counties.
...
mortality_sw <- nb2listw(mortality_nb,
                         style = "W",
                         zero.policy = TRUE)
...
> The OLS model is a better fit to the data. It has a lower RSS than the spatial lag model, and the ANOVA results show that the difference between the models is statistically significant (p < 0.001). This suggests that county-specific factors, along with year and month, explain mortality better than the spatial lag variable.  Essentially the information about the county is more significant than the information about the neighboring counties
...
> The OLS model shows that mortality varies by county, and many of the county coefficients are significant. The Year and Month variables are also significant, suggesting mortality changes over time. The model has an R² of 0.668, meaning it explains about 67% of the variation in monthly mortality, so it does a fairly good job overall.
...
> Not completely. The model explains a good amount of the variation in mortality (R² = 0.668), but the diagnostic plots show some outliers and violations of the regression assumptions. I think the model is useful for looking at general trends, but I would be cautious about relying on the predictions too heavily.
...
> The training set contains about 81.8% of the data and the testing set contains about 18.2%. This is very close to the standard 80-20 train-test split, with slightly more data in the training set.

#### Question 18B [0.5 points]
What caveats or limitations can you think of by using none of the 2020 data in the training set?

> One limitation is that the model does not learn any of the patterns from 2020, including the effects of COVID-19. If mortality changed a lot in 2020, the model may not predict those changes very well because it was trained only on data from earlier years.
...
> Out-of-bag (OOB) error is an estimate of how well a random forest predicts new data. Each tree is built using a random sample of the training data, and the observations left out are used to test the model. We use OOB error to evaluate model performance without needing a separate validation set. In this project, it gives us an idea of how accurately the random forest can predict mortality.
...
> mtry is the number of predictors that the random forest can consider at each split in a tree. For regression, the default is usually about one-third of the predictors. In this dataset there are only three predictors, so the default would not give many options. By testing mtry values from 1 to 3, the model can compare different settings and choose the one that gives the best performance.
...
> The best model used mtry = 1 because it had the lowest RMSE and highest R². R² can be negative, which means the model is doing a poor job of explaining the data. I don't think the RMSE can be directly compared to the earlier regression because the random forest used preprocessed data, so the models were not fit under exactly the same conditions.
...
> Cross-validation slightly improved the model. Unlike the OOB results, none of the R² values were negative, although they were still very low. The best model again used mtry = 1 because it had the lowest RMSE (0.984) and highest R² (0.035). The RMSE was only slightly lower than the best OOB model (0.987), so cross-validation did not lead to a major improvement in performance.
...
> The RMSE and MAE suggest that the model's predictions are still fairly inaccurate. The training and testing RMSE values are very similar, so the model is not overfitting. Instead, it seems to be underfitting because both the error is relatively high and the R squared values are very low, highlighting that the model is not capturing much of the variation in mortality.
...
> The model tends to underpredict high mortality values and overpredict low mortality values, so most predictions are pulled toward the average. This matches the low R² values we saw earlier. Even though the model isn't very accurate, it still suggests mortality in 2020 may have been higher than in previous years.
...
## Pull a larger portion of the dataset
offsets <- seq(0, 650000, by = 50000)

caliDeaths <- lapply(offsets, function(x) {
  fromJSON(
    paste0(
...
## Filter to Sept-Dec 2020, all causes, total population
...
## Download all records (default is only 100)
caliDeaths <- fromJSON(
  "https://data.chhs.ca.gov/api/3/action/datastore_search?resource_id=078185a9-e3a7-403f-a546-b8c582f0a9d8&limit=50000"
)

caliDeaths <- caliDeaths$result$records

## Filter to the months you were predicting
caliDeaths2020 <- caliDeaths %>%
  filter(
    Year == "2020",
    Month %in% c("09", "10", "11", "12"),
...
## Calculate monthly percent mortality

caliDeaths2020 <- caliDeaths2020 %>%
  mutate(Count = as.numeric(Count)) %>%
  left_join(census, by = "County") %>%
  mutate(monthlyPercMortality = Count / totalN * 100)
...
```{r}
## Only run if actual 2020 mortality data exist

if (nrow(caliDeaths2020) > 0) {

  comparisonDF <- data.frame(
    Actual = caliDeaths2020$monthlyPercMortality,
    Predicted = rfPredictions
  )

  ggplot(comparisonDF, aes(x = Actual, y = Predicted)) +
  ...
  cor.test(
    comparisonDF$Actual,
    comparisonDF$Predicted
  )

} else {

  print("Actual 2020 mortality data were unavailable from the API, so the comparison could not be performed.")
...
ANSWER: The scatterplot shows little relationship between predicted and actual mortality. The correlation was very weak (r = 0.047) and not statistically significant (p = 0.335), suggesting the model did a poor job predicting mortality in late 2020, likely because it was trained on pre-pandemic data.
...
> The Month variable captures seasonality, but time lags also capture how mortality in previous months influences current mortality. Similar to spatial lags, they provide additional information that Month alone cannot capture and may improve predictions.
...
> The time-series lagged variables appear to improve the model because the testing MAE decreased from 0.745 to 0.129. Since the training MAE (0.134) and testing MAE (0.129) are very similar, it doesn't seem like there is  overfitting. The model seems to be doing pretty good and working with the new data.
...
> One way to improve the model would be to add more variables. Right now the model only uses a few predictors, so adding things like demographics, health factors, or COVID-related data could help explain more of the variation in mortality and improve predictions.
...
> We analyzed monthly mortality patterns across California counties using regression and random forest models. The results showed clear seasonal patterns and some spatial relationships between neighboring counties. However, models trained on 2017–2019 data struggled to predict mortality during 2020, likely because of the unusual effects of the COVID-19 pandemic.

Adding time-series lag variables greatly improved model performance, showing that recent mortality trends are important predictors of future mortality. Despite these improvements, the model is limited by the variables available and the unique conditions of 2020. Future work should include additional predictors and more advanced time-series methods to improve accuracy.
...
> No. Males have a noticeably higher proportion of injury-related deaths than females across all years. Ignoring sex would overlook an important source of variation and could bias the results.
...
> The most noticeable pattern is that males consistently have a higher proportion of injury-related deaths than females. There also appears to be a decline in injury-related deaths during 2020, especially after May. One possible explanation for the increase around May 2020 is that restrictions began to ease in some areas, leading to increased travel and activity compared with the strict stay-at-home period earlier in the pandemic.
...
> There is not strong evidence from these maps alone that the stay-at-home orders changed injury-related deaths. The patterns look fairly similar across years. To better assess the effect, I would compare maps before and after the shutdown and create separate maps for males and females.
...
> The gray counties are most likely not true missing data. In counties with very small numbers of injury-related deaths, the data may be suppressed to protect privacy or avoid reporting unstable estimates. When those values are suppressed, they appear as missing and are shown in gray on the map.
...
injuryDeathSex <- injDeaths %>%
  group_by(biologicalSex, COUNTY_NAM = County) %>%
  summarise(
...
ggplot(injuryDeathSex) +
  geom_sf(aes(fill = deathsInjuriesPercent), color = "gray20", size = 0.2) +
  scale_fill_viridis_c(name = "% Injury Deaths") +
  facet_wrap(~ biologicalSex) +
  theme_bw() +
  labs(
    title = "Percent of Deaths Due to Accident or Injury by Sex",
    subtitle = "California Counties, 2017-2020"
  )


```
ANSWER AND ANALYSIS: The maps suggest that males have a higher proportion of injury-related deaths than females across much of California. Several counties show noticeably higher values for males, while female rates are generally lower and more uniform. This supports the idea that biological sex is an important factor and should be included in the analysis.
...
> The correlation was 0.28 (p < 0.001), indicating a positive relationship between sex ratio and injury-related deaths. Counties with relatively more males tended to have a higher proportion of injury-related deaths, so controlling for sex ratio may be a good idea.
...
> Yes. The plot agrees with the correlation coefficient because the best-fit line has a positive slope. Counties with higher female-to-male ratios tend to have a higher percentage of injury-related deaths. However, the points are fairly scattered, which is consistent with the relatively weak correlation (r = 0.28).
...
> Data Science Question: Did the COVID-19 stay-at-home orders change the proportion of deaths due to accidents and injuries across California counties?

Hypothesis: The proportion of deaths due to accidents and injuries decreased during the stay-at-home period because reduced travel and movement lowered the risk of accidents.

Additional Census variables worth considering include the female-to-male ratio, retiree ratio, population over age 65, and total county population. These factors may influence injury-related mortality and could help explain differences between counties.

I would use a random forest model because it can capture non-linear relationships and interactions between variables. I would also include a spatial lag because neighboring counties may have similar mortality patterns. Ideally, I would include temporal lags as well since previous analyses showed that past mortality values were strong predictors of future mortality.
...
I AM PICKING HEART DISEASE TO ANALYZE.

The following is the code with graphics and data output followed by the analysis and interpretation of the code.

CODE:

```{r}
## For this analysis, I chose to examine heart disease (HTD). Heart disease is a leading cause of death and may ##have been affected by changes in healthcare access and behavior during the COVID-19 pandemic. I will explore ##spatial and temporal patterns in heart disease mortality and examine whether counties with older populations ##experienced higher rates of heart disease mortality.

## Join the sex deaths table with the census data; then filter for HTD
htdDeaths <- sexDeaths %>% 
  full_join(census, by = "County") %>% 
  ## Filter for heart disease deaths
  filter(leadingCauseDeath == "HTD") %>% 
  ## Join total deaths
  full_join(totalDeaths[, 1:4], by = c("County", "Year", "Month")) %>%
  ## Rename the death count column
  rename(heartDiseaseDeaths = sexDeaths) %>% 
  ## Calculate metrics
  mutate(
    deathsHeartDiseaseProportion = heartDiseaseDeaths / totalDeaths,
    femaleMaleRatio = femalesN / malesN,
    femaleMaleRatio65 = females65plus / males65plus,
    retireeRatio = over65 / totalN,
    Date = as.Date(
  ...
  mutate(
    deathsHeartDiseaseProportion =
      ifelse(deathsHeartDiseaseProportion == "NaN",
             0,
             deathsHeartDiseaseProportion)
  )
```
...
```{r, echo = FALSE}
## Plot heart disease deaths by sex and year
htdDeaths %>% 
ggplot(aes(y = deathsHeartDiseaseProportion,
           x = biologicalSex,
           fill = as.factor(Year))) +
  geom_boxplot(alpha = 0.7) +
  labs(
    x = "Biological Sex of Decedent",
    y = "Proportion of All Deaths Due to Heart Disease",
    fill = "Year",
    title = "Proportion of All Deaths Due to Heart Disease",
    subtitle = "California Counties, Jan 2017 - Aug 2020"
  ) +
  scale_fill_manual(values = skittles[c(5, 9:11)]) +
  theme_classic()
...
```
NOTES:
Males consistently show a higher proportion of heart disease deaths than females across all years. Heart disease mortality appears relatively stable over time, although 2020 may show a slight decline compared with previous years.

```{r, echo = FALSE}
##### Calculate the proportion of all deaths due to heart disease
heartDiseaseSummary <- htdDeaths %>% 
  ## Group by year, county, and sex
  group_by(Year, County, biologicalSex) %>% 
  ## Change the name to make the join easier
  rename(COUNTY_NAM = County) %>% 
  ## Mean proportion of heart disease deaths
  summarise(
    deathsHeartDiseasePercent =
      mean(deathsHeartDiseaseProportion, na.rm = TRUE) * 100,
    .groups = "drop"
  ) %>% 
  ## Join to get geometries for mapping
  full_join(cali_sf, by = "COUNTY_NAM", relationship = "many-to-many") %>% 
  st_as_sf()

#### Find the five-number summary
summ <- fivenum(heartDiseaseSummary$deathsHeartDiseasePercent)

paste0("The five-number summary of % of deaths that are heart disease-related is: ")
...
heartDiseaseSummary <- heartDiseaseSummary %>%
  mutate(
    deathLabels = cut(
      deathsHeartDiseasePercent,
      breaks = quantile(
...
## Initialize the plot
ggplot(heartDiseaseSummary) +
  
  ## Draw county polygons
  geom_sf(aes(fill = deathLabels),
          color = "gray20",
          size = 0.2) +

  ## Apply fill scale
  scale_fill_manual(
    name = "% of Deaths Due to Heart Disease",
    values = c4a(palette = "brewer.or_rd", n = 3),
    na.value = "gray70",
    drop = FALSE
  ) +
  ...
  labs(
    title = "California Annual Percent Deaths Due to Heart Disease, by County",
    subtitle = "Cumulative Monthly Deaths, Jan 2017 - Aug 2020",
    caption = "Source: California DPH Vital Records"
  ) +
...
htdDeaths %>%
  ...
  scale_color_manual(values = skittles[c(5, 9:11)]) +
  labs(
    title = "Monthly Heart Disease Mortality by Year",
    subtitle = "California Counties, Jan 2017 - Aug 2020",
    x = "Month",
    y = "% of Deaths Due to Heart Disease",
    color = "Year"
  ) +
  theme_minimal()
```
...
cor.test(
  ...
  htdDeaths$deathsHeartDiseaseProportion,
  use = "complete.obs"
)
```

There was a weak negative correlation between retiree ratio and heart disease mortality (r = -0.233, p < 0.001). Counties with more retirees tended to have slightly lower proportions of deaths due to heart disease.


```{r}
htdDeaths %>%
  ggplot(aes(x = retireeRatio,
             y = deathsHeartDiseaseProportion * 100)) +
  geom_point(alpha = 0.6, color = skittles[5]) +
  geom_smooth(method = "lm", color = skittles[8]) +
  labs(
    title = "Heart Disease Mortality vs Retiree Ratio",
    subtitle = "California Counties, 2017-2020",
    x = "Retiree Ratio",
    y = "% of Deaths Due to Heart Disease"
  ) +
...
The scatterplot agrees with the correlation coefficient. The fitted line has a negative slope, indicating that counties with higher retiree ratios tended to have slightly lower proportions of deaths due to heart disease. Although the relationship is statistically significant, the wide spread of points suggests the association is relatively weak.



ANALYSIS:
Analysis of Heart Disease Mortality (HTD)

For my analysis, I chose to examine heart disease (HTD) mortality across California counties from 2017–2020. I explored heart disease mortality using a boxplot, choropleth map, monthly trend graph, and a correlation analysis with retiree ratio.

Key findings:

Boxplot by sex: Males consistently had a higher proportion of deaths due to heart disease than females across all years. Heart disease mortality was fairly stable from 2017–2020, although there was a slight decline in 2020.
Choropleth map: Heart disease mortality varied across California counties, with some counties showing consistently higher percentages of heart disease deaths than others. The overall geographic pattern remained fairly stable over time.
Monthly trend analysis: Heart disease mortality was relatively stable from 2017–2019, generally ranging between 10% and 12% of all deaths. In 2020, the proportion of deaths due to heart disease declined during the spring and summer months, suggesting that the pandemic may have influenced mortality patterns.
Correlation and scatterplot: The correlation was weakly negative (r = -0.233, p < 0.001), suggesting that counties with more retirees generally had slightly lower proportions of heart disease deaths. The scatterplot showed a lot of variation, indicating that retiree ratio alone does not strongly explain heart disease mortality.
Data Science Question

Did heart disease mortality patterns vary across California counties during the COVID-19 pandemic, and were these patterns associated with county demographics such as retiree ratio and biological sex?

Hypothesis
Heart disease mortality varies across California counties and demographic groups. Counties with different age structures and sex distributions may experience different levels of heart disease mortality, and these patterns may have changed during the COVID-19 pandemic.

Overall Conclusion
Overall, heart disease represented a substantial share of deaths across California counties and showed both geographic and demographic variation. Males consistently experienced a higher proportion of heart disease deaths than females, and heart disease mortality patterns appeared relatively stable before the pandemic. The decline observed during 2020 suggests that COVID-19 may have influenced heart disease mortality patterns, although additional analysis would be needed to determine the exact cause. Future analyses could incorporate spatial and temporal modeling to better understand the factors driving heart disease mortality across California counties.
